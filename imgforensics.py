...
def _wiener_denoise(x: np.ndarray, sigma: float, windows=(3, 5, 7, 9),
    ...
    **ここは実測で直した箇所**。最初の実装は空間領域でも ``x * gain`` と書いており、
    ...
    分かった —— 8 枚で ``corr(fingerprint, K_true)`` は **-0.027**(ほぼ無相関)、
    直した後は **+0.859**(同条件、``denoiser="wavelet"`` は +0.873)。
    PCE も 205 → **7246** と 35 倍になった。
...
def _estimate_step(vals: np.ndarray, max_step: int = 64, *,
    ...
    **残っている限界(実測。直していない、報告する)**: 係数の広がりが真のステップの
    **約 2.5 倍未満**になると、上振れが残る。合成の櫛(4096 標本・復号の丸め雑音
    σ=0.2)で測った結果:

    ========================= ========== ========== ==========
    広がり / 真のステップ      step=5     step=12    step=16
    ========================= ========== ========== ==========
    1.5                       6          25         34
    2.5                       5          12         16
    4.0                       5          12         16
    ...
    実画像では低周波係数の広がりは十分あり、この域に落ちるのは高周波係数か
    低品質の JPEG である。:func:`jpeg_quality_estimate` はそれを
    ``n_quantized``(櫛が読めた係数の数)として返し、足りなければ品質を答えない。
    ``tests/test_imgforensics.py::test_estimate_step_rejects_the_two_measured_traps``
    がこの表の 1.5 行も含めて固定してある —— **直したふりをしない**ため。
    """
