...
DOCS = {
    ...
    "xpil_contour": (
        ...
        "エフェクト）。\n\n"
        "a, b は未使用（固定カーネル）。\n\n"
        "カーネルは 3x3 の ``[-1,-1,-1; -1,8,-1; -1,-1,-1]``（scale 1、offset 255）で、"
        "出力は ``255 + (8*中心 - 8 近傍の和)`` を 0〜255 に飽和させたもの。平坦部は"
        "255（白）、周囲より暗い画素は暗く、周囲より明るい画素は 255 で飽和する。"
        "つまり段差の「暗い側」に 1 画素幅の黒線が乗り、明るい側は白のまま"
        "（``xpil_find_edges`` は同じカーネルで offset 0 なので両側が明るく出る）。\n\n"
        "入力は [0,1] に clip して ``*255`` の切り捨てで 8 ビットの L 画像にし、"
        "出力を ``/255`` で [0,1] に戻す（1/255 の量子化が入る）。画像の最外周"
        "1 画素は Pillow がフィルタせず入力値のまま返す。0 サイズの画像は"
        "ValueError で拒否され、fail-soft で入力のクリップ版に落ちる（台帳に記録）。"
        "ノイズにも反応するので、前段に ``xpil_smooth_more`` や ``median`` を置くと"
        "線が整理される。線を領域として使うなら ``invert`` で黒線側を明るくして"
        "から ``threshold`` に繋ぐ。"
    ),
    "xpil_find_edges": (
    ...
    "xpil_smooth_more": (
        ...
        "a, b は未使用（固定カーネル）。\n\n"
        "カーネルは 5x5 の重み付き平均（中心 44、その 8 近傍が 5、最外周 16 画素が"
        "1、合計 100 で割る）。中心の重みが大きいので同じ 5x5 の一様平均より"
        "ぼけは弱く、インパルスは中心 44% + 隣接 8 画素 5% ずつに広がる程度"
        "（実測: 255 のインパルス → 中心 112、隣接 13、外周 0）。3x3 の ``SMOOTH``"
        "（中心 5、近傍 1、合計 13）よりは広く効く。\n\n"
        "入力は [0,1] に clip して ``*255`` の切り捨てで 8 ビット L 画像にし、"
        "出力を ``/255`` で戻す（1/255 の量子化が入る）。画像の最外周 2 画素は"
        "Pillow がフィルタせず入力値のまま。0 サイズの画像は ValueError で拒否され、"
        "fail-soft で入力のクリップ版に落ちる（台帳に記録）。ぼかし量を可変にしたい"
        "なら ``gaussian`` や ``median`` を使う。エッジ検出（``xpil_contour`` /"
        "``xpil_find_edges``）や ``threshold`` の前のノイズ落としに。"
    ),
    "xpil_detail": (
