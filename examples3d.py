...
EXAMPLES = [
    ...
    {"id": "geodesic_dome", "task": "metrology", "data": "synthetic",
     "name": "測地ドーム — オイラーの公式が 5 角形を 12 個に縛る",
     "summary": "正二十面体を細分して球に射影する。f を 1→6 と上げると頂点は 12→362 に増えるが、**次数 5 の頂点はどれだけ細分してもちょうど 12 個**(V−E+F=2 の帰結で、11 でも 13 でも球にならない)。次数は別実装の graph_degree_table に数えさせる。半径を変えても位相は不変、半球は切り口を持つので頂点が減る。"},
    {"id": "minimal_surfaces", "task": "metrology", "data": "synthetic",
     "name": "極小曲面と管メッシュ — 定義そのものが門になる",
     "summary": "極小曲面とは「平均曲率 H が至るところ 0」の曲面で、既存 vertex_curvature がまさにそれを測る —— 「これは極小曲面だ」という主張を**作り方を知らない op が採点**する。カテノイド・ヘリコイド・エンネパー・シェルクの |H| 中央値は 0.00002〜0.00014 で、対照群の単位球は 1.00004・半径 1 の円柱は 0.50000(門が素通しでない証拠)。★ガウス曲率 K の一致は**必要条件にすぎない**(カテノイドとヘリコイドは等長なので K が一致するが、K が一致しても極小とは限らない)。等長な曲げの族は面積 17.6738〜17.6756 で不変。ジャイロイドは体積比 0.499928 / 0.499981 / 0.499991(体心反転の対称性・格子に依らない)だが、**節面近似の残差は隠さず出す**(|H| / 主曲率スケール = 0.1250)。円を中心線にした管はトーラスなので体積 2π²Rr²・表面積 4π²Rr が解析解で、既存 mesh_volume / mesh_area が比 0.997 / 0.999。まっすぐな区間を含む曲線でも半径が 0.150000000000 で崩れない(平行移動フレーム —— フレネ枠は直線部で法線が定義できない)。"},
    ...
    {"id": "rle_region_efficiency", "task": "metrology", "data": "synthetic",
     ...
     "summary": "vol_rle_encode が 192^3 部品マスクを dense bool の 1/73(実測)に、volume/bbox/centroid は run 直接演算で dense と厳密一致かつ 93x 速(実測)、和/積/差の集合演算 3.1ms(run のみ)、vol_rle_components の成分分解、vol_tiled_map は gaussian 全量計算と最大差 0、往復 bit 一致、改竄 RLE は fail-closed 拒否。"},
    {"id": "wall_thickness_probe", "task": "metrology", "data": "synthetic",
     "name": "virtual probe — パイプ壁厚をプローブ 1 本で計測",
     "summary": "vol_profile_line(異方 spacing の物理距離厳密)→ vol_edge_probe(サブボクセル 4 エッジ、極性 +,-,+,-)→ vol_wall_thickness が壁厚 2.042/2.042 mm(真値 2.000、誤差 0.042mm=0.085voxel)。measure1d の 3D 版。"},
    {"id": "gray_window_level", "task": "segmentation", "data": "synthetic",
     "name": "CT windowing — 窓の選択が「見える構造」を入れ替える",
     "summary": "vol_window_level の軟部窓は軟部 Δ0.50 が立ち骨は飽和、骨窓は骨が立ち軟部は背景と同化(実測)。vol_equalize(mask domain LUT)/ vol_gamma(γ=2 で 0.5→0.25 厳密)/ vol_stretch(パーセンタイル厳密写像)も検証。"},
    {"id": "vol_geometry_transform", "task": "registration", "data": "synthetic",
     "name": "3D 幾何変換 — rotate 90°x4 恒等・resize の物理体積保存・affine 厳密シフト",
     "summary": "vol_rotate は np.rot90 と bit 一致で回転方向規約を機械固定、vol_resize は spacing 再計算で物理体積 230.4 mm^3 厳密保存、vol_affine は pull 規約(out[o]=vol[M@o+offset])を voxel 単位で検証。"},
    {"id": "deconv_fft_restore", "task": "reconstruction", "data": "synthetic",
     "name": "顕微鏡スタック復元 — 3D FFT フィルタ + Richardson-Lucy",
     "summary": "vol_fft_lowpass+highpass=入力の恒等式、照明ドリフト成分を 1/17 に抑制(線形性で分離計測、ガウス伝達のトレードオフごと開示)、vol_richardson_lucy 50 回で RMSE 0.68x(漸進と正直に主張)+前方一貫性 0.021x+総強度保存。"},
    # -- depth ------------------------------------------------------------------- #
    ...
    {"id": "ct_porosity_and_fibre_morphometry", "task": "measurement", "data": "synthetic",
     "name": "産業CTの形態計測 —— 気孔径分布・局所肉厚・開/閉気孔・繊維配向",
     "summary": "真値の分かる合成ボリュームで、気孔径分布と体積分率、局所肉厚(ミリ換算)、貫通孔と閉気孔の別、配向の揃い具合を復元できることを数値で確かめる。"},
    ...
    {"id": "nearest_seed_partition", "task": "shape_analysis", "data": "synthetic",
     "name": "距離変換の「向き」— 最近の seed への変位で半径・肉厚・ラベルの縄張りを測る",
     "summary": "vol_nearest_seed_vector(円柱の表面 → 骨格への変位の長さ = 半径 r を厳密再現)、vol_nearest_label(2 つの球のラベルを零 voxel に配ると分割面が垂直二等分面に一致)、異方 spacing で最近傍が入れ替わる、torch があれば edt_jfa_vector が scipy 経路と厳密一致。"},
    ...
    {"id": "structured_light_scan", "task": "structured_light", "data": "synthetic",
     "name": "構造化光スキャナ 1 台を閉ループで回す(投影→撮影→復号→三角測量)",
     "summary": "描画した実シーン(球+段差箱+床)へ相補Gray 18枚+位相シフト4枚を投影し、absolute_phaseで2π次数を確定、triangulate_columnで視線×コラム平面を解いて深度へ。真値深度をRMSE 0.233mm(レンジ287mmの0.081%)で再現。Gray整数のみ(0.548mm)の2.4倍、位相のみ(209.8mm)の901倍良い。Gray復号は全16521画素で誤り0。"},
    ...
    {"id": "bspline_freeform", "task": "freeform_geometry", "data": "synthetic",
     "name": "B スプライン自由曲線・自由曲面の復元と計測",
     "summary": "直線・平面・円のような大域基底では表せない「くねる曲線」「うねる曲面」を区分多項式(B スプライン)で復元し、再サンプル・平滑・残差計測まで 1 本に通す事例。曲面側は既知の f(x,y)=0.7 sin(1.6x)cos(1.3y)+0.3xy を散布 600 点から fit_bspline_surface で双三次フィットし、学習外の内部格子で eval_bspline_surface した値が解析真値と RMS 1.5e-3(大域平均 null 0.…"},
    {"id": "curvilinear_proj", "task": "metrology", "data": "synthetic",
     "name": "曲座標展開: 極/円筒/Zernike/LiDAR円筒投影で回転体の m 回対称を一貫復元",
    ...
    {"id": "geometry_metrology", "task": "metrology", "data": "synthetic",
     "name": "幾何メトロロジー: 直線/平面/球/円の当てはめ→角度・距離・交線計測",
     "summary": "1 個の機械加工ブロック(2 面が稜線で交わり、面上に球と円穴が乗る)を舞台に、当てはめ op(fit_line_3d/fit_plane_3d/fit_sphere_3d/fit_circle_3d/ransac_line)の出力を計測 op(angle_3points/angle_between_lines/angle_between_planes/angle_line_plane/distance_point_plane/distance_point…"},
    {"id": "matching_localize", "task": "match_localize", "data": "synthetic",
     "name": "3-D テンプレート定位(NCC/形状/chamfer/Hough/MIP/曲率)",
     "summary": "同一の合成シーン(滑らかな充実球=ターゲット と、球と同一ピーク濃度の立方体=おとり を離して配置)に対し、match3d の 6 定位手法を全て当てて、球テンプレートの中心を真値±2 voxel(実測の 6 手法合議 spread は 0.87vox)で復元できることを検証する事例。球は表面点群(match_points_ncc 用)と解析的 smooth 占有場(voxel 5 手法用)を同一幾何から生成し(bounds=(0,N-1) で world…"},
    {"id": "metrics_eval", "task": "reconstruction", "data": "synthetic",
     "name": "評価指標4種の真値検証(F-score/RMSE対応/法線一致/voxel IoU)",
     "summary": "単位球点群(放射法線)と占有ボクセルを正解として合成し、metrics3d の 4 評価指標を解析真値と 1e-9 で照合する。fscore は 120 点厳密コピー+60 外れ点の再構成で precision=120/180・recall=120/150 を厳密に作り込み f=0.72727 を検証(完全コピー=1.0、無作為点 null≈0)。rmse_correspondence は恒等=0・既知オフセット |v|=0.1 の残差を厳密照合し、対応数…"},
    {"id": "motion_scene", "task": "scene_flow", "data": "synthetic",
     "name": "動く物体のシーンフローを点群・ボクセル・画像平面の3表現で復元",
     "summary": "2時刻の同一物体を3つの見え方から観測し、既知の真値(剛体運動 R_gt=2度・t_gt<0.3、ボクセル並進 shift=[1.5,-2,1]、画素並進)を握って合成し、5つの op を鎖でつないで運動を復元・検証する。表現1(点群): estimate_flow が小運動で最近傍対応恒等(実測 1.000)となりフローが真の変位と機械精度一致(誤差 0.0)、その対応から fit_rigid が Kabsch で (R,t) を復元(回転誤差 1.7e…"},
    {"id": "pcl_geodesic", "task": "shape_analysis", "data": "synthetic",
     "name": "円柱点群の前処理と測地距離・中心軸復元(SOR/radius/MLS + kNN/測地/距離リッジ)",
     "summary": "円柱(半径R=1, 高さ2)を「側面点群」と「中身入りvoxel」の2通りで合成し、6つのopを鎖にして数値的真値で検証する。側面点群(面2400点+遠方の飛び点40点)に対し statistical_outlier_removal と radius_outlier_removal がいずれも飛び点40/40を除去し面の点2400を1つも誤除去せず(SOR→radius合成で面のみ2400点が残存)。mls_smooth が各点の軸までの距離の真値Rからの…"},
    {"id": "refinement", "task": "pose_refinement", "data": "synthetic",
     "name": "姿勢・ピーク精緻化(Newton/LM/LK/回転GN/点-面ICP)",
     "summary": "粗いマッチ(整数ボクセル/±3度級)を連続座標・連続角へ締め上げる 5 種の精緻化器を、既知真値の合成データで一括検証する。帯域制限した滑らかな解析場 F を整数格子(scene)と既知の分数オフセット格子(template)からサンプルし、「同一の真の並進」を refine_peak_newton(相関スコア山の整数ピーク→サブボクセル)・refine_translation_lk(逆合成 LK, corner 規約)・refine_lm(LM, cen…"},
    {"id": "sensor_seg", "task": "segmentation", "data": "synthetic",
     "name": "センサ幾何と領域処理パイプライン(角シーンの denoise→傾き→面分割→計画格子)",
     "summary": "深度センサが捉えた「2つの傾いた面が稜線で出会う角」の1シーンを、実際の知覚パイプラインの順に8opで連結処理する例。清浄ガイドで joint_bilateral して段差を残しつつノイズを削り(RMS 0.112→0.019、素のGaussianぼかしnullは稜線段差を-5.8→-1.16に潰すが本opは-5.79を保存)、bearing_angle_image で各面の傾きを degrees(atan(s)) と厳密一致で数値化(左26.565°/右…"},
    {"id": "sfm_recon", "task": "reconstruction", "data": "synthetic",
     "name": "2視点SfMから表面再構成まで一つの球で通す",
     "summary": "中心[0,0,6]・半径1.5の単一の球を題材に、2視点SfMから表面再構成・観測合成までを6つのopで鎖状に接続し、すべて既知真値と照合する(ノイズ無し合成)。essential_8pointは球面上の対応点+Kから本質行列Eを復元し、真のE=[t]×Rと符号/スケールを除き|cos|=1.000000で一致・正規化エピポーラ残差1.1e-15を確認(直交並進+40度回転の誤E nullは|cos|=0.002・残差0.35)。triangulateは真…"},
    {"id": "shape_desc_pose", "task": "shape_descriptors", "data": "synthetic",
     "name": "大域形状記述子と姿勢照合",
     "summary": "3D 部品を姿勢に依らず「形」で同定し、次に実際の姿勢を復元する一連の流れを、大域形状記述子(D2 距離分布・A3 角度分布・PCA 広がり比 extent・主慣性モーメント)と姿勢照合(PCA 主軸整列・FFT 位相相関・log-polar/Fourier-Mellin)で示す。記述子側は厳密な数学則で検証: extent_signature と principal_moments は同一共分散の別表現なので、principal_moments から共分…"},
    {"id": "transforms_repr", "task": "representation", "data": "synthetic",
     "name": "3-D データ表現の相互変換ハブ(点群↔ボクセル↔メッシュ↔SDF↔深度↔TSDF)",
     "summary": "半径・中心が既知の球(と、登録用に非対称な段付きブロック)を共通の被写体に、fullseye の 3-D 表現変換 op を 1 本の鎖に繋いで「表現を変えても物体の幾何が保たれる」ことを解析真値で検証する事例。depth→depth_to_points で球面点を厳密復元(median|d-R|=6.7e-16)、mesh_to_voxel と gaussians_to_voxel が同一格子上に同じ殻を作り occupancy IoU=0.454(ずら…"},
    {"id": "primitive_fitting_3d", "task": "metrology", "data": "synthetic",
     "name": "3-D プリミティブ当てはめ(直線/平面/球/円/最小包含球)",
     "summary": "点群から直線・平面・球・円を最小二乗で当て、中心/半径/向き/残差を (depth,row,col) で復元(機械精度)。各残差は『わざと外した』null を桁違いに下回る。measure3d.fit_line3/fit_plane3/fit_sphere3/fit_circle3/smallest_sphere3。2-D fit_line/fit_circle の 3-D 版。"},
    ...
    # -- wave3: シーン/実データ(実恐竜対称/ステレオ/LiDAR分割/部分ICP/円筒計測)------ #
    {"id": "dl_mesh_symmetry", "task": "shape_analysis", "data": "download",
     "name": "恐竜骨格の左右対称面(矢状面)の復元",
     "summary": "スミソニアン三角竜骨格(CC0,10万頂点→4090点stride)をdetect_reflection_symmetryに渡し矢状面を残差2.48で復元(最薄主軸=左右方向に一致)。他2主平面4.28/4.30と区別、片側20%破壊で15.87(6.4倍)へ悪化=左右対称を判別的に検出。未取得時はSKIPしexit0。"},
    {"id": "stereo_depth_scene", "task": "depth", "data": "synthetic",
     "name": "ステレオ視差からの多深度パッチ奥行き復元",
     "summary": "校正済みステレオ対(f*B=96)に近8/中16/遠32mの3テクスチャパッチを視差12/6/3pxで合成し、disparity_map→depth_from_disparityで復元。パッチ内部の相対誤差0.00%・near>mid>far順序も正。最良定数null63.9%・視差ゼロnull(∞)を判別的に上回る。"},
    {"id": "lidar_scene_segmentation", "task": "segmentation", "data": "procedural",
     "name": "屋外LiDARシーンの地面除去→物体分割",
     "summary": "傾斜地面(~5.4度)上の4物体(球/箱/円柱/円錐)のLiDAR点群5316点を、fit_plane_ransac+height_above_planeで地面除去→euclidean_clustersで分割。検出4==K=4・重心を真物体へ全単射(最大0.128m)。地面除去なしnullは全物体癒着で1クラスタ。"},
    {"id": "partial_overlap_icp", "task": "registration", "data": "procedural",
     "name": "部分重なりスキャンの登録(FPFH+ICP)",
     "summary": "非対称ブロブを別方向2視点で部分スキャン(幾何重なり56%)。scan Bに55度回転+並進を掛け、register_pointclouds(FPFH+RANSAC→ICP)でAへ登録。回転誤差0.86度<4・RMSE0.150が床0.148水準。PCA主軸103度/単位行列ICP58度のnullを桁違いに下回る。"},
    {"id": "cylinder_axis_metrology", "task": "metrology", "data": "procedural",
     "name": "円筒軸メトロロジー(30%外れ値ロバスト)",
     "summary": "汚れた産業スキャン(30%グロス外れ値・2000点)からパイプの軸方向と半径を計測。fit_cylinder_ransacで半径誤差1.27%・軸誤差0.78°・面残差0.00165m。非ロバスト全点フィット(半径誤差101%)と誤プリミティブ平面RANSAC(残差0.058m)を5倍超マージンで判別的に上回る。"},
    # -- wave4: 再構成/記述子/対称(alpha shape/poisson/SH/FPFH/回転対称)------------ #
    {"id": "alpha_shape_topology", "task": "reconstruction", "data": "procedural",
     "name": "トーラス点群のalpha shape再構成で穴(genus1)を保持",
     "summary": "中実トーラス(主R1.0/管r0.35)9000点をestimate_alpha+alpha_shape_meshで再構成。z軸穴プローブ41点の内包率がalpha=0.000(穴を保持)、凸包null=1.000(穴を充填・厳密Delaunayでも1.000)。オイラー標数χ_alpha=0(トーラス)対χ_hull=2(球)でも判別的。"},
    {"id": "poisson_surface_recon", "task": "reconstruction", "data": "procedural",
     "name": "Poisson軽量表面再構成(向き付き点群→水密メッシュ)",
     "summary": "でこぼこ閉曲面(外向き法線を勾配で厳密算出)の向き付き点群6000点をrecon3d.poisson_liteで水密メッシュ(V11788/F23572)へ。真曲面との正規化chamfer0.01006が外接球null(0.081=8倍)・乱数法線null(0.041=4倍)より桁違いに小さい。"},
    {"id": "sh_descriptor_retrieval", "task": "shape_descriptors", "data": "procedural",
     "name": "球面調和記述子による回転不変な3D形状検索",
     "summary": "球/立方体/トーラス/円柱/円錐の5クラスをボクセル化しsh_descriptor化。一様ランダム3D回転したクエリをmatch_sh_descriptorで検索→30/30=100%正解・分離マージン0.312。非回転不変null(軸周辺分布)は回転で100%→37%に崩れSHが+63pt上回る。"},
    {"id": "fpfh_correspondence", "task": "features", "data": "procedural",
     "name": "FPFH記述子で部分ビュー間の点対応を張る",
     "summary": "同一物体の2部分ビュー(58度回転+並進・重なり1514点)で法線推定→FPFH記述子(33次元)を計算し記述子最近傍で対応。幾何正答率0.633がランダム対応0.0034・記述子シャッフル0.0020(チャンス率)を約185倍上回る。register全体でなく記述子マッチ品質を直接測る。"},
    ...
    {"id": "itokawa_regolith_hero", "task": "rendering", "data": "itokawa",
