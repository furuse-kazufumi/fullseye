...
EXAMPLES = [
    ...
    {"id": "cad_to_scan", "task": "registration", "data": "synthetic",
     ...
     "summary": "初期姿勢なしで CAD 設計形状を実物スキャン点群に合わせ、置かれた向きと位置を復元する(FPFH+RANSACで粗く→ICPで機械精度)。"},
    ...
    {"id": "ct_bone_segmentation", "task": "modeling", "data": "skeleton_ct",
     "name": "CTボリュームから骨をセグメンテーションし本数と体積を計測",
     "summary": "骨を閾値化し、関節で繋がる指骨を収縮で分離してから連結成分で数え、体積を測る。"},
    ...
    {"id": "itokawa_curvature", "task": "features", "data": "itokawa",
     ...
     "summary": "表面の主曲率・曲率度・形状指数を求め、平坦(着地適地)と尾根/窪みを見分ける。"},
