...
正直な限界(すべて ``tests/test_volcolor.py`` が測っている範囲):
  ...
  * **色は衝突しうる**。パレットは一様乱数なので、ラベル数が増えると近い色の対が
    必ず出る。実測(seed=0):最近接色対の距離は 16 色で 0.109、64 色で 0.050、
    256 色で 0.020(RGB ユークリッド、最大 sqrt(3)=1.732)。**色は識別子ではなく
    目印**であり、どの色がどの成分かは :func:`vol_label_legend` が返す表で読む。
    色だけの図を作らないこと。
  * **チャネルごとの最大値合成(MIP)は提供しない**。RGB ラベルボリュームを
    チャネル別に max すると、赤成分は部品 A・緑成分は部品 B から来た「どの部品にも
    属さない色」が出る。実測(下記 :func:`vol_label_volume_render` の docstring):
    3 成分・16^3 のボリュームで **1226 / 1500 の前景画素**がパレットに無い色になった。
    ``mode="max"`` は :class:`ValueError` で拒否する(黙って混ぜない)。
