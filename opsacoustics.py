...
#      (正弦 + 雑音、負値あり)を 19 op 全部に流して実測したところ、
#      **型に起因する拒否は 1 件も出ず、返る数値もすべて正しい**
#      (負値も正値も音圧・加速度として正当だから)。「任意の signal を
#      音響信号として渡せる」は嘘ではない ― 本当にそうなのである。
#
#   2. **危険なのは配列ではなく `rate` という別のスカラである。** 取り違えの
#      本体は「16000 Hz の記録を 48000 Hz として読む」であって、これは
#      **ndarray の型では原理的に防げない**(サンプリング周波数は配列の中に
#      入っていない)。専用型を作っても防げるものが増えず、既存 dsp / funct1d
#      1-D 族との接続を切る損だけが残る。防御はスカラ側に置いた:
