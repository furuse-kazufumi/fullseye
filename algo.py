 ...
 * Fail-soft -1.0 on an empty input / bad or truncated header / OOM (0.0 = "nothing in
 * common" is a valid answer, so it cannot be the sentinel). */
...
double graph_components(const double* a, int n_in) {
    ...
    for (int k = 0; k < m; k++) {
        ...
        /* raw-double guard BEFORE the cast (range short-circuits, then integrality):
         * (int)3e9 / (int)1e300 / (int)NaN are float-cast-overflow UB. */
        if (!(ud >= 0.0 && ud < (double)n && ud == (double)(int)ud)) { free(parent); return 0.0; }
        if (!(vd >= 0.0 && vd < (double)n && vd == (double)(int)vd)) { free(parent); return 0.0; }
