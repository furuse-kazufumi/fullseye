...
``measure3d.smallest_box3`` は凸包面法線・PCA・決定的ランダムを初期姿勢に取り、各を局所改良
(座標降下で体積を縮める)して最小体積の向きを選ぶ。PCA で軸を取るだけの箱
(``fit_box3`` / ``pcseg.obb``)は非対称形状で最小にならないので、そこも上回る。
(honest: 全凸形状で「証明された大域最小」ではない — O'Rourke の辺接ケースは列挙せず局所改良で
寄せる。直方体は機械精度で厳密、正四面体のような case-b でも真の最小に到達する。)
