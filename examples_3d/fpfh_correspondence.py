...
点には真の対応(同一物体点)を保持する。両ビューは同一サーフェスからの清浄サンプル(=別視点
レンダに相当。センサノイズは載せず、測るのは純粋に記述子の識別力そのもの)。両ビューで FPFH を
計算し、記述子最近傍で対応を張り、「結ばれた B 点を既知変換で A 座標へ戻すと真の A 点の近くに
落ちるか」で幾何的な正誤を数える。難しさはノイズではなく、大回転(58度)・部分重なりの境界での
近傍構成のズレ・全 B 点(重なり外の妨害点を含む)からの最近傍探索から来る。
