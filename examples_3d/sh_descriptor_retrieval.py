...
周辺分布を符号つき分位点ベクトルで並べたもの、コサイン距離)を同条件で回す。null は
「回転なし」なら別サンプルでも 100% 正しく引けるのに、ランダム回転を掛けた瞬間に各軸の
周辺分布が崩れて正解率が ~30% 台まで落ちる(≒当てずっぽう)。SH は回転あり 100% を保つ。
SH の回転あり正解率が null の回転あり正解率を大きく上回ること、かつ null 自身が
「回転なし 100% > 回転あり ~30%」で崩れることを assert し、勝因が回転不変性だと判別的に示す。
"""
import sys
