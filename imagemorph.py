...
★★ 座標順の落とし穴(2026-09-02 に明文化)★★
    ...
    **例外は出ない**。出るのは「それらしく見えるが間違った」モーフ。
    ...
    実測(96x96、円盤 A(30,30) -> B(62,66)、12 点の対応点、alpha=0.5): 同じ対応点で
    affine と TPS を掛けたときの平均差が、(row,col) を渡すと **0.00488**、正しく
    (x,y) に直すと **0.00260** —— 座標順を間違えると 2 つの補間法が別々の場所へ
    ワープするので食い違いが約 1.9 倍に開く(正しければどちらも同じ中間形状を
    作るので差は小さい)。
