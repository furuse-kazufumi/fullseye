...
# --------------------------------------------------------------------------- #
#: 各 ``sg_*`` 関数自身には docstring があるが、実際に登録される ``op.fn`` は
#: ``build()`` 内の ``_wrap`` が返すクロージャ ``inner`` であり、``__doc__`` を
#: 転記していないため素通しでは説明が消える（``backends_regions3`` と同じ穴）。
#: qualname が ``<lambda>`` を含まなくても症状は同じなので、ここに Japanese の
#: 説明を書き ops.py の登録ループで Op.doc に積ませる。キーは op 名。
DOCS = {
    "sg_slic_superpixels": (
        "SLIC 法によるスーパーピクセル過分割の境界格子を返す"
        "（``skimage.segmentation.slic``、HALCON に対応オペレータなし）。\n\n"
        "``a``（0〜1）がセグメント数とコンパクトネスの両方を振る: 大きいほど"
        "セグメント数が少なく（``n_seg = max(2, round(6+(1-a)*40))``）、"
        "コンパクトネスが高くなる（``compactness = 0.01 + a*0.2``、形が四角く"
        "整う）。``b`` は未使用。戻り値はスーパーピクセルの境界線（外側の輪郭）を"
        "なぞった region で、境界はコンパクトネスが低いほど輝度エッジにスナップ"
        "しやすい。"
    ),
    "sg_felzenszwalb": (
        "Felzenszwalb のグラフベース領域分割の境界を返す"
        "（``skimage.segmentation.felzenszwalb``、HALCON に対応オペレータなし）。"
        "\n\n"
        "``a`` はクラスタの粗さを決める ``scale`` を 40〜540 に振り（大きいほど"
        "大きな塊にまとまる）、``b`` は最小成分サイズを 15〜100 に振る"
        "（``min_size = round(15+b*85)``）。戻り値はラベル画像の外側境界"
        "（region）で、真の物体輪郭に沿いやすい。"
    ),
    "sg_gmm_segment": (
        "輝度値に対する 2 クラス・ガウス混合モデル（EM 法）で明るい方のクラスタを"
        "領域として返す（``skimage`` 非依存の自前 EM 実装、HALCON に対応オペレータ"
        "なし）。\n\n"
        "25/75 パーセンタイルで初期化した決定論的 EM で 1 次元 2 成分ガウス混合を"
        "フィットし、平均が高い方を「明るいクラス」とする。``a``（0〜1）は"
        "事後確率のしきい値 ``t = 0.25 + 0.5*a`` を振り、大きいほど「確実に明るい」"
        "と判定された画素だけを拾うようになる。``b`` は未使用。乱数を使わないので"
        "再現性がある。"
    ),
    "sg_kmeans_intensity": (
        "輝度値の k-means クラスタリングで最も明るいクラスタを領域として返す"
        "（自前の Lloyd 法実装、HALCON に対応オペレータなし）。\n\n"
        "``a``（0〜1）からクラスタ数 ``k = 2 + round(4*a)``（2〜6）を決める。"
        "初期値はパーセンタイルから決定論的に置くので乱数は使わない。``b`` は"
        "未使用。重心が最も高いクラスタに属する画素を領域として返す。"
    ),
    "sg_region_growing_seeded": (
        "画像中心を種として、輝度が近い画素を繋げていく領域拡張（シードグロウ"
        "イング）（HALCON ``regiongrowing`` は既に別 op でカバー済みのため、"
        "重複申告を避けて ``halcon=\"\"`` としている）。\n\n"
        "種は画像中心の 1 画素。``a``（0〜1）が同質性の許容差（種の輝度 ±a 以内"
        "なら繋がる）、``b`` が連結性を切り替える（``b > 0.5`` で 8 連結、それ"
        "以外は 4 連結）。結果は種を含む連結成分のみ。"
    ),
    "sg_normalized_cut_2": (
        "輝度グラフの正規化カット（スペクトラル 2 分割、Shi & Malik）で画像を"
        "明暗 2 領域に分ける（HALCON に対応オペレータなし）。\n\n"
        "計算量を抑えるため画像を ``sdim = round(10+b*14)`` 程度まで間引いて"
        "から、輝度差と距離で重みを決めたアフィニティグラフを作り、一般化固有値"
        "問題 ``(D-W)y = lambda*D*y`` の第 2 固有ベクトル（Fiedler ベクトル）を"
        "中央値でしきい値化して 2 群に分ける。``a``（0〜1）は輝度方向の帯域幅"
        "``sig_i = 0.05 + a*0.5`` を振り、``b``（0〜1）は間引きの解像度を振る。"
        "明るい方の群を最近傍で元解像度に拡大して region として返す。"
    ),
    "sg_watershed_gradient": (
        "勾配画像の h-minima マーカーからのウォーターシェッド分割の境界を返す"
        "（``skimage``、HALCON に対応オペレータなし）。\n\n"
        "Sobel 勾配の大きさを正規化し、深さ ``h = 0.02 + a*0.3`` の h-minima 変換"
        "で生き残った領域極小をマーカーとして（``a`` が大きいほどマーカーが少なく"
        "・粗い分割になる）、その勾配画像に対するウォーターシェッドを計算する。"
        "``b`` は未使用。戻り値は集水域（catchment basin/物体）を分けるダムライン"
        "（境界）を表す region。"
    ),
}
