...
def csi_peak_position(signal, z_step_um=0.05, z_start_um=0.0, wavelength_um=0.6,
    ...
    for a step of any size that stays inside the scan.

    mode picks the estimator, and the choice is worth six orders of magnitude in
    the noiseless case and *inverts* under noise (full table in the module
    docstring):

      * ``"peak"``      — the scan plane with the largest envelope. Error is the
                          step quantisation, measured 2.50e-02 um at a 0.05 um
                          step.
      * ``"centroid"``  — the intensity centroid of the whole envelope. Noiseless
                          error 1.0e-06 um, and the **best estimator under noise**
                          (RMS 0.0324 um at 1 % noise, 3.3x better than the local
                          fits). Its own failure is *window bias*: it is pulled
                          toward the centre of the scan. Measured on a 12 um scan
                          with a 2.8 um coherence length, a surface 2 um from the
                          edge reads **+0.19 um** high with no noise at all and
                          **+0.87 um** high with a 2 % noise floor. Centre the
                          scan on the surface, or use a local estimator.
      * ``"parabolic"`` — three-point parabola on the envelope. 4.1e-06 um.
      * ``"gaussian"``  — three-point parabola on the **log** of the envelope,
                          which is exact for a Gaussian coherence envelope:
                          4.3e-09 um. The default.
    ...
    min_visibility: refuse a scan whose envelope prominence
                    ...
                    scores 1.000; a pure carrier with no coherence envelope
                    (a sinusoid plus 10 % noise) scores 0.176 and is refused,
                    because "the largest of 256 equal values" is a position that
                    noise chose, not a surface.
    ...
    **Raises** ``ValueError``: everything :func:`csi_envelope` raises, plus an
    unknown *mode*, a non-positive *z_step_um* / *wavelength_um*, a *z_step_um*
    at or past the Nyquist ceiling, an envelope whose prominence is below
    *min_visibility*, and — importantly — an envelope peaking on the **first or
    last plane**, which means the surface is at or beyond the end of the scan.
    That last one is the trap this refuses to fall into: returning the boundary
    plane would be a height, it would be finite, it would be plausible, and it
    would be wrong.
    """
    ...
    if vis < vis_min:
        raise ValueError(
            ...
            "envelope is a position noise chose. Refusing to report it as a "
            "height." % (op, vis, vis_min))
    k = int(np.argmax(env))
    if k == 0 or k == env.size - 1:
        raise ValueError(
            ...
            "range [%g, %g] um, so its true position is unknown; returning the "
...
def csi_design(wavelength_um=0.6, bandwidth_um=0.1, z_range_um=12.0,
    ...
    *wavelength_um* (a source whose spectrum reaches zero frequency is not a
    ...
    return {
        ...
        "planes_per_envelope": float(lc / rec_step),
        "z_range_um": zr,
        "n_planes": n_planes,
