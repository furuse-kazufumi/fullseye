...
**探針の無い op を 151 → 48 本に減らした回**(同日)。`tests/conftest.py` の
探針バンクは 6 sort しか持たず、**901 op のうち 151 本(16.8 %)が契約ゲート
3 本(例外を投げない / 非有限を出さない / 決定的)を一度も実行されていなかった**
—— ラチェットで本数だけ凍結し「本来の直しは BANKS を全 in_sort へ広げること」と
自分で書いてあった。その本来の直しを入れ、`points` / `signal` / `counts` /
`matrix` / `keypoints` / `rgbimage` の 6 sort(103 op)を実際に走らせた。
★**そこで出た 2 件の赤はどちらも op の欠陥ではなく探針の設計ミス**だった:
特異行列の条件数が `inf` なのも、非連結な点群の測地距離が `inf` なのも
**数学的に正しい答え**で、正解が非有限になる入力を「非有限を出すな」の門に
渡していた。ゲートを緩めず**探針の方を直した**(緩めると本物の非有限が通る)。
