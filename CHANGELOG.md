...
- ★**日本語も翻訳先だった**。要約 935 本のうち **349 本は原文が英語**で、日本語の
  ヘルプを開いても英語のままだった —— 「原文だから ja は常に読める」と数えていたのが
  誤りで、実測の内訳は **ja 586 / en 373 / zh・tw・ko・de 各 50**。その 349 本すべてに
  日本語訳を入れ、**ja は 935 / 935** になった。`docs/ops/**` のノートは docstring の
  写し(単一真実源)なので触らず、`<op>.ja.html` を兄弟ページとして生やす形にした
  ―― 他の 5 言語とまったく同じ仕組みで、参照順も既にそうなっていた。
- **op 要約の対訳 第 1 波**: `imgmetrics` 24 本(5 言語)+ `math` 26 本(4 言語 ——
  原文が英語なので en は素通し)。加えて上記の日本語 349 本。
  zh / tw / ko / de は各 50 本で、ここが次の波。
