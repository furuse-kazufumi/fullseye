...
* **周長は数え方を 3 つ測ってから選んだ。** 半径 40 px の円板(真値
  251.33 px)で、境界画素を数えると 316 px(**+25.7 %**)、Serra の重みなら
  263.8 px(**+4.95 %**、大きさを変えても同じ率)、Crofton の 4 方向なら
  252.8 px(**+0.56 %**)。円形度が 0.908 で頭打ちになるか 0.988 まで届くかが
  ここで決まるので、Crofton を採った(:func:`_perimeter` に表)。
