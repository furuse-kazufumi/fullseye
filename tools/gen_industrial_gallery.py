...
def subject_stereo_obstacles(log=print) -> dict:
    ...
    # 孤立した誤マッチ領域を無効化 (Hirschmüller の speckle filter)
    disp, spk_ok = stereo.speckle_filter(disp, max_diff=1.0, min_size=200)
    # 深度不連続まわりの flying pixels(縁のにじみが物体間を橋渡しする)を除去
    from scipy.ndimage import maximum_filter, minimum_filter
    dj = np.nan_to_num(disp)
    edge_jump = maximum_filter(dj, size=7) - minimum_filter(dj, size=7)
    valid = spk_ok & (dj > 3.0) & (conf > 0.25) & (edge_jump < 2.5)
    disp = dj
    ...
    obst = P[P[:, 2] > 0.12]
    clusters = fs.euclidean_clusters(obst, tol=0.30, min_size=150)
    ...
    return {
        ...
        "caption": ("2 台のカメラ画像のズレ(視差)をブロックマッチングで求め、"
                    "speckle 除去と信頼度ゲートで「使える視差」だけ残して "
                    "Z = f·b/d で 3D 点群へ(無効画素は無彩色マスク)。高さ 12cm "
                    "超の点をクラスタリングすると 4 物体が 4 クラスタに分かれ、"
                    "視野扇形つきの鳥瞰障害物マップができる。移動ロボットの視覚の"
                    "定番パイプライン。"),
