...
**6 幕構成**(幕ごとに必ずパラメータが動く。静止画のクロスフェード集にしない)::
    ...
    5 点群             クラスタ許容距離 tol を対数で掃き、視点が 1 周する
...
* (a) 色数を落とした —— 全フレームを **RGB 各 12 段に一様量子化**してから
  書き出す。版面が平坦な明るい地なので実測で **最大 237 色/フレーム**に収まり、
  256 色以下 = Pillow の GIF パレットがこの色をそのまま取る。結果として
  ① LZW が効いて容量が落ち、② **ポスター PNG が GIF の 1 フレーム目と
  ビット単位で一致する**(24 bit PNG と 8 bit GIF の往復で色が動かない)。
  副作用は陰影の段 —— ぼけた光学像と 3-D の球面でわずかに見える。隠さず書く。
  段数は ``--levels`` で変えられるが、256 色を超えたら自動で 1 段ずつ落とし、
  落としたことを表示する(黙って画質を変えない)。
* (b) フレーム数は 1 幕 22 枚 + 幕間クロスフェード 5 枚 x 6 = **162 枚**
  (fps 12 = 13.5 秒)。仕様の 120〜170 枚の範囲内で下寄りに取ってある。
* (c) 寸法は 960x360 のまま落としていない。実測 2.3 MB で 4 MB 上限の 6 割弱
  なので、(b)(c) をこれ以上削らずに済んでいる。
...
FRAMES_PER_ACT = 22
FADE = 5
LEVELS = 12                     # RGB 各チャンネルの量子化段数(実測 237 色/フレーム)
...
def _distinct(frame, levels):
    """``levels`` 段に量子化したときの、そのフレームの相異なる色数。"""
    a = np.round(np.clip(frame, 0.0, 1.0) * (levels - 1)).astype(np.uint32)
    return len(np.unique((a[..., 0] * levels + a[..., 1]) * levels + a[..., 2]))


def _quantize(frames, levels=LEVELS):
    """RGB 各チャンネルを一様量子化する(容量対策 (a) / 上の docstring)。

    段数は ``levels`` から始めて、**全フレームが 256 色以下に収まる最大の段数**へ
    自動で落とす。256 色を超えると Pillow が GIF パレットを自前で作り直すため、
    (1) 書き出した色が入力と食い違い、(2) ポスター PNG と GIF 1 枚目の一致が
    壊れる。段数を落としたら必ずその旨を表示する(黙って画質を変えない)。
    """
    for L in range(int(levels), 3, -1):
        worst = max(_distinct(f, L) for f in frames)
        if worst <= 256:
            if L != levels:
                print("  量子化: %d 段では 256 色を超えたので %d 段へ落とした" % (levels, L))
            print("  量子化 %d 段 — 最大 %d 色/フレーム" % (L, worst))
            q = float(L - 1)
            return [np.round(np.clip(f, 0.0, 1.0) * q) / q for f in frames]
    raise RuntimeError("4 段でも 256 色に収まらない —— 版面を見直すこと")
...
def act_sdf3d(nf):
    ...
    for i in range(nf):
        ...
        # 面積比例サンプリングは画素ごとにポアソン分布するので、表側の点が
        # 1 つも落ちない画素で裏面が透け、胡麻塩になる。実測(中央値から
        # 1.5 以上外れた画素の数): 25 万点 = 715 画素、100 万点 = 35 画素。
        pts = fs.mesh_sample_points(Vr, F, n=1000000, method="area", seed=0)
