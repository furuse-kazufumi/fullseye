...
# 壁時計の実測時間は **図にもキャプションにも焼かない**。
#
# 「同じコマンドで再生成すると SHA-256 一致」と「図に焼く数字は実測のみ」は、
# 時間に関しては両立しない ― 実測時間は毎回変わるので、焼いた瞬間に画素が変わる。
# 有効数字 1 桁に丸める折衷も試したが、他の作業と CPU を取り合う環境では
# 同じ処理が 375 ms と 415 ms(丸めれば同じ 400)にも 17 ms と 32 ms
# (丸めると 20 と 30 で **変わる**)にもなり、安定しなかった。
# そこで図には **厳密に決まる量**(メモリの実測 nbytes、voxel 数、run 数、
# 集合演算の結果 voxel 数)だけを焼き、時間の実測値は ``_wing3d_meta.json``
# に置く。数字を捏造しないまま決定性を保つための線引き。
...
def ex_mpr(log) -> dict:
    ...
    return {
        ...
        "caption": ("同じ 1 点を 3 方向から見る MPR。axial(`vol[z]`)・coronal"
                    ...
                    "(3 直交断面そのものは配列の基本スライス、コントラストは "
                    "`vol_window_level`、線は `imagedraw` の op)。"),
        **info}
...
def main(argv=None) -> int:
    ...
    # --- キャプション原稿(記事に差し込まれる本文なので、運用メモは入れない)---
    ja = [
        "<!-- tools/gen_wing3d_gallery.py が自動生成。記事本体 (docs/articles/*.md)"
        " には手を入れていません。数値はすべて生成時の実測で、"
        "`_wing3d_meta.json` に同じ値が入っています。 -->",
        "",
        "# 紙面の科学館 ―― 3-D 計測ウィング",
        "",
        "ボクセルと点群を **測る** ための一角です。すべて Fullseye の登録 op の実出力で、"
        "素材は合成データのみ(実データ・AI 生成素材は使っていません)。"
        "図に焼き込んだ数字は 1 つ残らずその場の計算結果で、乱数は seed "
        f"`{SEED}` 固定なので再生成でバイト列が一致します。",
        "",
        "断層(スライスを送る)ものは静止画にせず GIF にしてあります。"
        "各コマに位置・単位・実測値を焼いてあるので、止めた 1 コマだけでも読めます。",
        "",
    ]
    en = [
        "<!-- Generated by tools/gen_wing3d_gallery.py. The article body"
        " (docs/articles/*.md) is left untouched. Every number is measured at"
        " generation time; the same values are in `_wing3d_meta.json`. -->",
        "",
        "# A Science Museum on Paper — the 3-D Metrology Wing",
        "",
        "A wing about **measuring** voxels and point clouds. Every picture is the real"
        " output of a registered Fullseye op, on synthetic data only (no real scans and"
        " no AI-generated material). Every number burned into a figure is computed on"
        f" the spot, and the seed is pinned to `{SEED}`, so a regeneration is"
        " byte-identical.",
        "",
        "Anything that steps through slices is a GIF rather than a still. Each frame"
        " carries its position, units and measured values, so a single paused frame"
        " still reads.",
        "",
    ]
    for m in ordered:
        ja.append(_md_line(m, "ja"))
        ja.append("")
        en.append(_md_line(m, "en"))
        en.append("")
    for path, lines in ((CAPTION_PATH, ja), (CAPTION_PATH_EN, en)):
        with open(path, "w", encoding="utf-8", newline="\n") as fh:
            fh.write("\n".join(lines))
        log(f"captions -> {path}")

    # --- 生成物の実測(運用側の記録。記事には入れない)---
    man = ["# wing3d 生成物の実測(読み戻して確認した値)", "",
           f"生成: `py -3.11 tools/gen_wing3d_gallery.py`  seed `{SEED}`", "",
    ...
    for m in ordered:
        if m["kind"] == "gif":
            ...
            man.append("| %s | %s | `media/%s.gif` | %d フレーム, %dx%d, %.2f MB%s | `%s` |"
                       % (m["exhibit"], "GIF+mp4" if m.get("mp4_bytes") else "GIF",
                          m["name"], m["frames"],
                          m["gif_shape"][1], m["gif_shape"][0],
                          m["gif_bytes"] / 1e6, extra, m["gif_sha256"][:16]))
        else:
            man.append("| %s | PNG | `%s.png` | %dx%d, %.0f kB | `%s` |"
