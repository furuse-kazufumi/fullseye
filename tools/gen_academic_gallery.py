...
def generate_ai(prompt: str, slug: str) -> tuple[str, dict] | None:
    """gpt-image-1 (quality medium), fallback dall-e-3. Cached; billed at most once."""
    ...
    key = _openai_key()
    if not key:
        log(f"  [skip] no OPENAI_API_KEY for {slug}")
        return None
    attempts = [
    ...
    for body in attempts:
        try:
            ...
            if "b64_json" in item:
                ...
                blob = http_get(item["url"])
            with open(cache, "wb") as f:
                f.write(blob)
            meta = {
                "title": f"AI-generated simulated data ({body['model']})",
                "source": "generated by OpenAI " + body["model"],
                "license": f"AI-generated (OpenAI {body['model']}) -- simulated data, NOT a real specimen/scan",
                "credit": f"OpenAI {body['model']}",
                "model": body["model"],
                "prompt": prompt,
            }
            with open(metap, "w", encoding="utf-8") as f:
                json.dump(meta, f, ensure_ascii=False, indent=1)
            AI_GENERATED_NOW.append(f"{slug} ({body['model']})")
            log(f"  [ai] generated {slug} with {body['model']}")
            return cache, meta
            ...
            try:
                ...
                pass
            log(f"  [ai] {body['model']} failed for {slug}: HTTP {e.code} {detail}")
        except Exception as e:
            log(f"  [ai] {body['model']} failed for {slug}: {e}")
    return None
