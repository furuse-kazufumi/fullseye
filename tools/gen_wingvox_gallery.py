    ...
    "mesh_turntable": {
        "ja": ("色付きメッシュのターンテーブル",
               lambda f: (
                   ...
                   "粒が縦に伸びて見えるのは**そのほうが正しい**からで、z の刻みが面内の "
                   "2.5 倍あるためである(展示 4 と同じ話)。"
...
def _write_exhibit_md(results: dict, lang: str, log) -> str:
    ...
    gen = {"ja": ("tools/gen_wingvox_gallery.py が自動生成。記事 md への挿入候補で"
                  "あり、このファイル自体は記事ではない。数値はすべて生成時の実測値。"),
           "en": ("Generated by tools/gen_wingvox_gallery.py. This file is a draft to "
                  "paste into the article, not the article itself; every number was "
                  "measured at generation time.")}[lang]
