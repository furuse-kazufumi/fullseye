...
def ex_cosmic():
    ...
    data = {"truth_px": int(hit.sum()), "detected_px": int(mask.sum()),
            ...
            "bad_threshold": bad, "n_bad_raw": n_bad_raw,
            "n_bad_cleaned": n_bad_cleaned, "sum_raw": sum_raw,
            "sum_cleaned": sum_cleaned,
            "removed_pct": 100 * (1 - sum_cleaned / sum_raw),
...
def _captions(meta):
    ...
    add("sqrt_n",
        ...
        "しか外れない**。下段の右 2 枚は残差そのものを**同じ尺度**で塗ったもので、"
        "1 枚では画面いっぱいに散っていたものが 64 枚では見えなくなる ―― "
        "星の位置には何も残っていない(発散配色。赤緑の対は使っていない)。"
