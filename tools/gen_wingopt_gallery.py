...
#: ここで使う 232 画素のタイルが実際に張る半画角は 0.764 度、cos^4 = 0.99964 で、
#: **絵として見えない量**だからである。周辺光量落ちを見せるのは展示 3
#: (cos4_falloff)の仕事で、そちらは系の実画角から別に計算している。
#:
#: 2026-09 まではもっと強い理由があった ―― ``visiondesign.image_formation`` の
#: cos^4 が**配列の正規化半径**に掛かっており、16x16 の切り出しでも 2448x2048 の
#: 全面でも角が一律「画角 45 度」= 0.2500 になっていた(既定のままだと実物の
#: 2800 倍の周辺光量落ちを焼くことになる)。現在は ``image_distance_mm`` を要求する
#: fail-closed になり、全面の角は 0.967176(``system_feasibility`` の 0.9671480 と
#: 一致)、この 232 画素タイルは 0.999647 を返す。**この修正で ``render_part`` 経由の
#: 展示 detect_map / pixel_pitch の実測値が動いた**(タイルの角が暗くならなくなった
#: ぶん検出が良くなる)。
    ...
    if args.verify and results:
        ...
        for name in list(results):
            ...
            if out["fps"] is None:
                ...
                info2 = _save_flipbook(frames, name, lambda _m: None)
            else:
