...
def cmd_toc():
    ...
    try:
        ...
        fp = "?"
    # ★入口は 6 言語で出す(2026-09-09)。葉(Studio の op ヘルプ)は 6 言語
    #   10,191 ページあるのに、**そこへ導く索引が日本語だけ**だった —— 訳は
    #   在るのに辿り着けない、という形の欠落。枠の文言は `T()` に載せるので、
    #   対訳の穴は既存の門(test_chrome_translation_table_has_no_holes)が見る。
    for lang in LANGS:
        top = _top_index_lines(lang, fp, dims, dim_counts)
        name = "INDEX.md" if lang == "ja" else f"INDEX.{lang}.md"
...
def _lang_nav(current: str) -> str:
    """言語の切り替え行。**どの言語版にも同じ行を置く**ので、どこからでも移れる。"""
    out = []
    for code in LANGS:
        ...
        if code == current:
            ...
            out.append(f"[{label}](./{'INDEX.md' if code == 'ja' else 'INDEX.%s.md' % code})")
    return " · ".join(out)


def _top_index_lines(lang, fp, dims, dim_counts):
    top = [f"<!-- fullseye {_VERSION} · op-registry fingerprint {fp} · generated by tools/opdocs.py -->",
           ...
           T("# Fullseye Operator Docs — AI 使い方コーパス", lang), "",
           _lang_nav(lang), "",
           T("**fullseye {a0}** の op ドキュメント。op ごとの使い方を Markdown で1件1ファイル"
             "(RAD コーパス形状)に持ち、Studio の HTML ヘルプはここから一括変換で生成する"
             "(`tools/opdocs.py html`)。この目次はフォルダ階層から自動生成。",
             lang).format(a0=_VERSION), ""]
    for dim in dims:
        nc, nt = dim_counts[dim]
        top.append(T("- [{a0} operators](./{a1}/INDEX.md) — {a2} ops / {a3} categories", lang)
    ...
    top.append(T("1. まず上の一覧で**ファミリ**を選ぶ(`2d`/`3d` だけでなく、光学・PIV・"
                 "トモグラフィ等の計測ファミリにそれぞれ固有の op がある)。", lang))
    top.append(T("2. そのファミリの**使い方ガイド**(`<family>/guides/`)を読む。", lang))
    top.append(T("3. 各 op ノートの**データ種 `in → out`** が繋がるように連鎖を組む。", lang))
    top.append(T("4. 挙動が不確かなら、そのノートの**実行できる例**を走らせて GT 出力で確かめる。", lang))
    top.append("")
    top.append("---")
