...
(a) 恒等に近い (b) 定数に潰れる (c) 非決定的 (d) 遅すぎる を落とし、残りを
記述子でビン分けして各ビンの代表 1 件だけ残す。**上位 N 件を取ると同じ発見
ばかりが残る**(多様性の崩壊)ので、順位ではなく格子で間引く。
代表の選び方は品質判定ではなく**簡潔さ**(op 数 → 秒 → seed)で決める。
...
CONST_EPS = 1e-9           # 相対標準偏差がこれ未満 = 定数に潰れた
