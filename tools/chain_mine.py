...
その 1 フィールドを落として完全にビット一致する。

**秒を判断材料に混ぜないこと**が決定性の生命線。実測で 2 度踏んだ:
① 格子の代表を ``(op 数, 秒, seed)`` で選んでいた時は 95 代表中 4 件が走る
たびに入れ替わった → 選抜は ``(op 数, seed)`` に。
② ``deterministic`` の検査を掛けるかどうかを ``sec > SLOW_S`` で決めていた時は
CPU 負荷で検査の有無が変わり、フラグが None <-> True で反転した
(閾値を跨がせる実験で 40 連鎖中 6 件が反転)→ 判定材料は中身だけに。

残る唯一の負荷依存は ``too_slow`` の足切り(遅すぎる連鎖を落とす目的そのものが
時間なので不可避)。既定 5.0s に対し実測の最遅連鎖は 0.48s と桁が離れており、
通常は発火しない。
...
#: 1 op に渡す**総要素数**の上限。バイト上限(128MB)は時間の暴走を止められない:
#: 実測で ``match_shape_3d`` が 161³ の volume 2 本(計 63.7MB = 8.3M 要素、
#: 上限内)を食って **5 分経っても返らず**、1500 連鎖の本番走行がそこで止まった
#: (犯人は ``big-input: match_shape_3d (64 MB)`` のログで特定)。
#:
#: 上限の根拠(すべて実測): 生成器が作る最大は voxel 16³ = 4,096 / image2d 32² =
#: 1,024 / points 160×3 = 480。実際に有用だった中間産物の最大は 33,792 要素
#: (fpfh 記述子)・23,040 要素(polar_unwrap)。2^20 = 1,048,576 はその 31 倍の
#: 余裕がありつつ、上記のストール入力(8.3M 要素)を確実に弾く。
#:
#: **時間ではなく要素数で切る**のが肝: 秒で切ると判定がマシン負荷に依存し、
#: 記録の決定性が壊れる(同じ轍を 2 度踏んだ — docstring「再現性」節)。
MAX_OP_ELEMS = 2 ** 20
