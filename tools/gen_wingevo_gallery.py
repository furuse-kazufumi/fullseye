...
測定は ``out/wingevo/`` にキャッシュする(``--refresh`` で測り直し)。キャッシュを
消しても同じ数字が出る(すべて seed 固定)ので、キャッシュは速度のためだけにある。
"""
...
EXHIBIT_ORDER = [
    ...
    ("stage_tile", "6. champion のパイプライン図", ex_stage_tile),
    ("pipeline_walk", "7. 同じ鎖を 1 段ずつ歩く", ex_pipeline_walk),
