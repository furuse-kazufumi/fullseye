...
# ★2026-09-08: この表に居た `tb_angle_3points` と `tb_indices_to_labels` は、
# 「図の定義域の外」ではなく **橋に載せてはいけない op** だった。前者は 3 本の
# ベクトルを取るので点群 1 本では呼べず、後者は 1-D を返すのに宣言 out が
# `labels`(→ volume = ndim 3)。どちらも登録されたまま**一度も走ったことが無く**、
# fail-soft がもっともらしい値を返していた。`backends_typed._OP_BRIDGE_SKIP` へ
# 移し、この表からは外した —— 図が出ないことの記録は在ったのに、走れないこと
# 自体の記録がどこにも無かった(門が 2 つあって、片方だけが気づいていた)。
DOMAIN_MISMATCH = {
