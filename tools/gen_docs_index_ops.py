...
#: 日本語であることの印。かな **と漢字**を見る。
#: ★2026-09-14: 長らく かな だけを見ていたので、「Studio 北極星」「実測記録」
#: のように **漢字だけで書かれた題に印が付かなかった** —— 非日本語版の読者は
#: それを英語の題だと思ってクリックする(印を付けないのは「読めない」という
#: 事実を隠すことで、無訳より悪い、というのがこの関数の趣旨そのもの)。
#: 題は常に日本語版ファイルから取る(``_doc_title(rel)``)ので、漢字を足しても
#: 中国語の題を誤って日本語と呼ぶことは起きない。
_KANA = re.compile(r"[぀-ヿ一-鿿]")
