...
def _with_progress(frames, height=5):
    ...
    読者がクリップのどこを見ているか分かるようにするため。かつては
    ``video.write_video`` の GIF 経路(Pillow)が**連続する完全に同一のフレームを
    1 枚に畳んだ**ので、静止の「間」を作るために同じ grab を並べると枚数が
    食い違った(18 枚書いて 6 枚しか戻らない)。現在の ``write_video`` は
    重複フレームも保存するため、このバーは畳み対策ではなく純粋に読みやすさの
    ためのものになった(``save_gif`` の枚数照合は残す)。
    """
    out = []
    n = len(frames)
...
def voxel_mesh_to_world(Vz, F):
    ...
    35746 とほぼ一致)、``[:, ::-1]`` 後は **-37294.7**。かつてこの状態を
    ``cadmap`` に渡すと裏面カリングが本来の遮蔽面を捨てて可視率が 85.7 % と
    過大に出た(正しくは面積比で約 51 %)。現在の ``cadmap`` は巻き方向を
    検めて直し ``winding_fixed`` で報告する(``cad_visible_faces`` は既定で拒否)
    ので黙って通ることは無いが、**呼ぶ側で正しく直すのが本筋**なので、ここでは
    面の巻き方向も同時に裏返して打ち消す。
    """
...
def ex_normals():
    ...
    lab += [
        ...
        (24, 52 + S + 36,
         "②と④は world 法線をそのまま RGB にしたもの。**色 = 向き** なので、"
         "面が滑らかに繋がっていれば色も滑らかに繋がる。ごま塩状にまだらなら"
         "巻き方向(向き付け)が壊れている、という読み方をします。",
         C_TEXT, 13, False),
        (24, 52 + S + 60,
...
def ex_depth3d():
    ...
    for i in range(n):
        ...
        lab += [
            ...
            (30, 52 + 300 + 78, "画素中心は整数添字(ライブラリ共通)", C_DIM, 12, False),
            (30, 52 + 300 + 96, "逆投影 -> 再投影の残差 rms %.2e px" % reproj_rms,
             C_VIOLET, 12, True),
            (30, 52 + 300 + 114, "+0.5 を足すと %.5f world 単位ずれる" % off_half,
             C_DIM, 12, False),
