...
EXAMPLES = [
    ...
    {"id": "poc_dic_strain", "task": "metrology", "data": "synthetic",
     ...
     "summary": "斑点の中心を変形写像で移してから描き直すので、真値が補間を通らず厳密。"
                "変位は偏り 0.004 px / 散らばり 0.008 px(ゼロ点比 43 倍)。"
                "**剛体回転 2 度で微小ひずみが -699 µε(鋼の降伏の 3 割)を返す**が、"
                "Green-Lagrange なら代数的に 0 —— ひずみの定義に既定を置いてはいけない。"
                "偏りは推定器だけの性質ではなく、斑点を太くすると 20 分の 1 に減る。"},
