...
EXAMPLES = [
    ...
    {"id": "poc_aoi_ct_traceability", "task": "metrology", "data": "synthetic",
     "name": "全数の 2-D 検査と抜き取りの 3-D 検査を対応づける(格子は自分に重なる)",
     "summary": "規則正しく並んだ接合部は点の配置だけでは装置間で一対一に対応づかない"
                "(6x5 の格子は 30 点すべての距離署名が縮退し、署名は 9 種類しかない)。"
                "3 辺が異なる基準マークを入れると対応が厳密に決まる。そのうえで AOI の"
                "見かけのボイド率は CT の体積率と相関 0.973、悪い順 5 個も 5/5 一致するのに、"
                "寿命に効く界面接触ボイド 27 個のうちその 5 個で拾えるのは 19 % だけ —— "
                "順位が合うことは危ない個体を拾えることを意味しない。"},
    ...
    {"id": "poc_lightfield_depth", "task": "depth", "data": "synthetic",
     ...
     "summary": "既知の深度から合成ライトフィールドを作り、EPI 傾き・焦点度・2 眼を並べる。"
                ...
                "という切り分けまで数字で置く。"},
    {"id": "poc_compound_eye", "task": "optics", "data": "synthetic",
     "name": "ハエの複眼は光場センサ(神経重ね合わせで「重ねると頑健」を光学で測る)",
     "summary": "個眼アレイをプレノプティック系として設計し、同一点を N 個眼で重ねたときの"
                "SNR 利得を測る。★小開口では √N がほぼ厳密(N=5 で 2.25 対 √5=2.24)、"
                "大開口では補間誤差が平均化されず**飽和する**(N=49 で 5.33 対 7.00)—— "
                "ハエの ~6 重ねは膝の手前。アレイでこそ距離画像が出ること、"
                "少数派の遮蔽者は median 重ねなら貫けることも実測する。"},
    {"id": "poc_spc", "task": "imgmetrics", "data": "synthetic",
     "name": "統計的工程管理を op の連鎖で(検査計測が管理下か・能力があるか)",
     "summary": "マシンビジョンの計測列に Xbar-R 管理図 / CUSUM / 工程能力 Cp-Cpk / 多変量 Hotelling T² を掛ける。"
                "+4σ の逸脱は管理図が即座に、+0.8σ の持続ドリフトは CUSUM が Shewhart 3σ(0 件)より早く捕らえ、"
                "中心が仕様中点なら Cpk=Cp、多変量 T² は相関する計測の同時ドリフトを 1 判定にまとめる。"
                "どれも標準・教科書の閉じた式で、ISO 8258 定数・CUSUM の傾き d−k・T² 平均行 0 を厳密に検査する。"},
    {"id": "poc_fly_vision", "task": "optics", "data": "synthetic",
     "name": "ハエの視覚前段を op の連鎖で(合成の空を回る/前進すると何が読めて何が読めないか)",
     "summary": "六角格子 721 個眼 → 受容野 → ラミナの DC 落とし → HR 相関器 → HS 対向和を繋ぎ、"
                "1/f の帯つき空を回ると自己回転の向きと波形が読める(相関 +0.785、膜 LP つき +0.880)。"
                "★ラミナ段(DC 落とし)を省くと相関器は DC × 高域通過の揺れを出して +0.504 に落ちる。"
                "前進は全視野だと回転に化け(偏り −0.689)、上半視野に限っても受容野が地平線を"
    ...
    {"id": "xlsx_report", "task": "workflow", "data": "synthetic",
     ...
     "summary": "mdio.report と同じ (見出し, value, sort) の列から現場が使う .xlsx を作る。測定表・点群・"
                "スカラはセルに、画像はサムネイルを 1 枚埋め込む(openpyxl、optional)。書いて openpyxl で開き直し "
                "セル値と埋め込み画像数を assert。jsonio=機械 / mdio=読む / xlsxio=配る の第 3 系統。未知 sort は断る。"},
    ...
    {"id": "raw_to_display_isp", "task": "color", "data": "synthetic",
     ...
     "summary": "黒レベル → 欠陥画素 → 周辺減光 → AWB(RAW 側)→ 双線形デモザイク → CCM → 色相彩度 → 明暗を"
                "全部閉じた式で。植えた台座・減光・かぶり・欠陥 12 画素を順に外し、デモザイク後の誤差は 95 % 点で 5e-4"
                "(最大は円の縁のジッパー 6e-2 を隠さず印字)。★gray-world は場面の平均が灰でないと真の照明の逆に"
                "ならない —— 一致しない数字を並べて段の限界を見せる。"},
    ...
    {"id": "poc_allsky_cloud_cover", "task": "photometry", "data": "synthetic",
     ...
     "summary": "魚眼(等距離射影)で撮った空の雲量。**同じ角半径 8 度の雲を天頂角 0 → 82 度へ動かすと推定が 0.00789 → 0.01142(真値はどちらも 0.00973)*"
                ...
                "合計 +29.7 % になる。"},
    {"id": "fix_text_in_image", "task": "verification", "data": "synthetic",
     "name": "画像の中の文字を正しい文字列に合わせて直す(JSON 一枚の入口、MCP と同じ形)",
     "summary": "生成 AI が出した画像の誤字を再生成せずに直す。repair_flagged は床を超えた字だけ置き換え"
                "正しい字に触らず、検証を通らない置換は元に戻す。rewrite_line は行を丸ごと同じ書体で描き直す。"
                "報告の mismatch が typo(誤字)か unrelated(元の字が指示と無関係)かを分け、直せないときは "
                "reason_code(固定語彙)で断る。縁取り文字は色が多峰なので断る例つき。"},
    {"id": "poc_glyph_typo_detection", "task": "verification", "data": "synthetic",
     "name": "画像の誤字を認識せずに見つけて直す(正しい文字列を入力で貰う)",
     "summary": "正しい文字列は入力で貰えるので 6000 通りの分類は要らず、各マスを指定の 1 字と比べるだけで済む。★閾値は勘でなく**書体雑音の 95 % 点**(0.058)から導く。★★距離は"
                "平均でなく **99 パーセンタイル** —— 取り違えは部首を共有したまま一部だけ入れ替わるので、平均だと 検/横 が 0.0171 で床 0.0245 の**下**に沈み原理的に検出できない。"
                "合成では 4/4 検出・誤検出 0、置換で距離 0.0805→0.0258 と全部が床を下回る。版面が怪しい画像は**断る**(字が壊れているのか切り方が外れているのかを取り違えない門)。",
     },
    {"id": "poc_larval_connectome_reservoir", "task": "verification", "data": "real",
     "name": "幼虫コネクトームを reservoir にして数字を読む(配線は効いていない)",
     "summary": "ショウジョウバエ幼虫の完全コネクトーム(Winding 2023、2,952 ニューロン・110,677 辺)を固定の再帰網にし、"
                "読み出しだけ閉形式の ridge で学習すると MNIST の部分集合で 91.9 %(生画素の ridge は 77.6 %)。★先行研究に無い対照を置く: "
                "**次数を保って辺を繋ぎ替えた** グラフ 91.6 %、同密度の乱数 91.9 %、ガウス乱数 91.5 % —— 差は +0.3 ポイント。"
                "読み出しが使っているのは reservoir という仕組みで、進化が決めた配線ではない。設定はコネクトームの検証分割で 1 度だけ選び全対照に同じ値。"
                "データは実行時に取得して手元にキャッシュ(repo には集計と図だけ)、取れなければ合成の代替で経路だけ走らせ、その旨を印字する。"},
    {"id": "poc_malecns_activity_wave", "task": "visualization", "data": "real",
     "name": "ハエの脳の立体の上で刺激の波が配線を伝わるのを見る(コネクトーム vs 次数保存 shuffle)",
     "summary": "MaleCNS で soma の座標を持つニューロンのうちシナプス総数上位 3,000 体の部分グラフ(344,719 辺)に右の視葉だけへ刺激を入れ、"
    ...
    {"id": "poc_eye_to_brain", "task": "visualization", "data": "real",
     ...
     "summary": "MaleCNS の視葉ニューロンに付いた六角柱(個眼の柱)を使い、右眼 892 柱の視葉ニューロン + 中枢のハブ 1,400 体(4,076 体・150,487 辺)に"
                "個眼 1 つ分の刺激を入れる。刺激柱を眼の一行に沿って動かすと応答が視葉の中を同じ向きに動く: 柱座標と応答重心の相関はコネクトーム −0.92、"
                "次数保存 shuffle +0.01(網膜部位対応は配線にある)。縦縞の像を fly_hex_resample で個眼に落として入れると応答が縞を追う。"
                "Studio の Tools ▸ Compound eye → brain で同じ部品が対話的に動く(マウスで刺激位置、ドラッグで視点、画像を眼に通す、shuffle 切替)。"
                "新モジュール eyebrain(データ・柱↔個眼の対応・刺激・波・眼の絵)。生データも部分グラフも commit しない。"},
    ...
    {"id": "annotate_paper_tour", "task": "drawing", "data": "synthetic",
     ...
     "summary": "annotate_text_path_layout の弧長・位置・傾き・used を閉形式と突き合わせ(L 字経路で 0/90 度)、"
                ...
                "★反転色の崖: 補色は中間調で消える(8bit で v ∈ [113,142] の 30/256 階調 = 11.7 %、"
                "v=128 の比は 1.014 = ほぼ同じ色)。明→暗の傾斜に線を引くと消える列は x=(1-v)(W-1) の"
                "予測どおり 97-122。逃げ道 mode=\"contrast\" は全階調で最悪 4.61(連続の下界 4.583)。"
                "★表(annotate_table): 桁幅はその桁の全セルを実フォントで測った最大(文字数で数えると"
                "和英混在で崩れる)、桁間は行高×0.6 なので font_size を 2 倍にすると桁間も 2.00 倍、"
                "align=\"auto\" は数の桁だけ右そろえで全部が同じ縁で終わる。"},
