...
EXAMPLES = [
    ...
    {"id": "poc_texture_rotation_identity", "task": "shape_descriptors", "data": "real",
     "name": "回しても素材を見分けられるか(実写テクスチャ 3 種 x 12 角度)",
     "summary": "問うのは「記述子がどれだけ動いたか」ではなく**「回した自分は、まだ別の素材より近いか」**(取り違えの回数 = 判断のレベル)。素材は CC0 の実写 brick / grass / gravel で、異方(brick の目地)と等方(grass, gravel)を混ぜてある。★GLCM は角度 0 度固定なので向きに依存する。**未使用だった `b` を 4 方向平均に配線**した(既定 b=0.5 は 0 度のままで既存の結果は不変)。★★予測を外した: 「異方な brick で平均が逆効果」と読んでいたが、効き方を決めていたのは素材の異方性ではなく**共起距離 a** だった —— 距離 1 で 3/36 -> 0/36 と効き、距離 4 では 3/36 -> **8/36 と悪化**する。原因は平均ではなく**分母**で、距離を伸ばすと 3 素材の 0 度での差そのものが 0.0448 -> 0.0221 と半分に潰れる。★LBP は「回転不変」符号化(uniform)でも 19/36 -> 18/36 としか変わらず、**名前に不変と付いていても実写では半分前後を取り違える**。最も異方な brick が 11/12 と一番悪い。"},
