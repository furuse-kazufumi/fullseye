...
gauge 自由度(相似変換 7-DOF)は **先頭カメラを固定**して回転+並進の 6 を除き、残る **scale は
「構造のカメラ 0 中心からの RMS 距離を初期値に保つ」残差**で固定する(2026-09-02 実測: scale を
放置すると LM は再投影 0 のまま scale を ×0.7〜×213 まで滑らせた — 再投影誤差は相似変換に
厳密に不変なので、拘束が無ければ解の scale は数値的偶然で決まる)。
