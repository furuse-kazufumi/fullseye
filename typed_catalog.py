...
PARAM_HINTS = {
    ...
    # --- DEM 解析(2026-09-06) ------------------------------------------
    # `cell_size` は 10 op で**必須**(既定値を置かない設計 —— 標高とセル寸法の
    # 単位が食い違うと傾斜が桁で狂うため)。ここに無いと 13 op 中 10 op が
    # 「引数が組めない」で永久にスキップされ、発見ゼロに見える。
...
OP_PARAM_HINTS = {
    ...
    ("add_metrology_object_rectangle2_measure", "l1"): lambda rng: 10.0,
    ("add_metrology_object_rectangle2_measure", "l2"): lambda rng: 6.0,
    ...
    ("add_metrology_object_ellipse_measure", "ra"): lambda rng: 10.0,
