...
**Documentation: [furuse.work](https://furuse.work/)** — the operator index (2,016
operators in the machine-readable index, `docs/OP_INDEX.json`: 931 single-input 2-D operators +
17 n-ary + 1,068 in the typed ledgers, each with a per-operator note), the family guides, and the
release notes.
...
<!-- Animated hero: 6 acts of real operator output (edge orientation, blob
     selection sweep, sub-pixel calipers, SDF -> marching cubes, LiDAR
     clustering, lens defocus). Regenerate with `py -3.11 tools/gen_hero_gif.py`
     (deterministic: same bytes on a re-run). A static first frame lives beside
     it as fullseye_hero_poster.png for renderers that do not animate. -->
[![Fullseye in motion — six acts of real operator output: edge orientation, connected-component selection, sub-pixel calipers, SDF marching cubes, LiDAR clustering, lens defocus](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/fullseye_hero.gif)](https://furuse.work/)
...
| Look up any of the 2,016 operators in the machine-readable index | `docs/ops/INDEX.md` (full TOC) · `docs/OP_CATALOG.md` (one-page catalog) |
