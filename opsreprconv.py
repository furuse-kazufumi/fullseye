...
産む op はあるのに食う op が無い型 = **死んだ語彙**。この台帳の 30 op は
そのうち **13 型** に出口を作り、``gaussians`` と ``score`` については
**産む op すら 1 つも無かった**ので入口も作った(入口と消費側は必ず一緒に、
がこの repo の規律 — opsmotionmag の ``video``、opsphoton の ``histcube`` と同じ)。
...
示してから型を足した。本モジュールにはその根拠が 1 件も無い —— 30 op すべての
...
#: 宣言 out 型と素の返りの橋渡し。**意図的に空** —— 30 op すべてが宣言型
...
#: 文書化済みの非有限を返す op。**空** —— 30 op のどれも非有限を返さない
