...
def shape_mahalanobis(model, shape, align: bool = True, eps: float = 1e-12):
    """モデルから見てその形がどれだけ**異常**か。→ float。

    ``sqrt(sum(score_i^2 / variance_i))``。分散がほぼ 0 の成分は
    (標本数が足りずに潰れた軸なので)**捨てる** —— 残すと 0 割りで無限大が出て、
    「少しずれた」と「モデルの外」の区別が付かなくなる。
    """
    m = _as_model(model)
    z = shape_project(m, shape, align=align)
    v = np.asarray(m["variance"], dtype=np.float64)
    keep = v > float(eps) * max(float(v.max()), 1e-300)
    if not np.any(keep):
        return 0.0
    return float(np.sqrt(np.sum(z[keep] ** 2 / v[keep])))
