...
AUTO_SPECS = [{'halcon': 'tan_image',
 ...
 {'halcon': 'get_region_contour',
  ...
  'doc': '領域の輪郭（境界線、1 画素幅）を抽出する。領域からその収縮版（1 回の二値収縮）を引いて輪郭を残す。**穴のある領域では外周だけでなく穴の輪郭も返る**（実測 2026-09-16: 8x8 の枠に 4x4 の穴を開けた入力で、穴に隣接する前景 20 画素のうち 16 画素が輪郭として立つ）。「領域 − 収縮版」は定義上、外側からも穴の側からも 1 画素を残すので、外周だけを取り出したいなら穴を先に埋めること。a, b は未使用。``boundary`` seed op '
         'と同じ実装。\n'
         '\n'
