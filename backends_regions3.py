...
DOCS = {
    ...
    "r3_clip_region": (
        ...
        "窓の大きさは ``wh = max(1, round(a*H))`` 行 × ``ww = max(1, round(b*W))`` 列"
        "で、左上を ``((H-wh)//2, (W-ww)//2)`` に置く（端数は上・左に寄る）。"
        "``a``, ``b`` は [0,1] に clip、非有限なら 0.5。``a=b=1`` で入力そのまま、"
        "``a=b=0`` でも中央の 1 画素は残る（全零にはならない）。入力は ``> 0.5`` で"
        "前景マスクに直すため、グレー値や NaN は背景扱い。\n\n"
        "返り値は入力と同形の float64 0/1 マスク。窓は領域の外接矩形ではなく画像"
        "全体を基準にする点に注意（領域が画像の隅にあると ``a=b=0.5`` で丸ごと"
        "消えることがある）。領域基準で相対クリップしたいなら ``hx_clip_region_rel``。"
        "窓の外を消すのであって窓自体を前景にはしない（領域が無い所は 0 のまま）。"
        "画像側で同じ中央窓を切るのは ``it_crop_domain``（こちらは ``a`` 1 つで"
        "縦横同じ割合）、輪郭の点を窓で残すのは ``xg_crop_contours``。例外時は"
        "0 マスクに落ち、backend_safe の台帳に記録される（strict モードでは再送出）。"
    ),
    "r3_eliminate_runs": (
