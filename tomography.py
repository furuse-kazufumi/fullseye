...
def metal_trace_interpolate(sinogram, threshold=None, mask=None):
    ...
    Measured on the Shepp-Logan phantom with a 6-px, density-8.0 implant added:
    normalised RMS error against the metal-free truth goes 0.0999 (uncorrected)
    -> **0.0299** outside the implant, a 3.3x improvement, while inside the
    implant footprint the reconstruction is now wrong on purpose (the metal is
    gone). Clinical practice puts the metal back afterwards from a separate
    thresholded reconstruction; that step is not here.

    :param sinogram: ``(n_angles, n_detectors)``.
    :param threshold: bins above this value are treated as metal; ``None`` ->
        ``mean + 3*std`` of the sinogram, which is a starting point and not a
        rule.
    :param mask: explicit boolean ``(n_angles, n_detectors)`` metal trace;
        overrides *threshold*.
    :returns: ``(n_angles, n_detectors)`` float64 sinogram.
    :raises ValueError: if the mask shape disagrees with the sinogram, if the mask
        is all True (nothing is left to interpolate from), or if any *row* is
        entirely masked.
    """
    op = "metal_trace_interpolate"
    sino = _as_sinogram(sinogram, op)
    if mask is None:
        thr = (float(sino.mean() + 3.0 * sino.std()) if threshold is None
               else _finite_scalar(threshold, "threshold"))
        m = sino > thr
    else:
        m = np.asarray(mask)
        if m.dtype != np.bool_:
