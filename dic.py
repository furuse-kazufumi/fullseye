...
以下がその棚卸しで、この層が 3 op しかない理由そのものである。
  ...
  E. 相関品質(点ごと)… **別物がある。測ったら肝心な失敗に盲目**
     ...
     同じ実験を `piv_synth_pair` の粒子像(``diameter_px=3.0``)で繰り返すと
     数字は動くが順位は動かない: 門なし 1.9122 → ``zncc>=0.8`` で 0.0036
     (**537 倍**、82.8 % 残る)、``peak_ratio>=1.2`` で 1.0952(1.7 倍)。
     **peak_ratio が役に立たないのではなく、桁が 2 つ足りない。**
     `tests/test_dic.py` が後者の数字で固定してある。
