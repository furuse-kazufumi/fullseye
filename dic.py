...
1. **輝度不変を仮定する推定器と ZNCC は、勝つ土俵が違う。** 照明が動かない
   合成データでは Lucas-Kanade のほうが精度が高い(下の実測)。ZNCC を採る
   理由は精度ではなく、**平均と標準偏差の両方を引くので、ゲインとオフセットが
   変わっても答えが動かない**こと。実験室ではこちらが効く。
2. **ひずみの定義を既定にしてはいけない。** 2 度の剛体回転が微小ひずみでは
   数百 µε の嘘を作る(`strain_from_displacement` の docstring に実測)。
   だから ``window`` も ``method`` も既定値を持たない。

★ 実測(256x256 の解析スペックル、斑点 3000 個、1σ 半径 1.6 px、MARGIN 40 の
内側だけで評価。`dic_dense` は subset=31 / step=8 / search=8。lk は
`optical_flow_lk(window=21, levels=3, iters=6)`)

  (a) サブピクセル掃引 —— **lk の勝ち**

        u 真値    zncc 偏り  zncc 散らばり |  lk 偏り   lk 散らばり
        0.000      0.0000      0.0000      |  0.0000     0.0000
        0.125     -0.0060      0.0068      | -0.0032     0.0051
        0.250     -0.0075      0.0091      | -0.0025     0.0067
        0.375     -0.0037      0.0093      |  0.0006     0.0077
        0.500      0.0007      0.0090      |  0.0038     0.0077
        0.625      0.0041      0.0091      |  0.0060     0.0074
        0.750      0.0058      0.0090      |  0.0067     0.0069
        0.875      0.0043      0.0083      |  0.0048     0.0058
        1.000     -0.0000      0.0074      |  0.0000     0.0044

      最大の偏り zncc 0.0075 px / lk 0.0067 px、最大の散らばり
      zncc 0.0093 px / lk 0.0077 px。**どちらの指標でも lk がわずかに良い。**
      ZNCC の偏りは 3 点ガウス当てはめが持つ既知の系統誤差(相関の峰は
      厳密にはガウスではない)で、枚数を増やしても消えない。

  (b) 一様ひずみ(window=31)—— **lk の勝ち**

        ε 真値 µε   zncc 平均 µε  zncc 散 µε |  lk 平均 µε   lk 散 µε
             100         96.5        41.5    |     101.2       23.9
             500        496.8        41.6    |     501.3       24.0
            2000       1996.3        41.9    |    2001.0       24.5
           20000      19961.9        45.5    |   20009.0       33.4

      平均はどちらも真値のまわり 0.4 % 以内。**散らばりは lk が 1.7 倍良い**
      (格子を内挿しているぶん ZNCC の点数が実効的に少ない)。

  (c) ★★照明変化(u=0.37 px のまま ``cur = 0.7 cur + 0.15``)—— **ZNCC の圧勝**

        推定器            偏り px    散らばり px
        zncc            -0.0037       0.0093     ← 照明なしと**完全に同じ値**
        lk              -0.3700       0.0000     ← 変位 0 と答える(全域で破綻)

      lk の偏り -0.3700 は「u=0 と答えた」ことを意味する。ゲイン 0.7 は
      輝度不変の仮定を根から壊すので、ピラミッド全段で解が 0 に落ちる。
      **ZNCC 側は小数点以下 4 桁まで照明なしと一致する**(平均と標準偏差の
      両方を引いた量なので、アフィンな輝度変換に対して代数的に不変)。

  (d) 剛体回転 2 度(window=31)—— ひずみの定義が作る嘘

        推定器 / 定義          exx 平均 µε
        理論(cosθ-1)            -609.4
        zncc  infinitesimal      -607.5
        zncc  green                -0.2
        lk    infinitesimal      -609.3
        lk    green                -1.5

      **2 度回っただけで微小ひずみは -600 µε を返す。鋼の降伏ひずみの 3 割。**
      Green-Lagrange にすると 1 µε 台に落ちる(残りは推定器の誤差)。

  結論: **精度で ZNCC が LK に勝つ場面は無い**(a も b も負け)。勝つのは
  (c) の照明変化だけで、そこは勝ち負けではなく「片方が動かない」。
  この層を出す理由は (c) と、LK が持っていない相関品質マップ・ひずみ op・
  真値つき合成器のほうにある。
...
def dic_correlate(ref: Any, cur: Any, subset: int = 31, step: int = 8,
    ...
    ★ ZNCC(zero-mean normalised cross-correlation)—— なぜ平均と標準偏差の両方か
      ...
      f 自身との相関に厳密に等しい。実測: ``cur`` を ``0.7 cur + 0.15`` に変えても
      変位は小数点以下 4 桁まで動かない(モジュール docstring の (c))。
      同じ条件で `optical_flow_lk` は変位 0 を返す = 全域で破綻する。
      **これがこの op を書いた唯一の理由。素の精度では LK に負ける。**
    ...
    ★ サブピクセル —— 3 点ガウス当てはめ
      ...
      **この当てはめは系統誤差を持つ。** 相関の峰は厳密にはガウスではないので、
      サブピクセル位置に依存する偏りが残る(実測、u を 0.125 刻みで掃引):

          u 真値   0.000  0.125  0.250  0.375  0.500  0.625  0.750  0.875  1.000
          偏り px  0.0000 -.0060 -.0075 -.0037 0.0007 0.0041 0.0058 0.0043 -.0000

      整数の u では厳密に 0、1/4 付近で最も負、3/4 付近で最も正。**枚数を
      増やしても消えない**(乱数ではなく当てはめの形が作る誤差)。
      0.01 px を主張するなら、この 0.0075 px を先に引き算できる根拠が要る。
      なお同じ条件の `optical_flow_lk` は最大 0.0067 px なので、**偏りでも
      散らばりでも LK のほうがわずかに良い**。
