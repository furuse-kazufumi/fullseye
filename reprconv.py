...
def keypoints_to_image2d(keypoints, shape=(64, 64)):
    ...
    最大 0.5 画素ずれる(実測 RMS 0.29 px = 一様量子化の理論値 1/sqrt(12) = 0.2887
    と一致)。同じ画素に 2 点が落ちると 1 点に潰れるので、往復で点数も減りうる。
...
def gaussians_to_voxel(gaussians, shape=(32, 32, 32), origin=(0.0, 0.0, 0.0),
    ...
    **不可逆**。損失は 2 つあり、どちらも数字で測れる:
      * **打ち切り** —— 3 sigma までなら球ガウシアンの質量の 97.07%(誤差関数の
        値。``selftest`` が実測で照合する)。
      * **格子化** —— 中心が格子の間にあると 1 voxel 未満の位置がずれる。
