...
【グラウンドトゥルース(数値で嘘を弾く)】
1. コピー&ムーブのシフトはこちらが決めた (112, 128)。keypoint 法・block 法の
   どちらも誤差 **0 px 厳密**で当てる。清浄画像では 1 件も出ない(偽陽性 0)。
2. 貼り付けは品質 60 で符号化してから貼った。JPEG ゴーストの最頻値が
   貼り込み内で **60**、台紙側で **90** ―― どちらも真値。
   jpeg_quality_estimate は台紙の **90 をブラインドで当てる**。
   無圧縮には ``quality=None`` と答える(「品質 100」とは答えない)。
   ELA は「その品質で不動点になる領域」を消す: 品質 90 で撮ると台紙が消えて
   内/外 = 318 倍、品質 60 で撮ると貼り込みが消えて 0.000 倍。
...
N = 256
SRC, DST, PATCH = (40, 32), (152, 160), 64          # コピー&ムーブの既知の座標
TRUE_SHIFT = (float(DST[0] - SRC[0]), float(DST[1] - SRC[1]))   # = (112, 128)
# 貼る矩形は **8 の倍数**に取る。JPEG は 8x8 の格子で符号化するので、格子から
# ずれた位置に貼ると貼り込み自身の量子化履歴が壊れ、ゴーストが出なくなる
# (実測: rows 130 から貼ると最頻品質が真値 60 ではなく 95 になった)。
SPLICE = (slice(128, 176), slice(16, 64))           # 別画像を貼る矩形
