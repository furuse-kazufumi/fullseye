...
# --------------------------------------------------------------------------- #
# 場面と機材を合成する —— ここで作った「正解」だけが後段の судья になる          #
# --------------------------------------------------------------------------- #
...
def main():
    ...
    print(f"1) 同一性(dct 64 bit): 提出 vs オリジナル = {d_same} bit  /  "
          f"無関係な 1 枚 = {d_other} bit")
    assert h_orig.dtype == np.bool_ and h_orig.shape == (64,)
    assert d_same <= 2 < d_other                # 同じ写真だと分かる
    print("   → 『同じ写真』とは言える。しかし **改竄の有無は何も言っていない** "
          f"(8x8 に潰した後では {100 * changed:.1f}% の改竄が {d_same} bit にしかならない)")
