...
1. ★★**劣化ゼロで測り直しただけで「劣化」が出る**。同じ形を密度も走査位置も
   姿勢も変えて 2 回測り、位置合わせまで済ませてから素朴な最近傍差分(C2C)を
   取ると 中央値 **21.07 mm**・最大 **42.64 mm** の「変化」が出る。真の最大劣化は
   30.00 mm なので**同じ桁**。法線方向に測れば同じ条件で 0.55 / 3.55 mm。
   しきい値 1.0 mm で「補修候補」を数えると、**劣化ゼロでも 3.562 m2 / 5.098 L**
   —— 本物(同じ数え方の真値 5.882 L)の **87 %**。
2. ★**その偽の劣化は点間隔でほぼ決まる**。予測「独立に撒いた同じ面どうしの
   最近傍距離 ≈ 0.5/√ρ」に対し、密度 300→2400 pt/m2 で実測 26.83→10.14 mm
   (予測 28.87→10.21、比 0.930〜0.994)。★★「密度を揃えればよい」は**逆**で、
   ``voxel_grid_downsample`` で両時点を 30 / 50 mm 格子に揃えると
   21.39 → 22.30 / 24.44 mm と**悪くなる** —— 揃えるとは「細かいほうを粗いほうに
   合わせる」ことなので、C2C の下限は**粗いほうの時点の点間隔**になる。
3. ★★**劣化が全域に広がっていると、合わせは平均を吸って端に逆符号を作る**。
   たわみだけを入れて全点で合わせると、支間中央のたわみ 2.999 mm は 0.933 mm に
   潰れ(吸われた割合 実測 0.689 / 閉形式 2/3 = 0.667)、支点付近には
   **-1.737 mm の「持ち上がった」という嘘**が出る(予測 -1.637)。
   変わっていない端だけで合わせても 0.346 は吸われたまま(予測 0.296)——
   **範囲を狭めてもゼロにはならない**。吸われる量は「使った範囲での劣化の平均」。
4. ★**位置合わせの残差が作る嘘は、測る前に幾何だけで出せる**。残差姿勢の作る
   見かけ変化は ``(t + ω×(p-c))·n``。その RMS に雑音床 0.750 mm を直交に足すと、
   ω = 0〜1.0e-03 rad の実測との比は **0.969〜1.010**。
   ★★同じ大きさの回転でも**軸が違えば効く面が違う**: 腹板(法線が ±y)には
   ``ω_y`` が厳密に効かず(予測 RMS 0.000)、下面・下フランジ(法線が -z)には
   ``ω_z`` が効かない(0.002)—— **1 枚の面だけで検算しても気づけない**。
5. ★**検出できる最小の劣化は角度で折れる**。ω を 0→1.0e-03 rad で振ると
   最小検出深さは 2.10 → 7.56 mm。**橋長 12 m では 1.0e-4 rad(0.0057 度 =
   20.6 秒)が支点で 0.60 mm** に相当するので、位置合わせの角度は「度」ではなく
   「秒」で管理しないと 1 mm の劣化は測れない。
6. ★★**点検を 3 回に増やしても「速度」は良くならない**。予想は「3 点の直線
   当てはめのほうが良い」だったが、**等間隔 3 点の最小二乗の傾きは両端の差分と
   厳密に同じ**(core ごとの差の最大 1.8e-15 mm/年)—— 中央の点の重みが 0 だから。
   速度の誤差 RMS は 0.863 mm/年 で、**1 mm/年 の進行は 2σ = 1.726 mm/年 の下**。
   中央の点が効くのは加速(2 階差分)だけで、その誤差は 1.179 mm/年²。
   ★これも予測(独立なら 2.570)より小さく、理由は測れる: t1 と t2 の誤差は
   同じ t0 に合わせているので相関 +0.562 を持ち、2 階差分で部分的に打ち消す。
7. ★**細い溝は「平均する」測り方には原理的に見えない**。幅 12 mm・深さ 2.50 mm
   のひび割れは足跡(半径 0.10 m)平均で **0.0955 mm**。閉形式 2dh/(πR) との比は
   0.995〜1.000 で、★**密度が式に入らない** —— 予想「密度を上げれば見える」は
   外れ。深さの半分として読むには全幅 251 mm(= 足跡の直径)が要る。
8. ★★**押し出し形状の軸方向は「決まらない」のではなく「雑音が決めてしまう」**。
   桁の n_x は設計上キャンバー勾配だけ(9.26e-05〜1.48e-07)なのに、点群から
   ``estimate_normals`` で測ると 1.02e-03〜1.07e-03 と**キャンバーに無反応**で
   設計値の 11〜7207 倍 —— 法線推定の雑音が「見かけの情報」を作っている。
   結果、ICP の x 残差は 8.28 / 3.30 / 9.24 mm と**単調ですらない**
   (y・z は 0.13 mm 以下)。★その嘘は桁の平面には出ない(法線が x に直交)のに、
   法線が ±x を向く**支承の円柱にだけ**「41.2 mm 水平に動いた」として出る
   (真値 0、測り方の床 1.64 mm)。★★さらに支承の沈下 4.00 mm は、たわみを
   吸った合わせのせいで **-0.14 mm** に消える(端だけで合わせれば 1.56 mm)。
   **面積の大半を占める大きな面の残差(1.0 mm 前後)を見ている限り、
   どちらにも気づけない**。
...
def section_observe() -> dict:
    ...
    print("    ★仕込んだ誤差の 99 %% 以上は戻せるが、残った数 mm が問題になる。"
          "しかも**軸ごとに理由が違う**:")
    print("      x は押し出し形状なのでキャンバーの傾きからしか決まらず(8 節)、"
          "z にはたわみを吸った分が乗る(5 節)。")
...
def section_control(sc: dict) -> dict:
    ...
    print("     ★(c) が真値の %.2f 倍になるのは、本物の欠損に**偽の劣化が上乗せ**"
          "されるから —— \n     測り方だけで %.2f L、劣化だけで %.2f L、"
          "両方だと %.2f L。1 個の数字の中で分けられない。"
          % (out["(c) 劣化 + 測り直し"][1] / v_true, fv,
             out["(b) 劣化のみ・測り方は同一"][1],
             out["(c) 劣化 + 測り直し"][1]))
