...
1. **ゼロ点(位置合わせせずに最近傍距離)は 19.09 mm** = 公差 0.10 mm の 191 倍。
   重心だけ合わせても 3.36 mm(34 倍)。**差分検査の主要な誤差源は測り方ではなく
   位置合わせそのもの**。
2. **姿勢の誤差[度]と偏差の誤差[µm]は別々に数える**。FPFH の粗合わせだけだと
   姿勢 1.685 度 / 点の移動 1.725 mm で、偏差の RMS は 324.8 µm。点-面 ICP まで
   通すと姿勢 0.0364 度 / 0.0886 mm、偏差 RMS 84.3 µm。★**真の姿勢を与えても
   21.3 µm 残る**ので、84.3 のうち姿勢由来は 63 µm ではない —— 差の正体は
   第 6 章の datum ずれ(反り)であって、姿勢推定の精度不足ではない。
3. ★**稜線では最近傍が隣の面へ飛ぶ**。**欠陥も雑音もゼロ・姿勢は真値**という
   対照でさえ、素朴に最近傍を採ると **63.4 mm^2 の偽の公差外領域**が出る
   (面を跨いだ対応 2.0 %、偏差 RMS 32.6 µm)。k 近傍の法線が 30 度以上割れる点
   = 7.5 %(621 mm^2)を「測れない」として外に出すと 1.1 mm^2 / 21.3 µm。
   **直すのではなく、測れない帯として名前を付けて外に出す**。
4. ★**法線を実測点から推定すると符号が裏返る。しかも予想より広く**。予想は
   「凹面(穴 2 本 + フィレット)= 面積比 14.5 % だけ」だったが、実測 **41.0 %**。
   面別の内訳が理由を言う: fillet 100 % / hole1 82 % / hole2 73 % は予想どおりだが、
   ★**平面が 0〜68 % とばらつく**(bottom 68 % / side_xm 45 % / top 11 % /
   side_xp 0 %)。平面では近傍重心が点の上にほぼ載るので「重心から離れる向き」に
   法線成分が無く、符号を決めているのは数値誤差。大域向き付け
   (``estimate_oriented_normals``)は 0.0 % で正しく向いた。
5. **雑音は必ず正へ偏る、が符号を付けると消える**。予測 σ√(2/π) は σ=20 µm で
   15.96 µm、実測 15.90 µm(比 0.997)。同じ点の符号付き偏差の平均は +0.079 µm ——
   **符号を付けるだけで偏りが 200 分の 1**。
...
8. ★**引かれ方は欠陥の大きさに比例し、傾きが閉形式と合う**。反りの振幅を
   ...
   割合が 2.47 % のまま変わらない** —— 「大きい欠陥ほど食われる」のは**面積**の話。
   ただし並進だけの閉形式 2πσ²/A = 0.93 % は 2.47 % を**過小に予測する**
   (へこみが部品中心から外れているので回転も吸うため)。
9. ★**予想が外れた: 対称の別解は、この部品では残差で見抜けた**。点-面 ICP は
   初期ずれ 75 度まで収束し、90 度で別解(姿勢誤差 91.3 度、残差は床の 14.1 倍)、
   180 度でも別解(179.9 度、4.6 倍)。予想は「残差が床のままで見抜けない」
   だったが、ボスが片側にあり穴が φ10 と φ8 で違うので対称が壊れている。
   **対照群がその境目を示す**: 特徴を取り去った素の直方体では 180 度ずれたまま
   残差が床の **1.01 倍** —— 残差が嘘をつくのは対称形のときだけ。
10. **欠測(片側スキャン)は姿勢を引くが、1 本の実行では分からない**。表の 1 行では
   欠測ありのほうが姿勢誤差が小さい行すら出る(0.0259 度 < 基準 0.0364 度)。
   種を 3 本振ると 0.0191 ± 0.0090 度 → 0.0674 ± 0.0035 度(3.5 倍)で確かに引く。
   ただし本当に危ないのは**測れなかった 4192 mm^2(50.7 %)を「公差内」と書くこと**。
