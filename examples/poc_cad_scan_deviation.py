...
"""CAD と実測点群の差分検査 —— 合わせた分だけ欠陥が消え、無い所にへこみが出る。
...
「測る誤差」を**別々に数える**ための台で、中心にあるのは
**位置合わせが吸える偏差場はたった 6 次元しかない**という一点です。
...
1. **ゼロ点(位置合わせせずに最近傍距離)は 19.03 mm** = 公差 0.10 mm の 190 倍。
   重心だけ合わせても 3.35 mm(34 倍)。**差分検査の主要な誤差源は測り方ではなく
   位置合わせそのもの**。
2. **姿勢の誤差[度]と偏差の誤差[µm]は別々に数える**。FPFH の粗合わせだけだと
   姿勢 0.291 度 / 点の移動 0.348 mm で、偏差の RMS は 105.4 µm(公差と同じ大きさ)。
   点-面 ICP まで通すと姿勢 0.023 度 / 0.093 mm、偏差 RMS 81.7 µm。★**真の姿勢を
   与えても 20.4 µm 残り、しかも 81.7 との差は姿勢の精度では説明できない** ——
   正体は第 6 章の datum ずれ(反り)。姿勢を良くしても消えない誤差がある。
3. ★**稜線では最近傍が隣の面へ飛ぶ**。**欠陥も雑音もゼロ・姿勢は真値**という
   対照でさえ、素朴に最近傍を採ると **66.5 mm^2 の偽の公差外領域**が出る
   (面を跨いだ対応 2.1 %、偏差 RMS 32.2 µm)。k 近傍の法線が 30 度以上割れる点
   = 7.7 %(636 mm^2)を「測れない」として外に出すと 1.4 mm^2 / 20.4 µm。
   **直すのではなく、測れない帯として名前を付けて外に出す**。
4. ★**法線を実測点から推定すると符号が裏返る。しかも予想より広く**。予想は
   「凹面(穴 2 本 + フィレット)= 面積比 14.6 % だけ」だったが、実測 **39.5 %**。
   面別の内訳が理由を言う: fillet 100 % / hole1 81 % / hole2 71 % は予想どおりだが、
   ★**平面・凸面が 1〜63 % とばらつく**(bottom 63 % / side_xm 37 % / top 14 % /
   side_xp 1 %)。平面では近傍重心が点の上にほぼ載るので「重心から離れる向き」に
   法線成分が無く、符号を決めているのは数値誤差。大域向き付け
   (``estimate_oriented_normals``)は 0.0 % で正しく向いた。
5. **雑音は必ず正へ偏る、が符号を付けると消える**。予測 σ√(2/π) は σ=20 µm で
   15.96 µm、実測 15.90 µm(比 0.997。σ を 16 倍振って比 0.997〜1.013)。同じ点の
   符号付き偏差の平均は +0.079 µm —— **符号を付けるだけで偏りが 200 分の 1**。
6. ★**参照点群の密度そのものが偏差の下限を決める**。無雑音・完全一致でも最近傍
   距離の平均は予測 0.5/√ρ(ρ = 点密度[点/mm^2])に従い、40000 点(ρ=4.84)で
   実測 0.2254 mm(予測 0.2273、比 0.992)—— **公差 0.10 mm を素で 2.25 倍超える**。
   法線へ射影した符号付きなら -0.0014 mm。ρ を 16 倍振っても比は 0.985〜1.004。
   ★一方**実測点のほうを 1/1〜1/50 に間引いても偏差は偏らない**(偏り +0.7 →
   +3.2 µm、分散 81.6 → 72.2 µm)。壊れるのは**公差外面積**で -8.5 → -16.7 % ——
   1/50 では 1 点が 17.2 mm^2 を代表するので、境界をその刻みでしか引けない。
7. ★★**中心的な主張: 位置合わせが吸えるのは 6 次元だけで、そこに似た欠陥ほど
   消える**。点-面 ICP が吸える偏差場は ``J = [n | X×n]`` の張る 6 次元で、
   残るのは ``d - J(JᵀJ)⁻¹Jᵀd``。読み(合否を決める値)で比べると ——
   局所へこみ 448.9 → 432.5 µm(薄まり 3.7 %、予測 437.8)、ボス全周の摩耗
   140.0 → 141.4 µm(薄まらない: 全周だと法線の平均が 0 で並進では吸えない)、
   ★**反りは真値 1.2 µm の部品中央に -121.1 µm の「存在しないへこみ」を作る**
   (閉形式の予測 -a/3 = -120.5 µm)。公差 ±100 µm なので**偽の不合格**。
8. ★**引かれ方は欠陥の大きさに比例し、傾きが閉形式と合う**。反りの振幅を
   0→600 µm と振ると ICP の並進 z が 0→-193 µm と直線に動き、傾き **-0.322**
   (予測 -1/3 = -0.333)。★**へこみは深さを 30 倍(50→1500 µm)にしても薄まる
   割合が 2.47 % のまま変わらない**。効くのは**面積**で、広がり σ を 2→10 mm と
   すると 0.81 → 8.49 %(実 ICP でも 7.50 %)。★ただし σ=15 mm では予測 17.1 % に
   対し実測 7.76 % で**予測が壊れる** —— へこみが上面(1934 mm^2)の 73 % を覆って
   法線の違う面にはみ出し、並進 1 自由度では吸えなくなるため。
9. ★**予想が 2 つとも外れた(初期姿勢の崖)**。(a)「0〜30 度のどこかで壊れる」——
   点-面 ICP は 60 度まで姿勢 0.002 度 / 点移動 0.0029 mm で**微動だにしない**。
   ところが点-点 ICP は**崖ではなく坂**で、0→30 度で 0.046 → 1.807 度・点移動
   0.048 → 1.087 mm と滑らかに崩れ、**公差 0.10 mm を跨ぐのは初期 6 度**。
   「ICP の崖」は手法を言わないと意味が無く、判定基準(収束したか / 公差を守れるか)
   でも場所が動く。(b)「対称の別解は残差では見抜けない」—— この部品では別解の残差が
   床の 5.1 倍で**見抜けた**。★対照群が境目を示す: ボスも穴も取り去った素の直方体
   では 180 度ずれたまま残差が床の **1.01 倍** —— 残差が嘘をつくのは対称形だけ。
10. **欠測(片側スキャン)は姿勢を引くが、1 本の実行では分からない**。表の 1 行では
   欠測ありのほうが姿勢誤差が大きい行も小さい行も出る(0.0639 / 0.0230 度)。
   種を 3 本振ると 0.0152 ± 0.0094 度 → 0.0527 ± 0.0085 度(3.5 倍、差は散らばりの
   4.0 倍)で確かに引く。ただし本当に危ないのは**測れなかった 4186 mm^2(50.6 %)を
   「公差内」と書くこと**。

【グラウンドトゥルース】部品は解析的な立体(直方体 60x40x12 + ボス φ18 + 根元
フィレット R2.5 + 貫通穴 φ10 と φ8)。表面積 8266.79 mm^2 と体積 28742.32 mm^3 は
閉形式(フィレットは Pappus の定理)で、体積のモンテカルロ検算と -0.064 % で一致。
面積重み一様サンプリングの面別点数比は面積比と 0.03 % 以内。偏差の真値は
「法線方向にどれだけ動かしたか」そのもの(構成で厳密)。仕込んだ欠陥の公差外面積は
へこみ 117.39 / 反り 2169.41 / 摩耗 659.69、合計 2757.18 mm^2。
...
datum 面を指定するのはこのため(第 7 章の初期姿勢の掃引だけは欠陥を入れずに
測っている —— 入れると datum ずれ 0.107 mm が下駄になって崖が見えなくなる)。

【3-D の落とし穴(踏んだもの)】点の列は (x, y, z) 固定で ``points_to_voxel`` は
その順のまま軸を作る(``(depth, row, col)`` へ並べ替えない)/ 法線の符号は
形状ではなく手法が決める(第 4 章)/ 単位は全編 mm で、公差・雑音・点間隔が
同じ土俵に乗っていないと比べられない / 最近傍距離は密度で変わる(第 5b 章)/
対称形では残差が姿勢の正しさを保証しない(第 7 章)。
...
def section_scan_density(ref: CadRef) -> dict:
    ...
    print("      (この分散は雑音 %.0f µm より大きい。第 6 章の datum ずれ"
          "= 場所ごとに一定の模様が入っているため、" % (1000 * NOISE))
    print("       密度の話としては『増えも減りもしない』ことだけを読むこと。)")
...
def section_pull(ref: CadRef) -> dict:
    ...
    print("        線形代数の予測(6 次元への射影)と実 ICP: σ=3.5 で %.2f/%.2f、"
          "σ=10 で %.2f/%.2f。"
          % (dils[1], icps[3.5], dils[3], icps[10.0]))
...
def section_basin(ref: CadRef) -> dict:
    ...
    i30 = angs.index(30)
    print("\n   ★予想は 2 つとも外れた。")
    print("   (1) 崖は**手法ごとに別の場所**にあり、「ICP の崖」という言い方に"
          "意味が無い。")
    print("       点-面 ICP は 0〜60 度で微動だにしない(姿勢 %.3f 度 / 点移動"
          " %.4f mm のまま)。" % (errs[i30], mm1[i30]))
    print("       ★点-点 ICP は**崖ではなく坂**: 0 → 30 度で姿勢 %.3f → %.3f 度、"
          "点移動 %.4f → %.4f mm と" % (errs2[0], errs2[i30], mm2[0], mm2[i30]))
    if over:
        print("       なだらかに崩れる。公差 %.2f mm を跨ぐのは初期 %d 度"
              "(そこまでは「収束した」ように見える)。" % (TOL, angs[over[0]]))
    print("       依頼の 0〜30 度に崖を探すと『無い』と答えてしまうが、"
          "**合否で見れば %d 度で既に壊れている**。"
          % (angs[over[0]] if over else 60))
...
def main() -> int:
    ...
    print("  * ★位置合わせが吸えるのは J = [n | X×n] の 6 次元だけ。局所へこみの"
          ...
          % (100 * (1 - pl["cases"]["へこみのみ"]["meas"]
                    / pl["cases"]["へこみのみ"]["true"])))
    print("    反りでは真値 %.1f µm の中央に深さ %.1f µm の**偽のへこみ**が出る"
          "(予測 -a/3 = %.0f µm)。"
          % (1000 * pl["cases"]["反りのみ"]["true"],
