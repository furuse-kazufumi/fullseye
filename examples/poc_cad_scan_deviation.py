...
class CadRef:
    ...
    def deviate(self, q):
        """(unsigned 最近傍距離, 符号付き偏差, 最近傍 index)。"""
        d, i = self.tree.query(np.asarray(q, float), k=1, workers=-1)
        s = np.einsum("ij,ij->i", np.asarray(q, float) - self.pts[i], self.nrm[i])
        return d, s, i
...
def main() -> int:
    ...
    print("  * ★欠陥は剛体 6 自由度に吸われる分だけ薄まる。局所へこみは %.1f %%、"
          "反りは %.0f %%。" % (100 * pl["cases"]["へこみのみ"]["rho"],
                                100 * pl["cases"]["反りのみ"]["rho"]))
