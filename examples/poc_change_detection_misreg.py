...
1. ★★**偽陽性は 0.3 px までは雑音の床、0.5 px から崖**。ずれ 0.1 / 0.2 / 0.3 px
   の偽陽性面積は **0 / 0 / 1 px**、0.5 px で 114 px、3 px で 7175 px。
   予想 A はエッジ総長(しきい値を超える辺の投影長 1870 px)× ずれ量の比例則で、
   3 px で実測の 0.78 倍と大筋は合うが、**0.1 px で 187 px と予測して崖を
   説明できない**。原因は PSF: 幅 σ のぼけたエッジをずらした差分の高さは
   C·δ/(σ√2π) で、これがしきい値 τ を超える δ* = τσ√2π/C = **0.351 px**
   (最も強いエッジ C=0.50 で)—— 実測(0.3 px で 1 px、0.5 px で 114 px)と
   一致した。PSF と雑音を入れた台帳予測は 0.5〜3 px で実測の **0.84〜1.07 倍**。
2. ★**台帳予測から漏れるのは樹冠テクスチャ**。森林の内側は「エッジ」を持たないが、
   樹冠の勾配 |∇I·d| が τ を超える画素は差分に出る。ずれ 1.5 px での偽陽性の
   内訳: 人工物のエッジ帯 3001 px / 森林内部 482 px / その他 65 px。森林の分は
   基準画像 1 枚の勾配から予測できて(409 px)、足すと全体の予測は実測の
   0.95 倍。★ただし勾配則は 1 次近似で、ずれが樹冠の σ(1.3〜2.2 px)に
   近づく 3 px では森林の予測 2060 px に対し実測 1653 px と過大になる。
   **エッジ総長だけではテクスチャ地物の偽陽性は数えられない**。
3. ★**回転は中央が無傷で端だけ壊れる**。1 度の回転で偽陽性率は中心から 32 px
   以内 0.5 % / 96 px 以遠 6.0 %。局所のずれは r·θ なので、崖の半径は
   r* = δ*/θ: 0.5 度なら 40 px、1 度なら 20 px —— 実測の内側ビン(<32 px)は
   0.5 度で 0.00 %、1 度で 0.5 %。台帳予測は 1 度で実測の 0.79 倍(足りない
   分は 2 と同じ樹冠)。**画像の隅で偽陽性が出る画像対は、平行移動ではなく
   回転を疑う**。
4. ★★**変化の種類ごとに壊れ方が違う**。ずれ+回転(1.3/−0.8 px、0.4 度)で
   検出率は 新設 0.88 / 伐採 0.31 / 水域 0.92。伐採は位置合わせが**完璧でも**
   0.33 —— 樹冠が消えた跡の平均輝度差は小さく、差分に出るのは樹冠 1 本ずつ
   の斑点で、面積では 1/3 しか拾えない。水域は幅 4 px の環で、ずれ 1.5 px
   相当なら環の外縁が削れる。**まとめた 1 つの IoU(0.20)にはこの内訳が無い**。
5. **位置合わせ 3 経路は同じずれ(1.3/−0.8 px、0.4 度)を残留 0.13 px 以下まで
   戻す**(PIV 相互相関→剛体当てはめ 0.048 px、特徴点→RANSAC→Procrustes
   0.127 px = 整数座標のキーポイントの床、密な LK フロー→頑健当てはめ
   0.017 px)。ただし★**唯一の位相相関経路は 3-D 用しかなく整数精度**で、
   残留 0.718 px(うち回転 0.4 度の分 0.652 px)。★★残留ずれと偽陽性面積は
   **位置合わせ後も同じ崖の上に乗る**(5 節の図): 「ずれだけ」の掃引を残留
   0.72 px で読むと 915 px、実測 995 px(1.09 倍)—— 残留の中身が回転でも同じ。
   ★経路ごとに弱点が違う: 照明差(利得 1.15)で LK は 0.275 px に悪化し
   特徴点は 0.118 px のまま、強い雑音(σ 0.03)では逆に特徴点が 0.360 px、
   LK が 0.027 px。★PIV は「正規化中央値検定だけ」では残留 0.87 px だった ——
   平坦な窓と直線エッジ 1 本の窓(開口問題)は近傍も同じように間違うので
   近傍検定では落ちず、残差で刈る LS を 4 回回して 0.048 px。
6. ★**照明差は位置合わせでは直らない**。照明差だけ(利得 1.15、オフセット
   0.03)でずれ無しでも偽陽性は 17198 px(明るい畑・屋根・森林が丸ごと「変化」)。
   線形の相対放射補正(利得・オフセットを当てはめる)で 0 px。
   ★``histogram_match``(順位で分布を合わせる)は 375 px で、**ずれも照明差も
   無い対でも 383 px 出す** —— 変化そのもの(水域拡大で暗い画素が増える)を
   分布差として消しにかかるので、変化検出の前処理には向かない。一方ずれだけ
   (照明差なし)は 3497 px で、放射補正しても 3525 px —— **同じ「偽陽性」でも
   原因は対照群でしか分けられない**。
7. ★**変化の大きさの崖は面積でなく画素で決まる**。予想は面積の重なり
   (1 − δ/(s√2))² だったが外れた(最大差 0.31)。実測は**画素被覆則**: 真値画素は
   ずれた足跡に被覆率 > τ/C = 0.2 だけ掛かれば拾われる。この則の予測と実測の差は
   全 36 点で 0.000。1 px 未満のずれでは画素は 1 つも失われず、δ=1 px(対角)
   で角の 1 画素(s=2 で 0.75、s=1 で 0)、(1 − τ/C)√2 = 1.13 px を超えると
   行と列を 1 本ずつ失う(δ=2 px で s=3 は 0.44)。オープニング(半径 1)で
   偽陽性の帯を消すと、**一辺 1 px の変化は δ=0 でも消える**(2 px は 0.75 残る)。
