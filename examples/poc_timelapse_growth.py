...
COLONIES = [
    ...
    (140.0, 81.2, 3.2),     # 3 ┘
    (55.0, 150.0, 2.4),     # 4 ┐ **ニアミス** —— 最後まで接しない
    (91.0, 150.0, 2.4),     # 5 ┘   (最終フレームで隙間 1.7 長さ単位)
...
def section4_merge_time(vol, times, labels, groups):
    ...
    print("  真値は接触(r_i + r_j = d)の時刻。**画素中心が円に入るまで**")
    print("  前景にならないので、観測は必ず**遅れる**はず —— 実測で確かめる。")
    print()
    print("  %6s %14s | %10s %8s | %10s %8s"
          % ("組", "真の合体時刻", "4 近傍", "遅れ", "8 近傍", "遅れ"))
    print("  " + "-" * 68)
    out = {}
    for i, j in PAIRS:
        f = None
        ...
        print("  %6s %14.3f | %10.1f %8.2f | %10.1f %8.2f"
              % ("%d-%d" % (i, j), tm, t4, t4 - tm, t8, t8 - tm))
        out[(i, j)] = (tm, t4, t8)
    print()
    print("  → ★遅れは**必ず正**(観測は真の接触より遅い)。予想どおり。")
    print("     4 近傍と 8 近傍の差が出るのは、接触の瞬間に 2 つの円が")
    print("     **斜めにしか触れていない**画素配置があるため。8 近傍は")
    print("     それを繋いでしまい 1 フレーム早く『合体』と言う。")
    print("     ★どちらが正しいかは**物理で決まる**(菌糸が斜めに繋がるか)。")
    print("     道具は既定を持たず呼び手に選ばせるべき —— `blob_label` は")
    print("     ちゃんと `connectivity` を引数にしている(既定 8 は注意)。")
    return out
...
def section8_figures(vol, times, labels, groups, rec, merge):
    ...
    figs.save_grid("frames", [vol[0], vol[T // 3], vol[-1], proj],
                   ...
                   caption="拡散律速 r = k sqrt(t + 4)。左上の 2 個は t≈8 で、"
                           "下の 2 個は t≈30 で合体する。右の 2 個は最後まで"
                           "接しない(隙間 1.7)。")
...
def section9_findings(rec, conn, merge):
  ...
  (2) ★合体時刻の観測は**必ず遅れる**(真値より後)。画素中心が円に
      入るまで前景にならないため。組 %d-%d で %+.2f フレーム、
      組 %d-%d で %+.2f フレーム。**遅い合体ほど遅れが大きい** ——
      接触直前の半径の伸びが遅い(1/sqrt(t))ので、同じ 1 画素の
      読み違いがより長い時間に化けるから。

  (3) ★★「時間の標本化が空間の分解能より先に効く」かどうかは
      **合体の時刻で逆転する**。早い合体(組 %d-%d)では
      時間 4 倍で %+.2f、空間 4 倍で %+.2f フレーム。
      遅い合体(組 %d-%d)では時間 4 倍で %+.2f、空間 4 倍で %+.2f。
      理由は (2) と同じ換算 —— 空間の粗さは
      「長さの誤差 ÷ 縁の伸びる速さ」で時間の誤差に化けるので、
      **成長が遅い局面ほど空間の粗さが時間の粗さを追い越す**。
      「どちらを削るか」は一般論では決まらない。

  (4) ★3-D 近傍の選び方が家族の分割を変える。6 / 18 / 26 で家族数
      %d / %d / %d。ニアミスの組を 26 が同じ家族にしたか: %s。

  (5) ★成長定数 k は**合体前のフレームだけ**なら数 %% で出る。
      切る位置を教えるのが時空間のラベル —— これが「体積として扱う」
      いちばん実用的な効き目。
""" % (PAIRS[0][0], PAIRS[0][1], merge[PAIRS[0]][1] - merge[PAIRS[0]][0],
       PAIRS[1][0], PAIRS[1][1], merge[PAIRS[1]][1] - merge[PAIRS[1]][0],
       PAIRS[0][0], PAIRS[0][1], e_s[2], e_p[3],
       PAIRS[1][0], PAIRS[1][1], e_s2[2], e_p2[3],
