...
1. ★★**フィレット高さの 71.2 % は、どのリングにも照らされない暗部にある**。
   接触角 θ = 18° の凹円弧は壁で 72° まで立つが、いちばん低いリング(仰角 10°)
   でも鏡面反射で見えるのは傾き 40° まで。幾何の予測は「見える高さ = 28.8 %」、
   色帯の傾きを素朴に積分する推定器 E1 の実測は真値の 0.284 倍(相関 0.9975)。
...
2. ★**色そのものではなく、色が変わる位置がフィレットを決める**。円弧なら傾き α の
   ...
   自由な円弧の範囲で相対誤差の中央値 +1.7 %(ばらつき 5.3 %)、真値との
   相関 0.9954。ただし h < 0.09 mm では色帯が 2〜4 px しか無く、境界 1 px の
   量子化がそのまま -4.9〜+28.4 % の誤差になる。**壁の推定位置が 0.5 px ずれる
   だけで h が 5 % 縮む**(壁の傾き 72° への外挿は cos が小さく感度が高い)。
3. ★**はんだ量が増えて爪先(toe)がパッド端に固定されると E2 は下に外れる**。
   自由円弧が成り立つのは V ≤ 0.0243 mm³ まで(予測)。固定後は爪先の傾きが θ
   より立つので、青→緑境界を「傾き θ」と読む E2 は R を小さく見積もる
   (V = 0.031 mm³ で -11.6 %、0.035 mm³ で -42.3 %)。そこは h/H ≥ 0.79 なので
   合否は変わらないが、相関は 0.9954 → 0.9452 に落ちる。
4. **合否の崖は IPC 境界の両側 ±0.038(h/H)**。境界付近を一様に 200 点振ると、
   |h/H − 0.25| < 0.038 の帯の中で誤判定 26.9 %、外では 0.8 %。
   予測は「境界 ±0.5 px → R ±9 % → 帯 ±0.022」だったが実測は 1.7 倍広い ——
   境界は 3 点ありそれぞれ ±0.5 px、しかも 2 節の外挿感度が乗る。
   はんだ量の推定誤差と合否の誤り率は別の量で、後者は
   「境界からどれだけ離れた個体が多いか」という工程の分布次第。
5. ★**部品の位置ずれ 0.16 mm で良品が「不足」に化ける**(はんだ量は同じ)。
   ずれた側の爪先の余地は 0.3 − Δx。余地がフィレット高さ 0.178 mm を切ると
   爪先が固定されて面が立ち、爪先の傾きが 30° を超えた瞬間に**緑帯が消えて**
   E2 の「爪先 = θ」が破れる。幾何の予測は Δx = 0.16 mm、実測の反転も 0.16 mm
   (フィレット全体が暗部に落ちる 40° 超えは 0.18 mm)。真値の高さはむしろ
   **上がっている**(0.178 → 0.196 mm)ので、誤判定はすべて「良品を不足と言う」向き。
   真値が本当に不足に落ちるのは Δx = 0.28 mm で、その間 0.12 mm が誤判定の窓。
6. ★**表面粗さ: 予想は外れ、壊れるのは赤帯の消失と同時**。GGX の粗さを
   0.1 → 0.8 に振ると、赤帯が緑に負けて消えるのは表(LUT)の予測で 0.5、実測も
   0.5、E2 の誤差が 12 % を超えるのも 0.5(-14.0 %)。予想は「その前にローブの
   漏れで暗→色の境界が壁側へ動き E2 が上に外れる」だったが、粗さ 0.40 まで
   暗部の始まりは 1.0° しか動かず E2 は ±2.4 % のまま。代わりに効いたのは
   **露光**: 窓の狭い赤(傾き幅 10°)のピークが 0.40 で 0.48 まで落ち、0.6 で
   全リングが暗部しきい値を割ってフィレット全体が暗部になる(判定 100 % 不良)。
7. **照明むらは鏡面ほど効かない**。赤リングだけ +u % 明るくしても、粗さ 0.15 では
   緑→赤の境界は u = 30 % でも 0.0 px(予測 0.1 px、境界がほぼ階段だから)。
   粗さ 0.40 では u = 20 % で 0.0 px(予測 0.6 px)、u = 30 % で 1.0 px
   (予測 0.9 px)—— 境界は 1 px に量子化される。
8. ★**ゼロ点(パッド平均色の距離 ΔE)は基準条件では 100 % 当たるが、現場の
   ばらつきで壊れる**。校正集合(良品 30 / 不足 30)で精度 100 %(しきい値 ΔE 12.59)
   なのに、ずれ ±0.1 mm・粗さ 0.10〜0.35・照明むら ±5 % を混ぜた 4 種 × 50 個体では
   NG 率が 良品 56 % / 不足 86 % / ブリッジ 68 % / 浮き 100 %。ブリッジの 68 % は
   良品の偽 NG 56 % と同程度 —— パッドの外で起きるブリッジは**見えていない**。
   E2 系の判定は正答 良品 98 % / 不足 98 % / ブリッジ 100 % / 浮き 88 %。浮きの
   取りこぼし 6 件は持ち上がり角 8.7〜11.0°: 電極上面は天頂リングの窓(0-15°)の中で
   青のまま、本体の前縮み 1.0 cos ψ も許容 0.02 mm 以下(予測の限界 11.5°)。
   小さな浮きと不足は、この照明では原理的に区別できない。
...
def section_roughness(v_thr: float) -> dict:
    ...
    print("     予想は「赤帯が消える前に、ローブの漏れで暗→色の境界が壁側へ動き E2 が上に外れる」"
          "だったが**外れた**: 粗さ 0.40 まで暗部の始まりは %.1f° しか動かず、E2 は ±%.1f %% のまま。"
          % (abs(TILT_MAX - 39.0), max(abs(e) for e in errs[:5])))
    print("     壊れるのは赤帯の消失と同時。窓の狭い赤(傾き幅 10°)のピークが先に落ち"
          "(0.40 で %.2f)、次に全体が暗部しきい値を割る —— リングの分離より**露光**が先に崩れる。"
          % lut_response(0.40)["R"].max())
