...
"""埋設管の漏水を音で位置決めする —— 伝わる速さを間違えると、掘る場所がずれる。

上下水道の漏水調査は、管路の 2 点に相関式漏水探知機を当て、**音の到達時間差 τ**
から漏水位置を出す仕事です。位置は ``x = (L + c·τ) / 2`` の 1 行で決まるので、
**音速 c の仮定がそのまま掘る場所の誤差になる**。相関がどれだけきれいに立っても、
c を間違えていればアスファルトを外して切ることになります。

EXTEND: 実際の調査に差し替えるなら :func:`make_records` が返す ``y1`` / ``y2`` を
2 台の振動センサ(または水中マイク)の同時記録に、``t1`` / ``t2`` を「掘って
確かめた漏水位置から計算した伝搬時間」に置き換えます。**真値として要るのは
掘削で確認した位置と、その区間の管種・口径・布設年の台帳**で、探知機が表示した
位置を真値にするのは不可(それはこの PoC が測っている推定値そのもの)。音速は
文献値を写すのではなく、**既知点(消火栓を叩く / 空気弁を開ける)で 1 回較正**
するのが筋ですが、7 番の所見のとおり**区間で管種が変わっていると較正では
直りません**。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. **ゼロ点(生の相互相関のピークをそのまま読む)は、条件が良ければよく当たる**。
   SNR 0 dB・音速が真値なら整数ピークで -0.071 m、サブサンプル補間つきの
   5 手法はどれも 0.071 m 以内(相関の主ローブ幅 c/(2B) = 0.893 m の 1/10 以下)。
   **ここで満足すると、以下の 4 番以降で掘る場所を外す**。
2. ★**量子化誤差は「散らばり」ではなく「その場所ごとの偏り」**。予測は
   「整数ピークは c/(2fs) = 0.076 m 刻みなので RMS は 0.0220 m を割れない」。
   漏水位置を 70-86 m で 41 点振ると実測 **0.0221 m**(予測と 0 % 差、
   サブサンプル版は 0.0007 m で 32 倍良い)。ところが**位置を 1 つに固定すると**
   +10 dB で 0.0054 m しか外さない —— この τ の小数部がたまたま 0.93 で
   毎回同じ側に丸まるからで、同じ場所を何度測っても同じだけ外します。
   PIV のピークロッキングと同じ現象が、1 次元の遅延推定でも起きる。
3. **崖の高さ(CRLB)は当たり、崖の位置は 7.6 dB 外した**。Knapp-Carter の
   下界をビンごとの SNR から数値積分すると、減衰を切った場面で 0.00660 m ——
   平坦帯域の閉形式 0.00660 m と 0.05 % 差(検算が通る)。減衰を入れると
   0.00908 m へ悪化する(高域が先に痩せ、Fisher 情報は f² で効く)。
   0 dB の実測は 整数 0.0474 / サブサンプル 0.0348 / 帯域制限 0.0107 m で、
   **いちばん良い手法でも下界の 1.2 倍**。★崖(取り違えが 5 % を超える点)は
   予測 -20.1 dB に対し**実測 -12.5 dB**。「σ_τ が主ローブ半幅に届く」は
   取り違えが**支配的になる**点であって**始まる**点ではなく、隣のローブが
   本ローブを追い越す確率はもっと手前から効く(Ziv-Zakai 型の閾値効果)。
   崖の**下**の予測は当たった: -30 dB の全試行 RMS 36.0 m に対し、
   探索窓に一様と仮定した 39.0 m と 8 % 差。
4. ★**前処理の順位は SNR で入れ替わる**。教科書どおりの GCC-PHAT(全帯域)は
   +10 dB でも 0.025 m と、帯域内 PHAT(0.003 m)の 8 倍悪い —— 信号が無い
   ビンまで等しく持ち上げるから。**白色化そのものではなく、どこを白色化するか**。
5. ★★**反射では予想が外れた。PHAT は勝たない**。センサの先 0.7 m / 1.1 m の
   継手で折り返す反射(主ピークを歪ませる近さ)を振幅 0.8 まで入れると、
   +10 dB で 生 0.134 / 帯域制限 0.126 / PHAT 全帯域 0.114 / PHAT 帯域内
   0.121 m —— **差は 1 割**。理由は数字が教える: RMS 0.121 m に対して
   偏りが -0.121 m で、誤差は**ほぼ全部が偏り**。PHAT が白色化するのは
   相互スペクトルの**振幅**だが、遅延を運んでいるのは**位相**で、反射は
   その位相を歪める。**対照群**で因果を確かめた —— 左右の継手を入れ替えると
   偏りは +0.126 m と符号が反転し、左右対称にすると +0.000 m。
   **偏りの向きは「どちら側の継手が遠いか」だけで決まる**(反射の強さではない)。
6. ★**音速の誤りは位置に線形に効き、中点ではまったく効かない**。予測
   ``Δx = (Δc/c)(x - L/2)`` に対し、+10 % の誤りで予測 +1.800 m / 実測
   +1.798 m(差 0.0018 m)。中点の漏水は音速を ±15 % 間違えても 0.0007 m
   以内、x = 78 m では同じ誤りが 2.701 m。**相関の形も相関係数も 1 ミリも
   変わらない** —— 変わるのは掘る場所だけ。
7. ★★**管種が途中で変わると、音速を較正しても直らない**。60 m で鋳鉄
   1250 m/s から樹脂 500 m/s に変わる管路の 78 m に漏水を置くと、真の τ は
   **ちょうど 0**。相関はきれいに 1 本立つのに、鋳鉄を仮定しても樹脂を仮定
   しても 2 つの平均を仮定しても答えは 60 m(-18.001 m)。``x = (L + c·0)/2``
   なので**掛ける相手が 0 のとき、何を掛けても L/2 にしかならない**。
   これはパラメータの誤りではなく**モデルの誤り**。管路全体に振ると最大
   45.003 m 外し、さらに**真の遅延そのものが探索窓 |τ| <= L/c の外へ出る**
   位置が 37 点中 19 点(x < 48 m と x > 99 m)—— そこでは相関器は窓の中の
   雑音を掴む。**「ピークが立った」ことは、真の遅延が窓に入っている証拠に
   ならない**。
8. ★**位相勾配は連続に壊れない**。``transfer_function`` の位相の傾きから
   群遅延を出す手法は +20 dB で -0.002 m と最良だが、0 dB では -4.267 m。
   unwrap が帯域のどこかで 1 周滑ると、傾き = 遅延がそのぶん丸ごとずれる。
   「少し悪い記録なら少し悪い答え」にならない手法は、現場では使いにくい。
9. **誤差は種類ごとに数える**(足し合わせない): 量子化 22 mm / サブサンプル
   11 mm / 音速 10 % で 1.8 m / 管種の取り違えで 18 m / 崖の下で 36 m。
   **桁が 3 つ違う。相関を良くする努力は左の 2 つにしか効かない。**

【グラウンドトゥルース】漏水源のスペクトル(帯域内で振幅平坦・位相一様乱数)、
区間ごとの音速、周波数比例の減衰 0.20 dB/m @ 1 kHz、センサ 2 点の位置、反射の
振幅と往復路長、雑音のビンあたりパワー —— **すべて仕込んだ値**。2 点の記録は
同じ源に ``exp(-2πi f t_i)`` で厳密な小数標本遅れを掛け、別々の広帯域雑音を
足して作る。CRLB は推定スペクトルではなく**この閉形式のビンごと SNR** から
計算する(最初は白色雑音を濾して作り、実測 RMS が CRLB を下回るという有り得
ない結果になった —— 原因は周期グラム比の推定誤差だった)。

来歴(公開文献のみ): Knapp & Carter, *The generalized correlation method for
estimation of time delay*, IEEE TASSP 24(4) 1976 —— GCC と PHAT、および遅延
推定の Cramér-Rao 下界 / Carter, *Coherence and time delay estimation*,
Proc. IEEE 75(2) 1987 —— コヒーレンス形の下界 / Ziv & Zakai, IEEE TIT 15(3)
1969 —— 閾値効果 / Hunaidi & Chu, *Acoustical characteristics of leak signals
in plastic water distribution pipes*, Applied Acoustics 58 (1999) —— 漏水音の
帯域と管材による音速・減衰 / Gao, Brennan & Joseph, *A comparison of time
delay estimators for the detection of leak noise signals in plastic water
distribution pipes*, J. Sound Vib. 292 (2006) —— 相関式漏水探知の手法比較。
"""
