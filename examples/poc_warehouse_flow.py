...
#: ★「人待ち」4 件 x 7.5 s と「通路の干渉」10 件 x 3.0 s は総量 30.0 s で**わざと同数**。
#: 現実に一致するとは限らないが、「同じ数字が違う原因から出る」ことは 1 例で示せる。
DUR = {"補充待ち": 18.0, "人待ち": 7.5, "通路の干渉": 3.0,
...
def critical_rack_height(x: float, y: float) -> float:
    ...
    return float(min(per_cam))          # どちらか 1 台でも見えれば見える
...
def section_scene(base: dict, clear: np.ndarray) -> dict:
    ...
    print("  ★人待ち %.1f 秒 と 通路の干渉 %.1f 秒 は**わざと同じ総量**に置いてある。"
          % (counts["人待ち"] * DUR["人待ち"],
             counts["通路の干渉"] * DUR["通路の干渉"]))
...
def section_sweep_id(clear: np.ndarray) -> dict:
    print("\n" + "=" * 78)
    print("9) ★★崖(3) ID の取り違え —— 壊れるのは「人待ち」だけ")
    print("=" * 78)
    ...
    for p in ps:
        ...
        for k in LOSS_TYPES:
            ...
            line += "%-9s" % ("%.2f" % v)
        print(line)
    print("\n  ★人待ち %.2f -> %.2f、補充待ち(単独)は %.2f -> %.2f。"
          % (rec["人待ち"][0], rec["人待ち"][-1], rec["補充待ち"][0],
             rec["補充待ち"][-1]))
...
def main() -> int:
    ...
    print("  * 崖は 3 本の軸で違う型を殺す: 標本間隔は短い待ち(干渉 %.1f 秒で半減)、"
          % swd["cliffs"]["通路の干渉"])
    print("    遮蔽は棚に近い型(補充待ち %.2f -> %.2f)、ID 誤りは人待ちだけ"
          "(%.2f -> %.2f)。"
          % (swo["rec"]["補充待ち"][0], swo["rec"]["補充待ち"][-1],
             swi["rec"]["人待ち"][0], swi["rec"]["人待ち"][-1]))
    print("    そのあいだ総滞留時間は %.1f -> %.1f 秒(遮蔽掃引の外)で、"
          "どれも「何も起きていない」に見える。"
