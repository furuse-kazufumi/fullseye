...
移動は「斜めの管」なので、**t 軸方向のオープニング**(長さ 2.5 秒の線分)で管だけが
...
1. ★★**ゼロ点の「総滞留時間」は 6 割しか待ちでない**。速度がしきい値以下の時間を
   足すだけだと 321.5 秒。真の待ちの合計は 198.0 秒(61.6 %)で、残りは
   **生産的な作業 60.0 秒(18.7 %)**と、待ちに入る/出るときの徐行
   63.5 秒(19.8 %)。「滞留を減らせ」と言われた現場は、まず削ってはいけない
   作業時間を削ろうとすることになる。
2. ★★**同じ数字が違う原因から出る**。待ちを 1 種類ずつ止めた対照群で、
   総滞留時間の減り方は 人待ち -39.0 秒 / 通路の干渉 -38.0 秒 —— 差は 1.0 秒
   (2.6 %)。ゼロ点への寄与が揃うように件数と長さを置いてあるからで、
   1 本の数字では「通路を広げる」のか「人を増やす」のかが決まらない。
   種類別なら該当の型だけが 4/4 → 0/4、8/8 → 0/8 と落ちて一意に分かる。
3. ★★**欠品は滞留として存在しない**。棚が空で待たずに別の棚へ回る損失は、
   総滞留時間を -17.0 秒(-5.3 %)しか動かさないのに、**余計な移動を 94.6 m**
   (= 移動時間 78.8 秒、滞留の動きの 4.6 倍)生む。滞留時間だけを KPI に
   すると、この損失は永久に見えない。★しかも測った経路長は標本間隔で縮む
   (Δt=0.25 秒で -5.5 %、4.0 秒で -30.1 %)ので、経路長を KPI にするなら
   標本間隔を固定しないと**カメラを替えただけで改善に見える**。
4. ★★**2-D の滞留ヒートマップは場所は当てるが中身を当てない**。予想は
   「積算は通路に埋もれて待ちが見えない」だったが、実測では上位 30 列のうち
   17 列が本当に待ちの場所だった —— 場所はだいたい当たる。当たらないのは
   中身で、**いちばん積算が大きいのは誰も待っていない下段通路(55 フレーム)**、
   18 秒待っている棚前(48)より大きい。3-D に戻すと通路のいちばん長い柱は
   3.0 秒、棚前は 24.0 秒。同じ 1 人の軌跡でも、1 つの列に居るフレーム数は
   待っている間 42 / 歩いている間 6 で 7.0 倍離れる。
5. ★★**崖は種類ごとに違う場所にあり、しかも予測できる**。柱と認めるのに
   3 標本要るので長さ T の待ちは Δt > T/2 で消える。徐行 2.5 秒ぶんが柱に
   足されるから予測は (T+2.5)/2 —— 実測の崖(検出率が 0.5 を切る Δt)は
   欠品 2.0 秒(予測 2.2)/ 通路の干渉 3.0(2.5)/ 人待ち 4.0(5.0)/
   システム待ち 6.0(6.2)/ 補充待ち 10.0(10.2)。**5 種類とも 1.0 秒以内**。
   ★★Δt を 0.5 → 2.0 秒にすると欠品の検出率は 1.00 → 0.20 になるのに、
   ゼロ点は -8.2 % 動くだけで、しかもそれは「滞留が減った」としか読めない
   (実際には測れなくなっただけ)。その -26.5 秒は「人待ちを 1 件残らず
   無くす」(-39.0 秒)と同じ order で、畳んだ数字では区別がつかない。
6. ★★**遮蔽は「棚に張りつく型」だけを殺す**。棚を高くすると天井カメラの死角は
   まず棚に挟まれた狭い通路に出る。補充待ちの検出率は 1.00 → 0.40、欠品は
   1.00 → 0.75 に落ちるのに、開けた場所のシステム待ち・人待ち・通路の干渉は
   1.00 のまま。★死角に入る棚の高さは撮る前に幾何で出せる —— 事象ごとの予測
   2.68 / 2.90 m に対し実測で消えたのは 2.8 / 3.2 m 段(掃引の刻みの中で一致)、
   主通路側の 2 件は「見え続ける」の予測どおり最後まで見えた。
   ★同じ型でも**場所で生死が分かれる**。
7. ★★**ID の併合が壊すのは「2 人の関係を読む型」だけ**。人待ちは「同じ柱の中で
   遅く始まったほう」、通路の干渉は「柱の中に 2 人」で決めるので、追跡器が
   2 人を 1 本の軌跡にすると両方壊れる: 人待ち 1.00 → 0.40、干渉 1.00 → 0.78。
   ★予想は「単独で決まる型は無傷」だったが**完全には無傷でない**(最小 0.95)
   —— 併合は ID を書き換えるので柱が途中で切れ、短い柱(欠品)は 3 標本を割る。
   長い補充待ちは切っても両側が残るので 1.00 のまま。
   ★★ゼロ点はこの掃引で 321.3 → 321.3 秒、**1 秒も動かない**。標本間隔・遮蔽・
   ID 誤りは違う型を壊すのに、畳んだ数字にはどれも同じ顔で現れる。
8. ★`voxel_to_mesh`(marching cubes)が返す面の巻き順は**内向き**で、
   `mesh_volume` は負(-429、ボクセル数 463 と 7.3 % 差)を返す。符号を
   「向きの検査」に使うなら規約を先に確かめること。

【グラウンドトゥルース】
平面図(棚・通路・作業台・払い出し口)は矩形の閉形式。軌跡は**折れ線 + 一定速度**で
決め打ちし、待ちは種類ごとに**固定長**(:data:`DUR`)で仕込むので、どの滞留が何由来かは
完全に既知(現場ではばらつくので崖はもっと鈍る)。待ちに入る/出る 0.15 m だけ徐行
(0.12 m/s)を入れてあり、これが「ゼロ点が待ち以外を拾う」源になる。加速度は
入れていない(実機ではゼロ点の水増しはさらに増える)。天井カメラの遮蔽は 2 台の
カメラ位置・棚の高さ・標識高さから**光線が棚の縁を越えるか**で決める閉形式。
ID の併合は近接した対に確率 p で起き、離れるまで続く。掃引は 5 通りの乱数で平均する。
...
def section_sweep_occ(clear: np.ndarray) -> dict:
    ...
    print("\n  ★★棚に張りつく型だけが落ちる: 補充待ち %.2f -> %.2f、"
          "欠品 %.2f -> %.2f。"
          % (rec["補充待ち"][0], rec["補充待ち"][-1], rec["欠品"][0],
             rec["欠品"][-1]))
    print("     開けた場所の型は無傷: システム待ち %.2f -> %.2f、"
          "人待ち %.2f -> %.2f、通路の干渉 %.2f -> %.2f"
          % (rec["システム待ち"][0], rec["システム待ち"][-1], rec["人待ち"][0],
             rec["人待ち"][-1], rec["通路の干渉"][0], rec["通路の干渉"][-1]))
    print("     (干渉は交差通路の**まん中**で起きるので、2 台のカメラから"
          "通路に沿って見通せる)。")
    print("  ★★同じ「補充待ち」でも場所で生死が分かれる —— 主通路側の 2 件は"
          "最後まで見え、狭通路側の 3 件が消えて %.2f になる。"
          % rec["補充待ち"][-1])
...
def section_sweep_id(clear: np.ndarray) -> dict:
    ...
    dip = min(min(rec[k]) for k in ("システム待ち", "欠品"))
    print("  ★予測は「単独の型は無傷」だったが、**完全には無傷でない**"
          "(最小 %.2f)。併合は ID を書き換えるので柱が途中で切れ、"
          % dip)
    print("     短い柱(欠品 %.1f 秒 + 徐行)は切られると %d 標本を割る。"
          "長い補充待ち(%.1f 秒)は切っても両側が残るので %.2f のまま。"
          % (DUR["欠品"], MIN_SAMPLES, DUR["補充待ち"], min(rec["補充待ち"])))
