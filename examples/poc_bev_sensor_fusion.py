...
# --- 場面(真値はすべて式で決まる)------------------------------------------- #
#: 障害物。``h`` は **0.2 m の倍数を避けて**ある(高さの量子化を見えるように)。
OBSTACLES = (
    {"name": "先行車", "cx": 10.1, "cy": 0.0, "lx": 4.2, "ly": 1.8, "h": 1.53},
    {"name": "その先の車", "cx": 22.0, "cy": 0.0, "lx": 4.4, "ly": 1.8, "h": 1.47},
    {"name": "路肩の車", "cx": 13.0, "cy": -4.2, "lx": 4.2, "ly": 1.8, "h": 1.44},
    {"name": "歩行者", "cx": 7.0, "cy": 3.2, "lx": 0.6, "ly": 0.6, "h": 1.72},
    {"name": "側壁", "cx": 16.0, "cy": 6.2, "lx": 24.0, "ly": 0.4, "h": 2.38},
)

#: LiDAR: バンパー高さ。**先行車の陰に完全に入る**のがこの高さの帰結。
LIDAR = {"C": np.array([0.0, 0.35, 0.55]), "rmax": 32.0, "sigma": 0.020,
         "az": 70.0, "daz": 0.15, "el": (-14.0, 6.0), "nel": 16}
#: 深度カメラ: ルーフ高さ。σ_z = ``kz``·z²(ステレオの視差量子化に相当)。
CAM = {"C": np.array([0.0, -0.35, 1.90]), "zmax": 25.0, "kz": 0.0008,
       "W": 320, "H": 240, "f": 265.0}
