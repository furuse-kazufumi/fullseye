...
扱い(:func:`free_mask`)が使えない、(c) 時刻ずれは自車のオドメトリで
しか推定できない —— の 3 点が変わります。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. ★★**画像の再投影誤差(px)で校正を報告すると、BEV の壊れ方が見えない**。
   換算則は単純で、再投影 1 px = 1/f ラジアン = 距離 R で **R/f メートル**。
   この光学系(f=265 px)なら 22.1 m 先で **0.083 m/px = 0.42 セル/px**。
   外部 yaw を 0.20 度ずらすと画像では **0.98 px** —— サブピクセルに近く
   校正レポートなら合格だが、22.1 m の BEV では既に 0.077 m ずれている。
   1.00 度なら 4.91 px(予測 f·tanθ = 4.63 px)= **0.386 m = 1.9 セル**。
   px は角度の指標、BEV は長さの指標で、**換算係数が距離そのもの** ——
   同じ校正が近くでは合格、遠くでは不合格になる。
2. ★★**融合が単センサに勝つのは「見えていない場所」でだけ**。LiDAR を左
   ミラー、カメラを右ミラー(横に 1.80 m)に置くと、先行トラックの影は
   19.85 m の断面で LiDAR が y=-4.87〜+1.60 m、カメラが -1.60〜+4.87 m と
   **食い違う**。そこに置いた 2 台(y=±1.85〜3.75 m)は、左の車を LiDAR が
   24 セル / カメラが **0 セル**、右の車を LiDAR が **0 セル** / カメラが
   52 セル取る。IoU は LiDAR 単独 0.4988 / カメラ単独 0.5417 に対し
   融合 0.7033(最大値則)・0.7591(平均則)。
3. ★**2 つのゼロ点は IoU では拮抗しているのに、中身は正反対**。LiDAR は
   適合率 0.914 / 再現率 0.523(正確だが疎)、カメラは 0.680 / 0.727
   (密だが深度雑音 0.0008 z² で滲む)。**同じ弱点の 2 台を足しても増えない**
   —— 融合の利得はこの相補性の分だけ。
4. ★★**予想が外れた: 崖の尺度はセルではなく物体の大きさ**。角度 θ の誤差は
   距離 R の点を Rθ 動かすので、セル c=0.200 m を跨ぐのは R > c/θ。点が
   実際にセルを跨いだ割合は幾何の予測(1-(1-|dx|/c)(1-|dy|/c))と**最大差
   0.0553** で一致する。ところが yaw 0.50 度で点の **68 %** が既にセルを
   跨いでいるのに、IoU は 0.7033 → 0.6627(5.8 % 減)しか落ちない。
   幅 1.9 m の車を 0.20 m ずらしても重なりは 89 % 残るから。IoU が 90 % を
   切るのは yaw **2.00 度**(BEV のずれ 0.772 m = 車幅の 41 %)。
   並進も同じで、c/2 = 100 mm に崖は無く(0 → 100 → 150 mm で
   0.7033 → 0.6353 → 0.6554 と**上下する**)、90 % を切るのは 600 mm
   (車幅の 32 %)。**半セル以下のずれは、どのセルに丸められるかの揺らぎ**。
5. ★**融合の規則で壊れ方の種類が変わる**。yaw 1.00 度で 最大値則は
   適合率 0.767(偽占有 96 セル)、平均則は 再現率 0.779(見逃し 85 セル)。
   IoU は 0.6583 と 0.6600 で**ほぼ同じ** —— 計画に使うなら偽占有(急停止)
   と見逃し(衝突)は別物なのに、**IoU 1 本ではこの違いが出ない**。
6. ★★**融合が単センサより悪くなる領域がある**。カメラ側だけを回すと、
   最大値則は yaw **3.00 度**(0.5154)でカメラ単独 0.5417 を下回る。
   並進なら 600 mm(0.5407)。誤差のあるセンサを足すと正しいほうまで汚れる。
   信頼度重み則は yaw 6 度でも 0.4371 で平均則 0.3333 を大きく上回るが、
   ★予想「距離に応じてなめらかに混ざる」は**外れ** —— 宣言校正誤差 0.30 度と
   雑音モデルから作った重みは評価セルの **80.4 %** で LiDAR のほうが大きく、
   両方が見える 2022 セル(評価セルの 37 %)では**実質「LiDAR 優先」の
   順位規則に退化**していた。雑音モデルが交差しないと重みは混ざらない。
7. ★★**同じずれ量でも、回転と並進では壊れる場所が正反対**。ずれ量を揃えて
   (yaw 1.00 度 = 20 m で 0.349 m、並進 349 mm)距離帯で割ると、点の平均
   移動量は 回転が 0.165 → 0.368 m(**2.22 倍**)、並進は 0.349 m のまま
   (1.00 倍)。カメラ単独の IoU 低下率も 回転 14 % → 38 %、並進 31 % →
   32 %。★そして**融合するとこの差が薄まる**(遠 / 近の比 0.76 → 0.97)——
   融合は誤差を消さずに**隠す**ので、校正異常を距離依存で見つける診断は
   融合の後では効かない。
8. ★**時刻ずれの正体は並進**。自車 14.0 m/s・ヨー角速度 5.0 度/s では
   Δt=80 ms が「並進 1120 mm + 回転 0.40 度」で、20 m での寄与は 8.0 倍
   並進が大きい。実測 IoU 0.5744 は 純並進 0.5761 と差 **0.0018**、
   純回転 0.6336 とは 0.0592(**34 倍**)。Δt=40 ms では差 0.0243 と 0.0164 で
   **丸めのゆらぎと同じ大きさ**になり、IoU では原因を分けられない。
9. **偽占有は「どれだけ外れた所」に出るかで数える**。真の障害物からの距離
   (ESDF)の中央値はどの条件でも 1 セル(輪郭の帯)だが、2 セルより遠くに
   出る割合は 誤差なし 0.081 / yaw 1 度 0.115 / 並進 150 mm 0.119。
   個数が近くても、輪郭に貼りつくか離れて散るかは別の量。
10. ★**高さは占有より素直**。誤差 0 の融合の高さ誤差中央値 0.080 m は、
    格子の刻み 0.200 m の量子化(最大 0.100・期待 0.050 m)の水準。
    ★★ただし**帯の上端 2.60 m より高い物は測れない**: トラック(真値
    2.73 m)は -0.23 m の系統誤差になる。これは量子化ではなく測り方の上限。
11. ★整形(closing + 穴埋め)は footprint に近づけるが**正しさを見ずに
    一律に太らせる**。誤差 0 で占有セル 1.21 倍・solid IoU 0.2313 → 0.2934、
    yaw 1.00 度では偽占有が 137 → 152 セル(1.11 倍)。★★そもそも
    ``closing_circle`` のつまみは半径 1-4 セル(= 0.8 m)しか動かず、
    車幅 1.9 m を埋めきれない —— **このやり方の上限は道具のつまみで
    決まっている**。

【グラウンドトゥルース】
場面は**解析的**: 地面 z=0 の平面と、軸平行直方体 6 個(先行トラック・左右の
遠方車・路肩の車・歩行者・側壁)。BEV の真値占有は
:func:`fullseye.ledger.grid_coords` で作ったボクセル中心に :func:`box_sdf` を
評価し :func:`sdf_union` / :func:`sdf_to_occupancy` で合成する —— **式で決まる**。
高さの真値も同じ式。観測は解析的な光線 - 直方体交差(slab 法)で作り、LiDAR は
距離雑音、カメラは深度の 2 乗に比例する雑音を載せる。評価窓は前方 3-30 m・
左右 9 m、評価対象は **真の校正で「自由」と言い切れたセル ∪ 実際に返りが
あったセル**(占有格子の 3 状態のうち「未知」を除く、:func:`free_mask`)。
車体内部のように**どちらからも見えないセルを分母に入れると、上限が幾何で
決まってしまい規則の差が埋もれる**。★障害物の面は**セル中心と境界の中間**に
置いてある(理由はコードの :data:`OBSTACLES` の注記 —— どちらに載せても
別々の人工誤差が出る)。
...
def section_shaping(rig: Rig) -> dict:
    ...
    print("\n  ★誤差 0 では占有セルが %.2f 倍になり solid IoU は %.4f -> %.4f。"
          "★ただし表面の返りは L 字なので、閉じても中は埋まらない —— "
          "solid に対する IoU は %.2f までしか上がらない(上限は幾何)。"
          % (a1[0] / a0[0], a0[1], a1[1], a1[1]))
    print("  ★同じ整形を yaw %.2f 度に掛けると偽占有は %d -> %d セル(%.2f 倍)、"
          "真値側は %.2f 倍。**整形は正しさを見ずに一律に太らせる**。"
          % (1.0, b0[2], b1[2], b1[2] / max(b0[2], 1), b1[0] / b0[0]))
    print("     ★★``closing_circle`` のつまみ ``a`` は半径 1-4 セル(= %.1f m)"
          "しか動かない。車幅 %.1f m を埋めるには足りず、"
          "**このやり方の上限は道具のつまみで決まっている**。"
          % (4 * CELL, OBSTACLES[1]["ly"]))
    return out
