...
★そしてこの検算は、真値との RMSE が見落とすものを 1 つ捕まえた:
**FBP は質量を保存しない**。この PoC を書いた時点では 180 本・検出器 363 bin で
総質量が **-3.34 %** ずれていた。RMSE のほうは 0.02498 で、これは「ほぼ正解」に
見える数字である。**画としては同じ、量としては桁違い** —— それを RMSE だけでは
検出できない、というのがこの節の主張だった。

**(2026-09-06 に塞がった)** 欠損の正体は検出器の数ではなく**ランプフィルタの
DC ビン**だった。``np.fft.rfftfreq`` は 0 にちょうどビンを置くので ``|f|`` の
DC ゲインが厳密に 0 になり、フィルタが**各投影の平均を丸ごと引いていた**。
引かれる量は ``sum(P) / n_pad`` なので、誤差は検出器の数ではなく**FFT のパッド
長**に従う —— 363 bin と 511 bin が同じ値を返していたのは両方 1024 に詰まる
からで、当時それを「検出器を増やすと改善する」と読んだのは**取り違え**だった。
いまは Ram-Lak の空間インパルス応答の DFT を使い、DC ビンは小さいが 0 ではない
(pad 1024 で 3.958e-04)。実測の質量欠損は **-0.0099 %** で、検出器を 363 →
727 bin に増やしても**まったく動かない**(それが正しい —— ``n_detectors`` は
検出器の**幅**を決める引数で、標本化の細かさではないので、対象がすでに収まって
いるなら増やしたぶんは空のビンが増えるだけである)。
...
def main() -> bool:
    ...
    print("   ★2026-09-06 に**塞がった**。以前はここが -3.34 % と -0.83 % で、")
    print("     この PoC は「検出器を増やすと質量欠損が 4 分の 1 になる」と書いた。")
    print("     **それは取り違えだった** —— 効いていたのは検出器の数ではなく")
    print("     ランプフィルタの FFT パッド長で、363 bin と 511 bin が同じ値を")
    print("     返していたのは両方 1024 に詰まるからだった。原因は ``|f|`` の DC")
    print("     ゲインが厳密に 0 で、フィルタが**各投影の平均を丸ごと引いていた**こと。")
    print("     いま(Ram-Lak の空間応答の DFT)は DC が小さいが 0 ではないので、")
    print(f"     質量欠損は {m_c:+.4%} で、検出器を倍にしても動かない。")
    print("     ★節の主張そのものは生きている: **RMSE は総質量の狂いに鈍い**。")
    print("     ランプは総質量が乗る最低周波数がいちばん再構成できない帯域なので、")
    print("     真値との RMSE だけを見ていると、この種の故障は一生見えない。")
    ...
    # 許容 1e-6(2026-09-07: 1e-9 は CI の py3.10 で一度だけ落ちた。同じ環境の前回は
    # 通っており、数万画素の総和をスレッド BLAS が取る以上、加算順で 1e-9 は揺れる。
    # 守りたいのは「検出器の数で質量が**動かない**」ことで、1e-6 でも十分に主張できる)。
    checks.append(("検出器の数では質量が動かない(幅の引数だから)",
                   abs(m_c - m_f) < 1e-6, f"{m_c:+.6%} vs {m_f:+.6%}"))
