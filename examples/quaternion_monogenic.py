...
1. Riesz 変換: 格子 cos(2π(u₀x+v₀y)) に対して R1=(u₀/|ω|)sin、R2=(v₀/|ω|)sin が**閉形式**。
   ...
   局所位相は閉形式 |wrap(2π·8x/W)| と 1e-14 で一致し、値域は [0, π] の半回転。
   明るい線の頂で 0、段差で π/2、暗い線の底で π —— 振幅は 1 のまま動かない。
2. 色回転: q·x·q* は既知の回転を**厳密に**与える。往復・ノルム保存も機械精度。
   Hamilton 積を 2 回(左に q、右に q*)組むと quat_color_rotate と 1e-16 で一致。
   同じ回転子でも左右は別物(標準正規の場で max|left-right| = 2.10、データの極値 4.06)。
   共役は対合(bit 単位で戻る)、q·q* はベクトル部が厳密に 0 でスカラー部が |q|²。
...
def run():
    ...
    print()
    print("PASS: quat 19 op すべてが閉形式のグラウンドトゥルースと一致し、"
          "勝つところ・勝たないところ・負けるところを実測で分けて示した")
    return out
