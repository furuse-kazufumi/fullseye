...
実データでできるのは §2 の Δh 感度(底面を上下させて在庫がどれだけ動くかを
...
4. ★**遮蔽の広さは幾何で厳密に予測できる**。円錐は**線織面**なので、母線ごとに
   ...
   実測 0.5539(差 -0.0171。距離・目線を 6 通り振って差は最大 0.0201)。
   ★差は模型の誤りではなく**格子の離散化**で、セルを 1.2 → 0.6 → 0.4 m と
   細かくすると差は 0.0339 → 0.0171 → 0.0120 と **1 次で縮みます**
   (境界は母線 1 本 = 直線なので、そこに並ぶセルの幅がそのまま面積の誤差)。
...
10. **2 つの誤差は足し算になりません**。底面の仮定だけ(全周・うねり)+1.79 %、
    遮蔽だけ(1 か所・平ら・真の底面)+23.42 %、水平底面でその遮蔽だけを見ると
    -2.49 %。足すと -0.70 % のはずが、両方入れた実測は **+0.51 %**。
    遮蔽の補間が**底面を決める外周まで動かす**ので、2 つは独立ではありません。
    誤差収支を「底面 x % + 遮蔽 y %」と足し上げる報告は、この時点で嘘になります。
11. ★**安息角は地面の上では読めません**。仕込んだ 37.0 度の法面は、平らな地面の
