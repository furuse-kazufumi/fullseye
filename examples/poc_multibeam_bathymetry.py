...
値でしかない)。実データでできるのは §9 の**隣接測線の重なりの食い違い**
...
1. **ゼロ点(表面の音速計だけを見て定数と仮定)は、直下ビームからもう失格**。
   水深 50 m・水柱の音速差 -40 m/s の場面で、表面音速 1520.0 m/s をそのまま
   使うと直下で **+0.670 m** 深く出ます。IHO S-44 Order 1a の鉛直不確かさ
   TVU(50 m) = 0.820 m の **82 %** を、いちばん素直なビーム 1 本で
   食い潰しています(70 度では -3.545 m)。
2. ★**平均音速を直下ビームで較正すると、誤差は「消える」のではなく
   「外側へ移動」します**。較正後の直下は誤差 0.000 m ちょうどですが、
   65 度は **-2.697 m**、70 度は **-4.159 m**。★これが厄介な非対称です ——
   現場は直下の深さをバーチェックや錘で確かめるので、**検査が届く所だけが
   直り、検査が届かない所に全部が寄ります**。
3. ★**崖の位置は測る前に閉形式で出せる。ただし「屈折だけ」の話として**。
   ラフな展開式 Δz ≈ (g D²/2c₀)·tan²θ の予測が **48.15 度**、厳密な円弧の
   閉形式の予測が **48.68 度**。エコー検出を止めた経路の実測は **48.68 度**
   (厳密式と 2.0e-11 m 一致)。★ところが**全経路(ビーム形成 + 振幅検出)
   を通した実測は 47.95 度**で、予測より **0.73 度 手前**に来ました ——
   閉形式に入っていない量(4. の検出偏り)があるからです。
   展開式のほうは 45 度までは 3.0 % 以内ですが、70 度では **19.4 %** 過大
   (厳密 -4.159 m に対し展開 -4.966 m)。「tan² で効く」は外側で崩れます。
4. ★★**予測を 1 つ外しました。「エコー検出の誤差は無視できる床」ではない**。
   直下では検出のずれ -2.9 µs(深さ -0.002 m)ですが、**70 度では -2828 µs
   ＝ -0.725 m**。ビームが照らす帯が 70 度では **21396 µs**(直下の 171 倍)
   に伸び、往復時間が角度に対して凸なのでエコーが非対称になり、振幅検出の
   頂点がビーム軸より手前へ寄るためです。実機が外側ビームで位相検出へ
   切り替える理由がここに出ました。★**「床は一定」と書いてはいけません**。
5. ★**層の切り方だけで TVU を割ります**。層内を等音速とみなす古い実装
   (境界の曲がりは `fs.ledger.snell_angle`)は、65 度・50 m で
   **2 層なら +1.299 m**、32 層で +0.079 m。層数 16 倍で誤差 16.6 分の 1 ＝
   **1 次収束**。★キャストを何層に切るかは前処理の趣味ではなく、
   精度そのものです。一方、層内の音速を 1 次とすれば層数を 1→256 に
   変えても走時は 1.6e-15 s しか動きません(円弧が厳密だから)。
6. **要因を 1 つずつ止めた対照群**(65 度)。勾配ゼロ + 真の平均音速では
   1e-12 m 未満、角度推定だけを入れても **0.000000 m**(`beamform_doa` の
   誤差は 0.0000 度)、エコー検出だけで -0.222 m、屈折だけで **-2.697 m**、
   全部で -3.024 m。**スマイルは角度誤差ではありません**。
7. ★**深い所ほど崖は浅い角度に来ます**。IHO の許容 TVU は深さに比例して
   増えますが(b·d の項)、屈折誤差も深さに比例して増えるので、深水では
   **崖の角度が深さに依らなくなります**。予測した漸近値
   arctan√(2·b·c₀/|Δc|) = **44.83 度**に対し、200 ケース(音速差 25 通り ×
   深さ 8 通りの格子、乱数ではない)の実測は 10 m で 64.47 度 → 200 m で
   **45.53 度**。漸近値との差は 19.64 → 0.70 度 と**単調に**縮みますが、
   ぴったりには乗りません(TVU の定数項 a = 0.5 m が 200 m でもまだ
   b·d = 2.60 m の 2 割ある)。★**「浅い所は危ない」は逆で、浅い所のほうが
   swath を広く使えます**。±65 度の swath は 200 ケース中 **156(78.0 %)**
   が端で Order 1a を割りました。
8. ★**上向き屈折(frown)は、外側ビームが海底に届きません**。音速が深さ
   とともに増えると sinθ = p·c が増え、θ₀ > arcsin(c₀/c_bottom) の光線は
   反転して戻ります。Δc = +60 m/s・50 m で予測した限界角 **73.90 度**に
   対し、実測で最後に返ったビームは 73.0 度、最初に返らないビームは
   74.0 度(1 度刻み)。★**深さが誤るのではなく、そのビームだけ何も
   記録されない** —— swath が黙って狭くなります(届いたビームは逆向きに
   曲がり、65 度で +5.919 m と深く出ます)。
9. ★**フットプリントの教科書式 d·Δθ/cos²θ は、電子的に振ったビームには
   足りません**。`beamform_delay_sum` で自分の配列のビーム幅を実測すると、
   ステア 0/45/70 度で 1.100 / 1.550 / 3.150 度 ＝ **1/cosθ で広がって
   います**(予測 1.058 / 1.496 / 3.092 度、比のずれは最大 6.5 %)。指数は
   2 でなく **3**: 70 度の等音速対照群で cos² 式 **8.21 m**、cos³ 式
   23.99 m、光線から直に測った実測 **24.14 m**(cos³ は -0.6 %)。
   **cos² 式は実測の 34 %** しかありません。なお屈折を入れると同じ
   フットプリントは 18.47 m へ **-23.5 %** 縮みます(別の要因なので分けて
   測りました)。
10. ★**隣接測線の重なりに段差が立ちます**。測線間隔を swath の半幅
    (101.4 m)= 重複 50 % にすると、重なりの**真ん中**では両測線が同じ振れ角なので差は 0.0000 m
    ですが、**帯の端**では片方の直下ビームと片方の最外ビームが同じ海底を
    測り、食い違いは **2.697 m**(TVU の 3.3 倍)。★重なりを平均で見ると
    ゼロに見えるので、**帯の端まで見ないと見つかりません**。これは真値を
    使わない検査なので、**実データでもできる唯一の検査**です。
11. ★**平らな海底が「地形」に見えます**。`dem_slope` で測ると、測線 1 本
    (継ぎ目なし)でも平均 1.494 度 / 最大 **2.985 度** の見かけの勾配が
    立ち(スマイルの曲がりそのもの)、測線 2 本を格子化すると重なり帯が
    **縞**になって最大 **49.4 度**。対照群(真に平らな海底を同じ手順で)は
    0.000000 度。どちらも海底図の上では「谷の縁」に見えます。
12. **サーモクラインは一定勾配で書けません**。混合層 + サーモクライン +
    深層の真値に対し、等音速処理の最大誤差 5.357 m を、表層と海底の 2 点を
    結ぶ一定勾配の当てはめは 1.253 m まで(77 %)しか落とせません。
    真のプロファイルで処理すれば 1.65e-07 m なので、**残りは処理の実装
    ではなくプロファイルの形の取り違え**だと切り分けられます。
    ★おまけに当てはめは**直下を +0.262 m 悪くします**(等音速側は直下較正
    済みで 0 だった)—— **外側を良くする代わりに、いちばん検査される所を
    犠牲にしています**。
13. **道具の穴**。`fs.<name>` / `fs.op.<name>` / `fs.ledger.<name>` /
    `fs.op_find(語幹)` の 4 層すべてを引いて、**音響測深そのものの op は
    1 つもありませんでした**(``sonar`` / ``swath`` / ``bathym`` / ``tvu``
    は 4 層とも 0 件。``sound_speed`` / ``footprint`` / ``crossline`` は
    ``op_find`` が 4 / 2 / 9 件を返しますが中身は ``flow_speed`` /
    ``sk_median_disk`` / ``piv_cross_correlate`` で無関係 —— ★**件数だけ
    見て「在る」と言ってはいけません**)。使えたのは `beamform_delay_sum` /
...
def section_dtm(scene):
    ...
    print(f"  見かけの勾配(dem_slope、**測線 1 本・継ぎ目なし**): "
          f"平均 {float(slope1.mean()):.3f} 度 / 最大 {float(slope1.max()):.3f} 度")
    print(f"  同(**測線 2 本・継ぎ目あり**): 平均 {float(inner.mean()):.3f} 度 / "
          f"最大 **{float(inner.max()):.3f} 度**")
    print(f"  対照群(真の平らな海底を同じ手順で)最大 {flat_slope:.6f} 度")
    print("  → ★2 つは別物。**継ぎ目なしの数度**はスマイルの曲がりそのもの")
    print("     (平らな海底が浅い谷に見える)。**継ぎ目の数十度**は測線の"
          "貼り合わせで、\n     海底には無い崖が 1 本立つ。地形図として読むと"
          "どちらも「地形」に見える。")
    if figs.enabled():
        figs.save_grid("dtm",
        ...
        # ★縦軸は**高さ**(= −深さ)にする。深さのまま描くと上下が逆さになり、
        #   「笑っている」形が「しかめ面」に見えてしまう。
        figs.save_plot(
            "across_track",
            [("測った海底", y_meas, -z_meas),
             ("真の海底", y_meas, np.full_like(y_meas, -DEPTH_REF))],
            xlabel="航跡直交距離 [m]", ylabel="海底の高さ = −深さ [m]",
            title="1 発の ping の断面 —— これが「スマイル」",
            caption=f"直下は較正済みで一致。端 65 度で "
                    f"{float(DEPTH_REF-z_meas[-1]):+.2f} m 持ち上がる —— "
                    f"平らな海底が口角を上げた形に記録される。")
...
def section_thermocline(scene):
    ...
    if figs.enabled():
        ...
        figs.save_plot(
            ...
            caption="等音速(水色)は直下の走時が真と一致するよう較正してある。"
                    "2 点の一定勾配(橙)は平均としては近いが、混合層の段を"
                    "持たないので直下ですら合わない。")
...
def section_op_holes():
    ...
    print("      ★これを書いていて**別の片道の穴**が出た: ベクトル版 `refract` の")
    print("      docstring は「1 本でも全反射があるとバッチ全体が None。1 本ずつ")
    print("      回すこと」とだけ書いていて、**光線ごとに TIR を判定する")
    print("      `refract_rays` が既にある**ことに触れていなかった(逆向きの参照は")
    print("      在った)。名前の見つけやすい側から辿ると遠回りさせられる ——")
    print("      2026-09-08 に両方の docstring へ相互参照と `eta = 1/c` を書いた。")
