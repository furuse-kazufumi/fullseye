...
**危険(物理)と停止判定(装置)を分けて数える**のがこの PoC の骨格です。
危険 = 真の分離距離 < S_geom(0.640 m、止め始めても間に合わない距離)、
停止判定 = 推定 < S_geom + Z_r + Z_d(既定 0.690 m)。**見落とし**(危険なのに
止めない)と**誤検知**(危険でないのに止める)を最後まで畳みません。

この PoC が示すこと(数字はいずれも実行時に印字される実測値。掃引は 4 試行 ×
41 フレーム = 164 フレーム、うち危険 35):

1. **ゼロ点(人を重心 1 点 + 半径 0.30 m の球で代表)を先に測る**。★予想は
   「真の距離は『腕の伸び + 体の厚み - 被せた半径』ぶん過大評価される」=
   +0.232 m。**実測は危険時の平均 +0.166 m、最大 +0.304 m** —— 桁も向きも
   予想どおり。危険の **14.3 % を見落とす**のに、誤検知は 0.8 % しか出ない。
   **壊れ方が片側**(推定は必ず「もっと遠い」側へ寄る)。
2. ★**足元 1 点(2-D 安全レーザスキャナ相当。脛より下の点だけ)はもっと悪い**。同じ
   164 フレームで過大評価は危険時 +0.199 m / 最大 +0.338 m、見落とし
   **28.6 %**(重心の 2 倍)。高さを捨てると「伸ばした手」が丸ごと消える。
3. ★★**見えている部位だけで測ると危険が消える**。全表面が見えれば推定は真値に
   張り付く(平均 +0.006 m、見落とし 0.0 %)。ところが背面カメラ 1 台では
   見える面が 32.5 % まで落ち、**危険フレームの 48.6 % で「推定の最小が真の
   最近傍部位とは別の部位から出て」**いる(うち 17.1 % は最近傍部位が 1 点も
   見えない完全な影)。過大評価は平均 +0.118 m / 最大 +0.230 m、見落とし
   **18.1 %**。2 台目(セル隅)を足すと **0.0 %** に戻る。
4. ★★**崖は 3 つあるが「誤りの向き」が違う**。理想(遮蔽なし・密・毎フレーム)
   でも誤検知は 3.4 % 出る —— S が S_geom より 0.050 m 大きい**設計どおりの
   余裕**ぶんで、これが基準線。遮蔽を入れるとその誤検知が **0.0 % に減り**、
   代わりに見落としが 18.1 % 出る。**遮蔽は誤りを危険な側へ移すだけ**。
   更新間隔(0.15 → 0.60 s)は両側に増やす(見落とし 22.9 % / 誤検知 3.9 %)。
   点密度は買い戻せる(100 点で 4.8 %、800 点で 0.0 %)が、**遮蔽は点を
   増やしても消えない**(1 台 800 点 18.1 % / 1 台 100 点 29.5 %)。
5. ★**点密度の予想は 2.7 倍外した**。標本間隔 s の球面上で最近点を s/2 逃すと
   過大評価は (s/2)²/(2r) —— 100 点(s=0.189 m)で 0.107 m と踏んだが、
   **実測は +0.040 m**。予想は「いちばん近い 1 点を必ず逃す」最悪を仮定して
   いて、実際には手の周りに数点が残るぶん甘くなる。更新間隔の予想
   (v_h·Δt = 0.102 m)も実測 +0.055 m で、やはり最悪側だった。
6. ★★**Z_d を測定から見積もると足りない**。最接近の姿勢を 24 回測り直した
   ばらつきから Z_d を出すと **0.036 m**(3σ)。ところが遮蔽による過大評価の
   95 % 点は **0.178 m** —— **5 倍**。**同じ点群を測り直しても遮蔽の形は
   変わらない**ので、繰り返し性には遮蔽が原理的に写らない。見落としを 0 に
   する最小の Z_d は 0.150 m で、そのとき機械が止まっている時間は
   18.3 → 24.4 % に増える。**安全の代金は稼働率で払う**。
7. ★**よく報告される指標(Chamfer)はこの危険に盲目**。真の表面点群と測定
   点群の Chamfer は 0.0388 m で「よく合っている」ように見えるが、Hausdorff は
   0.2481 m(**6.4 倍**)で、分離距離の過大評価(平均 +0.119 m / 最大
   +0.219 m)と同じ桁。ただし**フレームごとの相関は弱い**(Hausdorff で
   r = 0.099)—— 最悪値は「どこかに大穴がある」とは言えても
   「それが危険源の側か」は言えない。
8. ★**格子で測ると距離が遠くなる**。機械を占有格子にして ESDF を引く
   (`sdf_to_occupancy` + `esdf` + `query_distance`)と、厳密な CSG 距離に
   対して 40 mm 格子で +0.0104 m / 20 mm で +0.0056 m / 10 mm で +0.0024 m。
   **予想は半ボクセル(0.0200 / 0.0100 / 0.0050 m)だったが、実測は約
   1/4 ボクセル** —— 三線形補間が階段を均すぶん半分で済んでいた。符号は必ず
   正で格子に比例するので、これは Z_d に足す不確かさではなく**引き算で直す
   系統誤差**。

【グラウンドトゥルース】人は 10 本のカプセル(胴 2 + 頭 + 右腕 3 節 + 左腕 +
両脚 + 肩帯)、機械は 2 本のカプセル(リンク)+ 直方体(基台)、環境に治具台の
直方体。関節角と歩行は既知の時間関数。真の最小分離距離は**線分どうしの最短
距離 - 半径の和**(Ericson の閉形式。400×400 の総当たりと最大差 5.6e-05 m)と
**線分-直方体の凸 1 次元最小化**(三分探索 48 回、4001 点の総当たりより必ず
小さく、差 1.9e-04 m)で求める。表面点は解析サンプル(marching cubes の面積と
解析値の差 +5.2 %)、遮蔽は場面全体の SDF を視線上で評価するので**遮蔽の真値も
既知**。
...
# 歩行と手伸ばし
X0, V_WALK = 2.45, 0.17        # 初期位置 [m] と接近速度 [m/s]
# ★X0 と速度は「どの試行でも真の分離距離が正のまま(接触しない)」かつ
#   「危険な帯に十分な時間とどまる」ように選んである —— 速く歩かせると
#   危険な帯を数フレームで通り抜けてしまい、見落としを数える標本が足りない。
T_END, DT = 6.0, 0.15
REACH_T0, REACH_T1 = 1.8, 4.6  # 手を伸ばす区間 [s]

# センサ(既知の位置)。1 台目 = 入口の上に付けた背面カメラ(人の背中を見る)、
# 2 台目 = セル隅の高所。背面カメラは「伸ばした腕が胴の影に入る」典型的な配置。
SENSOR_BACK = (3.90, 0.05, 1.55)
SENSOR_COR = (2.20, 2.05, 2.35)
...
# --------------------------------------------------------------------------- #
#: 試行 = (横位置 y_h [m], 手を伸ばし始める時刻 [s], 歩行速度 [m/s])
TRIALS = ((0.22, 1.8, 0.170), (0.00, 2.4, 0.162),
          (-0.18, 1.4, 0.178), (0.35, 3.0, 0.172))
...
def section_sweep() -> dict:
    ...
    figs.save_table("sweep_occlusion",
                    ["条件", "見えた面", "過大評価 平均 m", "最大 m",
                     "見落とし", "誤検知"], occ_rows,
                    title="遮蔽は誤りを危険な側へ移す(誤検知が減り、見落としが出る)",
                    caption="理想でも誤検知が %.1f %% 出るのは、停止判定 S が"
                            "物理の危険 S_geom より %.3f m 大きい設計余裕ぶん。"
                            % (occ_fa[0], S - S_GEOM))
    print("\n  ★3 つとも崖を持つが**種類が違う**。")
    print("   1) 誤りの向き: **理想(遮蔽なし・密・毎フレーム)でも誤検知は %.1f %% 出る**"
          "\n      —— S が S_geom より %.3f m 大きい設計どおりの余裕ぶん。ここが基準。"
          "\n      遮蔽を入れるとその誤検知が %.1f %% に**減り**、代わりに見落としが %.1f %% 出る。"
          "\n      推定が必ず遠くなるので、**遮蔽は誤りを片側(危険な側)へ移す**。"
          "\n      更新間隔は両側に増やす(見落とし %.1f %% / 誤検知 %.1f %%)。"
          % (occ_fa[0], S - S_GEOM, occ_fa[2], occ_miss[2], lat_miss[-1], lat_fa[-1]))
...
def section_miss_map(t_ref: float = 4.6) -> dict:
    ...
    return {"miss_area": float(cell * np.count_nonzero(miss)),
            ...
            / max(1, np.count_nonzero(true_map < S_GEOM)),
            "over_max": over_near,
...
def main() -> int:
    ...
    print("  * 重心 1 点は危険時に %+.3f m / 最大 %+.3f m 過大評価する"
          "(予想 %+.3f m)。危険の %.1f %% を見落とし、誤検知は %.1f %%。"
          % (z0[1], z0[2], ts["pred"], z0[3], z0[4]))
    print("  * 崖は 3 つ: 点密度(16 倍疎で %.1f %%)・更新間隔(4 倍で %.1f %%)・"
          "遮蔽(1 台で %.1f %%)。前 2 つは買い戻せるが、遮蔽は点を増やしても"
