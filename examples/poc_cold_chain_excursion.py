...
この PoC は 12.0 m x 2.4 m のリーファー荷室の 12 時間を
**(t, y, x) の 1 つの体積**(720 分 x 60 x 12 セル)として作り、その体積を
fullseye のボリューム op で読みます。ロガーの記録は
:func:`fullseye.vol_profile_line` で体積から抜いた**線プローブ**そのもの、
逸脱は :func:`fullseye.vol_label` で数える **3-D の塊**、距離場はすべて
:func:`fullseye.esdf` です。

EXTEND: 実測に差し替えるなら :func:`make_scene` が返す ``vol`` (T, Y, X) を
実測の温度場に、``layout["is_product"]`` を「そのセルが製品か通路の空気か」に
置き換えます。**ロガー数点の記録だけでは足りません** —— この PoC の主題
(点の記録と荷全体の差)は点の記録には残らないので、CFD か多点マッピング
(空荷マッピングではなく**実荷**)の温度場が要ります。逆に真値が無くても
3〜5 章(崖の予測)と 7 章(3 指標の食い違い)は記録だけで再現できます。

【グラウンドトゥルース】
物理は既知の閉形式で組み立て、時間積分だけ厳密に解く。

* 供給空気 ``T_sup(y,x) = T_set + g·d_vent``。吹き出し口からの距離 ``d_vent``
  は :func:`fullseye.esdf`(占有 = 吹き出し口セル)。
* 壁からの侵入 ``h0·(1−e^{−d_vent/L_throw})·w(y,x)·(T_amb(t) − T_sup)``。
  ``w`` は 4 枚の壁それぞれの ``esdf`` から ``Σ e^{−d/λ_w}`` —— **隅は 2 枚
  ぶん効く**(1 枚の最短距離だと隅と辺の区別がつかず、いちばん危ない場所が
  消える)。吹き出し口の近くは風が持ち去るので侵入が効かない。
* 扉開閉は既知の時刻・幅(14/14/24/14/24 分、計 90 分)の矩形パルスで、
  扉からの距離に ``e^{−d_door/λ_d}``。
* 荷の熱容量は**1 次遅れ**。時定数は空気 5 分、製品はパレットへの潜り込み
  深さ(これも ``esdf``)で 64〜91 分。``T[n+1] = D[n+1] + (T[n] − D[n+1])
  ·e^{−Δt/τ}`` は駆動が階段状なら**厳密**。
* 真の劣化は Arrhenius(``ΔH/R = 10000 K``、基準 5 °C)の速度を時間積分した
  等価時間。真の MKT は Haynes(1971)の定義そのもの。

出発時はどのセルも規定内(最高 7.69 °C)—— そうでないと「逸脱」ではなく
「積み込み時点で不合格」になってしまう。

【この PoC が測って分かったこと(数字はいずれも実行時に印字される実測値)】

1. **ゼロ点(ロガー 1 個の「規定を外れた合計時間」)は置き場所で 0 分から
   585 分まで動く**。真の最悪製品セルは 598 分、製品 490 セル中 **108 セル
   (22.0 %)が本当に不合格**。それでも**製品セルに 1 個置くと 82.4 % の
   置き方が「合格」と言う**。
2. ★★**見逃しと空振りは違う要因から出る。要因を 1 つずつ止めると分かれる。**
   (a) 荷が均一なら偽合格も偽不合格も 0.0 %(「1 個で足りる」は均一を仮定
   している)。(b) 壁からの侵入だけ(扉は 1 度も開けない)で真に不合格な
   製品 6.9 %・**偽合格 95.5 %**、偽不合格は 0.0 %。(d) 壁を止めて扉だけ
   残すと真に不合格な製品は 2.0 % に落ちるのに**偽不合格が 3.5 % 現れる**
   —— 扉のパルスは空気だけを叩き、熱容量のある製品には届かないから。
   ★(c) 両方あると偽不合格は 0.0 % に**隠れる**(パルスが叩く空気のそばの
   製品は、壁のせいでどのみち傷んでいる)。
3. ★★**(d) では製品に貼ったロガーも役に立たない**: 真に傷んだ製品が 2.0 %
   あるのに、製品セルに置いたロガーは **100 % が合格**と言う。短いパルスは
   製品の中に入らないので、「製品に貼れば安心」も成り立たない。
4. ★**時定数の崖は先に予測できるが、教科書の式では足りない**。幅 W の矩形
   パルスを時定数 τ のロガーが見るとピークは ``A(1−e^{−W/τ})`` までしか
   上がらないので、余裕 M を割るのは ``τ > W/ln(A/(A−M))``。中央通路の
   4 か所(扉から 1.2〜4.2 m)で実測した崖は **138 / 63 / 35 / 19 分**。
   1 極の予測は 123 / 66 / 40 / 23、**「荷室の空気(τ=5 分)+ ロガー」を
   2 段に重ねた予測**は 113 / 58 / 34 / 19。★崖が短い側では 2 段の予測が
   **0〜3 %** に収まり、1 極は 21 % ずれる —— ロガーが完璧でも**空気そのもの
   が既に鈍らせている**から。逆に τ*=138 分の行だけは 1 極のほうが近く
   (11 % 対 18 %)、そこは余裕が 1.72 K しかなく外気上昇による底の drift が
   パルスと同じ大きさで、**どちらの式も前提を外れている**。
   扉のすぐ手前(余裕 0.31 K)は崖が 940 分と予測され、**そもそも測れない**。
5. ★**サンプリング間隔の崖は予測と完全一致**。真値が規定を超えている時間
   W_eff = 33 分のパルスは、Δt > W_eff で位相によって丸ごと落ちる。整数の
   位相を全部試した実測は Δt = 45/60/90 分で **26.7 / 45.0 / 63.3 %**、
   予測 ``1 − W_eff/Δt`` と**差 0.0 分ポイント**。
6. ★**量子化 0.5 K は逸脱時間を減らす側にしか効かない**。丸めた値が
   「> 8.0 °C」を満たすには真値が limit + q/2 以上要るので実効しきい値が
   上がる。真の逸脱 299 分のセルで q = 0.25/0.5/1.0/2.0 K の実測は
   **272 / 242 / 197 / 107 分**、予測(真値が limit+q/2 を超えた時間)と
   **差 0 分**。
7. ★★**同じ記録から出した 3 つの指標が合否で食い違う**。「12 °C に何分いて
   よいか」に換算すると 逸脱時間 ≤ 60 分 は **60 分** / MKT ≤ 8 °C は
   **276 分** / 劣化 ≤ 1.30 は **197 分** —— **4.6 倍**の開き。全 720 セルに
   ロガーを置いてみると **82 セル(11.4 %)で 3 指標が揃わない**(逸脱時間
   だけが不合格 26 セル / MKT だけが合格 56 セル)。
8. ★★**MKT は集計窓の長さで動く**。製品セル (y=44, x=0) は 12 時間で
   MKT 7.09 °C(合格)だが、逸脱を含む 4 時間で切ると **8.04 °C(不合格)**。
   逸脱時間は窓に入った分をそのまま足すだけで薄まらない(138 分 / 120 分)。
   **「MKT は 8 °C 以下でした」は窓を書かないと意味が無い。**
   逆に逸脱時間は**大きさに盲目**: 200 分 8.1 °C と 200 分 20.0 °C はどちらも
   逸脱 200 分だが、劣化は 1.047 と 2.381(**2.27 倍**)、MKT は 5.36 と
   11.88 °C。
9. ★**逸脱は 3-D の塊として数えられる**。``vol_label`` で (t,y,x) を切ると
   逸脱は **11 個**の塊で、いちばん大きい塊は t = 120 分に生まれて 600 分
   続き、5.8 m x 2.4 m(252 セル)に広がる。延べは 49732 セル分。
   ★``vol_region_props`` は同じラベルから曝露量 1978 m^2·min を返すが、
   同じ辞書の ``surface_area`` 8183 と ``sphericity`` 0.093 は**意味を持たない**
   —— 3 軸のうち 1 つが分、2 つが m だから(末尾「道具の穴 (g)」)。
10. **ロガーを増やすのは「独立に n 回引く」のと同じ**。製品セルにランダムに
    1/2/3/5/8/12 個置くと偽合格は **81.6 → 68.3 → 55.4 → 34.4 → 22.1 →
    9.3 %**、予測 ``p1^n``(p1 = 0.824)との差は最大 3.8 分ポイント。
    **10 % を切るのに 12 個要る。**
11. ★★**予想が外れた**。「慣用の 3 点(扉寄り・中央・吹き出し口寄り)は
    代表点を選ぶだけだからランダム 3 個より弱い」と踏んでいたが、実測は
    **規約のほうがずっと当たる**(3 点をそれぞれ ±1 m 揺らして偽合格
    **3.3 %**、ランダム 3 個は 55.4 %)。ただし内訳を見ると効いているのは
    **扉寄りの 1 点だけ**(単独 2.9 % / 中央 100.0 % / 吹き出し口寄り
    100.0 %)。規約が強いのではなく**この荷の最悪点が扉側にある**から
    当たった —— 送風の偏りで吹き出し口側が壊れる故障なら同じ 3 点は当たらない。

来歴(公開文献のみ): Haynes, *J. Pharm. Sci.* 60 (1971) 927 —— 平均動力学温度
(MKT)/ ICH Q1A(R2) *Stability Testing*(ΔH = 83.144 kJ/mol の既定)/
Seevers et al., *Pharmaceutical Technology* 33 (2009) —— MKT の誤用 /
...
def section_events(scene: dict) -> dict:
    ...
    if figs.enabled():
        ...
        body = hot[::4].astype(np.float32).transpose(2, 0, 1)   # (幅, 時間, 奥行き)
        proj = np.asarray(_L.render_volume_projection(
            body, azimuth=15.0, elevation=0.0, mode="xray"), np.float64).T
        figs.save_grid("excursion_body",
                       [np.repeat(np.repeat(proj, 4, 0), 4, 1),
                        plan_view(np.count_nonzero(hot, axis=0) * DT_MIN),
                        plan_view(np.asarray(fs.temporal_max(vol)))],
                       ["逸脱体の投影(横=時間 %d 分 / 縦=奥行き %.0f m)"
                        % (NT, NY * CELL_M),
                        "各セルの逸脱時間 [min]", "各セルの最高温度 [°C]"],
                       title="逸脱は (t, y, x) の 3-D の塊", ncols=1,
                       caption="1 枚目は render_volume_projection を幅方向から"
                               "掛けた xray 投影(明るいほど幅方向に厚い)。")
