...
def main():
    """章を順に実行する。import 時に走らせない(この PoC は 14.6 秒かかる)。"""
    ...
    # 「隣は綺麗なのに最後の 1 本だけ開く」は平均に潰れる形なので、継ぎ目を
    # 1 本ずつ並べる。右端(35-0)が閉じる継ぎ目。
    _si = np.arange(len(PAIRS_ADJ), dtype=float)
    figs.save_plot("seams",
                   [(nm, _si, np.array([seam_by_sys[nm][k] for k in PAIRS_ADJ]))
                    for nm in ("1 鎖(回転・ゼロ点)", "3 閉ループ拘束(厳密)",
                               "5 大域最適化(GN)")],
                   xlabel="継ぎ目の番号(右端 35 が最後に閉じる 1 本)",
                   ylabel="継ぎ目のずれ [px]",
                   title="誤差は消えていない。まだ測っていない 1 本に押し出されている",
                   caption="鎖はどの継ぎ目も綺麗なのに右端だけ跳ねる。"
                           "解き方を変えると跳ねが消え、全体も下がる。")
    _fi = np.arange(N_LOOP, dtype=float)
    figs.save_plot("pose_error",
                   [(nm, _fi, pose_by_sys[nm])
                    for nm in ("1 鎖(回転・ゼロ点)", "2 閉ループ誤差を等分",
                               "3 閉ループ拘束(厳密)", "5 大域最適化(GN)")],
                   xlabel="フレーム番号(方位角)", ylabel="真の姿勢からのずれ [px]",
                   title="閉ループ誤差だけを見て「良くなった」と言うと嘘になる",
                   caption="系 2(等分)は閉ループ誤差を下げるのに姿勢はかえって"
                           "悪化する。新しい観測を足さずに効くのは系 3。")
    ...
    _nn = np.asarray(N_LIST, float)
    _mm = np.array([np.mean(acc[n]) for n in N_LIST])
    figs.save_plot("drift",
                   [("実測(8 本の場面の平均)", _nn, _mm),
                    ("偏りが 0 なら(∝ √N)", _nn, _mm[0] * np.sqrt(_nn / _nn[0])),
                    ("偏りだけなら(∝ N)", _nn, _mm[0] * (_nn / _nn[0]))],
                   xlabel="鎖の枚数 N", ylabel="終端フレームのずれ [px]",
                   title="ドリフトの伸び —— ランダムウォークか、偏りか",
                   caption="実測 log-log 傾き %.2f。2 本の基準線は同じ N=4 から"
                           "引いてある。" % SLOPE)
