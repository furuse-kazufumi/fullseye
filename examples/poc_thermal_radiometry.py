...
1. **床はここ**。真値を植てて前向きに作った放射輝度を、同じ ε・T_refl・τ で
   逆算する往復の誤差は **1.75e-06 K**(校正表 0.05 K 刻みの補間誤差)、
   量子化を入れて 6.28e-03 K、NETD 30 mK を入れて 2.71e-02 K。
   ★ちなみに素直に 350.0 K で試すと往復誤差は**厳密に 0** になります ——
   その温度がたまたま表の節点に乗るからで、**0 を床と呼んだら嘘**でした。
   節点を外して測り直したのが上の 1.75e-06 K です。
2. ★**外れの正体は「実効波長の選び方」ではなく「式の欠け」でした**。
   帯域の実効 Planck 指数 n = c2/(λ_eff T) は、λ_eff を帯の中央に取ると
   実測(数値微分)から最大 **24.5 %**、放射輝度で重み付けた **1/λ** の
   平均に取ると **16.6 %** ずれます。どちらも高温・長波で**必ず過小**。
   温度微分に付く因子 e^x/(e^x−1)(x = c2/λT)を入れると誤差は
   **0.00 %** —— 素朴な形は **Wien 側(x ≫ 1)でだけ厳密**でした。
   MWIR 300 K は x = 10.9 でほぼ Wien(誤差 0.0 %)、LWIR 800 K は
   x = 1.80 で Rayleigh–Jeans 寄り(誤差 16.6 %)。同じ式の当たり外れの正体。
3. ★★**予測を 1 つ外しました**。「低温ほど崖が急」と印字してから測ったら、
   **絶対誤差では逆**でした。Δε/ε = 5 % の同じ取り違えが LWIR で
   305 K の **0.233 K** → 800 K の **16.907 K**。log|ΔT| を log T に
   `fs.poly_fit` で当てると傾き **2.717** で、内訳は T/n が 1.741 +
   反射因子 (1−L_r/L_o) が 0.977 = 2.718。**閉形式は最初からそう
   言っていて、自分の式を読み違えていました**。
4. ★★**「低温ほど急」は正しかったが、犯人が違いました**。現場が使うのは
   絶対温度でなく**周囲からの上昇** (T_obj − T_refl) です。その比で見ると
   放射率の誤差は 上昇 2 K で 4.7 % → 上昇 500 K で 3.4 %(**1.40 倍しか
   動かない**)—— T_obj → T_refl の極限で Δε/ε に収束し、**発散しません**。
   発散するのは **T_refl の取り違えと雑音の床**のほうで、T_refl を 5 K
   取り違えた誤差は 上昇 2 K で **13.2 %**、上昇 500 K で 0.0 %(**972 倍**)。
   上昇 10 K を切ると反射のほうが放射率より重くなります。
5. **低放射率は無条件に急**。**同じ絶対** 誤差 Δε = 0.02 が、塗装面
   (ε=0.95)で 0.871 K、酸化面(ε=0.60)で 1.365 K、研磨金属(ε=0.10)で
   **7.183 K** —— **8.2 倍**。★逆に **相対** 誤差 Δε/ε で見ると
   閉形式から ε が約分で消え、3 本の曲線が**完全に重なります**。
   「放射率を ±2 % で入れた」の ±2 % が相対か絶対かで、意味が別物です。
6. **MWIR は放射率に強く、反射には強くありません**。350 K で n が
   LWIR 4.020 に対し MWIR 9.526(**2.37 倍**)なので、Δε/ε = 5 % の誤差は
   LWIR 2.019 K に対し MWIR 1.407 K。ところが T_refl を 10 K 取り違えた
   誤差は ε=0.10 の面で LWIR **−89.6 K** / MWIR **−49.6 K** で、桁が同じ
   —— **(1−ε)/ε の重みは帯に依らない**からです。
7. **ゼロ点 3 つはどれも失格**。350 K・τ=0.949 の場面で、DN を 2 点校正で
   線形に温度と呼ぶと ε=0.95 で **−12.15 K**(ε=0.10 で −46.22 K)、
   ε=1 固定(= 見かけ温度)で **−2.12 K**(ε=0.10 で **−44.02 K**)、
   反射項を落とすと **+2.35 K**(ε=0.10 で **+244.12 K**)。
   ★いちばん質が悪いのは ε=1 固定です —— 絵は完全にもっともらしく、
   単位も ℃ で、**どこにも異常が出ません**。
8. ★★**独立 RSS の「95 % 区間」は 95 % を包みません**。酸化した金属面
   (ε=0.60±0.05、T_refl=300±5 K、τ=0.949±0.02、校正利得 ±0.5 %、
   NETD 1.98 DN)で ε と T_refl に相関 ρ=+0.7 を入れた現実を 20000 試行
   数えると、包含率は **RSS 88.03 % / 相関つき MC 94.44 %**。
   ★**床を先に測ってあります**: ρ=0 の現実では RSS **95.03 %** /
   MC 94.52 %(標本誤差 ±0.30 点)—— **相関が無ければ RSS は正しい**ので、
   落ちた 6.4 点は実装ではなく**相関を無視したこと**そのものです。
   真の合成不確かさ 5.6518 K に対し RSS は 4.5086 K = **区間が 20.2 % 狭い**。
   ρ=−0.7 なら RSS は逆に **99.64 %** と過剰に包みます。
9. ★**外れ方は片側に寄ります**。ρ=+0.7 の RSS の取りこぼし 11.97 % の
   内訳は 下側 **7.14 %** / 上側 **4.83 %**。逆算が ε で割る非線形なので
   伝播した分布が右に歪み(p95−中央 = 10.96 K に対し 中央−p5 = 8.04 K)、
   **対称な ±k·u は歪んだ分布を対称には外しません**。
10. ★★**guard band(合否判定)で誤合格が 5.6 倍になります**。「表面温度
    ≤ 80 ℃ なら合格」を 40000 試行(真値は 65〜95 ℃ の一様分布、真に不合格
    19833 個 / 真に合格 20167 個)で判定すると、誤合格率は 不確かさ無視
    **8.03 %**(3212 件)/ RSS の guard band **0.81 %**(323 件)/
    相関つき MC の guard band **0.14 %**(58 件)。誤不合格率は逆に
    6.69 % / 28.22 % / **40.03 %**。★**誤合格を減らす唯一の代償が誤不合格**
    で、選べるのは交換レートだけです。MC の band が広い(13.47 K 対 8.84 K)
    のは相関(u が 4.51 → 5.65 K)**だけでなく**歪みの分もあります ——
    対称な 1.96·u なら 11.08 K のところ、上側の 97.5 百分位は 13.47 K。
11. ★**単位の取り違えは「例外で止まる」ものと「静かに間違う」ものに
    分かれます**。8 通り試して **例外 3 / 静かに 5**。止まったのは
    (a) 波長を µm のまま SI 式に入れた(L が真値の 1.5e+07 倍)、
    (b) T_refl をセ氏の数値のまま入れた(22 K は表の外)、
    (c) T_refl を 295 ℃ と書いた**うち ε=0.10 の面**——
    どれも `fs.interp_linear(out_of_range='raise')` が校正表の外だと言って
    落ちます(fail-closed が効いた)。静かに間違うのは (d) 同じ 295 ℃ を
    ε=0.95 の面でやる(**−22.1 K**。568 K は表の中なので通る)、
    (e) DN を平均してから温度に直す(**+1.543 K**。L が T に凸なので
    Jensen の不等式ぶん必ず高く出る)、(f) 見かけ温度を表面温度と呼ぶ
    (−2.121 K)、(g) τ を二重に掛ける(**+2.485 K**)、(h) ΔT/T をセ氏で
    計算して n を出す(n を **4.46 倍**に見積もる = 感度を 1/4.46 に過小評価)。
    ★**止まるかどうかは取り違えの種類でなく「場面」で決まります** ——
    (c) と (d) は**同じ間違い**で、放射率が高い = 現場で安心な面ほど、
    間違いが静かに通ります。
12. ★**熱画像の上で「いちばん熱い所が いちばん冷たく見えます」**。
    塗装面(ε=0.95)の平らな発熱台の真ん中に研磨ボルト(ε=0.10)があり、
    **両方とも 375.0 K** の場面で、ε=1 の見かけ温度はボルトを **309.7 K**、
    すぐ周りの塗装面を 372.0 K と表示します —— 差 **62.3 K**。絵の上では
    ボルトだけ「冷えている」= 健全に見えます。ε 地図を入れて補正すると
    375.0 K に戻ります(全面の残差 rms 0.106 K、見かけ温度なら 8.361 K)。
    ★ただし**補正は偏りを消す代わりに雑音を増やします**: 補正後の残差 rms は
    塗装面 0.0571 K に対しボルト **0.3673 K**(6.4 倍)。ε の比 9.5 より
    小さいのは、ボルトの DN が低くて光子雑音も小さいから —— **2 つの効き方が
    逆向き**です。同じ絵の上で、DN を平均してから温度に直すと
    **+0.994 K** 高く出ます(11. の (e) を実画像で数えたもの)。
13. **道具の穴 —— この分野の op は 1 つもありません**。`fs.<name>` /
    `fs.op.<name>` / `fs.ledger.<name>` / `fs.op_find(語幹)` の 4 層すべてを
    引いて、``planck`` / ``emissiv`` / ``temperature`` / ``radiom`` /
    ``thermal`` / ``infrared`` / ``stefan`` / ``kelvin`` / ``atmos`` /
    ``guard`` は **4 層とも 0 件**。★``blackbody`` は `op_find` が **4 件**
    返しますが中身は ``morph_blackhat3d`` / ``cv_blackhat`` などのモルフォロジで、
    **件数を見て「在る」と読むと外します**。``uncert`` の 1 件は
    ``photon_uncertainty``(Poisson の誤差棒。使いました)、``montecarlo``
    の 1 件は ``tolerance_analysis``(レンズ製造公差専用で、任意の測定モデルの
    GUM 伝播には使えない)。使えた op は `interp_linear` / `stat_covariance` /
...
14. ★★**ついでに道具の穴を 2 つ見つけ、重いほうはその場で直しました**。
    (a) `fs.noise_sigma(method="mad")` は**整数値の画像で階段状**になります。
    MAD は |x−中央値| の中央値なので、値が整数なら MAD も整数 —— 返り値は
    1.4826 の倍数しか取れません。σ=1.983 の DN 雑音を丸めた画像で **1.483**
    (連続値なら 1.954、`method="clip"` なら 1.918)、σ=0.5 では **0.0** ——
    「この画像に雑音は無い」と読める、もっともらしい嘘です。14 bit の生 DN は
    まさに整数なので、赤外・可視を問わず生画像に対して普通に踏みます。
    ★**下流まで追いかけたら、もっと悪いことが起きていました**: 200x200 の整数
    フレームに**点目標を 2 個植てて `fs.star_detect` が 0 個**を返します(例外なし)。
    `star_detect` には `sigma <= 0` の門が**在った**のに、コメントが
    「完全に平坦 = 雑音が測れない」と書いていて —— **門の前提のほうが誤り**でした。
    σ が 0 になる道はもう 1 本あり、そのとき画像は平坦ではありません。
    ★**直しました**(2026-09-08、`astrostack.py`): 値を返す `noise_sigma` は拒否せず
    **警告**(呼び手が `method="clip"` を選べる)、答えを出す `star_detect` は
    **平坦なら空・平坦でなければ拒否**。空を返すのは保守的な答えではなく誤った答えです。
    同じフレームで `method="clip"` に替えると、植てた 2 個がちゃんと出ます。
    (b) `fs.interp_linear` は **1-D の問い合わせしか受けません**。画像
    ``(H, W)`` を校正表で引くのに、呼び手が毎回 ravel/reshape します(未修正)。
...
(最大 8.9e-16 の相対差。Simpson の節点を 33 → 129 → 513 と増やすと
積分そのものも 4.8e-08 → 1.9e-10 → 6.9e-13 と落ちる)。対照群は
(a) ε=1 の見かけ温度、(b) 反射項を落とした処理、(c) **相関 ρ=0 の現実**
(= RSS が正しいはずの床)、(d) MWIR 帯、(e) 表の節点に乗る温度と外した温度。
大気透過率は定数を書き写さず `fs.beer_lambert_transmittance` から出しました
(σ = 2.6e-05 /mm・光路 2 m → τ = 0.9493。σ は仮定)。
...
def section_scene():
    ...
    print("     (`method='clip'` は同じ画像で正しく出る。)")
    print("     ★**直した**(2026-09-08): この潰れ方をしたら `noise_sigma` は"
          "**警告**を出す。")
    print("     さらに下流の `star_detect` は、**平坦なら空・平坦でなければ拒否**"
          "へ道を分けた ——")
    print("     整数フレームに植えた点目標 2 個を **0 個**と返していたため"
          "(空は保守的な答えではなく誤答)。")
...
def section_cliff(scene):
    ...
    return {"idx_rows": idx_rows, "worst_pred": worst_pred, "worst_eff": worst_eff,
            ...
            "worst_corr": worst_corr, "slope": slope, "slope_tn": s_tn,
            "slope_fac": s_fac, "ratio_eps": ratio_eps, "ratio_refl": ratio_refl}
...
def section_units(scene):
    ...
    print("     止まったのはどれも `fs.interp_linear(out_of_range='raise')` が")
...
def scene_truth(h: int = 180, w: int = 240):
    ...
    # ★発熱部は**平らな台**にする。山にすると「ボルトと周りが同じ温度」が
    #   成り立たず、見かけの谷が放射率のせいなのか温度のせいなのか分けられない。
    rr = np.hypot(xx - 120.0, yy - 92.0) / 40.0
    s = np.clip((1.0 - rr) / 0.25, 0.0, 1.0)
    t = 305.0 + 70.0 * (s * s * (3.0 - 2.0 * s))          # smoothstep の台
    eps = np.full((h, w), EPS_PAINT)
    # 研磨ボルト 2 本。ひとつは台の真ん中、もうひとつは冷えた縁の上。
...
def section_image(scene):
    ...
    print(f"  補正後の残差 rms = {rms:.3f} K(雑音の床)。見かけ温度の rms = "
          f"{rms_app:.3f} K。")
    # ★補正は偏りを消すが、雑音を 1/ε 倍にする。図(d)でボルトだけざらつく理由。
    rms_bolt = float(np.sqrt(np.mean((t_corr - t_true)[bolt] ** 2)))
...
def section_op_holes(unc, units):
    ...
    print("    * `interp_linear` は **1-D の問い合わせしか受けない**。校正表で画像を")
    print("      引くたびに呼び手が ravel/reshape する(`BandTable.radiance`)。")
    print("      画像を扱う道具箱の校正表としては、ここは通したほうがよい。")
    print("  ★★**この PoC が見つけた本体の不具合 —— 見つけて、その場で直した**:")
    print("    * `noise_sigma(method='mad')` が**整数値の画像で階段状**になる(§1)。")
    print("      返り値は 1.4826 の倍数しか取れず、σ=0.5 の整数画像では **0.0** ——")
    print("      「雑音は無い」と読める、もっともらしい嘘。生 DN はまさに整数。")
    print("      最小再現: `fs.noise_sigma(np.rint(1000+rng.normal(0,0.5,(64,64))),")
    print("      method='mad')` → 0.0(`method='clip'` なら 0.567)。")
    print("    * ★下流はもっと悪かった: 整数フレームに植えた**点目標 2 個を")
    print("      `star_detect` が 0 個**と返す。門は**在った**が、コメントが")
    print("      「完全に平坦 = 雑音が測れない」と書いていて**前提が誤り**だった。")
    print("      しかも `_robust_background` のコード内コメントには「MAD が 0 に")
    print("      潰れる量子化された画像向け」と `clip` の存在が書かれていた ——")
    print("      **知識は在ったが、読む側(docstring と既定値)に無かった**。")
    print("    * 直し: `noise_sigma` は警告(値は返す)、`star_detect` は")
    print("      **平坦なら空・平坦でなければ拒否**。試験 2 本追加。")
    print("      `docs/HARDENING.md` に台帳として残した。")
