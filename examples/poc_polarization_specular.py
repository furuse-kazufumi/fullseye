...
def main():
    ...
    for sigma in (0.0, 1e-4, 1e-3, 2e-3, 3e-3, 4e-3, 5e-3, 1e-2, 3e-2, 1e-1):
        ...
        print(f"  {sigma:>10.0e}{verdict:>18}{frac:>12.5f}{e:>16.3e}"
              f"{e / bias70:>14.3f}")
    print("  → **既定の fail-closed が拒否に転じる境界は σ = 3e-3 と 5e-3 の間**")
    print("     (暗パッチの拡散 0.02 = 最小輻度 0.01 に雑音が届く点。12288 画素の")
    print("     うち 0.1% が負の最小輻度を吐いた時点で全体が止まる)。")
    ...
    print("     小さい**: 1 度あたり 1.14e-4 で δ に厳密比例する。20 度ずらしても")
    print("     70 度の偏りの 0.09 倍で、これは想定と逆だった。**この 3 つの壊れの")
    ...
    print("     しかも壊れ方が急峻: 飽和 0.4% で 70 度の偏りに並び、3% で 5 倍、")
    print("     6% で拡散平均の 21% を誤る。雑音(σ に比例)や較正誤差(δ に比例)")
    print("     と違い、**飽和は画素率に対して線形ではない**(飛んだ画素の中では")
    print("     偏光情報が完全に失われるので、当てはめが平気で嘘をつく)。")
