...
1. **ゼロ点(生画像に Pearson と Otsu-Manders)**: 漏れ込み α=β=10 % で真の共局在
   0 % の r が 0.203(理想 0.009)、M1 が 0.133(真値 0.000)。25 % では M1 0.254
   (真値 0.246)と**一見当たっている**が、50 % で 0.401(真値 0.482)、100 % で
   0.705(真値 1.000)—— 当たりは偶然。r は 100 % で 0.955 と壊れない:
   **漏れ込みは「無関係」を「少し関係」に見せるが、「完全に一致」は壊さない**。
2. ★**単染色対照から漏れ込み行列を推定して線形分離(``mat_lstsq`` + ``mat_solve``)
   すると Pearson は戻る**: α の推定 0.0996(真値 0.10)、β 0.0988。分離後の r は
   0.007 / 0.171 / 0.383 / 0.933(理想 0.009 / 0.183 / 0.406 / 1.000)。
   ★**ところが Manders は戻らない**: 分離後も 0.076 / 0.198 / 0.342 / 0.691 で、
   100 % でさえ −31 pt。**Otsu より下の Gaussian の裾が領域に入らない**からで、
   ピーク p の点のうち領域内の蛍光は閉形式 1−(T−台)/p。その予想 0.756 に対し
   実測 0.705(予想は 5 pt 甘い —— 雑音がしきい値の縁を虫食いにする)。
3. ★★**Costes のシャッフル検定は漏れ込みを見抜けない**。真の共局在 0 %・
   α=β=15 % の画像で r=0.293、ブロック並べ替え 200 回の p 値は 0.000
   (=「有意に共局在」)。検定が答えているのは「r は偶然か」であって
   「r は共局在か」ではない。分離後は r=0.002、p=0.470(漏れ込みなしの 0 % は p=0.195)。
4. ★**崖(Pearson)は幾何で先に当たる**。A, B が無相関なら対称な漏れ込み
   α=β で ``r = 2α/(1+α²)``、r=0.5 は α=2−√3=0.268。実測の交点は 0.282
   (雑音が分母を太らせるぶん少し遅い。13 点すべてで予想との差 ≤ 0.023)。
   **片側だけ(β=0)なら ``r = α/√(1+α²)`` で、30 % でも 0.289(予想 0.287)——
   0.5 には届かない**。同じ「漏れ込み 30 %」でも対称か片側かで r は 2 倍違う。
5. ★★**Manders の崖は Pearson とは別の場所にあり、途中に相殺点がある**。真の共局在
   50 % で、漏れ込みなしの値から Pearson が +0.10 動くのは α=β=0.067、Otsu-M1 が
   +10 pt 動くのは 0.178。M1 は α=0 で真値より −13.9 pt(裾落ち)から出発して
   漏れ込みで上がり、**α=0.222 で真値 0.482 を横切る** —— そこで「正確」に見えるのは
   裾落ち(−)と漏れ込み(+)が釣り合っただけ。真値だけ見て α を選ぶと、漏れ込み
   22 % の装置を「最良」と呼ぶ。
6. ★★**ぼけの崖は Manders に来る、Pearson には来ない**。漏れ込みなし・真の共局在
   0 % で PSF σ を 0.5 → 4.0 px に振ると r は 0.011 → 0.089(細胞質なしなら
   0.011 → 0.048。独立な点過程はぼかしても無相関で、残りは ROI の縁の共通の減衰)。
   M1 は 0.044 → 0.946。予想は「偶然の重なり = B の Otsu 領域が ROI に占める面積率」で
   実測との相関 1.000。**崖は σ=2.5 px**: Otsu のしきい値 T_B=0.083 が細胞質の台
   0.10 を割り、前景が「小胞」から「細胞体」に飛び移る(小胞のピークは PSF で
   0.265 倍 → 0.187 倍に潰れ、細胞と外の 2 山の方が大きくなる)。
   予想は「M1 はなだらかに増える」だったが、実際は 0.221 → 0.902 の**段差**。
7. **対照群で原因を分ける**(真の共局在 0 %): 漏れ込みだけ止めると r 0.203 → 0.011、
   ぼけだけ止めると 0.204、背景だけ止めると 0.205、雑音だけ止めると 0.210。
   **Pearson を作っているのは漏れ込み 1 つ**。Manders は漏れ込みを止めて
   0.133 → 0.072、ぼけを止めて 0.091 —— 2 つの原因が乗っている。
8. ★**しきい値の流儀で M1 が動く**(真の共局在 50 %、α=β=10 %、真値 0.482):
   Otsu(T_B=0.260)0.401 / 固定(台 + 3σ、0.187)0.508 / Costes 自動(0.096)
   0.960、M2 は 1.000。★**Costes 自動は漏れ込みで崩壊する** —— 暗い画素にも
   漏れ込みの相関が残るので「相関が消える所」が台まで落ち、ほぼ全画素が前景になる。
   分離後は 0.342 / 0.419 / 0.438 で **Costes がいちばん真値に近い**(しきい値が
   台の上 0.052 と低く、裾を拾うぶん 2 項の裾落ちが小さい)。固定しきい値の生 0.508
   が真値に見えるのは 5 項と同じ相殺。
