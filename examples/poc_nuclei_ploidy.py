...
も嘘になるか**」。答えは「面積は真値でも 15.4 % 誤分類、積分輝度は 0 %。ただし
**背景を引き忘れると分類は生き残ったまま DNA 指数だけが -20 % 壊れる**」。
...
【この PoC が示すこと(数字はすべて実行時に印字される実測値)】
 ...
 1. **積分輝度は保存される**。PSF でぼかしても総和は相対 0.0e+00 で変わらない。
    ★**予想が外れた**: 「マスクで積分すると裾が落ちるが、倍率の偏りだから比には
    効かない」と書きかけたが**効く**。裾落ちは 2n で **9.6 %** / 4n で **5.5 %**
    と大きさに依存する(落ちるのは周囲長に比例した縁の帯)ので、**DNA 指数
    (4n/2n)が測る前から 2.00 → 2.115(+5.8 %)**。
 2. ★★**ゼロ点(面積)と対比(積分輝度)**。全探索でいちばん良いしきい値でも
    誤分類は **面積(真値) 15.4 % / 積分輝度 0.0 %**。log の分離度は
    面積 **1.5 σ** / 積分輝度 **6.2 σ**。
 3. ★★**「測った面積」は面積ではない**。固定しきい値のマスク面積で分けると
    誤分類が **8.0 %** —— 真の面積(15.4 %)より**良い**。ln(測定/真値) を
    ln(明るさ) に回帰すると傾き **+0.26**、相関 **0.97**。明るい核ほどぼけた
    縁がしきい値を外側で切るので、**面積という名前で DNA 量を漏らしている**。
 4. ★**予想が外れた**。「焦点がぼければ面積分類器は壊れる」と踏んでいたが、
    PSF σ を 0.3 → 1.5 px にすると誤分類は **9.3 % → 4.5 %** と**良くなる**。
    漏れの傾きが **+0.18 → +0.49** に増えるため。ぼかすほど「面積」は DNA 量に
    近づく —— 性能が上がったのではなく、**測っている量が入れ替わっている**。
 5. ★★**背景の引き忘れ**。偽の項は面積に比例する: (測定 - 背景なしの測定) を
    面積に回帰した傾きは **背景レベル b と 6 桁一致**(b=396.851 → 396.851)。
    ところが分類は壊れない(b = 核輝度の 5 倍でも誤分類 4.3 %)。**壊れるのは
    量のほう** —— DNA 指数が **2.115 → 1.702**(-20 %)。★**分類だけを見て
    いたら気づけない**。対照群: 背景の中央値を引けば b をいくら振っても
    2.115 のまま、`fs.aperture_photometry` の環状背景なら **2.033** ——
    開口が裾まで含むので 1 節の歪みも同時に直る。**2 つは別の穴**。
 6. ★★**飽和は逆向きに効き、途中で 1 節と打ち消す**。飽和画素 **8.8 %** で
    DNA 指数は **2.002** —— 真値 2.00 に**ぴったり**。だが合っているのは
    裾落ちの +5.8 % と飽和の -5.3 % が釣り合っているだけで、どちらも直って
    いない。**逆向きの 2 つの失敗を 1 つの数字で見ると、いちばん壊れている
    条件がいちばん正しく見える**。飽和 64.6 % まで振れば 1.653。
 7. ★**融合**は 2n+2n を 4n に見せる。間隔を詰めると 76 塊中 24 が融合し、
    4n の割合の推定が真値 0.382 に対し **0.539**。`solidity`(単独 0.995 /
    融合 0.863)の 1 本のしきい値で検出率 1.00 / 誤検出 0.000、落としたあとは
    **0.327**。★融合は分子だけでなく**分母も減らす**(110 核 → 76 塊)。
 8. ★**混合比の推定**は Otsu でも EM でも当たる(積分輝度なら)。真値 0.05〜
    0.78 に対し最大偏差 **0.014**。同じことを面積でやると最大 **0.41** 外す
    —— **重なった 2 山は割り方を変えても割れない**。
...
def section5_saturation(nuc):
    ...
    base = rows[0][2]
    j = int(np.argmin([abs(r[2] - 2.0) for r in rows]))
    print()
    print("  → 飽和画素が %.1f %% のところで DNA 指数は %.3f(真値 2.00)。"
          % (rows[-1][1], rows[-1][2]))
    print("  → ★★**いちばん危ないのは真ん中の行**。飽和 %.1f %% で DNA 指数は"
          % rows[j][1])
    print("     **%.3f** —— 真値 2.00 に**ぴったり合っている**。だが合っているのは")
    print("     1 節の裾落ちによる過大(%+.1f %%)と飽和による過小(%+.1f %%)が"
          % (100 * (base / 2.0 - 1), 100 * (rows[j][2] / base - 1)))
    print("     **打ち消しているだけ**で、どちらも直っていない。露光を少し変えれば")
    print("     釣り合いは崩れる。**逆向きの 2 つの失敗を 1 つの数字で見ると、")
    print("     いちばん壊れている条件がいちばん正しく見える**。")
    print("  → 飽和は背景の引き忘れと**同じ向き**(指数が 2 より小さくなる)なので、")
    print("     2 つが同時に起きていると**見分けが付かない**。分けて数えるには")
    print("     飽和画素の割合を別に報告するしかない(道具の穴 (d))。")
    return rows, j
...
def section6_fusion():
    ...
    print("     ちょうど 1.000 になって、区別が不自然に簡単になる)。実測の")
    print("     solidity は 単独 %.3f / 融合 %.3f。" % (out[s][7], out[s][8]))
    print("  → ★**融合は「4n を増やす」だけではない**。%d 個の核が %d 塊に減るので"
          % (110, out[s][0]))
    print("     **分母も減る**。数え落とした核は 2n も 4n も含むので、割合の偏りは")
    print("     「融合した数 / 塊の数」より小さく出る —— 分子と分母が同時に動く量を")
    print("     1 つの割合で報告すると、どちらが効いたのか分からなくなる。")
    return out, truth4
