...
出ています。もう 1 つ、実データでは :data:`PITCH` / :data:`PLATES` の角度が
**スキャンから推定した値**になるので、3 節の予測(格子の位置)にも推定誤差が
乗ります。1 節の実測(角度の差 0.26°、線数の差 0.61 lpi)が、その誤差が
どれくらいかの目安です。
...
【所見(数字はすべて最終実行の実測値。★ は重要、★★ は最重要)】
 ...
 1. ★★**折り返しは幾何から厳密に予測できた**。スクリーン角 θ の網点格子
    ...
    0〜2p(0〜18.0 px)を 0.5 px 刻みで掃引して測ったところ、4 版 148 点の
    うち **144 点で予測と実測の差は 0.1096 px 以下**。残る 4 点は
    **セル境界のタイ**(セル余裕が 0.165 px 以下)で、そこは予測式の
    round() がどちらへ転んでもよい場所でした(4 節)。
 2. ★★**格子で簡約した残差で見れば 148 点すべてが合う**(最大 0.2382 px、
    境界のタイを除けば 0.1096 px)。つまり推定器は「間違えている」のでは
    なく、**格子で等価な答えの中から 1 つを返している**。どれを返すかは
    境界付近では雑音が決めます。別の推定器(窓ごとの相関 =
    :func:`fullseye.ledger.piv_cross_correlate`)でも同じ折り返しが出ました
    (格子で簡約した残差の中央値 0.0119 px)。
 3. ★★**同じ物理的なずれが、版ごとに違う見かけになる**。d=(+0.90,+9.60) px
    (|d|=9.64 px = 公差の 4.8 倍)を 4 版すべてに同じだけ掛けると、見かけは
    C(15°) 1.67 / M(75°) 3.32 / Y(0°) 1.09 / K(45°) 3.27 px。**同じ紙が
    同じだけずれた結果なのに、4 版中 2 版が「合格」に見えます**(5 節)。
 4. ★★**ゼロ点(インク重心)の壊れ方も閉形式で書けた**(2 節)。版どうしの
    重心差は誤差 34.28 px —— 絵柄が版ごとに違うので**ずれを測っていない**。
    「自版の設計との重心差」は折り返さない代わりに**縮尺が狂い**、応答の
    傾きは ``k = 1 - β``(β = 下地だけのインク量 / 全インク量)。
    **予測 0.3983 に対して実測 0.3711**(差 0.0272)、絵柄を一様な下地だけに
    した対照群は予測 0 に対して実測 0.0225。
 5. ★もう 1 つ出ました(2 節)。重心の応答は直線から **0.9864 px** 外れ、
    外れ方の周期は網点ピッチ 9.02 px。**窓の縁で網点の列が出入りする**ため
    で、テーパ窓を掛けた対照群では **0.0048 px** に落ちます —— 原因が縁だと
    確かめられました。★予測していなかった項目です。
 6. ★★**物差しを変えると勝者が入れ替わる**(8 節、Y 版)。公差 2.0 px 以内
    での RMS 誤差は 素の相関 **0.0071 px** 対 低域通過 0.6680 px(94 倍)対
    重心 0.8072 px。ところが 0〜18 px 全域での合否判定の一致率は
    低域通過 89.2 % / 重心 94.6 % に対して**素の相関は 67.6 % で最下位**
    (不合格の版を 12/37 回「合格」と言った)。
 7. ★★**2 つを組み合わせれば両方取れる。ただし成立条件つき**(8 節)。
    低域通過(格子が消えるので一意)で代表元を選び、素の相関で詰める二段は
    **0.0071 px / 94.6 %** で両方勝ちます。成立条件は「粗の誤差 < 基本セルの
    半径 4.511 px」で、**二段が壊れた 2 点はすべて粗の誤差がそれを超えた点**
    でした。絵柄をベタ近く(山 0.45→0.06)にすると 6 点中 3 点で丸ごと
    1 格子(9.0 px)跳びます。
 8. ★**対照群で原因を素材に切り分けられた**(6 節)。同じ絵柄・同じ推定器で
    スクリーンだけ FM(確率)に替えると、0〜18 px の全域で誤差は
    最大 0.0029 px、折り返しはゼロ(同じ AM は最大 20.402 px)。
    **崖の原因は推定器ではなく AM 網点の周期性**です。
 9. ★**レジストマークは当たる。ただしマークのある場所でしか当たらない**
    (7 節)。d0=(+0.60,+9.40) px を仕込んだ版で、マークを含む窓は誤差
    **0.0823 px**、同じ版の網点だけの窓は **9.0591 px**。そして版が 0.60 %
    伸びて 0.120° 回っていると、マークの値を版全体へ当てた誤差は距離に比例し
    (係数 0.006319 px/px)、公差 2.0 px を超えるのは
    **予測 316.5 px / 実測 322.5 px** から —— 紙の **44 %** が公差外です。
10. ★**用途外の登録 op を当てたら、自信満々で外した**(9 節 (c))。
    :func:`fullseye.frame_align`(星の対応で 2-D 変換を出す)に網点を
    食わせると `inlier_ratio` **1.00**、`rms_px` 0.645 を返しながらシフトは
    真値から **80.85 px** 外れていました。しかもその答えは網点格子の
    ベクトルですらない(格子で簡約した残差 2.625 px)ので、1〜2 の不定性
    とは**別種の失敗**です。**「確信度 1.00 で 80 px 外す」形の fail-soft**
    は報告する価値があります。★この PoC の指摘で **op 側を直しました**
    (2026-09-08): docstring に「繰り返し構造には使えない」を測った数字つきで
    書き、投票の**2 番手の山 / 1 番手**を ``vote_margin`` として返すように
    しました。賛成率はどちらの場合も 1.00 ですが、``vote_margin`` は
    **この網点で 1.000、星野で 0.143** —— 同じ賛成率でもどちらか分かります。

【グラウンドトゥルース(すべて式で置いた)】
* **網点**: スクリーン角 θ、ピッチ p = SCAN_DPI/LPI = 9.02 px の正方格子。
...
【節立て】
 ...
 8) ★★物差しを 2 つ —— 勝者が入れ替わる / 二段で両方取る / その崖
...
線数(133 / 150 lpi)は業界の慣行値で、この PoC はそれを**仮定として置いた**
だけです(規格から特定の数値を引用してはいません)。合否の公差 2.0 px
(42 µm)も、規格値ではなく**この PoC が置いた閾値**です。
"""
...
def section9_tool_gaps():
    ...
    print("      なる**。")
    # ★2026-09-08: この PoC の指摘を受けて op 側を直した。docstring に
    #   「繰り返し構造には使えない」を測った数字つきで書き、投票の
    #   **2 番手の山 / 1 番手** を `vote_margin` として返すようにした。
    #   賛成率では区別できない 2 つを、この数なら区別できる。
    assert "vote_margin" in fa, "vote_margin が返らない(op を直した意味が消えている)"
    print("      ★op 側を直した(2026-09-08): 賛成率では区別できないので、投票の")
    print("      **2 番手の山 / 1 番手**を `vote_margin` として返すようにした ——")
    print("      この網点で **%.3f**(山が並んでいる)。docstring には星野での"
          % fa["vote_margin"])
    print("      値(0.143)も測って書いてあるので、**同じ 1.00 の賛成率でも")
    print("      どちらの場合か分かる**。")
    ...
    return {"fa_err": fa_err, "fa_ratio": float(fa["inlier_ratio"]),
            "fa_margin": float(fa["vote_margin"]),
            "fa_lat": fa_lat}
