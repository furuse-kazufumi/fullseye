...
LPI = 150.0              # 網点の線数 [lpi](商業印刷の慣行値)
PITCH = SCAN_DPI / LPI   # 網点のピッチ [px] = 8.00
UM_PER_PX = 25400.0 / SCAN_DPI   # 1 px は何 µm か(= 21.17 µm)

SOFT = 0.9               # インクのにじみ + スキャナ MTF の幅 [px]
...
WIN = 64                 # 相関の窓 [px]
SWEEP = np.arange(0.0, 2.0 * PITCH + 1e-9, 0.5)   # 掃引する |ずれ| [px]
...
def main() -> int:
    ...
    print("  * 物差しで勝者が入れ替わる: 精度 相関 %.4f px 対 重心 %.4f px、"
          % (met["相関 AM"]["rms"], met["重心(ゼロ点 B)"]["rms"]))
    print("    判定一致率 相関 %.1f %% 対 重心 %.1f %%。"
          % (met["相関 AM"]["rate"], met["重心(ゼロ点 B)"]["rate"]))
    print()
    ...
    assert max(r["lat_err"] for r in allr) < 0.05, max(r["lat_err"] for r in allr)
    assert max(r["piv_lat"] for r in allr) < 0.05
    ties = [r for r in allr if r["raw_err"] > 0.5]
    assert all(r["margin"] < 0.35 for r in ties), [r["margin"] for r in ties]
    assert len(ties) < 0.1 * len(allr), len(ties)
