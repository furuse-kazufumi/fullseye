...
逆に **§9 の対照群(cos^4 / 大気 / 軸外ぼけ を 1 つずつ止める)は実データでは
...
2. **ゼロ点は思ったより強い**。誤検出を **1.0 件/フレーム**に揃えたうえで、
   「生の明るさで上位を採る」だけでも走査幅 **180.2 m** が出ます。整合フィルタ
   (白色トップハット)は **267.4 m** = **1.48 倍**。★**零点を置かないと
   「検出できた」としか書けません** —— 効いているのは 1.48 倍の分だけです。
3. ★**「背景を引いた」の 8 割は効きません**。画像全体の中央値と sigma で割るのは
   **アフィン変換**なので**検出の順位が 1 つも変わらず**、走査幅は 180.2 →
   **178.0 m**(差は検出上限 4000 個の打ち切りだけ)。効くのは**場所ごと**に
   引いたときで、横距離ごとに引くと **250.2 m**(1.39 倍)になります。
4. **走査幅の定義そのものを実証しました**。半幅 500 m の帯に一様に撒いた
   40 万個の目標を、(a) 実測した曲線 p(x)、(b) 幅 W の完全検出帯、
   (c) 底辺 2W の三角形、(d) 直下が見えない二峰形(側方監視レーダの nadir
   gap)で拾うと —— **面積が同じなら検出数は同じ**(実測 0.2669 / 0.2673 /
   0.2667 / 0.2676、予測 W/2X = 0.2674、モンテカルロの標準偏差 0.0007 の
   1.1σ 以内)。曲線の**形は消え、面積だけが残る**。これが「走査幅」という
   1 本の数字を計画に渡せる理由です。
...
6. ★**予測を外した(1): 実測の p(x) では、完全平行捜索が C=1 で 1.000 に
   届きません**(実測 **0.8514**)。``min(1,C)`` は p が幅 W の**矩形**である
   (定値域則)ことに依存していて、裾を引く実曲線では隣の航跡と裾が重なり、
   その重なりぶん取りこぼします。実曲線は平行 0.8514 と ランダム 0.6352 の
   **あいだ**に落ちました。
7. ★**予測を外した(2): 平らな曲線のほうが平行捜索に強い、は逆でした**。
   走査幅を 267 m に揃えて C=1 で比べると、直下に集中したトップハットの曲線が
   **0.8514**、直下を捨てて端を拾う「横距離ごとの背景」の曲線が **0.7777**。
   **矩形に近いとは『平ら』ではなく『W の内側に立ち、外へ裾を引かない』こと**
   —— 支持域/W が 2.15 対 2.30 で、広いほうが 2 本以上の航跡に重なって数えられ、
   そのぶん損をします。★同じ条件でも**ランダム捜索では 2 本が一致**します
   (0.6352 / 0.6377)—— そちらは §4 のとおり面積しか見ないからです。
   **走査の型を決めてからでないと、曲線の形を捨ててよいかは言えません**。
8. ★**走査幅 2 倍と捜索時間 2 倍は等価**(どちらも C を 2 倍にする)。実測でも
   (2W, t) と (W, 2t) が一致(平行 0.8513 / 0.8509、ランダム 0.6360 / 0.6338、
   最大差 0.0022 = モンテカルロ誤差)。★**だが画像側にその 2 倍はありません**。
9. ★**最適高度は内点に来る**。高度 100→850 m で W = 184.4 / 242.7 / 263.2 /
   **267.9** / 226.6 / 37.5 / 0.0 m。低高度で落ちるのは解像度ではなく
   **掃引幅そのもの**で、高度 100 m では視野の端でも p=0.764 のまま切れて
   おり(W は下限値)、高度 150 m でも 0.364 です。高高度では目標が暗くなって
   消えます。速度が高度に依らない機体なら最適は 300 m、低高度で減速が要る機体
   (``v ∝ min(1, h/600)``、置いた仮定)では **420 m** へ動きます。
10. ★**見張り役: 誤検出は端ではなく直下に集中する**。閾値 4.5σ・160 フレームで、
    横距離 0-32 m の帯に **117 件**、256-288 m の帯に **0 件**(それぞれ
    71 件/km^2 と 0 件/km^2)。目標も白波も同じ cos^4・同じ大気で暗くなるので、
    **いちばんよく見える所がいちばん吠えます**。検出率だけの表はこれを隠します。
11. ★**走査幅は誤検出率と一緒にしか語れない**。同じ画像・同じ検出器で、閾値を
    動かすだけで W は **417.4 m(誤検出 23.4 件/フレーム)から 188.6 m
    (0.39 件/フレーム)**まで動きます。「走査幅 400 m」という報告は、
    誤検出率が書いていなければ**何も言っていません**。
12. **対照群で原因を切り分け**: 基準 263.9 m に対し、白波を止めると **374.2 m**
    (+110.3)、cos^4 を止めると **334.7 m**(+70.8)、軸外ぼけを止めると
    **311.0 m**(+47.1)。★**大気だけは +3.8 m** で、これは同じ条件を別の種・
    別のフレーム数で測り直したときの振れ幅(3.5 m = 0.6σ)と同じ程度です ——
    端と中央の透過率の**比**が 0.76 しかない上に、大気を止めると**かすみの
    路程放射(= 光子雑音の源)も一緒に消える**ので、頑健 sigma が下がって
    閾値が 6.78 → 15.42 に上がり、差し引きで残らないからです。
    ★**足し算になりません**(4 つの差の和 +232 m に対し実際は相互作用する)
    —— 止めると誤検出も減って閾値が下がるからです。うねりを止めても
    -0.2 m しか動かず、これが §2 で海面のぼけを軸上 1 本で近似したことの根拠です。
13. ★**道具の穴を 4 つ見つけました**(本体は直していません、報告のみ)。要点は
...
から作ったガウス斑、光子雑音は :func:`fullseye.photon_sample`(Poisson)、
読み出し雑音は加法ガウス。目標は直径 2.4 m の救命いかだで、コントラストは
0.45〜2.4 倍の対数一様(同じ「いかだ」でも塗色・向き・波しぶきでこれくらい
散ります)。検出は :func:`fullseye.star_detect`、整合フィルタは
``fullseye.op.gray_tophat``、ぼけは ``fullseye.op.gauss_filter``、
cos^4 の検算は :func:`fullseye.relative_illumination`、面積は
...
【実行時間】この機械(Windows 11 / py 3.11)で約 64 秒。内訳はおおよそ
