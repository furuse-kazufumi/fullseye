...
12. ★**雑音とビームハードニングでは壊れ方が違う**。フォトン数を 3e6 -> 3e3 と
    1000 分の 1 にしても層数は 17 -> 16 枚、層厚は 0.173 -> 0.175 mm。壊れるのは
...
プローブは縞を打ち消す(この PoC を書いている途中で実測: ガス空隙のセルで
コントラストが 0.45 -> 0.25 に落ち、17 枚のうち 4 枚しか数えられなかった)。
プローブは**スライスの中心**に置く。
...
def _probe_points():
    ...
    プローブは縞を打ち消してしまう(この PoC を書いている途中の実測: 空隙のある
    セルでコントラストが 0.45 -> 0.25 に落ち、17 枚の層のうち 4 枚しか数えられ
    なかった。**いまは中心に置いてあるので、この数字は出力には現れない**)。
    """
