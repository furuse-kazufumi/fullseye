...
1. ★★**「DoD は斜面で cos だけ体積を間違える」は間違い**。予想は「傾斜 40 度
   では体積が cos40 = 0.766 倍に縮む」(= -23.4 %)だったが、鉛直差分を
   **水平投影面積で積分する**限り cos は約分して消える(式は 2 節で先に出す)。
   雑音を止めた格子標本で測ると 傾斜 0→40 度の掘削体積の誤差はどれも
   **-0.011 %**、傾向が無い。間違うのは体積ではなく**厚さ**のほう。
2. ★**DoD の「深さ」は法線厚さを sec θ 倍で過大に言う**。崩壊中心の
   DoD 深さ / M3C2 厚さの比は 10 度で **1.003**、25 度で **1.105**、
   40 度で **1.247**(予測 sec θ = 1.015 / 1.103 / 1.305)。「1.2 m 掘れた」は
   鉛直の話で、40 度斜面の法面実厚は 1.007 m。**同じ現場の 2 つの数字は別の量**。
3. ★★**変化なしの対照が 83.8 m3 の偽の土量を出す**。同じ地形を 2 回撮って
   引くだけで、偽掘削 83.8 m3 / 偽堆積 90.7 m3(真の掘削 164.2 m3 の 51 %)。
   雑音の正負を別々に足しているので、**偽正味 +6.9 m3 は小さいのに内訳は巨大**。
   有意性でしきると 18.1 / 18.7 m3、偽変化面積は 2702 → 194 m2 に落ちる ——
   しきい値は飾りではなく本体。
4. ★**M3C2 の利得は傾斜からしか来ない**。足跡面積を 1.00 m2 に揃えて比べると
   ...
   傾斜(cell/sqrt12)と水平位置誤差の寄与が落ちる。「M3C2 は常に良い」は嘘。
5. ★★**真値に近い 1 個の数字は、逆向きの 2 つの誤差の和かもしれない**。
   M3C2 のしきらない掘削は 162.1 m3(真値 -1.3 %)で一見よく合うが、内訳は
   「測れなかった core 13.2 % ぶんの欠け」と「雑音の水増し(対照で +34.9 m3)」。
   有効率で割り戻すと 186.7 m3(+13.7 %)。**被覆と雑音は別々に報告する**。
6. **点密度の崖**: 最小検出厚は 0.5 pt/m2 で 0.408 m(この崩壊の最大深さ
   1.20 m の 34 %)、20 pt/m2 で 0.095 m。★壊れ方が手法で違う —— DoD は空セル
   84.1 % でも残り 15.9 % で測り続けるが、M3C2 は有効 core 率が 42.7 %
   (5 pt/m2)→ 0.0 %(0.5)と落ちて**止まる**。★ゼロ点その 2 の C2C
   (点群間の最近傍距離)は変化ゼロでも 0.767 → 0.142 m を返し、符号も無い。
7. ★★**系統誤差が生む偽体積は DoD も M3C2 も同じ**。0.30 m の平行移動に対し
   予測 540.5 m3、実測 DoD 543.2 m3、M3C2 は生値 467.2 だが有効 core 率
   86.8 % で割り戻すと 538.5 m3。M3C2 が有利なのは厚さと有意性であって
   体積ではない(法線方向の見かけ変化は cos 倍小さいが、体積に直すとき
   1/cos するので約分する)。位置合わせで 543.2 → 1.1 m3。
8. ★★**合わせた分だけ変化が消える**。変化のある点群対を全対応で ICP に掛けると
   正味土量は -32.3 → -3.8 m3(真値 -30.4 の 12.6 %)に潰れる。変化域が窓の
   33 % もあると ICP はそれを系統誤差と読み、鉛直に +0.0105 m 引き上げる ——
   平均変化量 -0.0113 m とほぼ同じ量。上位 60 % だけ使う trimmed ICP なら
   -30.8 m3(101.1 %)まで戻る。
9. ★★**位置合わせで決まらない成分は、測定にも現れない**。斜面に沿った 0.292 m
   の面内ずれを与え、両時期を同じ窓に切って区画の縁の情報を消すと、純平面では
   ICP 後も 0.278 m(95 %)ずれたまま。それでも DoD の偽正味は -4.2 m3。
   ★予想は「純平面なら常に決まらない」だったが、**縁ごと**なら 0.099 m まで
   詰まる —— 区画の縁が拘束になる。うねりがあれば窓で切っても 0.228 m。
10. ★★**遮蔽で直すのは代表値ではなく分類の窓**。地面分類の取りこぼしは 0.71 %
    (遮蔽 60 %)なのに、標準偏差で引いた LoD は 0.162 → 1.450 m と 8.9 倍に
    飛ぶ(MAD なら 1.3 倍)。★予想「セル代表値を中央値にすれば直る」は外れで
    318.8 m3(+129.1 %)。対照で追うと汚染は「セルの点が全部樹冠」——
    地面点が 1 つも届かないセルが 36 個。最低点を 3x3 セルで探す分類にすると
    そのセルは**空になり**、掘削は 167.5 m3(+20.4 %)で遮蔽 0〜60 % 横ばい。
11. ★**法線の符号は道具では決まらない**。``estimate_normals`` は「近傍重心から
...
def section_density() -> dict:
    ...
    print("  ★★崖の形が 2 つの手法で違う。DoD は空セルが %.1f %% になっても残り %.1f %% で"
          "測り続けるが、" % (emp[0], 100 - emp[0]))
    print("     M3C2 は有効 core 率が %.1f %%(%.0f pt/m2)-> %.1f %%(%.0f)-> %.1f %%(%.1f)と"
          "**崩れて測れなくなる**"
          % (rate[3], dens[3], rate[2], dens[2], rate[0], dens[0]))
    ...
    print("     これは点間隔そのもの(1/sqrt(密度)で %.2f 倍、予測 %.3f)で、"
          "しかも**符号が無い**ので掘削と堆積を分けられない。C2C で土量は測れない。"
          % (c2c[-1] / c2c[0], 1 / math.sqrt(ratio)))
    print("     ★最初は速度のため両時期を 6000 点へ間引いて測ったら、どの密度でも "
          "0.42 m の一定値になった —— **間引いた後の点間隔**を測っていた。"
          "密度の話をするなら密度を変えてはいけない。")
