...
def section_occlusion(tr: dict, lod0: float) -> dict:
    ...
    print("  ★★しきい値を正しく引いても**足りない**: 平均 DEM のままだと遮蔽 %.0f %% で"
          "掘削 %.1f m3(誤差 %+.1f %%)。" % (occ[-1], adapt[-1],
                                             100 * (adapt[-1] / truth(lod=lmad[-1])["ero"] - 1)))
    print("     しきい値が抑えるのは**雑音由来の偽陽性**であって、混ざった樹冠そのものは"
          "しきい値を軽々と超える。")
    print("  ★★セル代表値を平均から**中央値**に替えると %.1f m3(誤差 %+.1f %%)。"
          % (med[-1], 100 * (med[-1] / truth(lod=lmad[-1])["ero"] - 1)))
    print("     1 セルに数点の汚染なら中央値は動かない。平坦な面では中央値は平均より "
          "sqrt(pi/2) = 1.25 倍ばらつくが、その代償のほうがずっと安い。")
    print("  -> 遮蔽率は「手法を替えろ」ではなく**「しきい値をその場で、"
          "外れ値に強い散らばりで測り直し、代表値も外れ値に強いものにしろ」**という指示。")
