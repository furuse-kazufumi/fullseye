...
def section_occlusion(tr: dict, lod0: float) -> dict:
    ...
    print("\n   遮蔽率 地面点  取りこぼし 実効密度  LoD(std)  LoD(MAD)  "
          "MAD-LoD の掘削 誤差%   分類なし")
    occ, gr, leak, lstd, lmad, adapt, raw, rows = [], [], [], [], [], [], [], []
    for p in (0.0, 0.15, 0.30, 0.45, 0.60):
        ...
        e2 = 100 * (r_ada["ero"] / t_ada["ero"] - 1) if t_ada["ero"] > 1e-6 else float("nan")
        occ.append(100 * p)
        ...
        print("   %5.0f %% %6.1f %% %8.2f %% %7.1f %9.3f %9.3f %13.1f %+7.1f %11.1f"
    ...
    print("  -> 遮蔽率は「手法を替えろ」ではなく**「しきい値をその場で、"
          "外れ値に強い散らばりで測り直せ」**という指示。")
...
def main() -> int:
    ...
    print("  * 面内のずれは窓で切ると ICP が決められない(純平面で %.3f m 残る)が、"
          ...
          % (sy["plane"]["純平面・窓で切る"][1],
             sy["plane"]["純平面・窓で切る"][3]))
    print("  * 遮蔽 %.0f %% では LoD が %.3f -> %.3f m に上がる。しきい値を据え置くと"
          "掘削は %.1f m3(誤差 %+.0f %%)、その場で測り直せば %.1f m3。"
          "分類しなければ %.1f m3。"
          % (oc["occ"][-1], oc["lod"][0], oc["lod"][-1], oc["fixed"][-1],
             100 * (oc["fixed"][-1] / truth(lod=ctrl["lod"])["ero"] - 1),
             oc["adapt"][-1], oc["raw"][-1]))
