...
【★★ 番号つき所見(すべて実測。予想が外れたものはそう書く)】
  ...
  1. ★★**時間軸に交点は無かった**(いちばん大きな予想外れ)。終端
     (6 % ひずみ、端の総変位 7.9 px)で累積 61 µε / 直接 1878 µε ——
     **累積が 31 倍良く、どの時刻でも勝ち続けた**。交点は**雑音の軸**に
     あって、σ を 0.02 → 0.40 と上げると σ=0.20 で初めて直接が勝つ。
     「どちらが良いかは**時刻**で入れ替わる」ではなく
     「**画質**で入れ替わる」が、この条件での正しい言い方。

  2. ★★**累積の誤差はランダムウォークではなかった**。雑音を切っても
     誤差はほぼ変わらず(-60 → 68 µε)、散らばりの log-log 傾きは 0.5 では
     なく -0.03。支配しているのは揺らぎではなく **PIV の系統誤差**。
     しかも散らばり(14 µε)より偏り(60 µε)のほうが大きい。

  3. ★★**コマを間引くと累積は悪くなる**(教科書と逆)。歩数 24 → 4 で
     誤差 -60 → -606 µε。効くのは歩数ではなく**1 歩あたりの変形の大きさ**
     (1 歩 2500 µε で相対 0.1 %、1 歩 15000 µε で 1.0 %)。
     雑音が小さければ**コマは多いほど良い**。σ=0.20 では逆転するので、
     **最適なコマ数は雑音で決まる**。

  4. ★直接測定の弱点は探索範囲(1/4 則)でも窓の大きさでもなく、
     **窓の中で変位が変わること**(subset distortion)。窓 32 → 64 では
     -1863 → -1684 µε とほとんど変わらないのに、多段 `piv_multipass` に
     すると -585 µε(3.2 倍改善)。**既存 op で直せる**。

  5. ★★ひずみ速度: **因果(先を見ない)移動平均の偏りは、遅れ (w-1)/2 の
     閉形式でほぼ説明が付く**(w=11 で予測 447、実測 556 µε/コマ)。
     クリープ速度は単調に落ちるので、遅れはそのまま「速すぎる」方向の
     系統誤差になり、w=11 で真の速度の 25 %。中央移動平均なら同じ w で
     11 µε/コマ。**揺らぎは両方とも 5.6 → 0.9 µε/コマ**へ下がる。
     窓を広げる利得は揺らぎ、代償は偏り —— 両方を別々に持っていないと
     窓は決められない。

  6. ★採点の落とし穴: ひずみ速度の偏りを「窓ごとに端を除いた範囲」で
     測ると、**窓が広いほど良く見える**(評価している時刻が違うだけ)。
     全部の窓で同じ t ∈ [11, 21) に固定して測り直した。
...
def true_rate2(t):
    """速度の 2 階微分 ``d³E/dt³ = A n (n-1)(n-2) (t+t0)^(n-3)``。

    移動平均のなまり(偏り)の閉形式 ``(w²-1)/24 x d²f/dt²`` に使う。
    """
    t = np.asarray(t, float)
    return A_COEF * NEXP * (NEXP - 1.0) * (NEXP - 2.0) * (t + TOFF) ** (NEXP - 3.0)


#: ひずみ速度を採点する時刻の範囲。**全ての窓幅で同じ範囲**を使う
#: (窓ごとに範囲を変えると、なまりでなく別の時刻を測ってしまう)。
#: 下限 11 は因果移動平均 w=11 が履歴を使い切れる最初の点。
EVAL = slice(11, 21)
...
def section5_direct_breaks(frames):
    ...
    print()
    print("  → ★探索範囲は**原因ではない**(最後まで超えていない)。窓を")
    print("     32 → 64 に広げてもほとんど改善しない(%.0f → %.0f µε)ので、"
          % (1e6 * (res["窓 32"][-1] - tru[-1]), 1e6 * (res["窓 64"][-1] - tru[-1])))
    print("     窓の大きさそのものでもない。")
    print("  → ★★効いているのは**窓の中で変位が変わること**(subset distortion)。")
    print("     多段 `piv_multipass` は前段の予測で 2 枚目の窓をずらすので、")
    print("     窓の中に残る変形が減り、誤差が %.0f → %.0f µε(%.1f 倍)。"
          % (1e6 * (res["窓 32"][-1] - tru[-1]),
             1e6 * (res["多段 64→32"][-1] - tru[-1]),
             abs((res["窓 32"][-1] - tru[-1]) / (res["多段 64→32"][-1] - tru[-1]))))
    print("     **直接測定の弱点は既存 op で 3 分の 1 に減らせる** —— ただし")
    print("     それでも累積(56 µε)には届かない。")
    return res, tru
...
def section6_rate(frames, cums, dirs, tru):
    ...
    print("  → ★揺らぎは w とともに %.1f → %.1f µε/コマ へ下がる"
          % (rows[0][3], rows[-2][3]))
    print("     (中央、w=1 → 11、およそ 1/sqrt(w))。**窓を広げる利得はここ**。")
    print("     偏りが増えない")
...
def section8_findings(out, rec, direct_res, tru, rate_rows, sweep):
    ...
    print("""
  (1) ★★**教科書の「時間軸の交点」は現れなかった**(予想が外れた)。
      終端(6 %% ひずみ)で累積 %.0f µε / 直接 %.0f µε —— 累積が %.0f 倍良く、
      どの時刻でも累積が勝ち続けた。交点は**時間軸ではなく雑音の軸**に
      あった: σ を上げていくと σ=%.2f で初めて直接が勝つ。
      「どちらが良いかは時刻で入れ替わる」ではなく
      「**画質で入れ替わる**」が、この条件での正しい言い方。

  (2) ★★**累積の誤差はランダムウォークではなかった**(予想が外れた)。
      対照 1(雑音を切る)で誤差はほぼ変わらず(%.0f → %.0f µε)、
      散らばりの log-log 傾きも 0.5 でなく %.2f。累積を支配するのは
      揺らぎではなく **PIV の系統誤差**。

  (3) ★★**コマを間引くと累積は悪くなる**(これも予想が外れた)。
      歩数 24 → 4 で誤差 %.0f → %.0f µε。効いているのは歩数ではなく
      **1 歩あたりの変形の大きさ**(1 歩 2500 µε で相対 0.1 %%、
      1 歩 15000 µε で 1.0 %%)。雑音が小さいなら**コマは多いほど良い**。
      σ=0.20 では逆転するので、**最適なコマ数は雑音で決まる**。

  (4) ★直接測定の弱点は探索範囲(1/4 則)でも窓の大きさでもなく、
      **窓の中で変位が変わること**。窓 32 → 64 では %.0f → %.0f µε と
      ほとんど変わらないのに、多段 `piv_multipass` にすると %.0f µε ——
      3.2 倍改善する。**既存 op で直せる**が、それでも累積には届かない。

  (5) ★★ひずみ速度: **因果(先を見ない)移動平均の偏りは、遅れ (w-1)/2 の
      閉形式でほぼ説明が付く**(w=11 で予測 %.0f、実測 %.0f µε/コマ)。
      クリープは速度が単調に落ちるので、遅れはそのまま「速すぎる」方向の
      系統誤差になり、w=11 では真の速度の 25 %% に達する。
      中央移動平均なら同じ w で偏り %.0f µε/コマ。
      揺らぎはどちらも %.1f → %.1f µε/コマ へ下がる。
      **窓を広げる利得は揺らぎ、代償は偏り。両方を別々に持っていないと
      窓を決められない。**

  (6) ★真ひずみ(対数ひずみ)を使うと、増分の和が全体とちょうど一致する。
      **工学ひずみで同じことをやると、累積と直接が別の量を推定してしまい、
      比べているのが手法なのか定義なのか分からなくなる。**
""" % (c24, d24, d24 / max(c24, 1e-9),
       sweep["sigma"][int(np.argmax(np.asarray(sweep["dir"])
                                    < np.asarray(sweep["cum"])))],
       rec["err"][0.0][0], rec["err"][0.02][0], out["cum"]["logslope"],
       rec["err"][0.0][0], rec["err"][0.0][-1],
       1e6 * (direct_res["窓 32"][-1] - tru[-1]),
       1e6 * (direct_res["窓 64"][-1] - tru[-1]),
       1e6 * (direct_res["多段 64→32"][-1] - tru[-1]),
       rate_rows[-1][4], rate_rows[-1][2], rate_rows[-2][2],
       rate_rows[0][3], rate_rows[-2][3]))
