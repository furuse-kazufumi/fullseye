...
【この PoC が示すこと(数字はすべて実行時に印字される実測値)】
 ...
 5. ★★**太陽まわりの偽陽性**。RBR(赤/青)は散乱で太陽の周りが白むので、
    **雲がゼロの晴天でも雲量 7.25 % が出る**。閉形式 1-cos(ψ_t) との差は
    ...
    ★★しかも**内訳が単調でない**: t=0.70 で光学的厚さ 0.20 の雲は 0.27
    検出され、0.35 の雲は 0.00。太陽からの角距離が 24 度と 90 度だから
    —— **同じ厚さの雲が、空のどこに居るかで検出されたりされなかったりする**。
 6. ★★**幾何の誤差と検出の誤差は別物で、しかも打ち消し合う**。対照群
    (真のマスク = 検出を止めた条件)で幾何だけの誤差は **-5.02 %**、
    重みつき + RBR で検出だけの誤差は **+37.95 %**。ところが素朴な
    「RBR x 画素数比」は **+29.73 %** —— 足し算(+32.93 %)より**小さい**。
    ★**符号が逆の 2 つの失敗が打ち消して、いちばん素朴な数え方がいちばん
    まともに見える**。重みを掛けると幾何は 0.00 % まで直るが検出は残る。
 7. ★**地平線マスク**を 90 → 70 度に絞ると、真値そのものが 0.1811 → 0.2284
    (**+26 %**)動く。画素数比の偏りは **-5.02 % → -0.48 %** と縮み、
    重みつきは **どの角度でも +0.01 % 以内**。★「マスクを厳しくしたら一致した」は
    直った証拠にならない(いちばん偏っている帯を捨てているだけ)。
    **マスクの角度を書かずに雲量を報告した数字は比較できない** ——
    同じ空・同じ検出器で 0.1811 〜 0.2284 を名乗れる。
...
CAPS = [(15.0, 200.0, 12.0, 1.00),
        ...
        (78.0, 20.0, 10.0, 1.00),
        # ★最後の 3 つは**薄い雲**(光学的厚さちがい)。しきい値を上げると
        #   薄いものから順に落ちる —— 4 節の階段はこれで出る。
        (28.0, 60.0, 9.0, 0.20),
        (45.0, 220.0, 11.0, 0.35),
        (62.0, 95.0, 10.0, 0.55)]
#: 薄い雲だけの添字(4 節の検出率を数える対象)
THIN = (5, 6, 7)
