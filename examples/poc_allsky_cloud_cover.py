...
答えは「**同じ雲が、天頂にあれば -19 %、地平線側にあれば +17 % に読める**。
...
【この PoC が示すこと(数字はすべて実行時に印字される実測値)】
 ...
 1. **検算**。球冠の閉形式と数値積分は α=10/20/30 度で 6e-5 以内。立体角重み
    map の総和は 1.000000。
 2. ★★**ゼロ点(画素数比)は雲の位置で系統的に偏る**。角半径 8 度の同じ雲を
    天頂角 0 → 82 度へ動かすと、雲量の推定は **0.00789 → 0.01142**(真値は
    どちらも 0.00973)。**推定/真値 = 0.811 → 1.174**、つまり同じ空が
    **1.45 倍**違って読める。★閉形式 (θ/sinθ)/(θmax²/2) と 0.003 以内で一致。
 3. ★★**重み sinθ/θ を掛けると直る**。同じ 6 通りで推定/真値は 0.9999〜1.0010。
 ...
 4. ★**同じ真値・違う配置**。雲を天頂寄りに集めた空と地平線寄りに集めた空で
    真の雲量を揃えると、画素数比は **0.0940 と 0.1300**(1.38 倍)。重みつきは
    **どちらも 0.1152**(真値 0.1152)。
 5. ★★**太陽まわりの偽陽性**。RBR(赤/青)は散乱で太陽の周りが白むので、
    **雲がゼロの晴天でも雲量 7.25 % が出る**。閉形式 1-cos(ψ_t) と 4 桁一致
    (ψ_t = 21.9 度)。しきい値を上げれば減る(0.90 で 0.64 %)が、
    ★**薄い雲の検出率が 1.00 → 0.00 に落ちる**。両方を 1 つの誤差にまとめると
    最適点が偽物になる。
 6. ★★**幾何の誤差と検出の誤差は別物**。全部入りの空で重みつき推定は真値より
    **+47.2 %** —— そのうち **+47.2 pp 全部**が太陽の偽陽性で、幾何の寄与は
    **0.00 %**(真のマスクを使う対照群で確認)。**重みは幾何しか直さない**。
 7. ★**地平線マスク**を 90 → 70 度に絞ると、真値そのものが 0.1536 → 0.1879 に
    動く(地平線側は雲が少ない空だったから)。画素数比の偏りは
    **-4.65 % → -0.70 %** と縮み、重みつきは **どの角度でも ±0.02 %**。
    ★マスクを変えると「推定の誤差」も「真値」も動くので、**マスクを書かずに
    雲量を報告した数字は比較できない**。
