...
1.  STFT 往復: max|x - istft(stft(x))| が 1e-12 未満(窓・ホップを変えて)。
    COLA は窓ではなく (窓, ホップ) の対の性質 —— 同じ blackman が hop=win/4 で
    COLA、hop=win/2 で 19.05 % ずれる。矩形窓 50 % 重なりの和はちょうど 2。
...
7.  次数: 既知次数は速度ランプを通しても同じ bin。共振は逆に滲む。
    角度領域リサンプルの回転数は台形積分の閉形式に一致し、その出力を自分で DFT
    すると order_spectrum と最大差 1e-17 で一致する(= 同じ計算)。
8.  A 特性は 1 kHz で厳密に 0 dB(構成上)。正弦の Leq = 10log10(A^2/2)。
    apply_weighting は 1 kHz 正弦を素通しし、100 Hz 正弦を 0.110373 倍にする
    (閉形式 10**(-19.1428/20))。1/3 オクターブ帯域は upper/lower = G^(1/3)、
    center = sqrt(lower*upper) が厳密。分母が偶数だと 1 kHz は中心でなく境界。
