...
def main():
    ...
    print("     のか「原理的に無理」なのか区別できない。返り値に対応の質を示す指標が無い")
    print("     (inlier 数も、2 位との差も返らない)。")
    print("  2. register の docstring は部分重なりに init='feature' を勧めているが、")
    print("     5-a の表ではこの形状で 'pca' と大差ないか悪い。推奨の前提(局所形状が")
    print("     識別的であること)が書かれていないので、平面部品に適用すると外れる。")
    ...
    print("  6. icp / point_to_plane_icp は収束したかを返さない。tol で止まったのか")
    print("     max_iter で打ち切られたのかが外から分からず、反復予算の設計ができない。")
    print("  7. register(init='auto') は毎回 FPFH 初期値を計算するので PCA 経路の 20-50 倍")
