...
1. **真値は 24 枚**。決め方を 1 つに頼らない —— (a) 面積の平坦域:
   ...
   変換(半径 19〜31 を明示)が投票のしきい 0.30〜0.50 で 24 個、
   (c) Sobel + 穴埋めでも 24。**3 つが一致したものを真値と呼ぶ**。
   ★さらに **1 対 1 の検算**をする —— 数が合っていても同じものを数えて
   いるとは限らない(2 枚くっついた塊 1 個 + ごみ 1 個でも「24」になる)。
   円 1 個をちょうど 1 個の成分が含み、成分の外に落ちた円は 0 だった。
2. **有名な照明勾配は、数え上げを壊していない**。素の大域 Otsu + 穴埋め +
   面積 150 で 24 枚ちょうど。前処理は要らなかった。★ただし**測るほうは
   すでに壊れている** —— 24 個のうち 1 個は面積 9076 で、中央値 1501 の
   6 倍、Hough 半径の上限から言える上限 π·31² = 3019 の 3 倍。明るい 1 枚が
   背景へにじみ出ている。**「枚数が合った」と「領域が正しい」は別**。
2b. ★**fullseye 自身の ``hough_circle_trans`` は、この用途では真値に届かない**。
   半径レンジがコードに直書き(4〜19)で ``b`` が本当に何もしていなかった
   ので、この PoC を書くついでに ``b`` を半径の上限に配線した
   (``b=0.5`` は従来と完全に同一)。それでも非極大抑制して峰を数えると
   b=0.5 で 13、b=1.0 でも **19** にしかならない —— 半径の刻みが 3 で、
   しかも全半径の最大に潰しているから。**引数を足しても足りないことがある**
   ことまで含めて数字にしておく。
