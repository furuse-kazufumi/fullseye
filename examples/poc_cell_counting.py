...
1. **ゼロ点(大津の二値化 + 連結成分)を先に測る** —— 重なりを強めていくと
   単調に下振れし、いちばん密な条件で 78 個中 25 個を取りこぼす。失点の正体は
   全部「過統合」で、「取りこぼし(どの予測ラベルにも触れられていない細胞)」は
   ゼロのまま —— **この 2 つは別物**である。
2. ★**計数が合っていて分割が外れている点が実在する** —— 種を間引く量を振ると
   過分割と過統合が逆向きに動くので **どこかで打ち消し合う**。実測ではその点で
   個数の偏りが +0.3 個(真値の 0.4 %)になるのに、分割誤りは 13.3 件残り、
   1 対 1 に対応した細胞は 60/78 個(最良の設定なら 69 個)。**個数だけを
   報告していたら最良の設定として通る**。ゼロ点の疎な条件(偏り +2.0)も同じ形で、
   内訳は 過統合 8.0 件 と 偽物 11.7 件 の相殺だった。
3. **過分割と過統合は別々に数えないと見えない**。さらに実測では
   ★**過分割は重なりに反応しなかった** —— 重なりを 5 段階振っても過分割の列は
   ほぼ一定で、動くのは過統合だけ。**2 つの欠陥は原因が別**(過分割は種の撒き方、
   過統合は重なり)なので、密度を振る実験では過分割の良し悪しは測れない。
4. **最適な h は基準で変わり、基準によっては密度でも動く** —— 偏りを基準に取ると
   最適 h は密度で 1.1 → 0.75 → 0 と動くが、1 対 1 対応の数を基準にすると動かない。
   「最適な h は密度で動く」は **基準を書かないと真偽が決まらない**主張である。
5. ★**大きさが 3 倍違う集団についての予想は外れた** —— 「大型と小型で最適な h が
   分かれる」と踏んでいたが、実測ではどちらも h≈1.0 で分かれなかった。代わりに
   出たのは (a) 種を細かく撒いたときの過分割が **大型に集中する**(大きさ比 1 倍で
   8 個中 1.3、3 倍で 5.7)、(b) ★**道具の都合で指定できる h の下限が
   0.42 → 0.80 画素へ上がる**(``xsk2_h_maxima`` の h は正規化画像に対する比なので、
   画像に大きい細胞が 1 つ入るだけで小さい細胞の側の下限まで上がる)。
6. **雑音と背景ムラは壊し方が逆** —— 雑音は前景を粒々にして **偽物** を増やし
   (過分割ではない)、背景ムラは大津の閾値を動かして前景率を 3 倍にし **過統合**
   を増やす。前者は面積による棄却、後者は背景補正が要る。
7. **縁で切れた細胞をどう数えるか**で推定個数が 10 個(真値の 13 %)動く。ただし
   真値の側を同じ規約で数えれば偏りはどの規約でも小さい —— 危ないのは規約の選択
   ではなく、**推定と真値で違う規約を使う**ことのほう。
8. **面積は「見えている面積」までしか取れない** —— 隠れていない細胞では
   「見えている面積との差」と「真の面積との差」がほぼ一致するのに、隠れた帯では
   前者が当たったまま後者だけが外れる。分割の巧拙ではなく **誤差の床** である。
   さらに、よく隠れた細胞は **そもそも 1 対 1 に対応しない**ので形を測る対象に
   残らない = 床の存在自体が標本から消える。

★この PoC が出した fullseye の道具の穴は
...
def label_paint(lab):
    """ラベル画像を図に載せるための塗り分け(表示だけ。数字には触らない)。

    ラベル番号をそのまま塗ると **隣り合う細胞が似た色**になって境界が見えない。
    番号を固定の擬似乱数で入れ替えてから塗る。背景は 0 のまま。
    """
    lab = np.asarray(lab, np.int64)
    k = int(lab.max())
    lut = np.zeros(k + 1)
    if k:
        lut[1:] = 0.15 + 0.85 * np.random.default_rng(0).permutation(k) / max(k - 1, 1)
    return lut[lab]
...
def main():
    ...
    print("  → 左の 2 行はほぼ同じ値で止まっている。pack 1.25 以上ではコロニーが")
    print("     そもそも重ならないので、残っているのは **常に重なる分裂鎖のぶん**の底")
    print(f"     ({100 * 12 / 78:.0f} % 前後)。この底があるおかげで、疎な条件でも")
    print("     『重なりのある場面』が消えず、疎↔密の比較が同じ課題の上で行える。")
    ...
    print(f"  → ★1 行目に **すでに打ち消し合いが出ている**。偏りは"
          f" {zero[SPARSE]['bias']:+.1f} 個とほぼ完璧に")
    print(f"     見えるが、内訳は 過統合 {zero[SPARSE]['merge']:.1f} 件(個数を減らす)と")
    print(f"     偽物 {zero[SPARSE]['spurious']:.1f} 件(デブリを細胞と数えた = 個数を増やす)"
          "で、たまたま近い。")
    print("     **偏りの絶対値が小さいことは、内訳が小さいことを意味しない**。")
    ...
    print("  → 「偽物」の列も見ること。ゼロ点と全極大の分水嶺はデブリをそのまま")
    print("     細胞として数えているが、形の事前知識は面積の下限で落としている")
    print("     (0 件台)。**同じ偏りでも、内訳が違えば直し方が違う**。")
    ...
    print("  → ★答えは **どの手法でも過統合が先**、というより **過分割は重なりに")
    print("     反応しない**。過分割の列はどの手法でも pack を通してほぼ一定である。")
    print("     過分割は『種の撒き方』が決めるもので、重なりが決めるものではない ——")
    print("     つまり **2 つの欠陥は原因が別**で、密度を上げる実験では過分割の良し")
    print("     悪しは測れない。合計 1 つの数字にしていたら、この切り分けは消える。")
    print("  → 全極大の分水嶺だけは pack 0.85 から過統合も急増する。種が細かすぎて")
    print("     境界が引けないのではなく、**深く重なると距離変換の尾根が繋がって")
    print("     極大が 1 つになる** ため。同じ『過統合』でも機序が違う。")

    print("\n=== 5. 崖 (b) 大きさのばらつき —— 単一スケールの前提はいつ壊れるか ===")
    RATIOS = (1.0, 1.5, 2.0, 2.5, 3.0)
    HS_SIZE = (0.0, 1.0, 1.6, 2.4, 3.4)
    ...
    print("  → ★**予想は外れた**。「大型と小型で最適な h が分かれる」と踏んでいたが、")
    print("     この場面ではどちらも h=1.0 前後で、**最適な h は分かれなかった**。")
    print("     壊れ方は別の形で出ている:")
    print(f"     (1) 種を細かく撒く(h=0)と **過分割が大型に集中する** —— 大きさ比 1 倍で")
    print(f"         大型 {size_tab[(1.0, 'n_big')]} 個中 {size_tab[(1.0, 0.0)][0]:.1f} 個、"
          f"3 倍で {size_tab[(3.0, 0.0)][0]:.1f} 個"
          f"({100 * size_tab[(3.0, 0.0)][0] / size_tab[(3.0, 'n_big')]:.0f} %)。")
    print("         大きい物体ほど距離変換の尾根が長く、尾根の上の凹凸を種に拾うため。")
    print("         小型の過分割は比によらずほぼ一定なので、これは **大きさの効果**。")
    print("     (2) 小型の過統合は h とともに単調に増えるが、**大きさ比には反応しない**。")
    print("         大型が混じっても小型どうしの分離は悪くならない = 干渉していない。")
    ...
    print("  ★『実効 h』の列に注目 —— 要求した h が小さいと **要求どおりに設定でき**")
    print("     **ない**。``xsk2_h_maxima`` は h を正規化画像に対する 0.05〜0.35 の比で")
    print("     しか受け取らないので、h < 0.05 x max(距離変換) は下限に張り付く。")
    hcurve = {}
    for p in (SPARSE, MID, DENSE):
    ...
    print(f"  → **最適な h は基準によっても密度によっても動く**: 偏り基準で {hb_list}、"
          f"1対1 基準で {ho_list}。")
    if len(set(hb_list)) > 1:
        print("     偏り基準の最適 h は密度で動くので、疎な場面で決めた h をそのまま")
        print("     密な場面へ持っていくと壊れる。1 対 1 基準では **動かなかった** ——")
        print("     『最適な h は密度で動く』は基準を書かないと真偽が決まらない主張。")
    print("  → もう 1 つ: h=0(全極大)の行だけ **偏りが正**で、他は全部負。h の")
    ...
    print("  → 分水嶺を通す 2 つは偽物が桁で少ない。形の事前知識は面積の下限で明示的に")
    print("     落としているが、h-maxima のほうは **意図せずそうなっている** ——")
    print("     デブリは距離変換が小さく h に届かないので種が立たず、マスク内でも")
    print("     どの流域にも属さないまま残る。**同じ『強い』でも、片方は設計、片方は")
    print("     副作用**。副作用のほうは細胞が小さい標本に移した途端に裏返る。")
    # 「同じ画質の悪さ」で壊れる列が違う、が一目で読める形にしておく
    # (雑音 → 偽物の列だけ跳ねる / 背景ムラ → 前景率と過統合が動く)。
    figs.save_table("noise_vs_shading",
                    ["条件", "前景率", "偏り", "過分割", "過統合", "偽物"],
                    noise_rows, col_w=120,
                    title="雑音と背景ムラは壊れる列が違う(ゼロ点で測る)",
                    caption="真値の前景率 %.1f %%。雑音は偽物を、背景ムラは前景率と"
                            "過統合を動かす —— 直し方が逆。" % (100 * truth_fg))
    ...
    #: いちばん隠れている帯のうち、細胞が 3 個以上入っているもの。**よく隠れた細胞は
    #: そもそも 1 対 1 に対応しない**(隣とまとめられる)ので、いちばん上の帯は
    #: 空になりうる —— それ自体が「重なった細胞の形は測れない」ことの現れである。
    occ_hi = max(i for i in bins if len(bins[i]["dv"]) >= 3)
    print(f"  (帯の分布は **両極に寄る** —— ほとんどが「{OCC_LBL[0]}」で、真ん中の帯は"
          "ほぼ空。少しだけ")
    print("   隠れる状況が少ないのではなく、**よく隠れた細胞は 1 対 1 に対応しない**")
    print("   (隣とまとめられる)ので、形を測る対象から先に落ちるためである。")
    print(f"   3 個以上残っているいちばん隠れた帯は「{OCC_LBL[occ_hi]}」)")
    ...
    print(f"\n=== 11. 速度(この機械での実測、{IMG}x{IMG} の 1 枚あたり)===")
    ...
    print(f"     とはいえ全部が 1 枚 {max(speed.values()):.0f} ms 以内なので、"
          "この規模では速度は選択理由にならない。")
    ...
    lines = (
        ...
        ("大きさが 3 倍違うと",
         f"小型に最適な h では大型 {size_tab[(3.0, 'n_big')]} 個中 "
         f"{size_tab[(3.0, size_tab[(3.0, 'argh')][1])][0]:.1f} 個が割れる。"
         f"指定可能な最小 h も {size_tab[(1.0, 'hmin')]:.2f}→"
         f"{size_tab[(3.0, 'hmin')]:.2f} px へ上がる"),
        ("★いちばん危ない読み違え",
