...
1. **単一画像の拡大では bicubic の零点を上回れない。** nearest / bilinear /
   五次スプライン / Lanczos-3 のどれも、倍率 2・3・4 のすべてで PSNR の差は
   **0.12 dB 以内**。分解能(縞の変調度)で見ても、**全部が低解像側のナイキストで
   揃って死ぬ**。「勝てなかった」がこの節の結果である。
...
4. **複数フレームは、標本化が足りていないときだけ本当に増やす。** 副画素ずれのある
   ...
   **+18.7 dB**、ナイキストより細かい周期 6 の変調度が **0.03 → 0.40** に立ち上がる。
   十分に標本化された条件(σ = 0.85)では PSNR は上がるのに、**分解能の列は
   1 つも動かない** —— 上がったぶんは情報ではない。分解能の表を併記しないと、
   この 2 つは PSNR では区別できない。
...
def main():
    ...
    print("     動くのは (a) 鮮鋭化 —— 下へ、(b) IBP —— 順モデルを知っているから。")
