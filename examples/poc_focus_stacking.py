...
EXTEND: 実際の顕微鏡 Z スタックに差し替えるには、``_scene`` が返す ``stack`` を
...
2. ★**絵と深度は別物** —— 全焦点画像は 35.9 dB(ゼロ点 21.0 dB)まで行くのに、
   同じ融合が出した深度は平坦部でも 0.43 mm ずれ、無テクスチャ領域では
   **掃引全域にほぼ一様な乱数**を返す。絵が綺麗でも深度は嘘をつく。
3. ★**信頼度の作り方を間違えると、嘘の上に嘘が乗る** —— 焦点評価のピーク突出度
   ``(最大 - 中央値) / 最大`` は、無テクスチャ領域で **0.9873** と、有テクスチャ
   領域の 0.9625 より**高く**出た。相対量は絶対水準がゼロ近傍では意味を失う。
   絶対値で見ると両者は 24000 倍離れており、そちらで棄却すると全体 RMS が
   1.505 -> 0.878 mm に下がる(有テクスチャ側の誤棄却 0.0 %)。
4. **フレームを細かくしても、ある所から良くならない** —— 5 / 9 / 17 / 33 枚で
   量子化下限は 0.72 -> 0.09 mm と 8 倍下がるのに、平坦部 RMS は 0.77 -> 0.48 mm
   で止まる。9 枚から先の誤差は標本化ではなく焦点評価そのものが持っている。
...
★ この PoC が出した道具の穴(op 本体は直していない。詳細は末尾の節を印字):
  ...
  (b) **2-D op 層(``fs.op``)の微分系はフレームごとに自分の最大値で割る**
      ...
      **スタックを跨いで比較できない**。この場面での係数の振れ幅は 1.41 倍、
      鏡面ハイライトを一点足すと 1.88 倍まで開き、平坦部 RMS が 0.425 -> 0.509 mm
      に悪化した。例外は出ないし、API のどこにも「割った」と書かれない。
  (c) **唯一の非正規化な合焦指標 ``xcv2_lap_var`` は 1.0 で飽和する**
      (``min(1.0, 分散 * 20)``)。鮮鋭な側 —— オートフォーカスで一番効いてほしい
      側 —— で 15 枚中 9 枚が厳密に 1.000000 で並び、順位が付かない。
...
def fuse(stack, fm, focus_mm):
    """画素ごとに評価が最大のフレームを採る。返りは ``(全焦点画像, 深度, 峰の高さ)``。"""
    k = fm.argmax(axis=0)
...
def main():
    ...
    for dz in (1.0, 4.0, 8.0):
        ...
        print(f"  合焦面から {dz:>4.1f} mm 外れた面の錯乱円 = {coc:.4f} mm"
    ...
    print(f"  {'領域':<16}{'RMS [mm]':>10}{'偏り':>9}{'散らばり':>10}")
    for label, m in (("全体", np.ones_like(flat)), ("それ以外", plain),
                     ("段差帯", band), ("無テクスチャ", flat)):
        e = err[m]
        print(f"  {label:<16}{rms(e):>10.3f}{e.mean():>9.3f}{e.std():>10.3f}")
    print(f"  {'ゼロ点: 定数深度':<16}{rms(depth - null_const):>10.3f}"
          f"{0.0:>9.3f}{depth.std():>10.3f}   ({null_const:.2f} mm と答え続ける)")
    print(f"  → ゼロ点比は全体で {rms(depth - null_const) / rms(err):.2f} 倍にしかならない。"
          "PSNR の圧勝とは別の話。")
    print(f"     それ以外の領域だけなら {rms(depth - null_const) / rms(err[plain]):.2f} 倍。"
          "つまり悪いのは段差帯と無テクスチャで、そこが全体を支配している。")
    ...
    print("     無テクスチャの散らばりはどの評価でも掃引幅と同程度 —— 誰も救えない。")
...
def _blur(v, sigma):
    """PoC 内でだけ使う分離可能ガウス。ぼけ半径を px で直に指定したいので自前。"""
