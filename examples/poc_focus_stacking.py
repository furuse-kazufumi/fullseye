...
2. ★**絵と深度(距離画像)は別物** —— 全焦点画像は 35.89 dB(ゼロ点 = 中央の
   1 枚 20.98 dB)まで行くのに、**同じ融合が出した深度(= 距離画像)**はテクスチャの
   ある所で 0.44 mm ずれ、無テクスチャ領域では **掃引全域にほぼ一様な乱数**を返す。
   領域ごとに「その領域では一つの深度と答え続ける」ゼロ点と比べると、テクスチャの
   ある所では 6.81 倍勝つのに、無テクスチャでは **0.12 倍 = ゼロ点に 8 倍負ける**。
   絵の PSNR だけ見ていると、この負けは一切見えない。
3. ★**信頼度の作り方を間違えると、嘘の上に嘘が乗る** —— 焦点評価のピーク突出度
   ``(最大 - 中央値) / 最大`` は、無テクスチャ領域で **0.9914** と、有テクスチャ
   領域の 0.9623 より**高く**出た。相対量は絶対水準がゼロ近傍では意味を失う。
   絶対値で見ると両者は 24157 倍離れており、そちらで棄却すると全体 RMS が
   1.517 -> 0.890 mm に下がる(有テクスチャ側の誤棄却 0.0 %)。
