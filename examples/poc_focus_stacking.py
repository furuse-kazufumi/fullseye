...
1. **真値を自分で作れる** —— 半導体ワイヤ・ボンディング風の深度地図(ダイと
   リードフレームの高さ段差 + ボンドワイヤの弧 + ベアダイの無地面)とテクスチャ
   から、錯乱円の式で深さに応じたぼけを掛けて画像列を合成する。錯乱円の直径は
   幾何光学の閉形式 ``c = f^2 / (N (z_f - f)) * |z - z_f| / z``(f = 焦点距離、
   N = F 値、z_f = 合焦距離、z = 被写体距離)。これを画素ピッチで割って画素数に
   する。この式は ``fullseye.ledger.defocus_blur`` の中身そのもので、PoC は式を
   写経せずその op を呼ぶ。
2. ★**絵と深度(距離画像)は別物** —— 全焦点画像は 33.69 dB(ゼロ点 = 中央の
   1 枚 28.52 dB)まで行くのに、**同じ融合が出した深度(= 距離画像)**はテクスチャの
   ある所で 0.47 mm ずれ、無テクスチャ領域では **掃引全域にほぼ一様な乱数**を返す。
   領域ごとに「その領域では一つの深度と答え続ける」ゼロ点と比べると、テクスチャの
   ある所では 2.17 倍勝つのに、無テクスチャでは **0.16 倍 = ゼロ点に 6 倍負ける**。
   絵の PSNR だけ見ていると、この負けは一切見えない。(実画像は平滑金属が大半で
   単一フレームでも合焦しているため、AIF の伸びは全面テクスチャ場ほど大きくない
   —— 現場の正直な姿。)
3. ★**信頼度の作り方を間違えると、嘘の上に嘘が乗る** —— 焦点評価のピーク突出度
   ``(最大 - 中央値) / 最大`` は、無テクスチャ領域で **0.9575** と、有テクスチャ
   領域の 0.9381 より**高く**出た。相対量は絶対水準がゼロ近傍では意味を失う。
   絶対値で見ると両者は 1038 倍離れており、そちらで棄却すると全体 RMS が
   0.918 -> 0.817 mm に下がる(有テクスチャ側の誤棄却 0.0 %)。
4. **フレームを細かくしても、ある所から良くならない** —— 5 / 9 / 17 / 33 枚で
   量子化下限は 0.49 -> 0.06 mm と 8 倍下がるのに、テクスチャのある所の RMS は
   9 枚の 0.42 mm から先 0.47〜0.51 mm で頭打ち(むしろ微増)。9 枚から先の誤差は
   標本化ではなく焦点評価が持っている。
...
★ この PoC が出した道具の穴(op 本体は直していない。詳細は末尾の節を印字):
  ...
  (b) **2-D op 層(``fs.op``)の微分系はフレームごとに自分の最大値で割る**
      ...
      **スタックを跨いで比較できない**。この場面での係数の振れ幅は 1.84 倍あり
      (ダイの回路パターンと明るいワイヤでフレームごとに最大値が動く)、テクスチャの
      ある所の RMS が 0.468 -> 0.510 mm に悪化した。例外は出ないし、API のどこにも
      「割った」と書かれない。
  (c) **唯一の非正規化な合焦指標 ``xcv2_lap_var`` は 1.0 で飽和する**
      (``min(1.0, 分散 * 20)``)。高コントラストの被写体を 17 枚掃引すると
      **17 枚中 15 枚が厳密に 1.000000** で並び、順位がほとんど付かない
      (クリップ前の分散なら最良は一意に決まる)。
...
#: 画像の一辺 [px]。
SIZE = 160
#: ダイ(左)とリードフレーム(右)の境目 [px]。ここに高さの段差が出る。
STEP_X = 84
#: ボンドワイヤの根元(パッド)がある行。5 本を掃引で立体的に拾う。
WIRE_ROWS = (26, 56, 88, 118, 148)
#: 無テクスチャ(一様な金属面)の矩形 —— 焦点評価が意味を失う場所。ダイ左下の
#: 無地な金属(パッドもワイヤも無い)を置く。実機でもベアなダイ面は無地。
FLAT_BOX = (slice(100, 144), slice(6, 30))


def _draw_wire(depth, tex, y0, size):
    """1 本のボンドワイヤを描く(明るい弧 + 弧の高さを深度に焼き込む)。

    パッド(ダイ上・x≈48・203 mm)から立ち上がり、ループ頂点(≈197 mm =
    対物レンズに最も近い)を越えてウェッジ(リード上・x≈124・201.5 mm)へ降りる。
    Z を掃引すると根元・頂点・先端が順に合焦する = 合焦点法が高さを拾う筋。
    """
    t = np.linspace(0.0, 1.0, 480)
    wx = 48.0 + (124.0 - 48.0) * t
    wy = y0 + 7.0 * np.sin(np.pi * t)                        # わずかに垂れる(配線の弛み)
    wz = 203.0 * (1.0 - t) + 201.5 * t - 6.0 * np.sin(np.pi * t)   # 頂点 ≈ 197 mm
    for xk, yk, zk in zip(wx, wy, wz):
        iy, ix = int(round(yk)), int(round(xk))
        for dy in (-1, 0, 1):
            for dx in (-1, 0, 1):
                y, x = iy + dy, ix + dx
...
def ground_truth(size=SIZE, seed=7):
    """半導体ワイヤ・ボンディング風の既知シーンを作る。返りは ``(depth[mm], texture)``。

    合焦点法(depth-from-focus)が現場で使われる場面そのもの —— **ダイ上のボンド
    パッドから金線がリードフレームへ弧を描いて渡る**。ワイヤのループは対物レンズへ
    向かって立ち上がるので、Z を掃引するとパッド・ワイヤ頂点・リードが順に合焦し、
    画素ごとに高さ(= 距離画像)が出る。

    honest な作り込みは 3 つを必ず入れる: (1) ダイとリードフレームの**高さ段差**
    (境界にハローが出る)、(2) ワイヤの**なめらかな弧**(微分可能だが曲率がある)、
    (3) ボンドパッドやベアダイの**完全に無地な金属**(焦点評価が意味を失う)。
    テクスチャは乱数一色にせず、金属の微細粒 + パッド/ワイヤの明るい構造を混ぜる。
    """
    ...
    for y0 in WIRE_ROWS:
        r0, r1 = max(0, y0 - 7), min(size, y0 + 7)
...
def main():
    ...
    print(f"  → ゼロ点比 {psnr(fused, tex) - p_single[n_frames // 2]:+.2f} dB。"
          "絵の側は文句なく効いている。")
    sel = np.unique(np.linspace(0, n_frames - 1, 8).round().astype(int))
    figs.save_grid("stack",
                   [stack[i] for i in sel] + [fused],
                   [f"焦点 {focus_mm[i]:.1f} mm" for i in sel] + ["融合(全焦点)"],
                   title="焦点をずらした画像列と、その融合", gray=True, ncols=3,
                   caption=f"{n_frames} 枚のうち {len(sel)} 枚と融合結果。どの 1 枚も一部しか"
                           "合っていないが、融合は全面が合う(強度画像なのでグレースケール。"
                           "ボケ→シャープが素直に見える)。")

    print("\n=== 3. 距離画像(深度地図)—— 同じ融合が出した、もう一つの答え ===")
    print("  焦点評価が最大のフレーム番号 -> その合焦距離、で画素ごとの距離が出る"
          "(全焦点画像と同じ argmax の副産物)。")
    err = dmap - depth
    ...
    figs.save_grid("depth_map", [depth, dmap, err],
                   ["真値の距離 [mm]", "融合が出した距離画像 [mm]", "誤差(推定 - 真値)"],
                   title="同じ融合が出したもう一つの答え —— 距離画像(深度地図)", ncols=3,
                   signed=[False, False, True],
                   caption="合焦点法は全焦点画像と同時に距離画像も出す。ただし左下の"
                           "無テクスチャの四角だけ、誤差が掃引全域にばらけた乱数になって"
                           "いる(段差帯のハローも見える)—— 絵ほど距離は当てにならない。")
    ...
    # 相対量(突出度)と絶対量(峰の高さ)が**逆のことを言う**のがこの節の主張。
    # 峰は 5 桁以上にまたがるので対数で塗らないと無テクスチャの穴が見えない。
    # 突出度は 0.96〜0.99 の狭い帯に密集するので、その帯だけを塗り分ける
    # (全域で塗ると外れ値に潰されて「無テクスチャのほうが高い」が見えない)。
    figs.save_grid("confidence",
                   [np.clip(prom, 0.90, 1.0), np.log10(np.maximum(peak, 1e-30)),
                    keep.astype(float)],
                   ["突出度(相対)", "峰の高さ log10", "棄却後に残る画素"],
                   title="信頼度の作り方を間違えると嘘の上に嘘が乗る", ncols=3,
                   caption="左下の無テクスチャの四角が、相対量(0.90-1.00 に切って"
                           "表示)では最も明るい = 自信ありに見え、絶対量(対数)で"
                           "だけ「何も見えていない」と出る。")
