...
- 14 関数は **全部動いた**。「見えていなかった理由 = 壊れている」ではなかった。
  スロット幅の偏りは ``measuring1d`` -0.011 px / ``metrology`` +0.012 px、
  ゼロ点(大津の整数幅)-0.500 px。**ゼロ点比およそ 40 倍**。
  円穴の直径 -0.048 px、角度 +0.003 度。
- ただし穴が 4 つ: (1) ``measure_pairs`` が ``transition`` を取らない、
  (2) 回転が無い(平行移動だけ)、(3) ``metrology`` の矩形は最小面積**外接**
  矩形なので外れ値 1 点が幅を丸ごと動かす、(4) 信頼度を返さない。
- 単発の測定には **真値の小数部に依存する周期誤差(S 字)** が残る
  (PSF sigma 0.5 で 0.089 px、sigma 3 で 0.014 px。ぼけは味方)。
  **真の幅を整数に取るとこの誤差は左右で打ち消して見えなくなる** ——
  検査の真値を「きりの良い数」に置くと測定系の弱点が隠れる。
- 崖は「PSF が大きいと壊れる」ではなく「**エッジ間距離 / PSF 幅** が小さいと
  壊れる」。w/sigma = 3.1 を切ると偏りが 0.05 px を超え、しかも幅は必ず
  **大きく** 出る(エッジ対が互いを押し広げる)。w/sigma = 1.6 まで
  ``measure_pairs`` は「対が見つかった」と答え続ける —— 失敗を返さない。
- 丸い縁は必ず小さく出る。半径 rho・ぼけ sigma で直径が -sigma^2/rho
  (実測 / 予測 = 0.94〜1.24、rho = 8〜34 px)。**校正で引ける偏り**。
- 測定線が斜めのとき **cos 補正は入っていない**(そういう設計)。cos を掛けて
  戻せば 60 度までは 0.04 px 以内、70 度で 0.14 px 残る。
- 面取りは PSF に対して大きくなった瞬間に **答えが黙って飛ぶ**(この合成では
  5 px と 6 px の間で 11.7 px)。丸み rf=8 px では上面幅と底面幅が 16 px 違い、
  50% 交差はそのどちらでもない第三の値に落ちる。**サブピクセルの 0.01 px を
  争う前に、縁の定義で 3 桁大きい量が動いている。**
"""
...
def section_dimensions(img):
    ...
    print(f"        60 点中 1 点が 1.5 px 外れただけで幅が "
          f"{2 * (f1['l2'] - f0['l2']):+.2f} px 動く。最小二乗なら "
          f"{2 * 1.5 / len(ep):.3f} px の話。")
    print("        `fit_rectangle2` は**最小面積の外接矩形**であって当てはめではない ——")
    print("        値を決めているのは凸包の極値点だけで、内側の 58 点は寄与しない。")
    print(f"        rms は上がるので **検出はできる**({f0['rms']:.2f} -> {f1['rms']:.2f})。")
    print("        だから rms を必ずゲートにすること。値そのものは外れ値から救えない。")
...
def section_cliff_blur():
    ...
    figs.save_plot("blur_cliff",
                   ...
                   caption="偏りはどちらも正(対が互いを押し広げる)。符号が一定"
                           "なので繰り返し測っても消えない。下端 -4 は表示の"
                           "打ち切り(|偏り| < 1e-4 px)。")
...
def section_cliff_noise():
    ...
    print("    **『雑音で寸法がずれる』の実体は散らばりであって偏りではない** ——")
    print("    対策は平均化(次表)で、照明の作り直しではない。")
    print("  * SNR 14 を切ると M と F の性格が分かれる。M は勾配ローブの**グレー差**を")
    print("    しきい値にするので偽エッジを落とせるが、F(平坦部の中点)は平坦部の")
    print("    推定そのものが雑音で動く。")
    print("  * SNR 7 では両方とも崩壊する(-12 px)。それでも **成功率は 100%** ——")
    print("    `measure_pairs` は『対が 1 つ見つかりました』と答え続ける。")
    print("    ゼロ点(Z)のほうが崩壊しないのは、整数量子化が誤差を丸め込むから。")
    print("    **雑音が乗った領域では、粗い方式のほうが『まとも』に見える。**")
...
def section_cliff_illum():
    ...
    print("  『流れ』は g=0 からの差(偏りそのものではなく、照明で動いた分)")
    print(f"  {'g':>7} | {'Z 偏り':>9}{'Z’ 偏り':>10}{'Z’ 流れ':>10} | "
          f"{'M 偏り':>9}{'M 流れ':>9} | {'F 偏り':>9}{'F 流れ':>9}")
    zz, zs, mm, ff = [], [], [], []
    lvl0 = None
...
def section_findings():
  ...
  (A2) docstring と実装の食い違い(9 節で実測):
      * ``measure_pairs`` の説明は「立ち上がり/立ち下がりエッジのペア」だが、
        実装は **極性が交互なら順序を問わない**。暗い構造の幅が返ることがある。
      * ``metrology`` の矩形は ``add`` で (phi, l1, l2) を受け取るのに、
        ``apply`` は **l1 >= l2 に正規化して** 返す。l1 < l2 で入れると
        phi が 90 度回って軸が入れ替わる。往復しない。
      * ``metrology`` の直線の ``angle_deg`` は **col 軸から**の角度。
        「鉛直からの傾き」を期待すると 90 度ずれる(12 度の面が 78 度と出る)。
