...
この PoC が示すこと(数字はすべて実行時に印字される実測値):
 ...
 1. ★★**「水位が高いほど誤差が大きくなる」という予想は外れた**。行番号を
    ...
    アンカー(0.00 / 2.00 m)の上では誤差 0、その間で最大 **-6.6 cm**
    (水位 1.0 m 付近)。単調ではありません。透視の写像は行 = (aZ+b)/(cZ+d)
    という一次分数関数で、直線で近似すると**弦と双曲線の差**になるからです。
 2. ★**符号はアンカーの取り方で変わる**。同じ画像・同じ水面線でも、
    下のペア(0.00/0.50 m)で較正すると **+18.6 cm**(高く出る)、
    上のペア(1.50/2.00 m)なら **-6.3 cm**(低く出る)。内挿は一方の符号、
    外挿は逆の符号 —— 「誤差の向き」は現場の較正表で決まります。
 3. **4 点のホモグラフィで正対化すると誤差は 1 桁以上落ちる**(素朴法の
    最悪 6.6 cm に対して 0.5 cm 台)。残っているのは透視の誤差ではなく
    **水面線の検出誤差**で、対照群(検出を使わず厳密な交点を入れる)では
    1e-4 m 台まで落ちます。
 4. ★**波立ちだけならロバスト当てはめは要らない**。零平均の波 ±4 cm では
    最小二乗もロバストも同じ(比 1.0 倍)。効くのは**泡で検出が外れた列**が
    混ざったときで、そこでは RANSAC が最小二乗の **9 倍**良い。
    「ロバストにしておけば安心」ではなく、**外れ値があるときだけ効く**。
 5. ★★**反射は系統的に水位を低く見せる**。護岸が水面に映ると、水面直下も
    壁のように明るいので、しきい値の交差が下へずれる —— 実測 **-9.9 cm**。
    対照群(反射率 0)では -0.3 cm なので、これは反射の効果です。しかも
    **偏りは常に片方向**(低く出る)なので、平均しても消えません。
 6. 夜間の低コントラスト(コントラスト 1/4 + 雑音)では誤差が σ とともに
    増え、σ=0.02 で標準偏差 3.5 cm。偏りではなくばらつきが増えます。
