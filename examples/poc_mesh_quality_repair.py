...
2. ★★**6 種類の欠陥のうち 5 種類は χ を 1 も動かさない**。裏返り 6 面、
   非多様体辺 5 本、退化三角形 16 枚、重複頂点 18 個、自己交差 6 頂点 ——
   どれも χ = 0 のまま。χ が動くのは穴(6 個で χ = -6)だけ。
3. ★★**その 1 つの数字は打ち消しで騙せる**。穴 6 個(χ -6)と重複面 6 枚
   (χ +6)を同時に入れると **頂点 5604・辺 16812・面 11208・χ = 0 で
   健全な部品と 1 つも違わない**。種類ごとに数えれば、同じメッシュに
   境界辺 18 本と非多様体辺 18 本が同時に立っている(水密 False)。
4. **種類ごとに数えると内訳まで 1 個の狂いもなく合う**。ただし★★どの数字も
   「その欠陥だけ」を数えてはいない: 境界の輪は 23 個 = 穴 6 + 割れ 12
   + テント 5 で、輪の数を穴の数だと思うと **3.8 倍**に読み違える。
   ★★**順番が両向きに効く** —— 先に頂点を溶接すると割れの輪 12 個は消えて
   11 = 穴 6 + テント 5 になるが、**退化三角形も 16 → 0 枚に消えて数え
   損ねる**(潰れ頂点は元の頂点から 4e-11 mm、溶接の許容差の内側)。
   退化は溶接の**前**、穴は溶接の**後**に数える。重複頂点も 26 個と出て
   真値 18 個に合わない —— 差の 8 個は潰した辺が置いた頂点で、
   **退化と重複は t が小さいところでは分けて数えられない**。
5. ★★**「直った」と「戻った」は別の指標**。ゼロ幅の割れは 2 通りに直せて
   **どちらも水密になる**が、頂点を溶接すると表面積の誤差は 0.0000 %、
   穴埋めで塞ぐと **+0.1201 %(面が 36 枚増える)**。水密かどうかだけを
...
6. ★**穴の大きさの崖は閉形式で予測できる —— 体積だけ**。球から半頂角 θ の
   ...
   (扇が平らなので冠の体積がそのまま抜ける)。★ところが**表面積は予測が
   外れる**(θ = 45° で予測 -2.145 %、実測 **+6.868 %** —— 符号すら逆)。
   縁が円ではなく**階段**だからで、θ=45° の縁は実周長が円周の 2.13 倍、
   張られた扇は平らな円 πa² の **1.72 倍**ある。★★しかもこれは穴の
   大きさの関数ではない —— θ = 32° と 80° では扇/πa² が 1.00 と 1.01 で、
   **縁が voxel 格子のどこに落ちるかで決まる**。予想は「穴が大きいほど
   単調に悪化する」でしたが、実測は 45° だけ跳ねました。
...
8. ★**簡略化の誤差則は途中から変わる**。幾何からの予測は「相対誤差 ∝ 1/F」
   (弦の矢 ∝ h² ∝ 1/F、両対数の傾き 1)で、実測の傾きは面数 1680→224 の
   区間で **1.18** とほぼ予測どおり。ところが 5604→1680 の区間では
   **2.52** で予測よりずっと急 —— 削り始めは「消しても損しない頂点」から
   消えるので、誤差はしばらく予測より小さいまま。**崖は 70 % 削減あたり**
   (体積誤差が 0.1 % を超える)。表面積の誤差はどの削減率でも体積より
   1 桁小さく、95 % 削減では -0.022 % と**むしろ縮む**(打ち消し)。
9. ★**退化三角形は「検出される」より「壊れる」ほうが 1 桁早い**。面積比
   1.1e-5(t = 1e-5)では ``remove_degenerate_faces`` は 16 枚中 **0 枚**
   しか落とさないのに、潰れ面の法線は既に 4 枚が 1° 以上(最大 **4.95°**)
   親の平面からずれている。水密も多様体も χ も健全のまま。★同じ族の中で
   「退化」の定義が 3 通りある: t = 1e-6 で ``face_normals`` は**メッシュ
   全体を拒む**が、外積が厳密に 0 なのは 2 枚、
   ``remove_degenerate_faces`` が落とすのは 4 枚。
10. ★★**自己交差はどの位相検査もすり抜ける**。頂点を 6 個だけ内側へ
    0.75 mm(襟の管径 0.60 mm より深く)突き刺すと、水密 True・辺多様体
    True・χ = 0 のまま **表面積は +4.414 %、体積は -0.332 %** —— **13 倍**
    の食い違い。体積だけを合否にしていると通る。fullseye には自己交差の
    検出 op が無いので、辺長の外れ値で 6 頂点を拾った(下の「道具の穴」)。
