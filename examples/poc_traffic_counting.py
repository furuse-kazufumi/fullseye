...
1. **ゼロ点はそもそも別の量を測っている**。フレームごとの連結成分の最大値は
   「同時に写っている台数」であって通過台数ではない。実測でも通過 10 台に
   対して最大値 7(-30 %)。★同じ画像列から「同時台数」として比べても
   158/180 フレームで下振れする(平均 1.26 台不足)—— 融合の分。
2. ★**疎な自由流では、スリット法は仮想ループ(1-D)に勝てない**。どちらも
   10/10 で同点。同じ計数列の情報しか使っていないのだから当然で、
   **2-D にした見返りは台数ではなく速度**(誤差 0.02 % 以下)と、
   壊れ方が絵で見えることのほう。ここを混ぜて「スリット法は強い」と
   書くと嘘になる。
3. ★★**同じスリット画像から 2 通りの数え方が出て、壊れ方が逆向き**。
   帯を全部数えるとフレーム間隔を空けたとき千切れて過大(Δt=16 で 10→49)、
   計数列と交わる帯だけを数えると過大は起きず見逃しだけになる(最大 10)。
4. ★★**破綻の条件が 3 つとも同じ式で書ける: L / V**(車長 ÷ 速度)。
   (a) 車間の時間間隔が L/V を切ると帯が融合する —— 予測(空隙 1 px の
   幾何)と実測が 8/8 条件で一致し、L/V = 9.0 frame のすぐ上の 11 frame
   から 7 台に分かれる。(b) フレーム間隔 Δt が L/V を超えると計数列に
   一度も標本化されない。★ただし予測 Σ min(1, L/(V·Δt)) は**位相について
   の期待値**なので、1 つの位相で測った 1 本の数字と比べても合わない
   (差 -1.0〜+2.2 台)。位相を 4 通り平均すると -1.08〜0.00 台に収まる。
   (c) 停止(V→0)は L/V → ∞ で、この 3 つの破綻からは遠ざかる。
5. ★★**2-D の連結は画像全体で効く —— それが代償**。車線内の速度がばらつくと
   帯が**計数列から離れた場所で交差して融合**し、計数が壊れる(headway 14
   で 7 → 4)。1-D の仮想ループはこの壊れ方をしない。
6. ★★**予想が外れた 3 件**。(i) 背の高い車は計数を「増やす」と踏んだが実測は
   **減らした**(遠い車線 4 → 1)。対照群(トラックだけを流す)では帯が
   確かに 6 本立つので、書き込みは起きている —— 遠い車線の帯と交差して
   融合するほうが勝つ。(ii) 台帳の ``blob_features["angle"]`` は主軸が寝て
   使えないと踏んだが、**行ごとの重心当てはめより良かった**(0.20/0.52 %
   対 0.84/4.16 %)。端で切れた行を落とすと当てはめが 0.02/0.01 % に下がる
   ので、悪かったのは角度ではなく重心のほう。(iii) フレームレートを落とすと
   「速度が先に壊れる」と踏んだが、**台数が先**(台数 Δt=6、速度 5 % 超は
   Δt=12)。
7. ★★**渋滞で壊れるのは検出器ではなく背景モデル**。時間中央値の背景は停止
   した車を**背景として学習する**(先頭車の前景率 0.86 → 0.00)。通過台数は
   3 → 1、最終フレームの検出は 6 → 0。**真の空き路面に替えるだけで両方とも
   真値に戻る**ので、数え方をいくら工夫しても 1 台も戻らない。
8. ★合成の側のバグは、推定器の欠陥に化けて見える。渋滞の軌跡を最初
   「制動開始以降だけ停止位置で頭打ち」と書いていて、停止位置を追い越した
   車が**後ろへ飛んで**いた。絵では気づかず、帯が 1 本余計に融合して
   「対照群でも 2/3 しか取れない」という嘘の結論が出かけた
   (:func:`_jam_positions` に単調性の assert を置いてある)。
