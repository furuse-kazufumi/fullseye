...
| `perception_on_video.py` | a **real mp4/gif clip** → optical flow → motion-energy events → global-motion removal → moving-region segmentation → multi-frame point tracking, with an honest photometric reconstruction-gain metric (falls back to a synthetic clip if none given) |
...
py -3.11 examples/perception_on_video.py path/to/clip.mp4 --save out/demo   # or no path -> synthetic
```
