...
def main() -> int:
    ...
    if strong:
        ...
        #   狭く(実測 0.0258 対 0.0581)、撮り方の違いだけで無事な字まで超えてしまう
        #   (実測 7 本中 5 本)。**この環境では言えないことを言わない**のが正しく、
        #   閾値を緩めて通すのは測定を捨てるのと同じ。CI には CJK を 2 本入れてある。
        print("  ※ 書体が 1 本しかないので、床は妨害から作った狭いもの。"
              "**誤検出ゼロは主張しない**(見逃しゼロと修復の効きだけを固定する)。")
