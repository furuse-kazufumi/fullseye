...
【この PoC が示すこと(数字はすべて実行時に印字される実測値)】
 ...
 1. **ラスタライズの床**。正対・雑音なしで面積の誤差は **+0.17 %**。以降の
    誤差はすべてこの床の上に乗っている。
 2. ★★**ゼロ点(1 枚目だけで較正)を距離で振ると、面積は 2 乗で動く**。
    Z/Z0 を 0.90 → 1.10 と振ると面積誤差は **+23.58 % → -17.36 %**。閉形式の
    (Z0/Z)²-1 とは最大 **0.17 pp** しか違わない(= 床そのもの)。線形近似の
    -2ε は ±10 % で **2.6 pp** 外れるので、**「2ε」は 1 次項であって法則ではない**。
    ★撮影距離が 5 % 違うだけで面積が **9.1 %** 動く。
 3. ★★**k の推定が壊れる**。日ごとに距離が +1.2 % ずつ漂う(ジッタ 1.5 %)
    8 セッションで、真の k = 0.1200 /day に対しゼロ点は **0.1440 ± 0.0044**
    (**+20.0 %**)。**漂い 1.2 %/day の 2 倍 = 2.4 %/day が指数の肩に乗る**という
    予測どおり。毎回較正しなおすと -3.2 %、正対化すると **+0.3 %**。
 4. ★**毎回較正しなおしても傾きは直らない**。1 本の長さで較正する方式は距離の
    ずれをほぼ完全に消す(遠 495 mm で -0.58 %)が、**傾き 20 度で -9.1 %**。
    ホモグラフィで正対化すると同じ場面で -1.2 %。
 5. ★**しきい値と較正を同じ軸で比べると、較正が 4〜6 倍支配的**。公称値からの
    相対摂動 12.5 % に対し、面積は較正 **26.6 %** / しきい値 **4.75 %**
    (ぼけ σ=0.45 px)・**6.50 %**(σ=2.4 px)動く。**ぼかすとしきい値の感度が
    上がる**が、それでも較正には届かない。
 6. ★★**正対化の残差は再標本化ではなく標識の置き場所で決まる**。対照群
    (**真の**ホモグラフィで正対化)の誤差は最大 **0.10 %** —— つまり
    warp + 二値化の床は 0.1 % 台。推定ホモグラフィだと 1〜2 % に増える。
    標識を創面の**そば**(中心間 61 mm)から**創面を囲む**配置に変えると
    平均 |誤差| が **1.37 % → 0.27 %** に下がる。**標識は測る場所を囲め**。
 7. ★**予想が外れた**。「較正しなおせば必ずゼロ点に勝つ」と踏んでいたが、
    遠くから傾けた 1 場面ではゼロ点 -22.4 %・長さ較正 **+15.6 %** で、
...
def section2_zero_point():
    ...
    print("     %.3f pp(= 1 節の床そのもの)。線形近似 -2eps との差は最大 %.2f pp。" %
          (dev_law, dev_lin))
    print("     **「2ε」は 1 次項であって法則ではない** —— ±10 %% では 2.6 pp 外れる。")
