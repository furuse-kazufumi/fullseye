...
2. 空だけの視野の雑音は ``sqrt(sky + read^2)``。MAD 推定はそこに 2 % で当たる。
   星が 30 個詰まった視野では素の ``np.std`` が **20 倍**外し、MAD でも星の裾で
   1.5 倍ほど上振れする —— 頑健推定は「星に無関係」ではなく「桁を外さない」。
...
def main():
    ...
    print(f"PASS: astrostack 14 op —— 一晩 {N_FRAMES} 枚から 1 枚を作り、"
          f"ディザを 0.15 px 以内・宇宙線を再現率 1.000・雑音を 1/√N(誤差 3 % 以内)・"
          f"drizzle のフラックス保存を 1e-12・カタログのフラックスを 2 % 以内で"
          f"再現した。壊れる側(κ-σ の 50 % 破綻、PSF の 1/12 補正、drizzle の"
          f"縁こぼれ)も同じ数で示した")
    return True
