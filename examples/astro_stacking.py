...
4. ディザも自分で決めたので、``frame_align`` の誤差を **px で**言える ——
   シーイングが揺れる本番で最大 0.433 px、揺れを止めた対照で最大 0.103 px。
   **誤差の出どころが「星が太った枚」だと切り分けられる**。自由度の多いモデル
   (affine)ほど、その揺れを吸って並進を外す(0.086 → 0.627 px)。
...
9. 広い開口の測光は与えたフラックスをそのまま返す(雑音の無い 1 星で相対
   9.1e-14)。開口面積は ``pi r^2``。既知の空を足しても環状背景がちょうど
   引き戻す(1e-9)。合成画像でも **孤立星**なら誤差 0.13 %(最大 0.38 %)。
10. ただし **混み合った星は 15.8 %(最大 128 %)上振れする** —— 開口が隣の星を
   拾うからで、測光 op の精度ではなく星表の混み具合で決まる。この差を分けて
   出さないと「測光は 1 割ずれる」という間違った要約になる。
"""
...
def main():
    ...
    print(f"PASS: astrostack 14 op —— 一晩 {N_FRAMES} 枚から 1 枚を作り、"
          f"ディザを 0.10 px(揺れ無しの対照)・宇宙線を再現率 1.000・"
