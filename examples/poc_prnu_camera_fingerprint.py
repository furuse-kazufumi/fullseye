...
1. ★**ゼロ点は AUC では負けない —— 予想は外れた**。予想は「平滑成分を
   引かない生の平均は被写体まみれで使えず、最尤 + 適応 Wiener が桁で勝つ」。
   実測(30 枚)は生の平均でも真の K との相関 0.429・AUC 1.000。各枚から
   ガウス高域を引いて平均するだけのゼロ点 1 は 0.862 で、最尤重み付き
   (fullseye ``sensor_fingerprint``、Wiener 残差)の 0.858 と**同点**。
   場面が毎枚違えば平均が被写体を消してくれる。差が出るのは PCE の桁
   (最尤 2192 vs 生の平均 300、7.3 倍)で、**AUC だけ見ていたら見えない**。
2. **照合は清浄条件では 2 桁離れる**。カメラ A の指紋(30 枚)に対して
   A の写真 30 枚の PCE は中央値 2192(最小 1929)、B の写真は 15.7
   (最大 23.8)、AUC 1.000。★別カメラのピーク位置は毎回ばらばら((0,0) は
   0/30)、同一カメラは 30/30 で (0,0) —— PCE より先にこの位置が証拠になる。
   帰無分布(別カメラ 30 組)の 99 % 点は 23.4、同一カメラの最弱 1 枚でも
   z = 111.8。
3. ★**指紋は √N で育つ。N=1 では最尤がゼロ点に負ける**。最尤の N=1 は
   相関 0.205、ゼロ点 1 は 0.351 —— ΣWI/ΣI² は 1 枚だと W/I になり、暗い画素で
   雑音を割り増す(N≥3 で並ぶ)。corr(N) = 1/√(1 + (1/r₁² − 1)/N) で N=1 から
   外挿すると N=50 の予測 0.935 に対し実測 0.898(差 0.038 = 被写体の
   漏れ込みが作る下限で、N を増やしても縮まない)。PCE は 109 → 2411。
4. ★★**JPEG 相当の 8×8 DCT 量子化: PCE は品質 91 相当で 53 %、84 で 26 %、
   50 で 3.6 %** —— 「係数ごとの量子化利得の平均²」で予測した 3.8 % と
   ほぼ的中。予測が外れるのは品質 28 相当より下(品質 20 で予測 0.0 %、
   実測 0.8 %): ショット雑音がディザになって量子化幅より小さい信号も少し通す。
   ★それでも **AUC は品質 38 相当まで 1.000 を保つ**(品質 28 で 0.888、
   20 で 0.711)。PCE が 1/50 になっても判定はまだ壊れない。
5. ★★**縮小の崖は 0.5× にあり、消したのは幾何ではなく残差抽出器**。予想は
   「0.5× は 2×2 箱平均で相関が 1/2 → PCE 25 %、非整数倍の 0.9× は画素位置が
   合わず 0.5× より致命的」。実測は 0.9× で PCE 38 %・AUC 1.000・ピーク
   (0,0) 30/30(**非整数倍のほうが安全**)、0.5× で **1.5 %**・AUC 0.900・
   (0,0) 25/30。K 自身を同じ往復に通した幾何の上限は 20 %(予想どおり)
   なのに、往復した写真から残差を取ると 2.4 % —— **縮小で 2×2 になまった
   指紋を、デノイザが『被写体』として取り去る**。0.95× でも 44 %。
6. ★★**被写体は指紋に化ける**。K=0 のカメラ C(指紋が無い)でも、同じ背景
   を写した 30 枚から作った「指紋」に別カメラ B の同じ背景の写真を当てると
   PCE 1706(AUC 1.000)—— **指紋が無いのに完全に「同一カメラ」と出る**。
   その「指紋」と真の K の相関は −0.015。場面を毎回変えると同じ C の PCE は
   −14.7(AUC 0.519)。★壁だけで作った A の指紋は模様入りより真の K との
   相関が高い(0.908 vs 0.858、PCE 2536 vs 2192)。**学習にはつまらない
   写真がいちばん良い**。
7. ★**デノイザで PCE は 17 倍動くが、壊れるのは指紋ではなく照合側の残差**。
   16 枚・同じ写真で、fullseye wavelet を 1 として scipy Wiener 0.85 /
   メディアン 0.65 / sk_wavelet 0.58 / fullseye wiener 0.56 / ガウス 0.49 /
   TV 0.06 / NLM 0.06 / ``xsp_dct_denoise`` は消滅(PCE −10、残差 RMS が
   雑音の 8 倍 = 被写体ごと削る圧縮器)。★TV / NLM は**指紋そのものは
   悪くない**(相関 0.725)のに PCE 216 / 195 —— 照合側の残差だけ Wiener に
   替えると 969 / 1009 に戻る。残差 RMS(0.029 / 0.036)が雑音(0.014)より
   大きい = 照合画像の被写体が残差に漏れ、PCE の分母を持ち上げていた。
...
def main() -> int:
    ...
    print("まとめ")
    print("=" * 78)
    print("  * 場面が毎枚違えば、生の平均でも指紋は出る(AUC 1.000)。差は PCE の桁(7 倍)。")
    print("  * 清浄なら PCE は 2 桁離れる。JPEG 品質 90 相当で半分、0.5× 縮小で 1/70(消したのは残差抽出器)。")
    print("  * 同じ背景を写した写真は、指紋の無いカメラでも指紋に化ける。学習は壁で。")
    print("  * デノイザで PCE は 17 倍動くが、壊れるのは指紋ではなく照合側の残差。")
