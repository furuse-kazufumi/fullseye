...
1. **保存則**: 拡張ガウス像は方向の計数ヒストグラムなので、総和は法線の本数に
   **厳密に等しい**。ブロックの 6 面は 6 つの bin に落ち、実測でその 6 bin が
   全質量の 81.6 % を占める(残りはボスが半球へ薄く散る)。
2. **閉形式**: 半径 R の球面の主曲率は両方とも 1/R。ボス R = 2.00 mm なので
   **0.500**、実測の中央値 0.5036(+0.7 %)。
3. **循環相関の厳密性**: b が a を s0 だけ ``np.roll`` したものなら、相関ピークは
   **厳密に** s0 に立つ(整数格子なので丸めも無い)。
4. **可逆性**: 位置・シフト・記述子・制御点の往復は **bit 一致**。回転倍率は
   max|Δ| = 7.1e-15、極形式は 3.8e-16。
5. **周回積分**: 単位円上の ∮dz/z = 2πi。極形式で r = 6.283175(真値 2π =
   6.283185、差は 2048 点の離散化)、θ = 90.000000 度ちょうど。

【この例が出す正直な結論】
**「往復して戻った」だけでは何も言えない。** 戻らない 4 つを、量を付けて残す:
  * 添字 → ラベル → 添字 は bit 一致だが、**逆向き**は末尾の背景 439 個を落とす。
