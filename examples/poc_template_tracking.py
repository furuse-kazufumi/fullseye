...
def main():
    ...
    print(f"  繰り返し模様:   ピーク値 {a_rep[0]:.3f} / 突出度 {a_rep[1]:.3f}"
          f"   → 突出度の勝ち")
    print(f"  そっくりな別物体: ピーク値 {a_tw[0]:.3f} / 突出度 {a_tw[1]:.3f}"
          f"   → 突出度の勝ち")
    print("  遮蔽では相関が素直に落ちるのでピーク値が効く。繰り返し模様と別物体では")
    print("  **正解と同じ高さのピークが別の場所に立つ** のでピーク値はほとんど何も言えない。")
    print("  そこを見分けられるのは 2 位との差だけである。")
    print(f"\n【結論 2】**ドリフトでは信頼度が嘘の側で高く出る。**")
    print(f"  毎フレーム更新(64 フレーム × 種 3): ピーク値 AUC {a_dr[0]:.3f}"
    ...
    print("  この崖は **ピーク値でも突出度でも検出できない**。真値か、往復追跡か、")
    print("  別の観測(1 枚目のテンプレートとの照合を並走させる)が要る。")
    print(f"\n【結論 3】1 本のしきい値では両立しない。平坦な遮蔽で校正した "
          f"ピーク {THR_PK:.3f} を")
    ...
    print(f"  同じしきい値を そっくりな別物体 に当てると "
          f"{sum(ch5t[f]['ff'] for f in ch5t)} / {sum(ch5t[f]['nl'] for f in ch5t)} 。")
