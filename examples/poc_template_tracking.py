...
どれも「壊れました」とは言わない。
  ...
  (2) **静かにずれる** —— テンプレートを更新すると、1 フレームあたり
      ...
      合わせているのだから当たり前)。
  (3) **自信満々で間違える** —— 繰り返し模様の上や、そっくりな別物体が
      一緒に流れてくる場面では、正解と同じ高さのピークが別の場所に立つ。
      実測では **ずれていた 152 フレームの 152 フレーム全部** が、
      遮蔽なしで校正したピークのしきい値を通って「見つけた」と報告した。
...
【章立て】
 ...
 3. 系の比較 4 種 × 変形の強さ 2 条件 —— 順位が入れ替わる
 4. ★ 崖 (a) フレーム間の移動量 —— 局所探索の窓を超えた瞬間に何が起きるか
 5. ★ 崖 (b) 遮蔽率 0 → 80 %(平坦な遮蔽物 / そっくりな別物体)と誤報告率
...
def main():
    ...
    # 相関は符号つき(-1..+1)なので発散 LUT。0 が中間色になり、山と谷が見える。
    # パネルの題は短くする(幅 156 px に収まらないと examplefig が書き出しを拒む)。
    figs.save_grid("ncc_maps", [corr_maps[k] for k in TARGETS],
                   list(TARGETS),
                   title="動かす前の相関マップ —— 失敗する対象はここで分かる",
                   ncols=2, signed=True,
                   caption="2 位の相関は " + " / ".join(
                       "%s %.3f" % (k, profile[k]["second"]) for k in TARGETS)
                       + "。繰返 は同じ高さの峰が周期的に並び、直線縁 は縁に沿った"
                         "尾根になる(その方向は位置が決まらない)。")
    ...
    occ_x = np.array(sorted(ch5)) * 100.0
    tw_x = np.array(sorted(ch5t)) * 100.0
    figs.save_plot("occlusion_vs_twin",
                   [("平坦な遮蔽物", occ_x,
                     np.array([ch5[f]["err"].mean() for f in sorted(ch5)])),
                    ("そっくりな別物体が並走", tw_x,
                     np.array([ch5t[f]["err"].mean() for f in sorted(ch5t)]))],
                   xlabel="遮蔽率 [%]", ylabel="平均誤差 [px]",
                   title="崖は「隠された量」より「似た物が居るか」で決まる",
                   caption="同じ遮蔽率でも、そっくりな別物体が視野に居るだけで"
                           "崖がはるかに手前へ来る。")
    ...
    print("-" * 68)
    # 誤差を時間で描くと「静かにずれる」が形として出る(表の 1 数字では出ない)。
    figs.save_plot("drift_curves",
                   [(lab, tt, ch8[lab]["err"])
                    for lab in ("毎フレーム", "4 フレーム", "16 フレーム", "更新なし")],
                   xlabel="フレーム", ylabel="位置誤差 [px]",
                   title="更新は変形についていき、同時に丸め誤差を溜める",
                   caption="更新なしは平らなまま。毎フレーム更新だけが数 px の"
                           "台へ離れていく —— しかもその間、相関ピークは最も高い。")
    s1 = ch8["毎フレーム"]["slope"]
    ...
    print("\n[足すとよい op(この PoC が実際に自前で書いたもの)]")
    print("  ncc_map(image, template) -> image")
    print("      相関マップそのもの。これが無いと突出度も 2 位の距離も出せない。")
    print("      穴 (a) を塞ぐ最小の 1 手で、以下 3 つはすべてこの上に載る。")
    print("  ncc_locate(image, template=None, roi=None, subpixel=True, top_k=1)")
    print("      引数でテンプレートを渡す(穴 c)/ roi=(x, y, r) で探索範囲を切る(穴 d)/")
    print("      重心で副画素へ(穴 b、3 次元側の _subvoxel_com と同じやり方)/")
    print("      top_k で 2 位以降も返す(突出度を呼ぶ側が計算できる)。")
    print("      既定を top_k=1・subpixel=False にすれば今の返り値と互換にできる。")
    print("  match_confidence(corr_map, suppress=None) -> table")
    print("      peak / prominence / peak2_distance / roi_width を 1 つの表で返す。")
    print("      **roi_width を一緒に返すのが肝** —— 第 9 章のとおり、探索窓を変えると")
    print("      突出度の意味が変わる。窓を書き添えない突出度は比較できない。")
    print("  shape_locate(..., angle_step=30.0, angle_range=(0, 360), pad='reflect')")
    print("      刻みを引数に出す + 回転テンプレートの四隅をゼロで埋めない(穴 f)。")
    print("  track_template(frames, template, update='none'|'every'|'k'|'confidence',")
    print("                 roi=None, predict='constant_velocity') -> table")
    print("      第 3 章の 4 系を 1 つの op に。返りは位置列・信頼度列・更新した")
    print("      フレームの一覧。★ 既定は **update='none'**(ゼロ点)にすべきである ——")
    print("      第 8 章のとおり、更新は条件によっては黙って誤差を 5 倍にする。")
    ...
    print(f"\n  (所要 {el:.1f} 秒)")
    # 予算 300 秒。手元(Windows, 2026-09-06)は 40 秒台、GitHub の共有ランナーは
    # **134 秒**(2026-09-07 の CI 実測)で、90 秒の自己制限が CI でだけ落ちた。
    # ここで守りたいのは「桁で遅くなっていない」ことなので、ランナー差の 3 倍を
    # 見込んだ上限にする(壊れたループなら数千秒になる)。
    assert el < 300.0, f"300 秒を超えた ({el:.1f} 秒)"
    print("\nPASS")
