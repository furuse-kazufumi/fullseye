...
★★この PoC の芯は 7 つ:
  ...
  2. **線形 Attention は結合則そのもの。** (QKᵀ)V == Q(KᵀV) が **1.18e-15**。
     O(T²d) と O(Td²) は同じ数の別の括り方で、**演算回数の比は厳密に T/d**
     (T=2048 で 32 倍、整数)。交差点が **T == d** にあるのはこの式の帰結である。
     ★予言は途中で外れる —— **実測の時間比は 32 倍に届かず 15〜16 倍で頭打ち**
     になる(二次の側が帯域律速)。
     ★★ただし時間は機械が決める。同じ commit で手元は T=1024 で 11.6 倍、
     CI の走者は **2.0 倍**だった。だからこの PoC は **演算回数の恒等式だけを
     合否にし、時間は `[測]` として報告する**。2026-09-25 に時間を assert して
     CI の 3 シャードを全部落とし、ここを直した。
...
def measured(label, detail):
    """★合否を持たない**実測の報告**。

    時間は機械が決める。同じコードでも BLAS も熱も違えば別の数が出るので、
    速さを合否にすると「手元は緑、CI は赤」になる —— 実際 2026-09-25 に
    「交差点は T == d」を assert して CI の 3 シャードすべてを落とした
    (手元 T=1024 で比 3 超、CI では 2.0)。**主張の芯は恒等式の側**なので、
    時間はここに落として数だけ見せる。
    """
    print("  [測] %s ---- %s" % (label, detail))
...
def run_checks():
    ...
    #: ★合否にするのは**厳密に数えられる側**だけ。(QK^T)V は 2T^2 d + 2T^2 d 回、
    #:   Q(K^T V) は 2Td^2 + 2Td^2 回の積和なので、比は **厳密に T/d** である。
    #:   これは機械に依らない整数の主張で、交差点が T == d にあることの中身でもある。
    flops = [(4 * tt * tt * d2, 4 * tt * d2 * d2) for tt in ts]
    exact = all(q * d2 == li * tt for (q, li), tt in zip(flops, ts))
    check(exact, "★演算回数の比は厳密に T/d(交差点が T == d である理由)",
          "T=%d で %d 対 %d = %d 倍(整数)"
          % (ts[-1], flops[-1][0], flops[-1][1], ts[-1] // d2))
    out["flops_ratio_2048"] = ts[-1] // d2
    #: 時間は機械が決めるので**報告だけ**。合否にすると手元緑・CI 赤になる。
    measured("実測の時間比(機械依存なので合否にしない)",
             "T=64 で %.1f 倍 / T=1024 で %.1f 倍 / T=2048 で %.1f 倍 —— "
             "演算回数は 32 倍を言うのに実測が届かないのは、二次の側が"
             "帯域律速で頭打ちになるから"
...
def draw_figures(out):
    ...
    figs.save_plot("linear_crossover",
                   ...
                   caption=("**答えは同じ**(相対差 %.1e)。違うのは括り方だけで、"
                            "演算回数の比は厳密に T/d ―― 交差点が **T == d = 64** に"
                            "あるのはその帰結。★予言は途中で外れる —— T=2048 では"
                            "式が言う 32 倍でなく **%.1f 倍**で頭打ちになる"
                            "(二次の側が帯域律速)。★時間は機械に依るので、"
                            "この図の速さはこの走者の値であって主張ではない。"
