...
1. **ゼロ点(光条の輝度が最大の画素をそのまま高さにする)は 1 桁負ける**。
   θ = 36 度・雑音 1.5 %・光条 1σ = 1.4 px で高さ RMS は 最大値 0.02458 mm /
   重心(固定窓)0.00562 mm(4.4 倍)/ 対数放物線 0.00326 mm(7.5 倍)。
   fullseye の ``lines_gauss`` から列ごとに中心を取ると 0.04165 mm で
   **ゼロ点より悪い(1.7 倍)** —— HALCON の同名 op が返す「中心線 + 線幅」
   ではなく Frangi リッジの二値化なので、サブピクセルが出ない。
2. ★★**「重心を使った」だけでは推定量が決まらない —— 窓の決め方で光条幅
   への向きが逆になる**。予測は 固定窓 σ^-1.00(太いほど良い)/ 幅に追従
   する窓 σ^+0.50(細いほど良い)。実測は σ^-0.61 と σ^+0.88 で**符号は
   予測どおり**。★大きさのずれは対照群が説明した: 固定窓の ``clip(I-BG,
   0, None)`` という 1 行が雑音を片側に整流していて、**切り上げをやめると
   σ^-0.98 に戻る**。★偏りと雑音も別物で、追従窓は雑音ゼロでも 0.00498 mm
   残る(しきい値が副画素位置で跳ぶ)のに対し固定窓は 0.00000 mm。
   ★3 点当てはめ(対数放物線)は幅への感度が桁違い(σ^+1.56)で、細い側にも
   床があり 1σ = 0.7 px が最良(0.00134 mm)、0.5 px では 1.9 倍に跳ね返る。
3. ★★**遮蔽の崖は 2 種類あり、形がまるで違う**。幾何から予測: 左母材面は
   傾きが一定 tan53 = 1.327 なので **θ = 37.0 度で全部いっせいに背を向ける**
   (段差の崖)。左アンダーカットは溝の斜面が母材の傾きに上乗せされる分だけ
   早く、**θ = 20.8〜34.5 度から少しずつ欠ける**(なだらかな崖)。実測では
   左側 4 量の「測れた率」が 24 度の 100 % から 28 度 90 % → 32 度 40 %
   → 36 度 8 % → 40 度 0 % と落ちる。★**予測(母材面の 37 度)より早い**
   のは、つま先の判定にその**まわりの列**が要るから —— 溝が先に陰へ入ると
   つま先そのものが見えていても判定できない。
4. ★★**いちばん深い欠陥がいちばん先に隠れる**。左アンダーカットの予測
   遮蔽開始角は深さ 0.43 mm の断面で 21.2 度、0.06 mm の断面で 33.6 度。
   深い溝ほど斜面が急なので早く陰に入る —— **通してはいけない欠陥から
   消える**。
5. ★★**「測れた率」は 100 % のまま値だけが浅くなり、そのあと生存者バイアス
   が来る**。θ = 28 度では全断面が「測れた」のに溝の区間の 25 % が欠測して
   いて、誤差は 0.0652 mm(生き残った断面の真値 平均 0.244 mm の 27 %)。
   さらに θ を上げると**深い断面から「測れない」に落ちる**ので、生き残った
   断面の真値の平均が 0.260 → 0.244 → 0.105 → 0.047 mm と浅いほうへ流れる。
   平均誤差だけを見ると 0.0652 → 0.0192 → 0.0227 mm と「良くなった」ように
   見えるが、**消えた断面がいちばん危ない断面**なので改善ではない。
6. ★★**最適角は測定量ごとに違い、1 つの角度で全部は取れない**。全断面が
   測れる中での最小は 凸み 24 度 / 左脚長 24 度 / のど厚 24 度 /
   右脚長 36 度 / 左アンダーカット 20 度 / 右アンダーカット 64 度。
   ★「誤差が最小の角度」と「全断面が測れる角度」も別物で、凸みは 32 度の
   0.0095 mm が最小だがそこで測れているのは 40 % の断面だけ、全断面なら
   24 度の 0.0201 mm(2.1 倍)。★**近側の脚長まで 40 度で消える** ——
   脚長ものど厚も**根**(2 枚の母材面の交線、溶接金属の下)が基準なので、
   遠側の面が 1 枚見えなくなると手前の量まで道連れになる。
7. ★★**校正誤差(高さ倍率 +1 %)は量ごとに 3.2 倍違い、しかも脚長と溝
   深さは同じ母材面で真逆**。予測は 脚長 = sin²(母材角)·ε、溝深さ =
   cos²(母材角)·ε(**和がちょうど 1**)。実測は 左 脚長 0.674 + 溝 0.359
   = 1.033、右 脚長 0.394 + 溝 0.634 = 1.029。★凸みだけ予測 0.958 に対し
   実測 1.152 と外れたが、対照群で切り分けた: しきい値 τ が**絶対値**なので
   高さが伸びるとつま先が -0.133 / +0.135 mm 外へ這い、弦の両端が下がって
   +0.186 増える(0.958 + 0.186 = 1.145 ≈ 実測)。
8. ★**法線から描いた遮蔽の地図は投げかけ影を落とす —— ただし格子が粗いと
   符号すら反転する**。0.1 mm 角の格子では「法線で背向き」⊂「水平線で
   不可視」が全角度で成立(差 +0.7〜+3.4 pp = 投げかけ影)。ところが
   0.5 mm 角では θ = 55 / 65 度で差が -1.4 / -0.5 pp と**逆転**する
   (稜線をまたぐセルの法線が平均化されて背向き側に倒れる)。★正射版
   ``normals_from_depth`` は 1 画素 = 1 単位と決め打つので、x 0.05 mm・
   y 0.5 mm の生の走査格子をそのまま渡すと傾きが 10 倍ずれる。
9. ★のど厚は 3-D の距離場でも測れる。溶接面の点群 → ``occupancy_grid``
   → ``esdf`` → 根の位置で ``query_distance``。閉形式 3.3722 mm に対し
   3.3291 mm、差 -0.0431 mm(ボクセル 0.05 mm の 0.86 個ぶん)で
   **60 断面すべて負** —— 距離場は最近傍ボクセル**中心**までを測るので、
   面を離散化した時点で根に近い側の中心が拾われ、系統的に短めに出る。
...
def main() -> int:
    ...
    for s in ("L", "R"):                     # 余りはつま先の這い(7 章で説明)
        assert abs(cal["got"]["leg" + s] + cal["got"]["uc" + s] - 1.0) < 0.05, "sin²+cos²=1"
