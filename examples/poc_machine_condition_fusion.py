...
:func:`mode_params` の**重症度 1 つから 3 センサすべてを決める**構造も
実機に合わせて置き換えます —— ここを「センサごとに独立な乱数」にすると
束ねただけ良くなる嘘の結果が出ます(それがこの PoC の反例)。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. ★★**基準条件では、束ねても 1 ポイントも増えない**。3 センサ融合の 6 クラス
   識別率は 100.0 %、しかし**振動のみでも 100.0 %**。熱を抜いても形状を抜いても
   100.0 %。融合が仕事を始めるのは**振動が壊れてから**で、加速度計の雑音を
   σ=0.05 → 1.60 に上げると 振動のみ 45.8 % に対し融合 78.1 %(+32.3 pt)。
   ★予想は「融合は常に少し良い」だったが、実測は**振動が効いているあいだは
   1 pt も足さない**。
2. ★★**モードごとに必要なセンサの数と種類が違う**。基準条件では 6 モードとも
   センサ 1 個で 3 センサに並ぶが、**届く 1 個は同じではない** —— 芯ずれは
   振動 / 熱 / 形状 の**どれ 1 個でも 100.0 %**、軸受外輪傷は 振動 か 熱、
   残る 4 モードは**振動だけ**。芯ずれで 3 つが揃うのは軸心のずれ 1 つが原因で、
   2X 振動も継手の発熱も**その 1 つの数字の別の顔**だから。真値の重症度との
   相関は 2X 振動 0.843 / 継手温度 0.841 / 芯ずれ量 0.978 で、独立な 3 つの
   証拠ではない。
3. ★★**1 つでは絶対に分けられない組がある**。熱だけだと 正常・アンバランス・
   ゆるみ の 3 モードが**互いの中で 48/48 回まわる**(仕込んだ軸受の発熱が
   2.5 / 2.7 / 2.6 W でほぼ同じだから、原理的に分けられない)。形状だけだと
   芯ずれ以外の 5 モードが 1 つの塊(識別率 6.2〜37.5 %)。だから振動を抜くと
   アンバランス 100.0 → 50.0 %、ゆるみ 100.0 → 31.2 % に落ちる ——
   ★落ちるのは**軸受外輪傷(100.0 % のまま)ではなかった**。
4. **ゼロ点(センサ 1 つ・しきい値 1 個)は異常を見つけるが名指しできない**。
   振動 RMS の「健全の平均+3σ」は異常検出率 100.0 %(誤警報 0.0 %)に届くのに、
   返せるのは正常/異常の 2 値だけで**5 つの故障モードのどれも名指しできない**。
   熱の最高温度は 60.0 %、芯ずれ量は 21.2 %。
5. ★**崖 1(記録長)の予測 (a) は当たった**。ゆるみの 0.5X は次数分解能
   = 1/(整数回転数)が 0.5 を切る **T > 2/f_r = 68.6 ms** から測れる。実測でも
   予測を跨ぐ 50 → 70 ms の 1 段で d' が 2.45 → 4.32(+76 %)へ跳び、その前の
   40 → 50 ms の段は +4 % しか動かない。
6. ★**崖 1 の予測 (b) は外れた —— そして外れ方が有益だった**。軸受の側帯波
   (保持器周波数 FTF = 11.62 Hz 間隔)は「分解能 1/T < FTF」= 86.1 ms から
   分離できると予測したが、側帯波比が真値 0.275 に近づくのは実測 150 ms 以降で、
   100 ms ではまだ 0.730(真値の 2.7 倍)。原因は測ってある: ピークを読む窓の
   半幅 0.6/T Hz が側帯波間隔の半分(5.81 Hz)を切る **T > 1.2/FTF = 103 ms**
   のほうが厳しい。**分解能ではなく読み取り窓が崖を決めていた**。
7. ★**崖 2(熱の画素ピッチ)も 崖 3(回転数変動)も予測どおり**。高温域の
   半値半径は閉形式で 軸受 33.1 mm / 機械全体 92.9 mm。広がりの d' は
   ピッチ 32 mm まで 6.8 を保ち、半値直径 66 mm を跨ぐ 64 mm で 4.30、
   96 mm で 0.00 へ落ちた。回転数変動では**高次から先に落ち**、振幅が 80 % を
   割る δ は 2X で 2.54 %、櫛(3-6X)で 1.20 %。★ただし比は予測の 3.0 倍
   (最高次 6X / 2X)でなく **2.1 倍** —— 櫛は 3X〜6X の和なので低次が落ち方を
   ならす。「最高次で決まる」は上界であって実測値ではない。
8. ★★**崖はどれも特徴 1 個の上には出るのに、6 クラスの識別率には出ない**。
   熱の広がりが d' 0.00 まで死んでも熱のみの識別率は 潤滑不良 81.2 → 50.0 %
   に留まり、融合は 100.0 % のまま。**冗長性はセンサ間だけでなくセンサ内にもある**
   —— 崖を測るなら特徴の上で測らないと見えない。
9. **折り返し(エイリアス)だけは測り直すしかない**。共振 3000 Hz を 3200 Hz へ
   間引くと、衝撃列の k=29 の線が 167.9 Hz へ移ると予測でき、実測 168.0 Hz
   (差 0.13 Hz)。★**間隔は BPFO ちょうど**なので「軸受らしい」姿になるのに、
   絶対位置は機械のどの部品にも属さない。他の崖は測り方で下がるが、これは違う。
...
def section_sweeps() -> dict:
    ...
    out["noise"] = sweep("sweep_vibration_noise",
                         ...
                         "noise", ("v",), VIB_FEATS,
                         (("d' 1X アンバランス/正常", "v_o1", "アンバランス", "正常"),
                          ("d' BPFO 軸受/正常", "v_bpfo", "軸受外輪傷", "正常")),
                         ("軸受外輪傷", "ゆるみ"),
                         "雑音 σ", "振動の雑音を上げる(熱・形状は据え置き)",
                         "d' は特徴 1 個の情報量。識別率より先に落ちる。",
                         fmt="%8.2f")
    nz = out["noise"]
    gain = 100 * (nz["fuse_all"] - nz["solo"].mean(axis=1))
    k = int(np.argmax(gain))
    print("\n  ★★**融合が初めて役に立つのはここ**。σ=%.2f までは 振動のみ = 融合"
          "(差 %+.1f pt)だが、" % (nz["x"][0], gain[0]))
    print("     σ=%.2f では 振動のみ %.1f %% に対し融合 %.1f %%(%+.1f pt)——"
          " 振動が壊れて初めて熱と形状が仕事をする。"
          % (nz["x"][k], 100 * nz["solo"].mean(axis=1)[k], 100 * nz["fuse_all"][k], gain[k]))
    print("     ★予想は「融合は常に少し良い」だった。実測は**振動が効いている"
          "あいだは 1 pt も足さない**。")
    ...
    out["dur"] = sweep("sweep_record_length",
                       ...
                       "dur", ("v",), VIB_FEATS,
                       (("d' 0.5X ゆるみ/アンバ", "v_o05", "ゆるみ", "アンバランス"),
                        ("側帯波比(真値 0.275)", "v_sb", "軸受外輪傷", None)),
                       ("ゆるみ", "軸受外輪傷"),
                       "記録長 T [s]", "記録長を縮める —— 予測した 2 つの崖",
                       "予測は 68.6 ms(0.5X の次数ビン)と 86.1 ms(FTF 側帯波)。"
                       "側帯波比の真値は変調度 0.55 の半分 = 0.275。横軸は"
                       "周波数分解能 1/T —— 予測はこの軸の上で FTF = 11.62 Hz と"
                       "f_r/2 = 14.58 Hz。側帯波は予測より遅れて直り、"
                       "読み取り窓の条件 FTF/1.2 = 9.68 Hz のほうが合う。",
                       pscale=(1.0, 10.0), xplot=lambda v: 1.0 / v,
                       xplot_label="周波数分解能 1/T [Hz]")
    dz = out["dur"]
    x, d05, sb = dz["x"], dz["d"][:, 0], dz["d"][:, 1]
    j = int(np.argmax(x > t_order))                 # 予測を跨ぐ 1 段
    print("\n  ★予測 (a) は当たった。予測 %.1f ms を跨ぐ段 T=%.0f -> %.0f ms で"
          "0.5X の d' が %.2f -> %.2f(%+.0f %%)へ跳ぶ。"
          % (1000 * t_order, 1000 * x[j - 1], 1000 * x[j], d05[j - 1], d05[j],
             100 * (d05[j] / d05[j - 1] - 1)))
    print("     それ以前の段(%.0f -> %.0f ms)の変化は %+.0f %% しかない ——"
          "**跳ぶのは予測した段だけ**。"
          % (1000 * x[0], 1000 * x[1], 100 * (d05[1] / d05[0] - 1)))
    k = int(np.argmax(sb < 0.5))
    print("  ★予測 (b) は**手前すぎた**。側帯波比が真値 0.275 に近づくのは"
          " T=%.0f ms からで、予測の %.1f ms の段ではまだ %.2f(真値の %.1f 倍)。"
          % (1000 * x[k], 1000 * t_side, sb[k - 1], sb[k - 1] / 0.275))
    print("     原因は測ってある: ピークを探す窓の半幅が 0.6/T Hz で、これが側帯波"
          "間隔の半分(%.2f Hz)を切るのは **T > 1.2/FTF = %.0f ms**。"
          % (FTF / 2, 1000 * 1.2 / FTF))
    print("     分解能そのものではなく**読み取り窓**が崖を決めていた。修正した予測"
          " %.0f ms は実測の区間 %.0f〜%.0f ms の中に入る(元の %.1f ms は外)。"
          % (1000 * 1.2 / FTF, 1000 * x[k - 1], 1000 * x[k], 1000 * t_side))
    ...
    out["pitch"] = sweep("sweep_pixel_pitch", (1, 2, 4, 8, 16, 32, 64, 96), "pitch",
                         ("t",), THR_FEATS,
                         (("d' 広がり 軸受/潤滑", "t_spread", "軸受外輪傷", "潤滑不良"),
                          ("広がり mm² 軸受", "t_spread", "軸受外輪傷", None)),
                         ("軸受外輪傷", "潤滑不良"),
                         "画素ピッチ [mm]", "熱画像の画素を粗くする",
                         "軸受(局所)と潤滑不良(全体)を分けているのは広がりだけ。"
                         "予測の崖は半値直径 %.0f mm。" % (2 * rh_b),
                         fmt="%8.0f", pscale=(1.0, 2.0e-4))
    pz = out["pitch"]
    px, pd = pz["x"], pz["d"][:, 0]
    ...
    out["jitter"] = sweep("sweep_rpm_variation",
                          ...
                          "jitter", ("v",), VIB_FEATS,
                          (("2X 振幅 芯ずれ", "v_o2", "芯ずれ", None),
                           ("櫛 3-6X 振幅 ゆるみ", "v_comb", "ゆるみ", None)),
                          ("芯ずれ", "ゆるみ"),
                          "回転数変動 δ", "回転数が揺れると高次から壊れる",
                          "櫛(最大 6X)のほうが先に落ちる。予測は 3 倍差。",
                          fmt="%8.4f", pscale=(50.0, 50.0))
    jz = out["jitter"]
    jx, a2, ac = jz["x"], jz["d"][:, 0], jz["d"][:, 1]

    def knee(v):
        """振幅が δ=0 の 80 % を割る最初の δ(線形補間)。"""
        r = v / v[0]
        i = int(np.argmax(r < 0.8))
        if i == 0:
            return float("nan")
        f = (0.8 - r[i - 1]) / (r[i] - r[i - 1])
        return float(jx[i - 1] + f * (jx[i] - jx[i - 1]))

    k2, kc = knee(a2), knee(ac)
    print("\n  ★予測「高次から先に落ちる」は当たった。振幅が 80 %% を割る δ は"
          " 2X で %.2f %%、櫛(3-6X)で %.2f %% —— 櫛のほうが %.1f 倍早い。"
          % (100 * k2, 100 * kc, k2 / kc))
    print("  ★予測の**倍率**は外れた。最高次で数えると 6X/2X = 3.0 倍のはずだが"
          "実測は %.1f 倍。" % (k2 / kc))
    print("     櫛の特徴は 3X〜6X の**和**なので、低次の 3X・4X がまだ生きていて"
          "落ち方をならす。「最高次で決まる」は上界であって実測値ではない。")
    print("  ★1 ビン条件の予測(2X で δ<%.2f %%、6X で δ<%.2f %%)より実測の膝は"
          "だいぶ後ろ —— 読み取り窓 ±%.2f 次(±%.2f Hz)のほうが緩いから。"
          % (100 / (2 * 2.0 * FR * DURATION), 100 / (2 * 6.0 * FR * DURATION),
             0.08, 0.08 * FR))
    out["knee2x"], out["kneecomb"] = k2, kc
    return out
...
def main() -> int:
    ...
    st = sw["d05_step"]
    print("  * 崖は**特徴 1 個の上で先に予測できた**。当たり: 0.5X の次数ビンは"
          " T>%.1f ms(実測 %.0f->%.0f ms の段で d' %.2f->%.2f)、"
          % (1000 * sw["t_order"], 1000 * st[0], 1000 * st[1], st[2], st[3]))
    print("    熱の広がりは画素ピッチ > 半値直径 %.0f mm(実測 %.0f mm から崩れる)、"
          "高次ほど先に落ちる(80 %% を割る δ は 2X %.2f %% / 櫛 %.2f %%)。"
          % (2 * sw["r_half"], sw["pitch_step"][0],
             100 * sw["knee2x"], 100 * sw["kneecomb"]))
    print("    ★外れ: 側帯波は「分解能 1/T < FTF」= %.1f ms と予測したが実測は"
          " %.0f ms 以降。読み取り窓 0.6/T のほうが厳しく、修正 1.2/FTF = %.0f ms。"
          % (1000 * sw["t_side"], 1000 * sw["sb_step"][0], 1000 * sw["t_side_fix"]))
    print("    ★外れ: 高次が先に落ちる**倍率**は最高次比 3.0 のはずが実測 %.1f ——"
          " 櫛が 3X〜6X の和で、低次が落ち方をならすから。" % (sw["knee2x"] / sw["kneecomb"]))
    print("    ★そして**6 クラスの識別率はそこまで落ちない** —— 同じセンサの他の"
