...
1. ★★**基準条件では、束ねても 1 ポイントも増えない**。3 センサ融合の 6 クラス
   ...
   1 pt も足さない**。
2. ★★**モードごとに必要なセンサの数が違う**。6 モードとも「センサ 1 個で
   3 センサに並ぶ」が、その 1 個が同じとは限らない —— 芯ずれは**形状だけで
   100.0 %**(3 センサと同点)。芯ずれでは軸心のずれ 1 つが原因で、2X 振動も
   継手の発熱も**その 1 つの数字の別の顔**。真値の重症度との相関は
   2X 振動 0.843 / 継手温度 0.841 / 芯ずれ量 0.978 で、独立な 3 つの証拠ではない。
