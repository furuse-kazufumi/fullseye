...
倉庫・宅配・輸出の現場では、パレット 1 枚ごとに **積載率(体積占有率)**を
出して「積み方が下手」「もう 1 個載る」「はみ出しているから積めない」を
決めます。門型の 3-D 計測器は 1 パレット 2 秒で天端を測れるのに、そこから
出す 1 つの数字が**まったく違う 2 つの不良を同じ値にしてしまう**。
ここでは箱の寸法と位置を全部既知にし、その 1 つの数字が何を隠すかを測ります。

EXTEND: 実測に差し替えるなら :func:`scan` が返す (N,3) を、門型スキャナ /
天井 ToF / ステレオの点群に置き換えます(**真上からの視点であること**が
この PoC の前提そのもの)。:data:`LOAD_A` の直方体一覧は真値なので実物では
手に入りません —— 代わりに **WMS の出荷明細(箱の SKU と寸法・個数)**が
「実の箱の体積」の台帳になります。それが無いと §3 の内訳のうち
「見えない空洞」だけは**原理的に測れません**(上から見えないので)。
パレット外形と高さ制限は運用で決まっている既知量として使えます。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. **上から測れるのは「押し出した体積」であって箱の体積ではない**。荷 A は
   中段が空洞に架かるので、箱 1.2804 m3 に対し上から見える体積 1.3476 m3
   —— 差 0.0672 m3(見える体積の 4.99 %)は**原理的に見えない**。
2. **ゼロ点(荷を 1 個の外形とみなす)は 3 通りともよく外す**。荷 A で
   AABB +6.93 pt / 凸包 +6.47 pt / 主軸の箱 OBB +21.50 pt。★荷 B では
   **AABB が積載率 113.90 %、OBB が 166.44 %** —— はみ出しと突出を
   体積として数え込むので **100 % を超える**。高さマップだけが上から
   見える形を保つ(A +3.38 / B +0.28 pt)。
   ★最初に外形を「天端の点だけ」で作ったら、AABB も凸包も**天端の起伏を
   包む薄い板**(0.36 m3)になった —— 外形を出すには**既知のパレット
   デッキ(z=0)を点として足す**必要がある。
3. ★★**見かけの積載率が同じ 2 つの荷を、閉形式で作れる**(荷 B の中段を
   995.0 mm に解いた。差 0.02 pt)。同じ 62.65 / 62.67 % なのに中身は逆:
   荷 A は **3.11 pt が上から見えない空洞**で、はみ出しも高さ超過もゼロ。
   荷 B は空洞ゼロだが **はみ出し 2.10 pt(90 mm 張り出し)+ 高さ超過
   0.58 pt(天端 1911 mm / 制限 1800 mm)**を含む。**処置も逆で、A は
   積み直せば載り、B は降ろすしかない**。1 つの数字はこの区別を持たない。
4. ★★**同じ 1 つのつまみ(高さマップのセル寸法 g)が、逆向きに 2 通り
   壊す**。セルの中を最大値で畳む規約は**高さには安全側だが隙間には危険側**
   で、幅 w の隙間が取り戻せる割合は **max(0, 1 - g/w)**。実測は
   w=120 mm で 0.97 → 0.93 → 0.85 → 0.65 → 0.39 → 0.00(g = 5〜160 mm)、
   予測との差は最大 0.20。g = 40 mm では **20 mm の隙間が完全に消え、
   50 mm も 3 割しか残らない**。
5. ★**g = w ちょうどは「半分見える」ではなく、格子の位相で全か無かに割れる**。
   g = w = 20 mm でオフセットを 0/4/8/12/16 mm と振ると
   0.98 / 0.00 / 0.00 / 0.00 / 0.00 —— 平均 0.20 は**5 回に 1 回だけ
   全部見える**という意味で、1 回の測定の期待値としては読めない。
6. ★★**粗い格子は、1 mm も出ていない荷に「はみ出し」を作る**。境界をまたぐ
   セルに荷の天端が入るので、外側の面積ぶんが計上される。予測は
   **周長 P x g/2 x 天端 h**。荷 A の偽はみ出しは 0.0055 → 0.4117 m3
   (g = 5 → 160 mm)で、**g = 40 mm から本当に 90 mm 出ている荷 B の
   真のはみ出し 0.0450 m3 を上回る**。実測 / 予測は 42 %(g=5)から
   99 %(g=160)へ近づく —— 小さい g では格子がパレット端にそろう場合が
   相対的に効いて低めに出る。荷 B の実測も同じ偽の分を丸ごと含む
   (真値 0.0450 に対し g=160 mm で 0.4213 m3)ので、**粗い格子では
   どちらの荷も「はみ出している」で埋まる**。
7. ★★**一致度 1 個(IoU)はこの違いに構造的に盲目**。真の中身と上から
   押し出した形の IoU は荷 A 0.9493 / 荷 B 1.0000 —— どちらも
   「よく合っている」としか読めない。A の 0.0507 の食い違いは全部
   「中の空洞 0.0672 m3」で、B は 0。**1 個の一致度は「何が違うか」を
   持っていない**。
8. **代わりに使えるのは「あと 1 個入る最大の箱」**。自由空間へ
   :func:`fullseye.ledger.inner_box3` を掛けると、荷 A は天端より下に
   **高さ 400 x 奥行 1000 x 幅 100 mm**(= いちばん広い 120 mm の隙間、
   50 mm 格子で量子化)、制限まで含めれば **600 x 1000 x 1200 mm** の
   余りがある。積載率という割合と違って、**そのまま「この箱なら載る」と
   言える形**で出る。

【グラウンドトゥルース】パレット 1200 x 1000 mm、高さ制限 1800 mm
(使える体積 2.160 m3)。荷 A は下段に 400 x 400 x 420 mm の空洞を持ち、
中段の全面スラブがその上に架かる。上段は幅 20 / 50 / 120 mm の隙間を挟む
4 本の柱(天端 1180 mm)。荷 B は下段が -x 側へ 90 mm 張り出し、
300 x 400 mm の柱が 1900 mm まで突出する。中段の高さ 995.0 mm は
**見かけの積載率が荷 A と一致するよう閉形式で解いた**値。走査は真上から
標本間隔 3 mm・測距の雑音 σ = 3 mm、高さマップのセルは既定 10 mm、
デッキのしきい値 30 mm(= 10σ)。

来歴(公開文献のみ): ISO 6780(平パレットの主要寸法)/ 積み付けの
はみ出し・高さ制限の考え方は各社の運送約款に共通する運用 /
Bischoff & Ratcliff, *Issues in the development of approaches to container
loading*, Omega 23 (1995) 377 —— 積載率という指標の限界 /
O'Rourke, *Finding minimal enclosing boxes*, Int. J. Comput. Inf. Sci. 14
(1985) 183 —— 外接箱。
"""
