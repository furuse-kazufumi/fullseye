...
【実測(seed 固定。どの機械でも同じ数字が出る。所要 約 20 秒)】
 2 章  N=200 光子・背景なしでの距離 RMSE:
         ピーク位置そのまま      10.23 mm  ← ゼロ点
         重心そのまま             2.16 mm(背景がない時だけ強い)
         ゲート重心               2.17 mm
       背景を入れた瞬間に順位が入れ替わる。信号:背景 = 0.031 では
         ピーク  11.26 mm / 重心そのまま **416.76 mm** / ゲート重心 2.78 mm。
       ゼロ点比は **ピーク比 4.1 倍、素の重心比 150 倍**。
 3 章  N を 12.5 から 6400 まで 512 倍振って、ゲート重心の RMSE は CRB の
       **0.95〜1.07 倍**に収まり、log-log の傾きは **-0.500**。1/√N は成立する。
       一方ピーク位置は N=6400 でも 5.51 mm で止まる —— これは雑音ではなく
       **ビン格子への量子化バイアス**(真の到達時刻がビン中心から 0.28 ビン
       ずれている分)なので、光子を増やしても消えない。
 4 章  背景光: SBR を 0.008 まで落とすと RMSE が 5.54 → 256 mm に飛ぶが、
       **中央値誤差は 3.7 → 6.6 mm しか動かない**。壊れ方は緩やかな劣化ではなく、
       4.5 % の試行でゲートが背景のスパイクに食いつく **双峰の誤ロック**。
 5 章  パイルアップ: 先頭光子 TCSPC は距離を **近く**読む。バイアスは
       -8.85 mm / (光子/サイクル) でほぼ線形。同じ検出数でのショット雑音を
       超えるのは **1 サイクルあたり 0.02 光子**あたり(教科書の「1〜5 % 則」と
       一致)。Coates 推定量はこれを残差 0.024 mm まで厳密に戻す。
 6 章  多重反射: 2 面の間隔が σ_IRF の 0.5〜4 倍(16〜130 mm)のとき最大
       16.2 mm ずれる。これは **N を 16 倍にしても 1 mm も縮まない**(1/√N が
       止まる)。間隔が 6σ を超えるとゲートが遠い面を切り落とし、手前の面を
       0.06 mm で当てる —— 正しい数字を返すが、光子の 30 % を黙って捨てている。
