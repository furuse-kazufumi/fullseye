...
【所見(すべて下の節で実測。★ は重要、★★ は最重要)】
 ...
 1. ★★**同じレンズで「MTF 合格・迷光 不合格」を作れる**。裾の割合 g を
    ...
    6 段階の g のうち **3 段階で判定が割れる**(3 節の表)。

 2. ★★**予想が外れた**。書き始める前は「細かいチャートで測ると裾が見えない」
    と予想していた。実測すると**周波数の高低は関係が無い** —— 正弦チャートの
    絶対コントラスト(Michelson)は、周期 4 px でも 32 px でも
    ちょうど **(1-g)×コアの MTF** になる(実測と閉形式が 4 桁一致。
    周期が裾の広がり 448 px に近づく 128 px 以上でだけ 2 % 上振れする)。
 ...
 4. ★裾はどの周波数に居るか。全 PSF を `fs.psf_to_mtf` に渡すと MTF は
    f=0 の 1.0 から **f≈0.02 cyc/px までに 1-g の台地へ落ちる**(半分まで
    落ちるのは 0.0028 cyc/px = 周期 363 px)。一方 窓 ±16 px の SFR が持てる
    最低の非零周波数は 1/32 = 0.031 cyc/px ——**裾が居る周波数帯に、窓が
    1 本もビンを持っていない**。実空間で言い直すと、±16 px の窓には裾の
    エネルギーの 6 % しか入らない。95 % を入れるには ±202 px(標準の 13 倍)。
...
def section6_chart():
    ...
    print("     広がり(%d px)に近づく 128 px 以上では裾自身の MTF が効いて 2 %% ほど"
          % (2 * R_PSF))
    ...
    print("     絶対コントラストは g=0→0.20 で 20 %s 落ちるのに、周期 128 px を基準に" % "%")
