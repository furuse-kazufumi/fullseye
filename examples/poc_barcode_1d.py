...
1. ★**無傷では 6 つの読み方が全部 24/24 で読める。差はまったく出ない。**
   ゼロ点(1 本の走査線 + 固定しきい値 0.5 + run 数 43 の検査)も、
   9 行多数決も、9 行平均も、適応しきい値(Bernsen)も、
   :func:`fullseye.ledger.measure_pos` のサブピクセルエッジも、寛容デコーダも。
   **道具の良し悪しは壊してからしか分からない。**
2. ★★**「読めない」と言える実装かどうかが誤読率を 6 倍変える。**
   壊れ始めてからの 384 枚で、run 数を検査する**厳格**デコーダは
   成功 40.1 % / 誤読 **7.3 %** / 読み取り不能 52.6 %。同じ画像を、走査線を
   70 等分してモジュール中心を読む**寛容**デコーダ(構造を見ず必ず 9 桁返す)
   に通すと 成功 47.7 % / 誤読 **46.1 %** / 不能 6.3 %。
   ★寛容のほうが**成功率も高い** —— 読取率だけを見ると良い道具に見える。
   内訳を分けて数えないと、誤読を成功として買ってしまう。
3. ★検査数字は寛容デコーダの誤読を 46.1 % → **2.6 %**(約 18 分の 1)に
   するが 0 にはしない(10 桁に 1 つは偶然通る)。それでも
   **構造の検査(run 数)のほうが強い** —— 厳格は検査数字なしで 7.3 %。
4. ★★**傾きは「にじみ」ではなく「打ち切り」で死ぬ。** 1 本の走査線は
   傾いても run 幅が 1/cos 倍に伸びるだけで、桁ごとの正規化がそれを消す
   (16 度でも 1/cos = 1.040)。壊れるのは**走査線が符号の上下からはみ出す**
   とき。限界は幾何だけで決まり θ_max = atan(バー高 60 / 符号長 210) =
   **15.95 度**。実測の崖は 15.5 度(24/24)と 16.0 度(0/24)のあいだ ——
   **予測の 0.5 度以内**。モジュール寸法にもぼけにも雑音にも依らない。
5. ★★**汚れは大きいほうが安全**(非単調)。9 行多数決の成功数は汚れ
   0 → 1 → 2 → 3 → 4 → 6 → 8 モジュールで 24 → 24 → 19 → **15** → 18 →
   23 → 24 / 24。大きい汚れは走査線を**構造的に壊す**ので汚れた行は
   「読めません」と答えて棄権する。小さい汚れは**読めてしまう**ので、
   間違った票を投じて多数派を作る。
6. ★★**ぼけと傾きは「モジュール 1 本あたりの実効ぼけ」に畳めるか** ——
   **ぼけは完全に畳める**。モジュール寸法 m = 2 / 3 / 4 px の 3 通りで、
   50 % を切る位置が σ/m = **0.725 / 0.729 / 0.725** と一致した。
   **傾きは畳みきれない**。帯平均のにじみ(矩形)を同じ標準偏差のガウスへ
   換算すると σ_eq/m = **0.607** で、ぼけより **16 % 手前**。標準偏差を
   揃えても矩形とガウスは裾が違う —— 1 本の軸に乗る、と言えるのは 2 割まで。
7. ★**局所を見る方式ほど強い、とは限らない**。ぼけでは固定しきい値の 3 つが
   σ = 2.1 px まで持つのに、適応しきい値とサブピクセルは 1.8 px で全滅する。
   なまると「局所の差」そのものが消えるから。逆に明るさが持ち上がる条件
   (8 章の対照群)では、落ちるのは固定しきい値の 3 つだけ。
8. 既存 op :func:`fullseye.op` の ``decode_barcode`` は **バーの本数を数える
   だけ**(docstring にもそう書いてある)。無傷で 22 本を正しく数え、
   ぼけ σ = 2.2 px でもまだ 22 本、σ = 2.6 px で 7 本に崩れる。桁は返さない
   ので本 PoC のデコーダの代わりにはならない。**在るものを測ってから**書いた。
9. ★:func:`fullseye.ledger.measure_pos` の**エッジ本数が平滑化 σ に非単調に
   依存する**(9 章 (d))。無傷 24 枚のうち 44 本ちょうどになるのは
   σ = 0.4 / 0.6 で 24 枚、σ = 0.8〜1.2 で 15 枚、σ = 1.5 でまた 24 枚。
   例外は出ず、本数が 31 本まで静かに減る。手法 5 は σ を順に試している。
