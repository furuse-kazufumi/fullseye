...
【所見(すべて下の節で実測。★ は重要、★★ は最重要)】
 ...
 2. ★半径 R の 1 次式 ``err(R) = a + b·R`` に当てはめると、定数項 a が
    焦点距離+架台のドリフト、1 次項 b が主点ドリフトを表す。実測(ΔT=15 K):
    a = +224.5 ppm(片方だけ動かした対照条件の値 +225.3 ppm、差 0.4 %)、
    b = +0.2388 ppm/px(主点のみの傾き 0.2190 + f のみの傾き 0.0197 = 0.2387)。
    **分離は厳密ではなく「1 次で分かれる」** —— 9 % のずれは取り違えではなく
    交絡で、その内訳まで数字で追えている(4 節)。

 3. ★★**対照群を置かないと「効いている」と言えない**。雑音だけ(ΔT=0)の
    床は 1 枚 4.69 µm(117 ppm)、25 枚平均 0.94 µm(23 ppm)。中央のワークで
    ドリフトがこの床を超えるのは **ΔT = 1.6 K から**。それ以下では
    「温度の影響は見えない」が正しい報告(2〜3 節)。

 4. **面積は距離の 2 倍効く**。倍率誤差 ε に対し距離は ε、面積は 2ε+ε²。
    実測の比は 2.000(5 節)。「面積で見れば平均されて安全」ではない。

 5. 対策の比較(6 節、ΔT=15 K・半径 1000 px でゼロ点は +463.9 ppm)。
    毎フレーム再校正 = +0.0 ppm。**画面内の基準物によるスケール引き直しは、
    基準物をワークと同じ半径に置いたときだけ効く** —— 同じ半径で -27.9 ppm、
    中央固定の治具では +238.1 ppm(半分しか取れない)。同じ半径でも 0 に
    ならないのは、歪みの倍率が半径方向と接線方向で違うから。
    温度ログ+式は温度センサ誤差にほぼ比例(1 K で -30.9、3 K で -92.6 ppm)。
...
def section3_zero_point(floors):
    ...
    print("     ドリフトが %d 枚平均の床を超えるのは ΔT = %.1f K から。それ以下では"
          % (N_AVG, cross))
    print("     「温度の影響は見えない」が正しい報告 —— ΔT=1 K の 0.60 µm は")
    print("     床 %.2f µm より小さく、**床を出さずにこれを報告すると雑音を"
          % (1e3 * floors[0.0]))
    print("     読んでいるだけ**になる。1 枚だけで判定するなら %.1f K まで見えない。"
          % (cross * np.sqrt(N_AVG)))
    return dts, meas
