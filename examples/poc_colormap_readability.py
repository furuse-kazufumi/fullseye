...
1. **なめらかな場に、jet は 3 本・hsv は 4 本の偽の境目を立てる**(viridis と
   gray は 0 本、turbo 2 本、cividis 3 本)。色差の利得の山を median の 1.3 倍で
   数えた本数。★同じ場・同じ数値・同じ図の大きさで、**見えるものが変わる**。
2. ★★**どこに立つかは閉形式で当たる**。jet の明度は t = 0.375(シアン)/
   ...
   **最大ずれ 0.0002**(t の刻み 0.00098 のほぼ 1 目盛)。谷は緑 (t=0.5) では
   **ない** —— 色の名前では出てこないので、解いて初めて当たる。
3. ★**「明度が単調 = 安全」ではない**。cividis は L\\* の折返し 0 回
   (``PERCEPTUAL_SAFE`` の触れ込みどおり)なのに、色差の山は **3 本**立つ
   (t = 0.579 / 0.611 / 0.810、最大 / median = 1.77)。制御点 6 個の折れ線
   LUT が原因で、明度だけを見る検査では捕まらない。★★予想では 0 本だった。
4. **崖**。なめらかな傾斜(画素あたり 0.273 %FS)に本物の段差を混ぜて掃引すると、
   段差が**偽の境目より目立つ**のは jet で **0.296 %FS**、viridis で
   **0.050 %FS** から。**jet では本物の段差に 5.9 倍の高さが要る**。しかも
   この崖は**画像を一切見ずに配色の LUT だけから予測でき**、予測 0.296 / 0.050
   に対し実測 0.296 / 0.050(ずれ 0.001 / 0.000)。
   ★1 回目は外した(予測 0.57 / 0.29 に対し実測 0.30 / 0.05)—— 段差の列で
   読める差は Δ **+ 画素 1 個ぶんのランプ**で、それを足し忘れていた。
5. ★★**生の色差マップは場のせいを配色のせいにする**。2 次曲面(外側ほど急)を
   塗ると、gray でさえ生の色差の最大/median が **1.34** になる。段差はゼロ
   なのに。真の勾配で割ると **1.26** に戻り、1-D 掃引の 1.26 と一致する
   (6 マップのずれは最大 0.19)。真値なしで色差マップを眺めてはいけない。
6. ★**写し方(norm)のほうが配色より効く**。4.3 桁ある 1/r² の場では、区別
   できる階調の実効数が linear **1.4** / sqrt 2.7 / percentile 12.6 /
   log 27.4 / rank **76.0**(viridis 固定、L\\* を 1 単位で刻んだ度数分布の
   perplexity)—— **配色を変えずに 54 倍**。
   ★★**素朴な順位相関は使えない**: 生の L\\* と真値の Spearman ρ は
   どの norm でも 1.000 —— すべて単調写像だから当たり前で、**この指標では
   linear と rank の差がゼロに見える**。JND で刻んで初めて 0.522 / 1.000 と
   分かれる。
7. ★**外れ値 1 個の対照群で log が落ちる**。1000 倍の画素を 1 個混ぜると
   実効階調は linear 1.4 → 1.0、sqrt 2.7 → 1.5、**log 27.4 → 16.3(-41 %)**。
   percentile と rank は不変(12.6 → 12.6 / 76.0 → 76.0)。
   「対数にしておけば安全」は嘘 —— 上端が外れ値に引っ張られるから。
8. **範囲外**。広い丘(頂点 9.99 = 範囲内)と細い尖り(14.4 = 範囲外)を
   vmin=0 / vmax=10 で塗ると、``under`` / ``over`` 無しでは振り切れた
   **351 画素**を読み手は端の色から数えるしかなく、**1608 画素**と数える
   (**+358 %**)。番兵色を付けると 351 画素ちょうど(誤差 0)。
9. **二変量**。疎な領域(重み 1/50、雑音 σ が 0.30 → 2.12)の雑音が明るく
   光るので、素の viridis では「最も明るい 277 画素」の的中率が **38.6 %**。
   ``colorize_bivariate`` で重みを明度に載せると **86.6 %**。同じ数値・
   同じ配色で、拾える異常が変わる。
10. **質的**。番号をラスタ順に振った 8 領域では、隣接ラベル対の色差の最小が
    viridis 13.4 に対し ``colorize_categorical`` tab10 17.3 / wong 21.7。
    ★★ただし**崖がある**: 領域を 24 個(tab10 の色数 10 超)にすると
    tab10 も wong も **0.0**(循環して同じ色が隣り合い、JND 未満の対が 3 組)。
    連続マップ viridis (3.5) にも、乱数 RGB の ``colorize_labels``
    (10 種で 3.9〜13.0)にも**負ける**。質的パレットは色数以内でだけ正しい。
