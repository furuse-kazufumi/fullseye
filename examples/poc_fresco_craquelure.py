...
"""絵画のひび割れ網(craquelure)を測る —— 3 指標のうち撮影条件で壊れるのは分岐次数だけ。
...
(斜光で幅が片側に太る量、質感で偽陽性がどう増えるか、セル径が壁の分だけ小さく
出ること)を持ち込んでください。斜光の向きと強さは撮影記録から取れるはずで、
それが無い写真の網統計は比較に使えません。人為的な模造(乾燥ひびを熱で作る等)は
本 PoC では作っていない —— 2 種の真値の**中間**を自分で仕込んで同じ表を作ること。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. **ゼロ点(暗い画素を大津で数える)は、ひび画素率をそのまま返さない**。
   きれいな条件(質感なし・斜光なし)でも ひび画素率 真値 10.18 % に対して
   推定 28.10 %(2.8 倍。ぼけた縁まで数える)。質感(コントラスト 0.08)を入れると
   44.55 %(4.4 倍)—— 絵の具の色斑が「ひび」に数えられる。
2. **リッジ op → ヒステリシス → 面積オープニング → 骨格 → 枝刈り → 分岐点/端点/枝
   の op 列で網は取れる**。xsk_meijering は既定条件(質感 0.08・ぼけ 0.8 px・雑音)で
   中心線の再現率 0.978 / 適合率 0.961(許容 2 px)、斜光でもセル 50 個(真値 52)。
   ★**sk_frangi は分岐点で応答が落ちる(Frangi の設計どおり)ので、斜光でセルが
   16 個に崩れる**。★そして正直に書くと、**暗さそのもの(cv_blackhat)が 3 種の
   リッジ op より全条件で上**(再現率 0.996 / 0.995 / 0.987)—— ひびが常に暗い合成
   では当然で、リッジ op が要るのは暗さが一様でない実写のほう。以後は
   xsk_meijering で測る。
3. ★**3 指標の分離力と壊れ方は別物**。乾燥 vs 経年で、真値の平均セル径は 18.0 vs
   45.2 px(2.5 倍)、直線度(弦/弧)は 0.960 vs 1.000、次数 4 以上の分岐点の割合は
   0.20 vs 0.83。3 つとも 2 種を分ける。ところが経年型で、きれいな条件の
   次数 4 割合 0.87 が **質感で 0.64、斜光(強さ 1.0)で 0.70** と乾燥側へ動く。
   セル径は 41.7 → 40.4 / 41.7 px、直線度は 0.978 → 0.966 / 0.983 で動かない。
   ★予想は「斜光で直線度が壊れる(片側の影で骨格が蛇行する)」だったが、実測で
   壊れたのは**分岐次数** —— 質感は 3 差路の偽分岐を足して割合を薄め、斜光は
   4 差路を 3 差路 2 個に割る。セル径の推定は壁(骨格を 1 px 太らせた 3 px)の分だけ
   一定に小さい(乾燥 3.7 px / 経年 3.5 px 不足)ので引き算で戻る。異方性は真値 0.09
   に対して推定 0.20 と上に偏るが、条件を変えても 0.17〜0.20 で動かない。
4. ★**ひび幅の崖は来なかった(予想は外れた)**。幅 0.15 → 4 px の掃引で、幾何の予想
   (ぼけ後の点コントラスト = 深さ × erf(w / 2√2σ) が質感の 2 倍を割る幅)は 0.60 px
   だったが、実測は 0.5 px でも再現率 0.926、0.15 px でも 0.696 —— 崖ではなく緩い坂。
   線検出は線に沿って積分するので、点のコントラストでは予想できない。先に壊れる
   のは適合率(0.5 px で 0.784)と**幅の推定**: 真値 0.5 px → 2.41 px(4.8 倍)、
   1.0 px → 1.94 px、4.0 px → 3.63 px。ぼけ σ 0.8 px の半値幅 1.9 px が下限を作る。
5. ★**質感でも偽陽性は爆発しなかった(予想は外れた)**。予想は「色斑の最暗部 3σ が
   ひびの深さ 0.55 に届く c ≈ 0.18 で爆発」。実測の 偽陽性長/真値長 は c = 0.16 で
   0.141、0.32 で 0.273、0.64 で 0.382 と直線的に増えるだけ —— ヒステリシスの
   しきい値がリッジ応答の最大値に相対で、面積オープニングが小さい島を落とすから。
   代わりに再現率が 0.981 → 0.871 と削れる。ゼロ点(大津)は c = 0.08 で 44.6 %。
6. **斜光は幅を片側に太らせる**。強さ 0 → 1 で推定幅 2.49 → 2.86 px(真値 2 px、
   +0.37 px)、検出した中心線は光源側へ 0.75 px ずれる(影の側だけが暗いので、
   半値幅のマスクがそちらへ伸びる)。異方性は 0.20 → 0.19 で動かない。
7. **ぼけがセルの小ささの下限を決める**。セル径 8 / 12 / 16 / 24 px × ぼけ σ 0.5〜3 px
   の表で、セル数の再現率 0.9 を保つ最小径は σ=1 で 8 px、σ=2 で 12 px、σ=3 で
   16 px(径 8 px は σ=3 で 0.16)—— **径 < 5σ + 幅** で隣のひびがぼけで融合する、
   という目安(σ=2 → 12 px、σ=3 → 17 px)と一致。
8. **対照群**: 乾燥型は質感でも斜光でも次数 4 割合 0.25 → 0.21〜0.25 で動かない。
   壊れるのは**格子的な網の 4 差路だけ**で、乾燥ひびの統計は撮影条件に強い。

【グラウンドトゥルース】
網は**種点のボロノイ図**を閉形式で描く。種点は格子 + 乱れ(規則性 β: 1 で完全格子、
0 で一様乱数)、経年型は長方格子(縦横比 1.3)で異方性も仕込む。蛇行は座標を低周波
の正弦で歪める(ドメインワープ、逆写像は不動点反復)。ひび = ボロノイ辺(画像座標の
折れ線)までの距離が幅の半分より小さい画素で、被覆率で反エイリアス(幅 0.15 px も
描ける)。その上に 絵の具の色斑(ガウス平滑した乱数、コントラスト c)、ニスの光沢むら
(低周波の乗算)、斜光(溝の斜面の勾配 × 強さ s、光源の反対側の壁が明るく手前が影)、
ガウスぼけ、雑音を足す。真値の統計(セル面積・分岐次数・直線度・異方性)はボロノイの
頂点・辺から幾何で出す(画像処理を通さない)。分岐点は真値・検出とも 3 px で束ね、
次数は半径 4 px の環を横切る本数で数える。

来歴(公開文献のみ): Frangi らの管状構造フィルタ(MICCAI 1998)、Meijering らの
neuriteness フィルタ(Cytometry 2004)、Bucklow "The description of craquelure
patterns" (Studies in Conservation 1997、乾燥ひびと経年ひびの分類軸 = セル径・直線性・
網の規則性)、ボロノイ図(Okabe et al. 2000)。
"""
...
def section_width_sweep() -> dict:
    ...
    figs.save_plot("width_cliff", [("中心線の再現率", ws, rec), ("適合率", ws, prc),
                                   ("推定幅 − 真値幅 [px]", ws, [v - w for v, w in zip(wid, ws)])],
                   xlabel="ひび幅の真値 [px]", ylabel="再現率 / 適合率 / 幅の偏り [px]",
                   title="ひび幅の崖(ぼけ σ %.1f px、質感 %.2f)" % (BLUR_SIG, TEX_C),
                   caption="再現率の崖は 0.5 px より下。幅の推定は細いほど太る(ぼけの半値幅が下限)。")
...
def section_texture_sweep() -> dict:
    print("\n" + "=" * 78)
    print("4) 崖: 質感のコントラスト 0 → 0.64(経年型、幅 2 px)—— 偽陽性はどこで爆発するか")
    print("=" * 78)
...
def main() -> int:
    ...
    print("  * 3 指標(セル径 %.1f vs %.1f px / 直線度 %.3f vs %.3f / 次数 4 割合 %.2f vs %.2f)は"
          "どれも 2 種を分けるが、質感と斜光で動くのは次数 4 割合だけ(%.2f → %.2f / %.2f)。"
          % (t_d["diam"], t_a["diam"], t_d["straight"], t_a["straight"], t_d["deg4"], t_a["deg4"],
             table[("経年", "きれい")]["deg4"], table[("経年", "+質感")]["deg4"],
             table[("経年", "+斜光")]["deg4"]))
