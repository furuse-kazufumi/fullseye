...
(1) 投影数を減らすと体積誤差がどう動くか
    ―― 実測の答えは「**16 本以上ならほとんど動かない**」。16 → 128 本で再構成
    nRMS は 5 倍良くなるのに、体積は 0.05 % しか動かない(面内 128/256 画素の
    3 通りの格子すべてで 0.05 %)。ストリークは物体のまわりに正負が対称に出る
    ので、体積という 1 つの積分量では相殺して消える。**体積は「投影が足りない」
    を教えてくれない**。教えるのは体積ではなく**連結成分の数**。
...
(4) 二値化しきい値を動かすと体積がどれだけ動くか
    ―― しきい値 0.30-0.70 RHO で 6.0 % 動く(3 通りの格子で 3.2-6.0 %)。
    16 本以上の投影数の影響 0.05 % と比べると **しきい値のほうが 100 倍以上効く**。

★内訳を疑った結果(honest disclosure)。最初は「しきい値は投影数より 14 倍効く」
と書きかけた ―― 投影 8-128 本の振れ幅と比べた数字で、この格子では本当にそうなる。
しかし同じ部品を面内 256 画素で測り直すと **1.02 倍**、つまり同程度になる。14 倍は
部品の性質ではなく**格子の粗さの性質**だった。原因は 8 本の行が不安定なことで、
面内 128 画素では体積誤差 -0.03 %(成分 43 個)、256 画素では +3.4 %(成分 175 個)。
3 通りの格子すべてで再現したのは「16 本以上で 0.05 %」と「しきい値で 3.2-6.0 %」の
2 つだけなので、主張はそこに限る。8 本は「体積が正しく出る」例ではなく、
**体積が信用できない**領域である。
...
def main():
    ...
    r16 = next(r for r in rows if r[0] == 16)
    print()
    print("   ★★正直な結論 ―― **体積は「投影が足りない」を教えてくれない**。")
    print(f"     16 本 → 128 本で再構成 nRMS は {r16[1]:.4f} → {best[1]:.4f} "
          f"({r16[1] / best[1]:.1f} 倍の改善)。")
    print(f"     ところが体積は {r16[2]:.1f} → {best[2]:.1f} mm^3、"
          f"わずか {abs(r16[2] - best[2]) / v_true:.2%} しか動かない。")
    print("     理由は打ち消し: ストリークは物体のまわりに**正負が対称に**出るので、"
          "しきい値を")
    print("     越えて増えた画素と、割り込んで減った画素がほぼ相殺する。"
          "体積は 1 つの積分量で、")
    print("     符号の付いた誤差はそこで消える。")
    print(f"     壊れていることを教えるのは体積ではなく**連結成分の数**: "
          f"{worst[0]} 本で {worst[3]} 個、{r16[0]} 本で {r16[3]} 個、"
          f"{best[0]} 本で {best[3]} 個。")
    print(f"     なお 8 本の行({worst[2] / v_true - 1:+.2%})は**この格子に固有**で、"
          "面内 256 画素で")
    print("     測り直すと +3.4%(成分 175 個)になる。8 本は「体積が正しく出る」"
          "例ではなく、")
    print("     体積という指標そのものが信用できない領域である。")
    ...
    stable = [r[2] for r in rows if r[0] >= 16]        # 8 本は下で別扱いにする
    span_views_all = max(r[2] for r in rows) - min(r[2] for r in rows)
    span_views_16 = max(stable) - min(stable)
    print(f"   しきい値 0.30-0.70 の振れ幅  = {span_thr:7.1f} mm^3 "
          f"({span_thr / v_true:.2%} of 真値)")
    print(f"   投影数 16-128 の振れ幅       = {span_views_16:7.1f} mm^3 "
          f"({span_views_16 / v_true:.2%} of 真値)")
    print(f"   投影数 8-128 の振れ幅        = {span_views_all:7.1f} mm^3 "
          f"({span_views_all / v_true:.2%} of 真値) ← 8 本を含めると別の話になる")
    print(f"   ★16 本以上に限れば、しきい値の任意性のほうが "
          f"{span_thr / max(span_views_16, 1e-9):.0f} 倍効く。")
    print()
    print("   ★★内訳を疑った結果(honest disclosure)")
    print("     最初は「しきい値は投影数より 14 倍効く」と書きかけた。"
          "8-128 本の振れ幅と")
    print("     比べた数字で、この格子(面内 128 画素)では確かにそうなる。"
          "だが同じ部品を")
    print("     面内 256 画素で測り直すと **1.02 倍** ―― つまり同じくらい効く"
          "ことになり、")
    print("     14 倍は部品の話ではなく**格子の粗さの話**だった。"
          "原因は 8 本の行が不安定なこと:")
    print(f"     この格子では {rows[0][2] / v_true - 1:+.2%} なのに、"
          "面内 256 画素では +3.4%(成分 175 個)。")
    print("     再現できたのは次の 2 つだけなので、主張はここに限る:")
    print(f"       ・16 本以上では体積は {span_views_16 / v_true:.2%} しか動かない"
          "(3 通りの格子すべてで 0.05%)")
    print(f"       ・しきい値 0.30-0.70 は {span_thr / v_true:.1%} 動く"
          "(3 通りの格子で 3.2-6.0%)")
    print("     8 本は「体積が信用できない」領域であって、"
          "「体積が正しく出る」例ではない。")
