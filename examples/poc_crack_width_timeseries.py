...
1. **幅がよく当たる測り方と、伸びがよく当たる測り方は別**。12 期の平均で見た
   幅の偏りは 積分法 +0.00001 mm / 2 値化 -0.07379 mm(真の幅の -25.0 %)。
   ところが**成長率**は真値 0.0400 mm/年 に対し 積分法 0.0412(+3.1 %)、
   2 値化 0.1012(**+153.0 %**)—— ★**幅を 25 % 過小に言う測り方が、
   伸びは 2.5 倍に過大に言う**。一定の偏りは差分で消えるが、2 値化の偏りは
   幅に依存する(1 画素の階段)ので消えない。
2. ★★**環境だけ動かすと 2 値化は勝手に伸びる**。幅を 0.30 mm に凍結して
   ぼけ・照明・据え直しだけを 12 期ぶん動かす対照群で、見かけの成長率は
   2 値化 +0.0117 mm/年(真の成長率の 29 %)/ 積分法 +0.0005 mm/年。
   要因を 1 つずつ止めて分けると犯人は**ぼけ**(単独 +0.0122)で、
   照明 -0.0001・据え直し -0.0024 はほぼ無害。積分法がぼけに強いのは偶然では
   なく、**畳み込みは輝度欠損の総量を保存する**から(1 節で 1e-16 の桁で検算)。
3. ★★**据え直しの半画素が 2 値化を跳ねさせる。跳ねる量は経路の傾きで決まる**。
   ひび割れが視野に**水平**だと全列の画素位相が揃うので、幅は列ごとではなく
   **画面ごと**に 1 px = 0.15 mm 単位で跳ぶ(期ごとの散らばり 0.0546 mm)。
   傾き 0.35 なら 0.0023 mm(1/24)。★崖の位置は先に計算できる ——
   測る区間 259 列で中心線が 1 px 動けば位相は混ざるので臨界の傾きは
   1/259 = 0.0039、実測で半分を切るのは 0.002。★積分法の散らばりは
   0.0013 → 0.0013 mm で傾きに**無反応**(2 値化の 1/42)。予想は
   「傾けば断面が斜めに切れて悪くなる」だったが、そうはならなかった。
4. ★**「有意な成長」と言うのに何期要るかは閉形式で出る**。傾きの標準偏差の
   予測 σ_w/(σ_t·√N) と 12 サイトの実測の比は、積分法で 0.75〜1.33
   (σ の推定自体の相対誤差が ±21 % なので標本のゆらぎの範囲)。
   ★2 値化だけは 1.26〜1.73 と**一貫して 1 を超える** —— 誤差が白色雑音では
   なく 1 画素の階段に張り付くので、iid の閉形式は下振れする。
   0.040 mm/年 を 2σ で言い切るのに、積分法なら 3 期(0.5 年)、
   2 値化は 12 期(2.8 年)でも足りない。
5. ★★**「同じ壁を撮り返す」ことが誤差を 1/3 にする**。地の模様を毎回別の
   乱数にすると積分法の傾きの σ は 0.0044 mm/年、同じ壁(模様は同じで
   据え直しだけ変わる)だと 0.0015 mm/年。共通成分は差分で消えるので、
   **合成の独立雑音で評価すると実写より悲観的**になる。
6. ★**2 値化が死ぬ幅は、ぼけの σ から先に計算できる**。閾値 0.5 の半値幅規約は
   ピーク欠損 erf(w/(2√2σ)) が 0.5 を切ると何も返さないので、
   臨界幅 = 2√2·erfinv(0.5)·σ = 1.349σ。72 枚を w/σ で並べ直すと境界は
   **値が出た最小 1.221 / 出なかった最大 1.219** と刃物のように鋭く、
   予測 1.349 で線を引いて外れるのは 5/72 枚(6.9 %)。★予測は 11 % 高い ——
   「どれか 1 画素でも 0.5 を超えれば検出」なのでざらつきの最も高い 1 画素ぶん
   (実測 0.0494)しきい値が下がるためで、そこを入れた予測 1.197 は実測と
   -1.9 %。積分法は 0.10 mm(0.67 px)の帯でも成長率 0.0409 mm/年(+2.2 %)。
7. ★★**2 値化の成長率は「初期の幅」だけで 0.0241〜0.1038 mm/年 と 4 倍動く**
   (真値はどれも 0.0400)。1 画素の段差が観測窓の中に来たかどうかで決まる
   ので、**同じ速さで開いているひび割れでも、初期幅が違うだけで報告が変わる**。
   1 番の +153 % もこれ(0.24 mm から始めたから)。積分法の成長率の誤差は
   全幅で +1.7〜+2.2 % に収まる。
...
def section_cliff_width() -> dict:
    ...
    xs_ = stations()
    j0, j1 = int(round(xs_[0])), int(round(xs_[-1])) + 1
    lifts = []
    for k in range(N_EPOCH):
        sc = render(0.19, psf=1.0, dy=float(sch["dy"][k]), tex=tex, noise_seed=2400 + k)
        d = deficit_map(sc["img"])
        lifts.append(float(d[:, j0:j1].max() - sc["cov_blur"][:, j0:j1].max()))
    lift = float(np.mean(lifts))
    wc2 = 2.0 * math.sqrt(2.0) * erfinv(max(0.5 - lift, 1e-3))
    print("     ★予測 1.349 は実測 %.3f より %.0f %% 高い。向きは説明がつく ——"
          "「**どれか 1 画素でも 0.5 を超えれば検出**」なので、"
          % (min(ratio_live), 100 * (1.349 / min(ratio_live) - 1.0)))
    ...
    return {"w0": w0s, "int": gi, "bin": gb, "wc_mm": wc_px * PX_MM, "dead": dead,
            "live": live, "cliff_lo": min(ratio_live), "cliff_hi": max(ratio_dead),
            "wrong": wrong, "npair": len(pairs)}
...
def main() -> None:
    ...
    print("まとめ")
    print("=" * 78)
    print("  * 幅の偏りは 2 値化 %+.4f / 積分法 %+.4f mm、成長率は %.4f / %.4f mm/年"
          "(真値 %.4f)。**一定の偏りは差分で消えるが、2 値化の偏りは幅に依存する"
          "(階段)ので消えない**。"
