...
1. **ゼロ点(C2C)は沈下を見つけられない**。沈下ありの中央値 48.06 mm に対し、
   沈下ゼロで測り返しただけの対照群でも 47.74 mm(差 0.33 mm)。真の最大沈下
   8.0 mm の 6 倍の値が**変化ゼロでも出る**(正体は点間隔 —— 密度 120 pt/m²
   なら平均間隔 91 mm)。符号も無いので「沈んだ」と「盛り上がった」を区別
   できない。M3C2 なら同じデータで平均 -2.43 mm(真値の平均 -2.43 mm)。
2. ★★**平均は「全体が沈んだ」と言い、LoD は「沈んだのは半分」と言う**。
   551 core の平均は -2.43 mm。|d| > LoD を満たすのは 275/551(49.9 %)で、
   その平均は -4.34 mm。真値の陽性(|S| >= 2 mm)に対して TPR 88.7 % /
   FPR 3.6 %(陰性 = |S| <= 0.2 mm で数える)—— **有意判定は沈下の地図を
   よく復元する**が、「平均 2.43 mm 沈んだ」という 1 行は、有意な領域の
   -4.34 mm とも、本当に沈んだ領域の真値 -5.04 mm とも一致しない。
3. ★★**LoD の地図は沈下の地図ではなく、粗さと点密度の地図**。同じ 2〜4 mm の
   沈下でも、アスファルト(粗さ 1.5 mm)では 100 %、砂利の路肩(粗さ 15 mm・
   密度半分)では 30.6 % しか有意にならない。ゾーンごとの LoD 実測は
   3.58 / 0.51 / 0.83 mm。★素朴な閉形式 1.96·σ·sqrt(1/na+1/nb) との比は
   0.77〜0.94 で**一貫して 1 を下回る** —— 局所平面を当てるぶん、円筒内の
   粗さのうち平面で説明できる分(路肩なら 15.0 -> 11.4 mm)が法線に吸われる
   から。σ を平面当てはめの残差にすると比 1.02〜1.03。
4. ★★**法線を鉛直で代用すると横断勾配が LoD に化ける**。勾配 2.06 % の面を
   半径 0.6 m の円筒で切ると鉛直射影は σ = g·r/2 = 6.18 mm 散らばる ——
   この予測を入れたゾーン別 LoD は実測と比 0.88〜1.01。★**損をするのは
   滑らかな面のほう**で、アスファルトは 0.51 -> 1.46 mm(2.9 倍)、
   もともと粗い路肩は 3.58 -> 4.42 mm(1.2 倍)。直感と逆向き。
   沈下量そのものは両方とも当たっている(-2.43 / -2.53 mm)のに、有意な core は
   275 -> 233 個 —— **測れているのに「有意でない」になる**。
5. ★**551 回の検定をすれば数十個は偽陽性になる**。沈下ゼロの対照群で有意と
   出た core は 20/551(3.6 %。名目 5 % = 27.6 ± 5.1 個の 1.5σ 下で、
   ★LoD が保守的なのではない —— 帰無仮説での z の標準偏差は 0.989。core 間隔
   0.8 m に対し円筒半径 0.6 m なので隣と重なり、独立でないぶん揺らぐ)。
   Benjamini-Hochberg(FDR 5 %)で 0 個、8 近傍のうち 3 個以上という塊の規則で
   0 個。★同じ規則を本番に掛けても TPR は 88.7 -> 87.4 / 87.4 % しか下がらない
   —— **偽陽性は全部消えるのに真陽性はほとんど減らない**。
6. ★★**LoD は雑音しか見ていない —— 合わせの残差は素通りする**。沈下ゼロで
   鉛直に -0.70 mm だけずらした対照群では、有意な core が 20 -> 249(45.2 %)に
   跳ね、平均は -0.70 mm。BH 174 / 塊 220 と**桁で減らない**(残差ゼロなら
   20 -> 0 だった)—— 系統誤差は多重比較の問題ではないから。安定域(切羽の
   前方 114 core)の中央値 -0.72 mm を引くと 249 -> 20 core に戻る。
7. ★★**有意なものだけ足すと体積は必ず過小になり、その量は先に計算できる**。
   真の沈下体積 0.8872 m3(閉形式)に対し、全 core を足すと 0.8569 m3
   (-3.4 %、大半は core 格子の粗さ)、有意な core だけだと 0.7643 m3
   (**-13.9 %**)。残る割合の実測 0.892 に対し、★**core ごとの LoD** で
   切った予測は 0.867(差 2.5 %)。LoD を中央値 1 個で代表した予測は 0.960 で、
   欠け量を 3.3 倍**過小に**見積もる —— 粗い場所ほど LoD が大きく、
   そこの沈下がまるごと落ちるから。**「有意でない」は「ゼロ」ではない**。
8. ★崖の位置はそのゾーンの LoD。舗装ゾーンの検出率が 50 % を超えるのは
   最大沈下 1.00 mm から(0.25 mm では 11.3 %)で、そのゾーンの LoD 0.51 mm と
   同じ目盛り。LoD は点密度の平方根で落ちる(実測 / 予測 0.98〜1.01)——
   **4 倍撒いて半分**。
