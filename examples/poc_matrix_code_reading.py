...
def main():
    ...
    for br in (0.0, 0.15, 0.30, 0.40, 0.45, 0.50, 0.60, 0.75, 1.00):
        ...
        if br == 0.50 and figs.enabled():
            ...
            figs.save_grid(
                ...
                caption="誤り %d モジュールのうち構造(位置検出・分離帯・タイミング・"
                        "位置合わせ)は %d。1:1:3:1:1 はまだ読めるので、この段では"
                        "定位が生き残る(落ちるのは sigma/m 0.60)。"
                        % (int((bits ^ bk2).sum()),
                           int((bits ^ bk2)[functional].sum())))
    ...
    figs.save_plot("blur_cliff",
                   ...
                   caption="自力検出の線は sigma/m 0.50 を最後に途切れる(0.60 では"
                           "位置検出パターンが見つからない)。標本化はそこでまだ"
                           " BER 0.07 で読めている。")
