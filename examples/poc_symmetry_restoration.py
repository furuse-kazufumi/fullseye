...
"""対称性で欠けを補う —— 対称面が **1.4 度**ずれた時点で、素朴な穴埋めに負ける。

割れた土器・欠けた彫像・片側だけ残った化石を「左右対称だったはずだ」と仮定して
補う仕事です(文化財の 3-D 復元、古生物の欠損補完)。復元された形は展示にも
計測にも論文にも使われるので、**どこまでが実測でどこからが仮定か**を数字で
言えないと使えません。ここでは左右対称な仮面(高さ場 z=f(x,y) が x について
厳密に偶関数)を作り、完全形と対称面の両方を真値として持ったまま片側を球で
削り、対称復元の効きと壊れ方を測ります。

EXTEND: 実物のスキャンに差し替えるなら :func:`sample_mask` が返す ``pts``
(点群)と ``nrm``(法線)を実測に置き換えます。**``mask_height`` が返す真値は
実物では手に入りません** —— 完全形も真の対称面も無いので、この PoC の縦軸
(復元誤差)は測れなくなります。実物で代わりに測れるのは (1) 残った側どうしの
対称残差、(2) 復元した所と残った所の境目の連続性、(3) 面の推定を複数の方法で
やったときの**ばらつき**の 3 つだけ。だから実物では「復元の正しさ」ではなく
**面の推定のばらつきを崖の幅(1.4 度 / 1.4 mm)と比べる**のが唯一の検算です。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. **ゼロ点を先に測る**。何もしなければ RMS 13.28 mm。密な深度格子に
   ``fill_holes``(調和緩和)をかけた穴埋め補間で 1.60 mm。対称復元は
   **面が真値なら 0.81 mm** で、ゼロ点の 2.0 倍うまい。★ゼロ点にはわざと
   有利な条件(点群でなく画素 0.8 mm の隙間なし深度格子)を渡している。
2. ★★**崖は角度 1.39 度(84 分角)、位置 1.41 mm**。そこを超えると
   対称復元は穴埋め補間に負ける。仮面の幅 180 mm に対し位置の崖は 0.78 %。
   「だいたい左右対称に置いた」程度の面では簡単に超える。
3. ★**幾何で先に予測して当てた**。面を角度 α 倒すと点 p の鏡像は厳密に
   2 sin α・sqrt(d²+e²)(d = 面までの距離、sqrt(d²+e²) = **回転軸**からの
   距離)動く。ただし**面に沿った成分は面が吸う**ので表面誤差になるのは
   法線成分だけ。予測 (iii) 変位の法線成分 + 標本の床の平均相対誤差は
   **4.6 %**、素朴な 2 d sin α は 50.6 %、変位の大きさそのままは 72.7 % 外す。
   位置ずれでも同じで、鏡像は厳密に 2t 動くが表面誤差は |n_面・x| の RMS
   0.498 倍。素朴な 2t は 62.1 % 外し、法線成分は **1.9 %** で当たる。
   残る系統差は α ≥ 3 度で +6.6 %(最近傍距離は多数の候補の**最小値**なので
   対応点の変位より必ず小さい)。
4. ★★**壊れ方は 2 種類あり、片方は復元誤差に出ない**。「穴が埋まらない」
   とは別に「**無い面が生える**」。α = 0.5 度で足した点の 11 % が真の面から
   1 mm 以上浮き、1.5 度で 91 %。復元 RMS はまだ 1.69 mm(ゼロ点並み)なのに、
   形はもう 9 割が作り物。**別々に数えないと見えない**。
5. ★★**欠損は対称面を「ずらす」のではなく「別の軸へ飛ばす」**。
   ``detect_reflection_symmetry`` の PCA 3 候補のスコアは、完全形では
   x 軸が 1.09 で 1 位(y 軸 2.18)なのに、7.9 % を失うと 4.90(y)対 6.74(x)と
   **順位が入れ替わり、対称面が 90 度違う軸に飛ぶ**。境目は失った点が
   3.1 % を超えたあたり(半径 23 → 26 mm)。★しかもそのとき復元 RMS は
   2.85 mm で「何もしない 13.28 mm」よりずっと良く見える —— 足した点の 88 % が
   真の面から浮いているのに、**残差だけでは気づけない**。
6. ★**予想が外れた**。「欠損は重心を 3.86 mm 動かすから面もそれだけずれる」と
   踏んでいた。重心を通す粗い面は確かに 4.27 mm ずれる(復元 RMS の予測
   4.34 mm、実測 4.81 mm)が、**残差を掃引する精緻化は重心を使わない**ので
   0.105 mm まで戻す(粗いずれの 98 % を吸う)。軸さえ正しければ復元 RMS は
   欠損半径 10〜50 mm で 0.81〜1.56 mm とほぼ平ら —— **崖は欠損の大きさでなく
   軸の取り違えにある**。
7. ★**古典手法(鏡映 → ICP → 対応点の中点に平面)は欠損があると悪化した**
   (角度 3.42 度 / RMS 1.83 mm、残差掃引は 0.253 度 / 0.81 mm)。欠損側では
   対応が付かないまま偽の対が中点に混ざり、最小二乗の面がそれに引かれる。
   対称トリミング(鏡像に相手がいない点を落として推定し直す)は素直に効く
   (0.105 → 0.092 mm)。
8. ★★**本当は対称でない形に対称復元をかけると、実在しない対称性を作る**。
   右頬だけの装飾 3 mm と経年のねじれ 2.2 mm を入れる(左右差の RMS
   0.754 mm、最大 3.92 mm)。**面が真値でも**、欠損が無地の側なら装飾が
   3436 mm³ **捏造され**、装飾側なら 3495 mm³ **消される**。欠損部だけを見ると
   真の左右差は RMS 1.69 / 1.70 mm あったのに、復元後は**厳密に 0** ——
   復元した所は定義上ぴったり対称なので、**「対称だから正しい」という検算は
   原理的にできない**。非対称な形は対称面の推定も歪める(0.242 度 / 0.360 mm、
   対称な形では 0.021 度 / 0.027 mm)。

【グラウンドトゥルース】仮面は解析的な高さ場 z = f(x,y)(楕円ドーム + 眼窩・
眉・鼻梁・口・頬・縁の隆起)で、``deco`` / ``warp`` を 0 にすると x について
厳密な偶関数(検算: |z(x,y) - z(-x,y)| の RMS = 0.000e+00 mm)。真の対称面は
x = 0、法線 (1,0,0)。欠損は既知の球で削るので失われた点は既知。点群は面素
sqrt(1+|∇z|²) による棄却抽出で**面積一様**(20000 点、中央値間隔 0.681 mm)。
「面から浮いているか」は点群どうしの最近傍でなく**真の面までの直交距離の
閉形式**で測る(最初に最近傍で測って、面が真値のときですら 68 % が「偽の面」に
数えられた —— 標本間隔の裾を拾っていた)。

来歴(公開文献のみ): Mitra, Guibas & Pauly, *Partial and Approximate Symmetry
Detection for 3D Geometry*, ACM TOG 25(3) 2006 —— 部分対称の検出 /
Thrun & Wegbreit, *Shape from Symmetry*, ICCV 2005 —— 対称面からの形状補完 /
Combès et al., *Automatic Symmetry Plane Estimation of Bilateral Objects in
Point Clouds*, CVPR 2008 —— 鏡映 + ICP による対称面推定 / Gunz et al.,
*Principles for the virtual reconstruction of hominin crania*,
J. Human Evolution 57 (2009) 48 —— 化石頭蓋の鏡映復元とその不確かさ。
"""
