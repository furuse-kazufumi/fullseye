...
def main():
    ...
    print("  → 白パッチ法は無彩を全部外すと最悪になる。灰色世界は逆に**良くなる**")
    print("     (無彩パッチは灰色世界の仮定に何も足さず、有彩の偏りだけが残るため)。")
    print("     **同じ場面の改変が、2 つの手法を逆向きに動かす。**")
    ...
    print("     飽和画素を外す(最右列)だけで大半が戻る —— 実装は 1 行だが、")
    ...
    print(f"  画像ごと渡した後の各チャンネル最大値: "
          ...
          f" —— 全体の 1 個で割るので比が残る")
    print("  → 同じ op の、引数の渡し方だけが違う 2 通り。片方は使えて片方は壊れる。")
    print("     2-D op はほぼ全部グレースケール前提なので、**ch で回すほうが自然な")
    print("     書き方**であり、例外も警告も出ない。")
    ...
    assert bias_rows[0.6]["灰色世界 (p=1)"] > bias_rows[0.6]["何もしない"], \
        "有彩色 60 % でも灰色世界がゼロ点に負けない —— 崖 (a) が消えた"
    assert bias_rows[0.9]["灰色世界 (p=1)"] > 4.0 * bias_rows[0.0]["灰色世界 (p=1)"], \
        "偏りで灰色世界が悪化していない"
    assert bias_rows[0.9]["白パッチ (p=inf)"] < 2.0 * bias_rows[0.0]["白パッチ (p=inf)"], \
        "白パッチ法が偏りで壊れた —— 崖の軸が直交しているという所見が崩れた"

    # 5. 崖 (b): 白を外すと白パッチ法が崩れ、灰色世界は良くなる(逆向きに動く)
    a, b = "素のチャート(白あり)", "無彩を全部除く"
    assert wp_rows[b]["白パッチ (p=inf)"] > 3.0 * wp_rows[a]["白パッチ (p=inf)"], \
        "白を外しても白パッチ法が崩れない"
    assert wp_rows[b]["灰色世界 (p=1)"] < wp_rows[a]["灰色世界 (p=1)"], \
        "白を外して灰色世界が良くならない —— 逆向きに動くという所見が崩れた"

    # 6. 崖 (c): 飽和は例外を出さず、白パッチ法だけを超線形に壊す。マスクで戻る
    assert sat_rows[1.0][0] == 0.0, "露出 1.0 で既に飽和している(実験設計が壊れた)"
    assert sat_rows[5.0][0] > 0.05, "露出 5.0 でも飽和画素が出ない"
    sat_wp = [sat_rows[k][1]["白パッチ (p=inf)"] for k in (1.0, 1.2, 1.5, 2.0, 3.0, 5.0)]
    assert all(y >= x - 1e-9 for x, y in zip(sat_wp, sat_wp[1:])), \
        "飽和で白パッチ法が単調に悪化していない"
    assert sat_wp[-1] > 5.0 * sat_wp[0], "飽和で白パッチ法が壊れていない"
    assert sat_rows[5.0][1]["灰色世界 (p=1)"] < 2.0 * sat_rows[1.0][1]["灰色世界 (p=1)"], \
        "飽和で灰色世界まで壊れた —— 崖の軸が直交しているという所見が崩れた"
    assert sat_rows[5.0][2] < 0.5 * sat_wp[-1], "飽和マスクで白パッチ法が戻らない"
