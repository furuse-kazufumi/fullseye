...
2. ★**「効く手法」は無い。あるのは効く条件だけ** —— 同じ白パッチ法(max-RGB)が、
   白の在るチャートでは 11 光源の中央値 1.06 度で全手法中最良なのに、
   **24 枚のうち一番明るい 1 枚を外すだけで 8.45 度**(8 倍)、露出を 3 倍にして
   43 % の画素を飽和させると 13.61 度 = 「何もしない」と厳密に一致するところまで
   退化する。灰色世界は飽和にも雑音にもほとんど反応しない代わりに、有彩色が
   画面の 2 割を超えた時点で「何もしない」に負け、6 割で 29.79 度になる。
   **壊れる軸が直交している。**
3. ★**基準光源では全手法がゼロ点に負ける** —— 光源が既に等エネルギー白のとき
   「何もしない」の角度誤差は定義から 0.01 度。そこで灰色世界は 6.75 度、
   白パッチ法は 1.45 度**外す**。色かぶりが無い場面で AWB を回すのは純損失。
4. ★**真の光源を知っていても対角補正には床がある** —— von Kries(対角)で
   完全に正しい ``e`` を割っても、2500 K では ΔE00 が平均 5.07 / 最大 12.96 残る。
   光源推定をいくら詰めてもここから下がらない。角度誤差 0 度は「色が合う」では
   ない。しかも角度 1 度あたりの色差は一定でない —— 0.85 度付近では 1 度あたり
   ΔE00 0.24、4 度付近では 0.80、29 度では 0.44 と、**低誤差側でも高誤差側でも
   買えるものが減る**(11-(b) 節)。
5. **最適な p は場面で動く** —— Shades-of-Gray の最適 p は、素のチャートで
   ∞(= 白パッチ法)、白を外すと 1(= 灰色世界)、有彩色 60 % かつ白なしで 8、
   雑音下で 16、飽和下で 1。1 つの p を既定値として配る根拠は出てこない。

★ この PoC が出した道具の穴(op 本体は直していない。詳細は末尾の節を印字):

  (a) **統計ベースの光源推定 op が 1 つも無い**。台帳(``fullseye.ledger``、
      実行時 923 件)と進化 op(``fullseye.op``、885 件)の全名を "gray world /
      white patch / shades of gray / gray edge / white balance / AWB /
      von Kries / CAT / color constancy" で探して 0 件(末尾の自己検査で機械的に
      確かめている)。あるのは物理ベースの ``illuminant_from_dichromatic_planes``
      1 本だけで、これは**材質ラベルと鏡面ハイライトの両方**を要求する
      (10 節のとおり、無ければ正しく拒否する)。マット面のチャートしか無い
  ...
  (c) ★**``fs.op.sobel_amp`` にはカラー画像の正しい渡し方が無い**(2 通りとも
      壊れる)。① 画像ごと ``(H, W, 3)`` を渡すと、実装が ``ndimage.sobel`` を
      そのまま呼ぶため **色軸が 3 本目の空間軸として扱われ、``[1,2,1]/4`` の平滑が
      RGB を跨いで掛かる**。R にだけ縦エッジを置き G と B を全画素 0 にした検査
      画像で、**出力の G に 0.3333 が出る**(入力の G は恒等的に 0)。
      ② ``[fs.op.sobel_amp(img[..., k]) for k in range(3)]`` と書くと今度は各
      チャンネルが自分の最大(``ops._norm``)で割られ、**RGB の比が消える**。
      灰色エッジ法の角度誤差は、自前 Sobel 1.03 度 → 画像ごと 4.17 度 →
      チャンネルごと 27.86 度(ゼロ点 29.14 度にほぼ張り付く)。どちらも例外も
      警告も出ず、それらしいエッジ画像が返る。2-D op はほぼ全部グレースケール
      前提なので、②のほうが自然な書き方に見えるが、そちらのほうが酷い。
  ...
  (e) **色域外が黙って負値で出る**。``spectrum_to_srgb`` は負をクリップしない
      ...
      必要があり、本 PoC では 792 個の patch-channel のうち 19 個が該当した。
...
def narrowband(centers, widths, weights):
    ...
    光源を必ず 1 つ混ぜる —— 対角(von Kries)モデルの床(11 節)はここで一番
    高くなる。
    """
    s = np.zeros_like(NM)
...
def main():
    ...
    print("     基準光源では 0.01 度(丸めの床)。**そこでは、どんな AWB も改善できない。**")
    ...
    print("     白パッチ法には崖が無い —— 一度も悪化せず、f>=0.75 で 0.00 度になる。")
    print("     赤が広がるほど生き残る領域が左上の白パッチに絞られ、3 チャンネルとも")
    print("     そこが最大になるので、f=0 のときに残っていた 1.06 度(周辺光量落ちで")
    print("     チャンネルごとに別のパッチが最大を取っていたぶん)まで消える。")
    print("     **崖の位置も、崖の有無も、手法ごとに別物。**")
    print("     最右列(左から塗る)は f=0.2 で既に崩れる。塗り潰しが白パッチを")
    print("     隠すからで、これは「色の偏り」ではなく「白の消失」の効果。")
    print("     **2 つの軸を 1 つの実験で動かすと、原因を取り違える。**")
    _fr = np.array(sorted(bias_rows))
    figs.save_plot("bias_cliff",
                   [(c, _fr, np.array([bias_rows[f][c] for f in _fr]))
                    for c in ("何もしない", "灰色世界 (p=1)", "白パッチ (p=inf)",
                              "灰色エッジ p=1")],
                   xlabel="1 枚の有彩色が画面を占める割合", ylabel="角度誤差 [度]",
                   title="崖 (a) 有彩色の偏り —— 崖の位置も有無も手法ごとに別物",
                   caption="灰色世界はゼロ点(何もしない)の線を 0.1〜0.2 の間で"
                           "上抜けする = そこから先は回すだけ損。白パッチ法には崖が無い。")
    ...
    _ex = np.array(sorted(sat_rows))
    figs.save_plot("saturation",
                   [(c, _ex, np.array([sat_rows[k][1][c] for k in _ex]))
                    for c in ("何もしない", "灰色世界 (p=1)", "白パッチ (p=inf)")]
                   + [("白パッチ + 飽和マスク", _ex,
                       np.array([sat_rows[k][2] for k in _ex]))],
                   xlabel="露出倍率", ylabel="角度誤差 [度]",
                   title="崖 (c) 飽和 —— 白パッチ法だけがゼロ点まで退化する",
                   caption="露出 3 以上で白パッチ法の線が「何もしない」に重なる"
                           "(max が (1,1,1) に張り付く)。灰色世界は動かない。")
    ...
    print("     いくら詰めてもここが床になる**。角度誤差 0 度 = 色が合う、ではない。")
    print("     床の高さを主に決めているのは **基準からの隔たり**(2 節のゼロ点角度)。")
    print("     ただし分光の形も効く: 色かぶりの強さが近い 狭帯域 3 波長(11.30 度)と")
    print("     黒体 8000 K(13.61 度)を比べると、**かぶりが弱いほうの狭帯域が")
    print(f"     負ける**(ΔE00 平均 {floor_rows['狭帯域 3 波長'][0]:.2f} 対 "
    ...
    slope = {n: (bridge[n][1] - floor_b) / bridge[n][0]
             for n in ("灰色エッジ p=4", "SoG p=16", "灰色世界 (p=1)", "何もしない")}
    print(f"  → 床(真値で割った残差)が ΔE00 平均 {floor_b:.2f}。**最右列(床を引いた分)")
    print("     が推定誤差の実費**で、そこが 0.00 でない限り光源推定の話は終わらない。")
    print("     角度 1 度あたりの実費 [ΔE00/度]: "
          + " / ".join(f"{n} {v:.2f}" for n, v in slope.items()))
    print("     **比例しない。** 0.85 度では 1 度あたり "
          f"{slope['灰色エッジ p=4']:.2f}、4 度では {slope['灰色世界 (p=1)']:.2f} と 3 倍")
    print(f"     になり、29 度まで行くと {slope['何もしない']:.2f} に戻る(飽和する)。")
    print("     角度誤差を「0.5 度改善した」と言うとき、それが色として何を買ったかは")
    print("     **どこから 0.5 度動いたか**で決まる。角度誤差だけで語れない。")
    figs.save_table("angle_to_de",
                    ["手法", "角度誤差 度", "ΔE00 平均", "ΔE00 最大", "床を引いた分"],
                    [[k, "%.2f" % v[0], "%.2f" % v[1], "%.2f" % v[2],
                      "%.2f" % (v[1] - floor_b)]
                     for k, v in bridge.items()],
                    title="角度誤差 1 度は ΔE00 いくつに化けるか(黒体 3000 K)",
                    caption="最右列が推定誤差の実費。1 行目の床(真値で割った残差)は"
                            "誰にも引けない。角度と ΔE00 は比例しない。",
                    col_w=130)
    ...
    print(f"  → 0.74 Mpixel で {min(speed.values()):.1f}〜{max(speed.values()):.1f} ms。"
          "**計算コストは手法選択の理由にならない。**")
    print(f"     律速は微分(灰色エッジ)で、一番安い灰色世界の "
          f"{speed['灰色エッジ p=1'] / speed['灰色世界']:.0f} 倍。それでも 1 フレーム分。")

    # ------------------------------------------------------------------ #
    print("\n所見(想定と違ったこと):")
    print("   (1) 「白パッチ法は素朴だから弱い」と思っていたが、**チャートでは全手法中")
    print("       最良**だった(素のチャートで 11 光源の中央値 1.06 度、88 枚の")
    print(f"       アンサンブルでも {ens_stats['白パッチ (p=inf)'][1]:.2f} 度)。")
    print("       理由は手法の優秀さではなく、**チャートに白が写っているから**。")
    print("       24 枚のうち")
    print("       一番明るい 1 枚を外すだけで 8.45 度に落ちる(5 節)。")
    print("       ベンチマークの題材が手法の順位を決めていた。")
    print("   (2) 「灰色世界は頑健」も半分しか正しくない。飽和にも雑音にもほとんど")
    print("       反応しない一方、有彩色が画面の 2 割を占めた時点で「何もしない」に")
    print("       **負ける**(4 節)。頑健さは軸ごとに測らないと意味を持たない。")
    print("   (3) 基準光源(色かぶり無し)の行を入れたのは形式的な理由だったが、")
    print("       ここが一番厳しい行になった。ゼロ点が 0.01 度なので、**どの手法も")
    print("       必ず負ける**。「AWB を常時 ON」は、色かぶりが無い場面では")
    print("       純粋な劣化になる。")
    print("   (4) 角度誤差を 0 にしても色は合わない(11 節)。対角モデルの床は")
    print("       2500 K で ΔE00 平均 5.07 / 最大 12.96。しかも角度 1 度あたりの")
    print("       色差が一定でなく、床の近くでは 1 度詰めても 0.24 しか買えない")
    print("       (11-(b) 節)。**推定の精度だけで語れるのはここまで**、という")
    print("       上限が実験の側から出た。")
    print("   (5) 2 光源では「外したほうの誤差」が開きの半分を下回れない(8 節)。")
    print("       これは手法の性能ではなく **1 本のベクトルしか返さない API の形**")
    print("       が決めている限界で、どんなに良い推定器でも越えられない。")
    print("   (6) 一番外したのは道具の見立て。灰色エッジ用に fs.op.sobel_amp を")
    print("       使うつもりで書き始めたが、**カラー画像の正しい渡し方が無かった**")
    print("       (12 節)。しかも「自然に見える書き方」(ch ごとに 3 回)のほうが")
    print("       酷い。op が在ることと、その op が使えることは別。")

    print("\n所見(fullseye の穴 —— この PoC が出した道具の穴):")
    print(f"   (a) **統計ベースの光源推定 op がゼロ。** 台帳 {len(fs.ledger)} + "
          f"進化 op {len(fs.op)} を")
    ...
    assert bias_rows[0.1]["灰色世界 (p=1)"] < bias_rows[0.1]["何もしない"], \
        "f=0.1 で既に灰色世界が負けた —— 崖の位置が動いた"
    assert bias_rows[0.2]["灰色世界 (p=1)"] > bias_rows[0.2]["何もしない"], \
        "f=0.2 でも灰色世界がゼロ点に負けない —— 崖 (a) が消えた"
    ...
    assert floor_rows["基準(等エネルギー)"][1] < 0.05, \
        "基準光源で床が 0 でない(恒等変換のはず)"
    # 色かぶりが弱いほうの狭帯域光源が、強いほうの黒体より床が高い
    assert per_ill["狭帯域 3 波長"]["何もしない"] < per_ill["黒体 8000 K"]["何もしない"], \
        "狭帯域のほうが色かぶりが強い —— 比較の前提が崩れた"
