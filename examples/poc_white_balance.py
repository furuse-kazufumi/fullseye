...
def main():
    ...
    print(f"       最良**だった(素のチャートで 11 光源の中央値 1.06 度、88 枚の")
    print(f"       アンサンブルでも {ens_stats['白パッチ (p=inf)'][1]:.2f} 度)。理由は")
    print("       手法の優秀さではなく、**チャートに白が写っているから**。24 枚のうち")
    print("       一番明るい 1 枚を外すだけで 8.45 度に落ちる(5 節)。")
    print("       ベンチマークの題材が手法の順位を決めていた。")
    print("   (2) 「灰色世界は頑健」も半分しか正しくない。飽和にも雑音にもほとんど")
    print("       反応しない一方、有彩色が画面の 2 割を超えると「何もしない」に")
