...
"""poc_star_astrometry — 星の位置は何分の 1 画素まで測れて、どこで崖に落ちるか。

    py -3.11 examples/poc_star_astrometry.py

【この PoC が答える問い】
``examples/poc_astro_photometry.py`` は測**光**(明るさ)を扱った。こちらは
測**位**(アストロメトリ)—— 星が**どこ**にあるか、そしてそれを天球座標に
直すプレートソルブが、どこで壊れるか。問いは 4 つ。

    問 1  副画素の位置精度に理論限界はあるか。手法はそれを上回れるか。
    問 2  どこで崖に落ちるか(標本化不足 / 飽和 / 二重星 / 宇宙線)。
    問 3  プレート解は星が何個あれば**一意に**決まるか。間違った対応が
          「もっともらしく」通ってしまう確率はどれだけか。
    問 4  天球座標に直した誤差を、**1 つの数字にまとめてよいか**。

【なぜ数字が信じられるのか】
真値の経路を全部こちらが握っている —— 既知の天球座標 → 既知の接平面投影
(gnomonic / TAN)→ 既知のプレート定数(1.20 秒角/px、回転 17.0 度、原点
(127.3, 128.7))→ 既知の PSF(``erf`` による画素の厳密積分)。段 0 で
投影とプレートの往復が **5.1e-11 / 8.0e-14 秒角**(倍精度の丸めの桁)で
閉じることを確かめてから使う。理論限界も経験式ではなく、**描くのと同じ画素
積分プロファイル**から Fisher 情報 ``I = Σ(∂μ/∂x)^2 / (μ + read^2)`` を数値で
組んで出している。

    EXTEND: 実データ(自分で撮った星野 + 公開星表)に差し替えるには
    ``render()`` を捨てて実画像を読み、``build_field`` の返す ``cat`` を
    ``ra`` / ``dec``(星表の値)と ``row`` / ``col``(**採点用の真値が無いなら
    省く**)だけの dict に置き換える。段 0〜5 は真値を自分で作ることが前提
    なので実データでは動かない —— 実データで意味を持つのは段 6b(対応が
    未知のときの偽解)と段 7(集団別の残差)で、段 7 の「真値との差」は
    「プレート解の残差」に読み替える。``PLATE_ARCSEC_PX`` / ``PLATE_ROT_DEG``
    / ``CRPIX_*`` は**推定の初期値ではなく真値**として使っているので、
    実データでは段 6a が返す推定値へ差し替えること。

【実測(seed 固定。所要 9.9 s)】
 段 1  S/N 掃引(孤立星 64 個 x 6 実現 x 2 軸 = 768 標本 / 明るさ)。
       flux 1e5(S/N 298)での RMS / 理論下限:

           重心(ゼロ点)  5.56x    背景引き重心      1.03x
           ガウシアン当てはめ 1.24x    PSF 相関   1.21x

       ★ **理論限界を上回った手法は 1 つも無い**。ただし暗い端では
       ゼロ点が**下回って見える** —— flux 300 で散らばり 0.2790 px は
       理論下限 0.3471 px より小さい。正体は「感度」列(真のずれに対する
       応答の傾き)の **0.038** で、11x11 の箱に入る空 9680 e- が星 300 e- を
       圧倒し、返っているのは初期値の四捨五入そのもの。誤差は端数の一様分布
       で、その標準偏差 1/√12 = 0.2887 と一致する(実測 0.2790)。
       **散らばりだけを見ると「動かない推定器」が勝つ。**
       同じ縮みは背景引き重心にも弱く効く(S/N 2.6 で感度 0.591 = 真のずれの
       41 % を初期値へ引き戻す)。当てはめ系は 1.03/1.02 で縮まない。
 段 2  画素位相の系統誤差(雑音を切った期待値画像、山谷差 px):

           手法              FWHM 1.0  1.4     2.0      2.5     3.2     4.0
           重心(ゼロ点)      0.0859  0.0828  0.0827  0.0828  0.0838  0.0962
           背景引き重心      0.0181  0.0006  0.0000  0.0000  0.0011  0.0145
           ガウシアン当てはめ 0.0209  0.0045  0.0002  0.0000  0.0000  0.0000
           PSF 相関          0.0383  0.0274  0.0018  0.0000  0.0000  0.0000

       素の重心の値が FWHM に依らないのは標本化ではなく**空の希釈**:
       箱の空 9680 e- / 星 1e5 e- の比 0.0883 が端数をそのまま箱の中心へ引く
       (予測 0.0883、実測 0.0838)。背景引き重心は **U 字**で、左端は標本化
       不足、右端は箱の切り落とし —— 箱を 11→21 に広げると FWHM 4.0 の
       0.01447 が 3.6e-08 まで落ち、FWHM 1.0 の 0.0181 は 1.00 倍で動かない。
       ☆ **予想が外れた点**: 「PSF 相関は既知 PSF を使うから標本化不足に強い」
       と思っていたが**逆で、FWHM 1.0〜1.4 では 4 手法中で最悪**
       (1.4 で 0.0274 px = 背景引き重心の 46 倍)。「平滑後もガウシアンだから
       対数は厳密に放物線」が成り立つのは連続の話で、画素は箱で積分されて
       いるうえ ``gaussian_filter`` の核も離散。崖は **FWHM 2 px**(Nyquist)。
 段 2b 段 1 で当てはめ系だけ下限の 1.2 倍に張り付いた理由を追った。位相系統
       誤差は FWHM 3.2 で 0.00000 px なので**位相のせいではない**。
       ``psf_fit`` の残差が ``model - vals``(= 全画素の分散が等しいという仮定)
       だから。重みを ``1/sqrt(値 + read^2)`` にするだけで:

           flux 3000    1.10x → 1.06x        flux 30000  1.21x → 1.02x
           flux 100000  1.27x → **1.00x**

 段 3  飽和(満杯 65000 e-、雑音を切って飽和だけを見る)。背景引き重心の
       山谷差は 0.0011(未飽和)→ 0.0058(2x)→ 0.0482(8x)→ 0.0476 px(16x)。
       対策を 2 つ測った(山谷差 px):

           何もしない                  2x 0.00579  8x 0.04816  16x 0.04759
           潰れた画素を捨てて重心         0.38322     0.43886     0.49094
           潰れた画素を捨てて当てはめ     0.00007     0.00020     0.00039

       ☆ **予想が外れた点**: 「飽和画素は捨てればよい」は**捨て方による**。
       重心で捨てると 10 倍**悪化**する —— 捨てた穴の形が画素格子に量子化され、
       位相が 0.5 を跨ぐたびに穴が 1 画素ぶん非対称になり、残った翼の重心が
       その非対称をそのまま拾う(0.491 px = 半画素の桁)。同じマスクで
       **当てはめる**と 0.00039 px(121 倍改善)—— モデルが対称だから、
       欠けた画素があっても残りから中心が決まる。
 段 4  二重星(FWHM 3.2、σ 1.359、位置角 30 度、雑音あり x 8)。
       2 個に割れる境界は 等光度 **1.25 FWHM** / 不等光度 4:1 **2.00 FWHM**。
       理論の二峰条件は 2σ = 0.85 FWHM で、実測とは 1.5 倍ずれる ——
       「谷が在る」と「谷が雑音より深い」が別だから。落差 / 雑音は
       0.85 FWHM で **0.0**、1.00 で 2.0、1.25 で 13.7、1.50 で 26.5。
       1 個と誤認したときに返るのは主星ではなく**光心**: 分離 0.75 FWHM
       等光度で 主星まで 1.196 px / 光心まで **0.0099 px**。寄る量は
       分離 x f2/(f1+f2) で決まる(4:1・分離 1 FWHM なら 0.64 px = 0.77 秒角)。
 段 5  宇宙線(1 画素スパイク)を星と誤認する率。しきい値 5σ = 51 e- を
       跨ぐ 20〜60 e- で 23.7 %、60〜150 e- で 89.7 %、それ以上は
       94.8〜97.9 % で飽和 —— **弱いから安全ではない**。鋭さ(中心画素 /
       3x3 の和)で切ると 95 → 3 件(97 % 除去)で、本物の星は 1 個も
       落ちない(点源 0.155 vs スパイク 1.0 で分布が重ならない)。
       残った 3 件の内訳を数えたら **2 件は宇宙線が 2 発 1.1〜1.8 px 隣に
       落ちた組**で、``star_detect`` が返す 7x7 窓の重心が 2 発の中間に来て
       中心画素が空になる。残り 1 件は星から 2.8 px。原因は同じ ——
       **鋭さを測る場所が、峰ではなく検出器の返した重心**。
 段 6a プレート定数の誤差(孤立星 24 個から k 個 x 200 回、使わなかった星での
       予測誤差の 95 % 点): k=2 で 0.5551 → k=4 で 0.2920 → k=10 で 0.1574
       → k=20 で 0.1365 秒角。全 24 個でも回転誤差は 72 秒角あるが、腕の長さ
       (星の重心からの RMS 距離)89.6 px を掛けると 0.0315 px —— 段 1 の
       1 星の測位精度と同じ桁。**角度の誤差は腕を掛けて初めて意味を持つ**。
 段 6b 対応が未知のとき。相似変換は 4 自由度なので **k=2 ではでたらめな対応
       でも残差が厳密に 0**(100 % 採択)—— 「残差が小さいから正しい」に
       情報が 1 ビットも無い。**一意に決まるのは 3 個から**で、k=3 の
       4000 回では 0.5 px 未満が 0 件(最小 1.05 px)。
       ★ ただし **1 回の確率と総当たりの期待値は別**。k=3 の対応は 12144 通り
       あり、40000 回で測ると p(< 0.5 px) = 7.5e-05 / p(< 1.0 px) = 4.7e-04 ——
       期待偽解数は **0.91 件 / 5.77 件**。しきい値を 0.5 から 1.0 px に緩める
       だけで総当たりでは偽解が出る側に回る。落とし方は 1 つ、**少数で当てて
       多数で検証する**: 3 個で作った偽解 9 件を全 24 個へ投影すると
       1 px 以内に当たるのは最大 3 個(正しい解は 24 個)で、1 件も残らない。
 段 7  天球座標に直した誤差[秒角]。**1 つの数字にまとめない**:

           集団                    N   偏り    散らばり   中央    95 %    最悪
           一様・孤立(1 個抜き)  24  0.0009  0.1812  0.0559  0.4085  0.6614
             うち暗い 1/3           8  0.0814  0.2155  0.1126  0.4397  0.5889
             うち明るい 1/3         8  0.0254  0.0415  0.0426  0.0734  0.0774
             うち視野中心 1/3       8  0.0301  0.0648  0.0426  0.1333  0.1512
             うち視野外側 1/3       8  0.0540  0.2147  0.0853  0.4266  0.5889
           密集星団                 4  0.4068  1.4776  1.3434  2.1615  2.2935
           二重星                   4  0.3465  0.5569  0.5871  0.9495  0.9812
           飽和星                   3  0.0326  0.0373  0.0532  0.0532  0.0532
           全部まぜて 1 つの数字に 35  0.0489  0.5776  0.0575  1.3154  2.2935

       混ぜた中央値 0.0575 秒角は、孤立星 0.0507 とも二重星 0.5871 とも違う
       「どこでもない値」。最悪は中央値の 40 倍。二重星は**偏り**だけで
       孤立星の中央値の 6.2 倍あり、孤立星は散らばりが偏りの 195 倍 ——
       **散らばりは枚数で減るが偏りは減らない**ので、足して 1 つにすると
       「もっと撮れば良くなる」を誤る。
       飽和星の 0.0532 秒角 = 0.044 px は段 3 の 16x での山谷差 0.0476 px と
       整合する(別経路の測定が一致した)。

【この PoC で自分が間違えて、直した点(消さずに残す)】
1. **「散らばりが小さい = 良い推定器」で最初の表を書いた。** 素の重心が
   flux 300 で理論下限を下回ったので「限界を破った」と書きかけた。回帰の
   傾き(感度)を足したら 0.038 —— 星をまったく追っていなかった。
   ★ **不偏でない推定器に Cramer-Rao は効かない**。散らばりの表には必ず
   「真値に応答しているか」の列を足すこと。
2. **段 1 の下限比 1.2 倍を「画素位相のせい」と書いて、段 2 に否定された。**
   FWHM 3.2 の位相系統誤差は 0.00000 px だった。実際の原因は ``psf_fit`` が
   重みなし最小二乗であること(段 2b で 1.27x → 1.00x を実測)。
   ★ 原因は**次の段で反証されうる形**で書くと、自分の間違いが自分で潰れる。
3. **飽和の対策として「潰れた画素を捨てる」を先に書き、10 倍悪化した。**
   箱を 11→31 に広げても直らない(0.383 → 0.383)ので箱のせいではなく、
   マスクの形が量子化されることが原因だと分かった。当てはめに替えて解決。
4. **「PSF 相関は標本化不足に強い」も外れた**(段 2)。連続の性質を離散の
   道具で実装した時点で、その性質は保証されなくなる。
5. **偽解の確率を「1 回あたり」で 0 % と書きかけた。** 4000 回で 0 件は
   「起きない」ではなく「測れていない」。40000 回に増やし、さらに総当たりの
   通り数 12144 を掛けて初めて **期待 5.77 件**という使える数字になった。

【fullseye 側に見つかった穴(この PoC の副産物)】
* **指定座標に星を置く公開 op が無い。** ``synth_starfield`` は座標を乱数で
  決めるので、既知の天球座標を持つ星野が作れない。厳密な画素積分の描画は
  ``astrostack._gaussian_star_exact`` にあるが private。この PoC は
  ``render()`` を自前で書いた(20 行)。
* **接平面投影(gnomonic / TAN)も、プレート定数も、fullseye に無い。**
  ``grep`` しても "gnomonic" は 1 件も無い。測位を名乗るなら
  ``sky_to_standard`` / ``standard_to_sky`` / ``plate_forward`` に相当する
  op が要る(この PoC は 4 つとも自前)。
* **``psf_fit`` が重み付けを持たない。** 残差は ``model - vals`` 固定で、
  Poisson の分散を無視している。段 2b の実測で理論下限の 1.27x → 1.00x
  の差になり、**利用者側では取り戻せない**。
* **``psf_fit`` / ``star_detect`` に画素マスクが無い。** 飽和・不良画素を
  除いて当てはめる手段が無く、飽和した星は黙って偏った値を返す(段 3)。
* **``star_detect`` が座標しか返さない。** 鋭さも真円度もピーク画素も
  返らないので、宇宙線の切り分け(段 5)は利用者が書くしかない。しかも
  返るのが窓の重心なので、**その位置で鋭さを測ると 3 件取り逃す**。
* **``fit_transform.vector_to_similarity`` の docstring が 1 行**で、
  入力が ``(row, col)`` か ``(x, y)`` か、戻りが 3x3 か、``src`` と ``dst``
  のどちらへ写すのかが書かれていない。実測で確定した規約は
  ``dst ≈ mat @ [src_0, src_1, 1]``、``mat`` は 3x3、``mat[:2, 2]`` が並進
  (残差 8.5e-14 で確認)。1 行足せば済む。
"""
...
def main():
    ...
    figs.save_table(
        "sky_error",
        ["集団", "N", "偏り", "散らばり", "中央", "95 %", "最悪"],
        [[r[0].strip(), "%d" % r[1]] + ["%.4f" % v for v in r[2:]]
         for r in rows7 if r]
        + [["一様・孤立(1 個抜き)", "%d" % n_clean, "%.4f" % lb, "%.4f" % ls,
            "%.4f" % np.median(lr), "%.4f" % np.percentile(lr, 95),
            "%.4f" % lr.max()]],
        title="天球座標に直した誤差 [秒角] —— 1 つの数字にまとめない",
        caption="混ぜた中央値は孤立星とも二重星とも違う「どこでもない値」。"
                "偏りは枚数で減らないので、散らばりと足してはいけない。",
        col_w=130)
    ...
    print(f"   位置で割ると: 中心 1/3 {d_uni['  うち視野中心 1/3'][4]:.4f} / "
          ...
          f"★ **これを「視野外側は悪い」と読んではいけない** —— 1 群 8 個の"
          f"中央値で、明るさの中央値は 中心 {np.median(fl[uni & (rad <= rq1)]):.0f} / "
          f"中間 {np.median(fl[uni & (rad > rq1) & (rad <= rq2)]):.0f} / "
          f"外側 {np.median(fl[uni & (rad > rq2)]):.0f} e- とほぼ同じなので"
          f"明るさでは説明できず、投影の非線形でもない(下で切り分ける)。"
          f"**8 個の中央値の揺らぎと区別できない**というのが正直な結論")
    ...
    print(f"   偏りと散らばりを分ける意味: 二重星の集団は 偏り "
          ...
          f"{d_uni['二重星'][2] / np.median(lr):.1f} 倍ある。"
          f"孤立星は 1 個抜きでも 偏り {lb:.4f} / 散らばり {ls:.4f} で"
          f"**散らばりが {ls / max(lb, 1e-12):.0f} 倍**。"
