...
- 雑音なしの床は 1e-10 nm(倍精度の丸め)。段差 500 nm でも桁が変わらない。
...
def main():
    ...
    print("   (b) 最初、画素ごとの誤差を λ/2 で丸めて数えたら 0/±1 次に 3 割が"
          "落ちたので「縞次数の飛びだ」と読んだ。ヒストグラムを引いたら ±λ/2 に"
          "広がった単峰で、丸めた瞬間に 3 本の柱に見えていただけだった。"
          "**量子化を主張する前に、丸めていない分布を見る**。")
    print("   (c) 走査を細かくすれば精度が上がる、は成り立たない(5)。局所当てはめは"
          " argmax 近傍 3 点しか見ないので、面数を 5 倍にしても散らばりは変わらず"
          "時間だけ増える。")
    print("   (d) 再現性が最良の推定量が最良とは限らない(6)。`centroid` は"
          " `gaussian` の 1/3〜1/5 の散らばりを保ったまま、雑音 3%% で段差を"
          " %.0f%% 縮める。真値を仕込まない繰り返し測定では検出できない種類の"
          "誤りで、これが「真値は自分で決める」規律の根拠。" % (100 * (1 - g_mid)))
    print("\n所見(fullseye の穴):")
