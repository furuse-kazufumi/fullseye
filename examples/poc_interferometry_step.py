...
- 雑音なしの床は 1e-3 nm 未満。段差 500 nm でも桁が変わらない。
- 雑音 1%(振幅比)で、段差によらず偏り数 nm・散らばり ~10 nm。
  **不確かさは段差の大きさにほぼ依存しない**(相対誤差ではなく絶対誤差の世界)。
- ゼロ点 `peak` の誤差は「0 か ±走査ステップ/2」の 2 値。段差が走査格子の
  整数倍かどうかで決まる **構造を持った嘘**で、雑音を減らしても消えない。
- 走査ステップは細かくしても良くならない。Nyquist 上限 λ/4 の手前 0.14 µm で
  既に雑音なし誤差 −18 nm。**上限は安全な動作点ではない**。
- `centroid` は散らばりが `gaussian` の 1/8 なのに、雑音とともに **段差を縮める
  ゲイン誤差**を持つ(雑音 3% でゲイン 0.67)。繰り返し測定だけでは捕まらない。
...
def main():
    ...
    for step in STEPS:
        ...
        print("   %8.1f nm | %12.6f µm | %+10.4f nm" % (step * 1000, got, err_nm))
    print("   床 = %.4f nm。段差 500 nm でも桁が変わらない = 誤差は段差に比例しない"
          % floor_nm)
    ...
    print("   `peak` の誤差 %+.1f nm はちょうど走査ステップの半分(%.1f nm)。"
          % (peak_err, 40.0 / 2))
    print("   サブサンプル推定はこれを %.0f 万倍改善している(%.4f nm)。"
          % (abs(peak_err / gauss_err) / 1e4, abs(gauss_err)))
    assert abs(abs(peak_err) - 20.0) < 1e-6      # = dz/2 ちょうど
    assert abs(gauss_err) < 0.05

    # ゼロ点の誤差は「小さい」のではなく「格子に載るかどうか」で 2 値になる
    print("   ★`peak` の誤差は雑音由来ではない。段差が走査格子の整数倍かで決まる:")
    for dz, npl in ((0.02, 601), (0.04, 301), (0.05, 241), (0.08, 151), (0.10, 121)):
        got, _ = measure_step(0.100, 0.0, 0, "peak", dz=dz, n_planes=npl)
        ratio = 0.100 / dz
        print("      走査ステップ %.2f µm(段差 = %.1f ステップ)→ 誤差 %+6.1f nm"
              % (dz, ratio, (got - 0.100) * 1000))
    # 格子に載る (0.02/0.05/0.10) は厳密 0、載らない (0.04/0.08) は ±dz/2
    assert abs(measure_step(0.100, 0.0, 0, "peak", dz=0.05, n_planes=241)[0]
               - 0.100) < 1e-12
    assert abs(abs(measure_step(0.100, 0.0, 0, "peak", dz=0.08, n_planes=151)[0]
                   - 0.100) - 0.040) < 1e-12
    ...
    assert abs(sweep[0.14]) > 10.0 * max(abs(sweep[d]) for d in (0.05, 0.10))
    ...
    print("   (b) 最初、画素ごとの誤差を λ/2 で丸めて数えたら 0/±1 次に 30%% が"
    ...
    print("\n所見(fullseye の穴):")
    print("   (e) `csi_design` は `max_z_step_um` = λ/4 を返すが、5) のとおり"
          "その手前 0.14 µm で既に雑音なし誤差 −18 nm が出る。`recommended_z_step_um`"
          " = λ/8 を使えば実用上問題ないが、上限と実用限界の差が返り値からは読めない。")
    print("   (f) `on_invalid=\"fill\"` の画素拒否率は返り値に出ず、NaN を自分で"
          "数えるしかない(6 の右端列)。拒否率は測定の信頼度そのものなので、"
          "拒否数を返す口があると下流で使いやすい。")
