...
def main():
    ...
    print("  → 対照行(倍率だけ 0.45)を見ると、**壊れるのはアンミックス2 だけ**である。")
    print("     端成分を固定の反射率で持っているので、画像が一様に暗くなると和 1 の制約が")
    print("     行き場を失い、暗いほうの端成分(土)へ寄る。アンミックス3 は影 = 明るさの")
    print("     自由度を持っているので不変。大津も正規化指数も倍率に不変(大津の閾値は")
    print("     アフィン変換に等変、正規化指数は分母で割る)。**対照を置かずに『曇り』の")
    print("     行だけを見ると、ゼロ点が良くなったのを『曇りに強い』と読んでしまう** ——")
    print("     効いたのは倍率ではなく影が消えたほうで、悪条件が必ず全部を悪くする")
    print("     わけでもない。")
