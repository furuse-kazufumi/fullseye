...
1. **ゼロ点(大津の二値化を緑チャネルに掛ける)を先に測る** —— 指数を使う手法が
   ...
   +7〜+16 pp 上振れ**し、指数を使う 3 手法はいずれもその 1/3 以下に収まる。
...
6. **端成分を盲目的に取る(PPI)は、純画素が足りないからではなく雑音で壊れる**。
   反射率で σ=0.0005(0.05 %)の雑音を足しただけで、純粋な葉が 67 % ある場面で
   PPI は葉を 1 つも拾わなくなる。**純画素が多いほど壊れやすい**(孤立した 1 点に
   票が集中し、多数派の純画素は互いに票を割るため)。予想と逆だった。
7. ★**被覆率の数字だけが合っていて、画素が 1 つも当たっていない**場面が実在する。
   発芽期・湿った土でのゼロ点は被覆率の偏り -0.2 pp(ほぼ完璧)なのに、適合率も
   再現率も 0.000。**1 つの数字だけを報告していたら最良の手法として通る**。
...
def zoom(a, k=3):
    """図に載せるためだけの最近傍拡大。96x96 のままだとパネルの題が入らない。"""
...
def main():
    ...
    print("  → **影ゼロならゼロ点は悪くない**(繁茂期で -0.5 pp)。影を入れた途端に")
    print("     +9〜+11 pp に飛ぶ。影の中の土は緑バンドで葉より暗いので、『暗い側 = 植生』")
    print("     という規約に直接刺さる。壊れているのは閾値の計算ではなく **手掛かりの")
    print("     選び方**(明るさそのもの)である。")
    print("  → NDVI も影で +0.2 → +4.4 pp と悪化する。影の中の土は NDVI が負に落ちるので、")
    print("     大津の閾値がそちらへ引かれて土の一部を植生側に入れてしまう。")
    print("  → **ExG は繁茂期で逆に良くなる**(-4.9 → -1.2 pp)。影で土が青くなり ExG が")
    print("     わずかに上がるぶん閾値が上がり、それが元々あった下振れを打ち消している。")
    print("     『影に強い』のではなく **2 つの誤差がたまたま相殺している**だけなので、")
    print("     土や空の色が変われば相殺は崩れる。良くなった理由も確かめる必要がある。")
    print("  → 影の強さを通して 2 pp 以内に留まるのはアンミックス3 だけ。影は明るさの")
    print("     自由度だと分かっていれば、端成分として設計に入れられる。")
    # 影ゼロの左端では全手法が団子になり、影を入れた瞬間にゼロ点だけが跳ねる。
    # 「悪条件を入れない実験では差が出ない」ことが図の形で残る。
    _sv = np.array([0.0, 0.4, 0.8, 1.0])
    figs.save_plot("shadow_sweep",
                   [(n.split("(")[0],
                     _sv, np.array([shadow_rows[(2, v)][n]["bias"] for v in _sv]))
                    for n, _ in sh_methods],
                   xlabel="影の強さ", ylabel="被覆率の偏り [pp]",
                   title="影を入れるまで差は出ない(繁茂期)",
                   caption="影ゼロならゼロ点も悪くない。影は『暗さ』を手掛かりにする"
                           "手法に直接刺さる。")
    ...
    print("     数えて上振れ(最悪 +46 pp)、繁茂期は条間の混合画素を土と数えて下振れ")
    print("     (最悪 -36 pp)。どちらも『半分葉の画素を 0 と答えるか 1 と答えるか』しか")
    print("     選べないことの現れで、**閾値をどこに置いても直らない**。")
    print("  → 混合率が 4 割を超えたあたりから偏りが加速し、9 割超では二値手法が全滅する。")
    print("     アンミックス3 は混合率 100 % でも 5 pp 以内 —— ただしそれは端成分を")
    print("     知っている場合の話で、画像から推定しようとすると次節のとおりになる。")
    ...
    print("  → 雑音ゼロなら PPI は端成分をぴたりと当てる。**反射率で 0.05 % の雑音を**")
    ...
    figs.save_table("ppi_noise",
                    ["雑音 σ(反射率)", "端成分 NDVI 最大", "葉を拾えたか", "被覆率の偏り"],
                    [["%.4f" % nz, "%+.3f" % ppi_noise[nz][0],
                      "はい" if ppi_noise[nz][1] else "いいえ",
                      "%+.1f pp" % ppi_noise[nz][2]]
                     for nz in (0.0, 0.0005, 0.001, 0.002, 0.004)],
                    col_w=140,
                    title="PPI は雑音 0.05 %% で葉を見失う(真値 NDVI %+.3f)" % NDVI_LEAF,
                    caption="純粋な葉が %.0f %% ある場面。壊れる原因は純画素の不足では"
                            "なく雑音のほう。" % (100 * pure_hi))
    ...
    lo_wet = cond_rows[(0, "湿った土(暗い)")]
    print("  → 湿った土は緑バンドで 0.068、緑葉は 0.095。**『暗い側 = 植生』が反転する**。")
    print("     ゼロ点が壊れているのは閾値の位置ではなく **向きの規約**のほうで、")
    print("     大津をどれだけ丁寧に計算しても直らない。")
    print(f"  → ★この表でいちばん危ないのは 発芽期・湿った土の行で、被覆率の偏りは"
          f" {lo_wet[1]['大津・緑(ゼロ点)']['bias']:+.1f} pp と")
    print("     **ほぼ完璧に見えるのに、適合率も再現率も 0.000** = 当たった画素が 1 つも")
    print("     無い。植生と答えた画素の数だけが偶然合っていた。**被覆率という 1 つの")
    ...
    print("     変換に等変なので、この 2 行は一致しなければならない。")
    print("  ★ここは **この PoC が見つけて fullseye 0.1.10 で直した穴** である。")
    print("     直す前は ``otsu`` と ``cv_otsu`` が [0,1] に切り詰めてから数えていたので、")
    print("     12 bit の生カウントを渡すと **全画素が上側に落ちて判定が 100 %** になり、")
    print("     判定が otsu 13.34 pp / cv_otsu 13.92 pp ずれていた(警告もログも出ない)。")
    print("     原因は ``ops._otsu`` と ``backends._u8`` の ``np.clip(v, 0, 1)`` の 1 行で、")
    print("     ``_u8`` は backends.py だけで 23 op が通るので同じ穴が横に並んでいた。")
    print("     いまは値域が [0,1] の外のときだけ実データの範囲でビンを張る。")
    print("     残る ``cv_otsu`` の 0.5 pp は OpenCV が内部で 8 bit に量子化するためで、")
    print("     こちらは docstring どおりの挙動。門は tests/test_value_range_saturation.py。")
    aff = {}
    for op in ("otsu", "sk_otsu", "cv_otsu"):
    ...
    lines = (
        ...
        ("混合画素はどうするか",
         ...
         "ただし端成分を知っている前提"),
        ("端成分を画像から取れるか",
         "PPI は反射率 0.05 % の雑音で葉を見失う。純画素が多いほど壊れやすい"),
        ("いちばん危ないのは",
    ...
    # (11) ★この PoC が見つけて **fullseye 0.1.10 で直した**穴。
    #      3 つの大津が同じ入力に違う答えを返していた —— 同じ画像を 4095 倍
    #      しただけで判定が otsu 13.34 pp / cv_otsu 13.92 pp ずれ、sk_otsu だけが
    #      アフィン不変だった。原因は `ops._otsu` と `backends._u8` の
    #      `np.clip(v, 0, 1)`(0..255 の float が全画素飽和して「全部前景」になる)。
    #      いまは 3 つとも倍率で動かない。残っている `cv_otsu` の 0.5 pp は
    #      OpenCV が内部で 8 bit に量子化するためで、これは docstring どおりの挙動。
    #      門は `tests/test_value_range_saturation.py`。
    assert aff["sk_otsu"] < 1e-12, aff
    assert aff["otsu"] < 0.01, aff
    assert aff["cv_otsu"] < 0.01, aff
    # 直したあとは ExG(負が半分ある)でも otsu と sk_otsu がほぼ一致する
    k = "ExG(発芽期・負が半分)"
    assert abs(otsu_frac[(k, "otsu")] - otsu_frac[(k, "sk_otsu")]) < 1e-2, otsu_frac
