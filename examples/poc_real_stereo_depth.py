...
7. **実写の視差レンジで塗り分ける**。CIE L* の折返しは jet 3 / turbo 1 /
   ...
   gray 0.0068 > viridis 0.0064。★ここは**測ってみて言い方を変えた**ところ:
   「jet は分かれ方が不均一」と書きかけたが、段ごとの色差の最大/最小比は
   jet 1.73 で **turbo 2.57 / viridis 3.26 よりむしろ均一**だった
   (厳密に均一なのは gray の 1.00 だけ)。jet の欠点は**明度が 3 回折り返す**
   ことであって、段の均一さではない —— 「悪い配色」とひとまとめにすると、
   何が悪いのかを言い損なう。turbo は折返し 1 回と大きな色差を両立していて、
   ``colorize_disparity`` の既定になっている。
