...
この PoC が示すこと(数字はすべて実行時に印字される実測値):
 ...
 1. ★★**「50 % 法の偏りは減光係数 u に比例する」という予想は外れた**。
    u=0.2 の **-0.56 px** を比例で伸ばすと u=0.8 は -2.2 px のはずが、実測は
    **-12.76 px(5.7 倍)**。★**対照群(ぼけ σ=0)が機構を分けました**:
    u<=0.5 ではぼけを切ると偏りが -0.78 px 以下しか残らない(つまりこの領域の
    偏りを作っているのは減光ではなく**シーイング**)。u>0.5 では 50 % の
    等輝度面が**円板の内側に入り込む**ので、ぼけ σ=0 でも -12.65 px 残り、
    幾何だけの予測 `R(√(1-(1-0.5/u)²) - 1)` = -13.14 px とよく合います。
 2. ★★**ぼけの効き方は「どの明るさで縁と決めるか」で符号が変わり、
    打ち消し点が実在する**。u=0.6 でシーイングを 1 -> 4 px に強めたときの
    半径の変化は **+0.80 px(しきい値 0.2)から -0.25 px(0.7)**へ反転し、
    **0.26 付近で 0**。そこは「シーイングに強い」のではなく釣り合っている
    だけで、**対照群がそれを示します** —— u を 0.6 -> 0.3 にすると打ち消し点は
    **0.26 -> 0.38** へ動く。減光係数を知らずにしきい値は選べません。
    しかも単調でもない: しきい値 0.7 の「ぼけによる変化」が -0.25 px と
    小さいのは精度が良いからではなく、**誤差 -23.3 px のほとんどが
    ぼけに依らない幾何項**だからです。
 3. 勾配最大(fullseye のキャリパー ``measure_pos``)は 50 % 法より 1 桁良いが
    **無偏ではない** —— u=0.6 / σ=2 px で **-0.27 px**(50 % 法は -3.58 px)。
    減光で縁の内側にすでに傾きがあるぶん、勾配の山が内へ寄ります。
 4. ★**減光モデルを当てはめて外挿すると +0.036 px** まで戻り、u も 0.600
    (真 0.60)と当てる。★ただしその利得は**モデルが合っていること**から来て
    いて、当てはめという手続きから来ているのではありません: u=0 と決め打つと
    **-3.48 px** で、ゼロ点(-3.58 px)とほとんど変わらない。逆に σ を半分
    (1.0 px)と誤認しても **+0.06 px** しか動かない —— u が 0.615 へずれて
    吸収するからで、**効いているのは減光の形であってぼけの正しさではない**。
 5. ★**黒点が動かすのは半径ではなく中心**。縁に載った黒点で半径は 0.33 px
    しか動かないのに、**中心は 0.005 -> 0.591 px(122 倍)**。残差 2.5 MAD で
    刈ると 0.008 px まで戻り、落ちた点は 180 本中 **5 本(2.8 %)** ——
    **点の 3 % が中心を半画素動かしていた**ことになります。半径だけ見ている
    と気づけない壊れ方です。
...
def section_zero_point() -> dict:
    ...
    print("  %-20s %8.3f  %+7.3f       <- 減光を無視したモデル" % (
        "同 u=0 と決め打ち", m0["r"], m0["r"] - R_TRUE))

    ms = fit_limb_model(img, (c["cy"], c["cx"]), c["r"], seeing=1.0)
    out["減光モデル(σ を半分と誤認)"] = (ms["r"] - R_TRUE, float("nan"))
    print("  %-20s %8.3f  %+7.3f       <- σ を 1.0 px と誤認(真は %.1f)  u=%.3f" % (
        "同 σ=1.0 と誤認", ms["r"], ms["r"] - R_TRUE, SEEING, ms["u"]))

    print("\n  ★50 %% 法は %+.2f px。勾配最大は %+.2f px で 1 桁良いが**無偏ではない**"
          % (out["ゼロ点(50 %)"][0], out["勾配最大(caliper)"][0]))
    print("  —— 減光で縁の内側にすでに傾きがあるぶん、勾配の山が内へ寄ります。")
    print("  減光モデルは %+.3f px まで戻り、u も %.3f(真 %.2f)と当てる。"
          % (out["減光モデル"][0], m["u"], U_TRUE))
    print("  ★ただし利得は**モデルが合っていること**から来ていて、当てはめという")
    print("  手続きから来ているのではありません: u=0 と決め打つと %+.2f px で、"
          % out["減光モデル(u=0 と誤認)"][0])
    print("  **ゼロ点(%+.2f px)とほとんど変わらない**。" % out["ゼロ点(50 %)"][0])
    print("  逆に σ を半分と誤認しても %+.2f px しか動きません —— u が %.3f へずれて"
          % (out["減光モデル(σ を半分と誤認)"][0], ms["u"]))
    print("  吸収するからで、**効いているのは減光の形であって、ぼけの正しさではない**。")
    return out
...
def section_u_sweep() -> dict:
    print("\n" + "=" * 78)
    print("3) ★★減光係数 u を振る —— 比例ではなく、u>0.5 で幾何項が点火する")
    print("=" * 78)
...
def section_seeing_levels() -> dict:
    ...
    print("  ★ただし**単調ではありません**。しきい値 0.5 を超えると 50 % 面が円板の")
    print("  内側に入り、ぼけに依らない幾何項が支配するので、**ぼけによる変化は")
    print("  また小さくなる**(%+.2f -> %+.2f px)。誤差そのものが小さいのではなく"
          % (d[0.5][2], d[0.7][2]))
    print("  (しきい値 0.7 の誤差は %+.1f px)、**誤差のうちぼけのせいの分**が"
          % d[0.7][1])
    print("  小さいだけです。")
    print("  打ち消し点は「シーイングに強い」のではなく**釣り合っているだけ**で、")
...
def section_spot() -> dict:
    ...
    print("  (差 %.2f px)しか動かないのに、**中心は %.3f -> %.3f px** と %.0f 倍に"
          % (abs(a_r - z_r), z_c, a_c, a_c / max(z_c, 1e-9)))
    print("  なる。円の当てはめは、片側の縁の点が内へ引かれると中心をそちらへ")
    print("  寄せます —— **半径だけ見ていると気づけない壊れ方**です。")
    print("  外れ値を刈ると 半径 %+.2f px / 中心 %.3f px まで戻る。落とした点は"
          % (b_r, b_c))
    print("  %d 本 = 全 %d 本の %.1f %% で、**その %.1f %% が中心を半画素動かして"
          % (out_drop, N_RAYS, 100 * out_drop / N_RAYS, 100 * out_drop / N_RAYS))
    print("  いた**ということです。")
...
def main() -> None:
    ...
    print("  * 縁に載った黒点は、半径(%.2f px)より**中心**(%.3f px)を動かす。"
          % (abs(spot[("黒点 r=0.97R(縁上)", False)][0]
                 - spot[("黒点なし(対照群)", False)][0]),
             spot[("黒点 r=0.97R(縁上)", False)][1]))
    print("    残差 MAD で刈れば %.3f px まで戻る。半径だけ見ていると気づけない。"
          % spot[("黒点 r=0.97R(縁上)", True)][1])
