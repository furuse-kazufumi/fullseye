...
1. ★★**最終形状だけを見る予測器は、どんな形でも厳密にゼロを返す**。断面の
   重心の定義から Σ a_k (z_k - z_bar) = 0 なので、最終断面で評価した曲率は
   7 形状すべてで 2.712e-21 以下(= 数値のごみ)。**反りは層の履歴にしか
   入っていない** —— 出来上がった形を眺めても、原理的に何も分からない。
2. **ゼロ点(底面積・最大断面積・体積で危険度を出す)は順位を当てられない**。
   実測たわみとの順位相関は 3 つとも -0.25(層の履歴の閉形式は +0.79)。
   7 形状のうち 6 つは底面積が同じ 960 mm^2 なのに、たわみは
   0.3251 - 0.4111 mm と 26 % 開く。★いちばん底面積の大きい平板が、
   いちばん反らない(0.3251 mm)。
3. **閉形式はまず 2 層バイメタルで検算できる**。等厚・等 E の 2 層で
   κ = 0.750 ε/h(Timoshenko 1925 の閉形式と比 1.000000 で一致)。層を積むと
   κ h / ε は 0.7500(2 層)-> 2.4174(24 層)-> 2.5650(64 層)と**飽和**する
   —— 断面 2 次モーメントは層数の 3 乗で増えるのに腕は 1 乗でしか伸びないので、
   ★**背を高くしても反りは増えない**。
4. **予測(層の履歴の閉形式)vs 実測(Q6 増分積層 FEM)は 7 形状中 4 形状で
   0.4 % 以内**。崩れた 3 形状(段付き 0.969 / 中央 1 柱 0.939 /
   両端 2 柱 0.783)は**すべて断面が x 方向に一様でない**形。
5. ★★**層面積の履歴が 1 mm^2 も違わない三つ子**(全長に薄く / 中央 1 柱 /
   両端 2 柱。体積も底面積も同じ)で、実測たわみは
   **0.4109 / 0.3809 / 0.3271 mm(26 % 開く)**。閉形式は 3 つとも同じ
   9.1470e-04 を返す —— **面積の履歴は「どこに置いたか」に構造的に盲目**。
   当たるのは面積を全長に均した 1 つだけ(比 1.0001)。
6. ★**予想が外れた(その 1)**。「首を細くすると平面保持が壊れて閉形式が
   外れる」と踏んでいたが、首 16 -> 1 mm で実測 κ / 予測 κ は
   0.9998 -> 1.0030(誤差 0.3 % 以内)にしか動かない。
   **一様な層の収縮は純曲げで、せん断力が立たない**ので細い首でも伝わる。
   ★さらに首を細くすると反りは**減る**(0.4344 -> 0.3532 mm)—— 首は
   「弱いから危ない」場所ではなく、**縮む材料が少ない**場所だった。
7. ★**予想が外れた(その 2)**。「細長比 L/H が 3 を切ると崩れる」と予測して
   掃引したら、実測の崖は **L/H = 1.0**(誤差 中実 4.2 % / 首つき 8.4 %)で、
   L/H >= 2.0 では 0.4 % 以内。予想 3、実測 1.5。
   ★**力学の崖より先に表現の崖が来る**: 設計 0.50 mm と 1.00 mm の首は
   ボクセル 0.50 mm ではどちらも 1.00 mm としか測れず、0.25 mm では
   **跡形も無くなって部品が 2 つに分かれる**。
8. ★★**同じ形・同じ体積・同じ底面積でも、層厚を 1.00 -> 0.25 mm にすると
   たわみが 4.63 倍**(0.1971 -> 0.9118 mm)。ただしこれは「1 層あたりの ε が
   一定」という仮定の下で、**「単位高さあたりの収縮が一定」と置き直すと
   1.16 倍にしかならない**(0.3942 -> 0.4559 mm)。どちらの較正を採るかで
   結論が 4 倍動くので、**ε の定義を書かずに「層を薄くすると反る」と
   言ってはいけない**。対照群として ε だけを振ると反りは ε に厳密に比例し
   (比のばらつき 0.00e+00)、幅だけを 4 -> 16 mm と振っても
   たわみは 0.4344 mm から**まったく動かない**。
9. **剥離は応力では決まらない**。基板に平らに押さえたときの端の引き剥がし
   応力は要素寸法で決まらず(dx 2.00 -> 0.50 mm で 9.53 -> 11.89 MPa、
   25 % 増え続ける = 端は特異点)。★一方**端 6 mm に掛かる力の合計は
   0.3 % で収束**する(700.4 -> 700.5 N)ので、判定に使えるのは力のほう。
10. ★★**その力も予測できない**。力 / EIκ の変動係数は **97 %** で、閉形式の
   EIκ からは 1 桁も決まらない。決定的なのは例の三つ子で、EIκ がほぼ同じ
   (3003.0 - 3834.8 N mm)なのに端の力は **11.8 -> 573.5 N の 48 倍**違い、
   接着強さ 0.5 MPa に対する判定まで割れる(保持 / 剥離)。効いているのは
   **端で基板につながっている材料の背の高さ**で、これは層の面積の履歴には
   入っていない。★式で予測できないので実測で崖を探すと、底板 1 -> 8 mm で
   端の力は 1.5 -> 698.1 N(**479 倍**)、容量 96.0 N を越えるのは 6 mm ——
   **端を薄く逃がすと剥がれない**(反りの量そのものはほとんど変わらない)。
11. **危険度の地図は 2 つのつまみで向きが逆**。予測たわみ / 許容 0.20 mm は
   0.71 - 5.76(8.1 倍)動くが、首だけを 16 -> 1 mm にすると 0.72 倍に
   **減り**、層厚だけを 1.00 -> 0.25 mm にすると 4.62 倍に**増える**。
   反りを減らしたいなら細くするより**厚く積む**。
    ...
    for a_ in range(g.shape[0]):
        for b_ in range(g.shape[1]):
            img = np.asarray(fs.text_box(
                ...
                text_color=(0.0, 0.0, 0.0) if norm[a_, b_] > 0.55
                else (1.0, 1.0, 1.0)))
...
def main() -> None:
    ...
    print("  * 剥離は応力でも EIκ でも決まらない(応力は要素寸法で %.0f %% 動き、"
          "力 / EIκ の変動係数は %.0f %%)。\n    "
          "同じ面積履歴の三つ子で端の力が %.0f 倍違う —— **端の作り**で決まる。"
          % (100 * (smax[-1] / smax[0] - 1), 100 * cv, trio_spread))
