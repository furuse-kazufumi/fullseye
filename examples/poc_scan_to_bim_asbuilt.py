...
1. **ゼロ点(点群全体を BIM へ一括 ICP して平均距離を 1 個出す)は、施工誤差の
   有無をほとんど区別しない**。誤差ゼロの建物 2.20 mm、両方入り 3.61 mm ——
   差は 1.41 mm。同じ点群を要素ごとに見ると柱の半径 +6.5 mm、床の反り 12 mm と、
   桁の違う不良が入っている。**建物 1 個の数字は、場所ごとに符号の違う偏差を
   平均して打ち消す**。幅木(出 15 mm)は点の 5.0 % しかないので最初から出ない。
   ★もう 1 つの素朴な数字(対称 Chamfer 距離)は 25.67 → 26.17 mm と **0.50 mm
   しか動かない**うえ、値そのものが平均距離の 10 倍以上ある —— 後ろ半分が
   「設計面の標本から見た最近点」なので、**欠陥ではなく走査できなかった所
   (柱の裏・影・擦過角)の広さを測っている**。欠測の指標を欠陥の指標として
   読むと、スキャン位置を増やしただけで「品質が上がった」ことになる。
2. ★★**一括で合わせると、部屋という箱が剛体モードを吸い、その分を他の部材へ
   配る**。真値は「東西の壁が +3.00 mrad そろって東へ倒れ、床は -1.50 mrad で
   東へ下がり、天井は完全に水平」—— どれも「部屋が east へ傾いた」向きなので、
   合わせは 1 つの妥協角を取る。**ICP を走らせる前に閉形式で予測できる**:
   偏差の場を ``J=[p×n|n]`` の 6 次元へ射影すると吸われる回転は
   ω=(+0.07, **-0.89**, +0.02) mrad で、合わせたあとの読みは
   壁 +2.11 / 床 -0.61 / 天井 **+0.89** mrad。実測(全体 ICP)は
   +2.06 / -0.62 / +0.89 で、**予測との差は最大 0.05 mrad**。
   ★**無傷の天井に一番大きな偽の施工誤差が出る**(真値 0 に対し +0.89 mrad =
   6 m で 5.3 mm)。壁は真値の 69 %、床の勾配は 41 % に痩せる。
3. ★**吸われるのは剛体モードだけ**。南北の壁の「開き」(上で外へ、互いに逆向き
   = 剛体でない)は真値 +2.50 mrad に対し合わせても +2.46 mrad。床の反り
   (対称な 2 次曲面)も残る。合わせ方で動くのは傾きだけ(合わせない +2.83 /
   床と 2 壁を基準 +1.47 / 全体 ICP +2.06 mrad)。★**判定の食い違いは
   合わせない 0 件 → 全体 ICP 2 件 → 床と 2 壁を基準 4 件**と、合わせるほど
   増える。床と 2 壁を基準にすると天井は +1.69 mrad になり、**無傷の天井が
   不合格になる**(許容 ±1.0 mrad)。
4. ★**崖は「欠測率」では決まらない**。予想は「点が減れば σ/√N で悪くなる」
   だったが、**同じ欠測率でも 1 桁違う**。無作為に 90 % 落としても壁の傾きの
   誤差 RMS は 0.098 mrad(検出できる)。ところが**下から順に残す**構造的な
   欠測(床置きスキャナと擦過角で実際に起きる形)で同じ 90 % を落とすと
   1.234 mrad —— **12.6 倍**。効くのは残った点数ではなく**残った面の高さ L**
   (2.86 m → 0.19 m)。σ√12/(L√N) の予測は L >= 1.1 m では 21 % 以内で当たり
   (予測 0.161 / 実測 0.204 mrad)、L = 0.19 m では 62 % 外す —— 面が薄いと
   RANSAC のしきい値 8 mm が面の広がり(傾き × L = 0.57 mm)より大きくなり、
   平面適合が雑音を当てにいくから。
5. ★★**レジストレーション誤差は点を増やしても消えない**。同じ点数(壁 1 枚
   2514 点)で、測距の雑音だけなら偽の傾きは 0.040 mrad で済むのに、スキャン
   位置の姿勢誤差 1.0 mrad で 0.189 mrad(**4.7 倍**)、5.0 mrad で 0.848 mrad。
   3 mrad の施工誤差に対する信号対雑音は 75 → 15.9 → 3.5。**検出限界は測距の
   雑音でなくレジストレーションで決まる**(密度を上げても改善しない)。
6. ★**混合画素に壊されるのは幾何でなく推定器**。混合を 0 → 40 % と振っても、
   RANSAC の平面から出す壁の傾きの誤差は最大 0.058 mrad。同じ点群・同じ縁で、
   ``obb``(主成分の min/max)から出す窓の高さは最大 27.74 mm 外し、しかも
   **単調でない**(+24.9 / +8.1 / +25.1 / +9.3 / -2.7 mm)—— 主軸が数点で傾く
   ので、どの割合でも突然壊れうる。``obb`` の前に ``ransac_plane`` で面上の点
   だけにすると最大 0.60 mm(98 % 抑制)。★このとき**割り当ての正答率は
   99.87 → 99.01 % としか落ちない** —— 正答率という 1 個の数字はこの壊れ方に
   構造的に盲目。
7. **CSG の交差は角で厳密でない**。6 枚の半空間(``plane_sdf``)を
   ``sdf_intersect`` で重ねた部屋と ``box_sdf`` の厳密な箱を 24192 点の格子で
   比べると、部屋の**内側**は最大 0.0000 mm 差、**外側の角**では最大
   1006.6 mm の過小評価。距離そのものが要るなら箱の op を使う —— op の
   docstring の警告どおりだった。
8. ★**平面度 PV は雑音を丸ごと足す**。完全に平らな床でも測ると PV 4.89 mm
   (RMS 2.03 mm)—— 許容 ±7 mm の 70 % を雑音だけで使う。真値 12.0 mm の反りは
   18.44 mm と読まれる。**PV は分布の両裾を足すので、要約に使うなら RMS の
   ほうが素直**(2.03 対 4.89)。
...
(d) 柱の半径 +6.5 / -4.0 mm (e) 東の壁の面外の膨らみ 8.0 mm (f) 開口の位置ずれ
25.0 / 18.0 mm。測定は 3 か所からの走査で、視線に依存する欠測(柱の影・
擦過角 cos<0.12)・入射角依存の雑音 σ=1.2 mm/(1 m, 垂直入射)・段差の縁の
混合画素 12 %・レジストレーション誤差 0.40 mrad + 1.5 mm。すべて既知。
...
def section_zero_point(seed: int = SEED) -> dict:
    ...
    print("   ★Chamfer 距離が %.1f mm と大きいのは施工誤差のせいではない —— "
          "対称 Chamfer の後ろ半分は" % (1000 * keep["a"][4]))
    print("     「設計面の標本から見た最近点」なので、**走査できなかった所"
          "(柱の裏・影・擦過角)の広さを測っている**。")
    print("     欠測を測る指標を欠陥の指標として読むと、"
          "覆いを増やしただけで「品質が上がった」ことになる。")
