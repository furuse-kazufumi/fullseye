...
1. **ゼロ点(しきい値 + 連結成分、そのまま数える)を先に測る**。面積率
   3.8 % で D50 -0.8 % / D90 +0.3 % —— よく当たる。**ただし D10 は同じ条件で
   -16.5 %**。3 つの数字は一緒に壊れない。**小さい側が先に、しかも
   いちばん薄い条件から壊れている**。
2. ★**壊れ方が 2 つあり、向きが逆**。融合(触れた粒子が 1 個になる)は
   大きい側へ、縁切れ(視野の縁で切れる)は小さい側へ引く。面積率を
   1.9 → 28.2 % と振ると D50 の誤差は **-4.9 % → +4.5 %** と単調に動く。
3. ★★**2 つが打ち消し合う面積率が実在する**(実測 13.8 %、D50 誤差
   +0.55 % ± 3.44)。そこは「正確」ではなく、**融合 28 件・縁切れ 19 件
   (塊 140 個中)が釣り合っているだけ**。いちばん薄い 1.9 % のほうが
   誤差が大きい(-4.9 %)ので、**D50 だけを見ていたら濃いほうを
   「最良の条件」として選ぶ**。
4. **融合は形で見つかる。ただし直そうとすると行き過ぎる。** 充填率は
   単独 0.998±0.016 / 融合 0.918±0.075 で、しきい値 1 本の分離が 92.6 %
   (円形度は 85.6 % で負ける)。ところが★**その門で落とすと D50 は
   +6.5 % → -11.3 %**。**直さないほうがましだった** —— 融合した塊には
   本当に大きい粒子も混ざっているので、丸ごと捨てると上側が痩せる。
5. ★**縁の規約は 3 通りあり、選ぶと D50 が動く**(全部 -5.4 % / 触れたら
   捨てる +0.7 % / Miles-Lantuejoul の 2 辺規則 -2.1 %)。ただし
   **それぞれの規約に対応した真値と比べたときの値**であって、規約を
   取り違えて比べると更に -1.5 % ずれる。危ないのは規約の選択ではなく
   **推定と真値で違う規約を使うこと**。
6. ★★**個数基準と面積基準は別の分布**。同じ画像・同じ塊から出した D50 が
   26.9 µm と 44.7 µm(**1.66 倍**)。「D50 = 26.9 µm」と書くとき、
   どちらの基準かを書かないと意味が無い。
7. ★**予想が 2 段で外れた**。「画素を細かくすれば融合は減る」と踏んでいたが、
   融合した**塊の数**は 34 → 32 → 32 でほとんど動かない(予想どおり外れ)。
   それでも D50 の誤差は 8.65 → 4.60 % と縮むので、次に「画素で塗った面積の
   偏りだろう」と考えた —— **これも対照群が否定した**。融合も縁切れもして
   いない塊を、写している粒子と 1 対 1 で突き合わせると誤差は
   -0.11 / +0.03 / +0.01 % で、**どの画素の細かさでも当たっている**。
   効いていたのは**融合に飲まれた粒子の数**(80 → 71 → 70)だった ——
   粗い画素は粒子を少し太らせるので、塊の数が同じでも 1 塊に飲む数が増える。
