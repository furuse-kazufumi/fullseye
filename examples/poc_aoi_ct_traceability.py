...
この PoC が測る唯一の主張:
    ...
    **規則正しく並んだ接合部は、点の配置だけでは装置間で一対一に対応づかない(格子が
    自分自身に重なるので向きが読めない)。基準マークを入れれば対応は厳密に決まる ——
    そのうえで、AOI の見かけのボイド率は CT の体積率と相関 0.973 で、悪い順 5 個も
    5/5 一致する。にもかかわらず、寿命に効く「界面に接したボイド」を持つ 27 個のうち
    その 5 個で拾えるのは 19 % しかない —— 合否の順位が合っていることは、
    危ない個体を拾えていることを意味しない。**

各章は真値との厳密な突き合わせで検査します(下の assert):

    縮退              6x5 の格子は 30 点すべての距離署名が縮退する(署名は 9 種類)
    基準マーク         3 辺がすべて異なる三角形なら、回転も鏡映も一意に決まる
...
★最初にこれを **剛体変換(Kabsch)だけで解こうとして失敗しました**。Kabsch は「どの点が
どの点に対応するか」が既知である前提の手法で、番号が振り直された点群どうしには使えません。
しかも 6x5 の格子は 180° 回転と 2 つの鏡映で自分自身に重なるため、**対応を仮に総当たりで
探しても解は一意になりません**。実装検査の基板に基準マークが必ず入っているのは、この
縮退を壊すためです —— PoC の所見 1 はその実測です。
...
#: 基準マーク(フィデューシャル)。**わざと非対称**に置く —— 3 点の相互距離が
#: すべて違うので、鏡映も回転も一意に決まる。実装基板に必ず入っている理由がこれ。
FIDUCIALS = np.array([[-2600.0, -2100.0],
                      [+2600.0, -2100.0],
                      [-2600.0, +1500.0]])
...
def main():
    ...
    print("\nPASS: 格子は %d/%d 点が縮退するが基準マークで対応は全復元(残差 %.1g µm)、"
          "面積率と体積率の相関 %.3f・悪い順 5 個は 5/5 一致、それでも界面接触は %d/%d(%.0f %%)"
          "しか拾えない。抜き取り %d 個の外挿は真値との差 %.2f pt(標準誤差 %.2f)。"
          % (degen, n, resid, r, caught, total, 100.0 * caught / max(total, 1),
             N_SAMPLE, est - true_mean, se))
