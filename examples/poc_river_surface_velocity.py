...
映り込みは「空(滑らか)」と「岸の樹木(細かい)」を分けて持つこと —— 効くのは後者だけです。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. ★★**ゼロ点(斜め画像のまま相関し、画像中央の 1 尺度 2.07 cm/px で m/s に直す)は
   近岸で速く・遠岸で遅く出る**。窓ごとの速度誤差は近岸 2 m 以内 +0.314 / 遠岸
   -0.276 m/s(RMS 0.264 m/s)—— 地上分解能が近岸 1.4 / 遠岸 2.7 cm/px なので
   符号が岸で逆になる。**流量は -56.7 %**:流下方向の 1 尺度を川幅方向にも当てると
   **見かけの川幅が 3.3 m(真値 8 m)**になり、速度が合っている行も幅で外れる。
   速度の誤差(±0.3 m/s)と流量の誤差(-57 %)は桁も原因も違う。
2. ★**行ごとの尺度だけ直して歪めない中間案は、速度 RMS 0.089 m/s・流量 -8.3 %**
   まで戻る。**正射化してから PIV** すると 0.074 m/s・-7.8 %、理想の真上カメラ
   (同じ画素で撮った対照群)は 0.041 m/s・-5.5 %。★正射化は真上カメラに届かず、
   遠岸(0.070 vs 0.039 m/s)だけでなく近岸(0.068 vs 0.038)も悪い —— 遠岸の
   1 画素は世界で 2.7 cm で 2 cm 格子への補間は失った解像度を作れず、近岸も
   斜め画像の画素の足跡ぶんぼける。
3. ★★**流量の誤差の 2.5 % は速度と無関係だった**。対照群(反射なし・波紋なし・
   雑音なし)は速度 RMS 0.008 m/s なのに流量 -3.0 %。真値を窓の行で標本化して
   岸 0 との台形で積分するだけで -2.47 % —— u は岸から y^(1/7) で立ち上がるので、
   最初の行(0.31 m)までの台形は 43 % 取りこぼす。**積分則の誤差は速度の誤差と
   別に数えないと、速度を良くしても流量が動かない理由が分からない**。
4. ★★**トレーサ密度の崖は「峰が立たない」ではなく「外れ値になる」**。密度
   0.05 %(窓 32 px に粒子 0.5 個)で 1 対あたりの nan は 0.0 % なのに、
   正規化中央値検定の旗は 44.1 %。相関は必ず**何かの**峰を返す —— 黙って外れる。
   12 対平均で外れ窓(> 1 px)46.4 %・流量 -52.6 %、0.2 %(2 個/窓)で 6.6 %・
   -5.3 %、0.5 %(5 個/窓)で 0 %・-3.3 %。★予想は「アンサンブル相関(相関面を
   足す)が薄い密度を救う」だったが、外れ窓は 46 → 44 % と救わず、典型窓の
   中央絶対誤差だけ 0.793 → 0.619 px と良くなる。外れ窓は粒子が一度も通らない
   窓で、相関面を何枚足しても真の峰は積み上がらない。
5. ★★**窓を広げても岸の速度は桁でなまらなかった**。予想は「偏り ≈ 窓幅 × 勾配」
   (0.12〜0.28 m/s)、実測は岸 1 m 以内で窓 24〜64 px のとき -0.005〜-0.008 m/s
   —— 1 次項は窓の左右で打ち消し、残るのは 2 次項(窓² × 曲率 / 24、Hann 窓で
   更に 0.4 倍)で、Hann 重みの窓平均で予測すると -0.004〜-0.008 m/s と一致する。
   窓 16 px の -0.029 は粒子 5 個/窓の散らばりで、なまりではない。★窓の崖は速度
   ではなく**流量**に出る:窓 w の 1 行目は岸から w/2 px で、その帯を岸 0 との
   台形で埋めるので、積分則だけの誤差が 16 → 64 px で -1.1 → -7.0 %(実測
   -3.2 → -7.1 %)と窓に比例して増える。
6. ★★**動かない映り込みが流速をゼロへ引くのは「細かい」ときだけ**。予想は
   「静止模様は流速をゼロへ引く」だったが、空・雲の映り込み(相関長 8〜30 px)は
   コントラスト 0.60 でも中央の速度比 0.926 —— 窓の平均引きでほぼ消え、残る
   相関ピークは幅が広くて粒子の鋭い峰に負ける。岸の樹木の映り込み(相関長
   1.2 px)は同じコントラスト 0.15 で速度 RMS が 40 倍(0.019 → 0.758 m/s)。
   ★しかも 2 段で効く:0.10〜0.20 では静止ピークと移動ピークが重なって**引かれ**
   (速度比 0.701 / 0.274、部分的に引かれた窓 91 / 87 %)、0.30 以上では
   **どちらかが勝って**ゼロに張り付く(0.122 → 0.033、張り付いた窓 59 → 100 %)。
   平均は混合比で、傾きは 2 回変わる。時間中央値(固定カメラの背景)を引くと
   0.60 でも比 0.998。アンサンブル相関は静止ピークも積み上げるので効かない
   (0.696 / 0.304 / 0.040 と生とほぼ同じ)。
7. ★**dt を伸ばす崖は「1/4 則」の壁ではなく対の消失だった**。予想は「探索上限
   8 px を超える k=4(10 px)で設定の壁が物理より先に来る」。実測の外れ窓は
   k=3 で 12.1 % → k=4 で 42.8 % → k=5 で 83.1 %、探索上限を外し補正も切った
   相関でも 7.9 / 57.0 / 80.3 % と**同じ k=4 に崖が立つ** —— 崖を作るのは
   変位 10 px で窓の重なりが 69 % に落ちる対の消失と窓内の速度差。真値 > 8 px
   の窓(83 %)より外れが少ないのは、峰が探索の縁に掛かっても 3 点当てはめが
   縁の外へ伸びるから。k=3 で既に 12 % 外れるので、dt は**いちばん速い行**で決める。
8. **波紋は効く**(振幅 0.12・波速 0.35 m/s = 0.58 px/コマ):速度 RMS を 0.008 →
   0.071 m/s(流量 -3.0 → -7.6 %)動かす。基準条件(反射(空)0.15 + 波紋 + 雑音)の
   0.076 m/s はほぼ波紋の分で、空間ハイパス(``highpass_image``、波長 30 px を
   落として粒子 3 px を残す)で 0.047 m/s・-5.4 % まで戻る。雑音 σ=0.03 は
   単独で 0.000 m/s しか動かさない。
...
def section_zero_point(sc):
    ...
    assert width_app < 0.6 * B_WIDTH and q_naive < 0.6 * Q_TRUE, "幅が化けて流量は半分以下"
...
def section_rectified(sc, scales):
    ...
    assert -0.035 < q_rule / Q_TRUE - 1 < -0.015, "積分則だけで 2〜3 % 損する"
...
def section_density():
    ...
    pairs = [(k, k + 1) for k in range(N_SWEEP)]
    rows = []
    print("  密度 [%]  粒子/窓  nan/対 [%]  外れ値旗/対 [%] | " + str(N_SWEEP) + " 対平均: 外れ窓(>1 px) [%]  "
          "中央絶対誤差 [px]  全対欠測の窓 [%] | アンサンブル: 外れ窓 [%]  中央絶対誤差 [px] | 流量 [%]")
    for d in dens:
        sc = build_frames(density=d, n_frames=N_SWEEP + 1, refl_c=0, wave_amp=0, want_ortho=False)
        r = run_rectified(sc["obl"], pairs)
        ...
        rec = [rectify(sc["obl"][k]) for k in range(N_SWEEP + 1)]
...
def section_dt():
    ...
    print("  ★予想は「1/4 則の壁(探索上限 %.0f px)が物理より先に来る」だったが、探索を外して補正も切った"
          "相関の崖も同じ k=4(%.0f %% vs 既定 %.0f %%)。" % (limit, 100 * rows[3][4], 100 * rows[3][2]))
    print("    崖を作るのは設定ではなく**対の消失**(変位 10 px で窓の重なり %.0f %%)と窓内の速度差。"
          % (100 * (1 - 10.0 / WIN)))
    print("    真値 > %.0f px の窓の割合(%.0f %%)より実測の外れ(%.0f %%)が少ないのは、峰が探索の縁に"
          "掛かっても 3 点当てはめが縁の外へ伸びるから(9 px の並進で 8.9 px を返す)。"
          % (limit, 100 * rows[3][3], 100 * rows[3][2]))
    print("    k=3(7.5 px)で既に %.0f %% 外れる —— 崖は行ごとに来るので、dt は**いちばん速い行**で決める。"
          % (100 * rows[2][2]))
    assert rows[2][2] < 0.2 and rows[3][2] > 0.3 and rows[4][2] > 0.6
    assert abs(rows[3][4] - rows[3][2]) < 0.25, "探索を外しても崖の位置は変わらない"
    return rows
...
def section_figures(sc, zp, rc, ctrl, dens, wins, refl, dts):
    ...
    up = lambda a: np.kron(np.nan_to_num(a), np.ones((8, 8)))     # 窓格子 24x29 は小さすぎるので 8 倍
    figs.save_grid("map_speed", [up(tr), up(vel), up(vel - tr)],
                   ["真値 u(y) [m/s]", "正射化 → PIV [m/s]", "差 [m/s]"],
                   title="表面流速の場(窓 32 px、%d 対平均、上 = 近岸)" % N_PAIRS, ncols=3,
    ...
    figs.save_plot("profile",
                   [("真値", yy, profile(yy)),
                    ("ゼロ点(1 尺度)", *fin(zp["Yrow"], zp["row_u"])),
                    ("行ごとの尺度", *fin(zp["Yrow"], zp["row_ub"])),
                    ("正射化 → PIV", *fin(rc["rect"]["y"], rc["rect"]["u"])),
                    ("真上カメラ", *fin(rc["y_o"], rc["u_o"]))],
    ...
    figs.save_plot("density_rms",
                   [("%d 対の平均" % N_SWEEP, d, np.array([r[4] for r in dens])),
                    ("アンサンブル相関", d, np.array([r[5] for r in dens]))],
                   xlabel="トレーサ密度 [% of px]", ylabel="変位の中央絶対誤差 [px]",
                   title="薄い密度でアンサンブル相関が救うのは典型窓だけ")
...
def main() -> None:
    ...
    print("  * 密度の崖は外れ値で来る(%.2f %% で %.0f %%)。窓の崖は速度でなく流量。反射は細かい模様だけ"
          "(引かれてから張り付く)。dt の崖は設定でなく対の消失。"
