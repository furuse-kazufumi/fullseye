...
def synthetic_clip(t=16, h=96, w=128, seed=0):
    ...
    for i in range(t):
        ...
        oy += 1.0
        blob = 0.7 * np.exp(-((yy - oy) ** 2 + (xx - ox) ** 2) / (2 * 7.0 ** 2))
        frames.append(np.clip(bg + blob, 0, 1))
...
def analyze(frames, flow_kwargs=None, event_k=2.0, track_grid=6, save_dir=None):
    ...
    # ~0.1% fraction for HD) so the detector is neither object-blind on large
    # footage nor swamped by residual-warp specks on small clips.
    mask, segs = fs.motion_segments(u, v, threshold=thr, min_area=max(25, a.size // 1000))
