...
1. ★**ゼロ点(1 対の波長の比)は思ったより強い**。64 x 64 通りの band 対を
   総当たりして最良の 1 対(**1721 / 2021 nm**)を選ぶと、綺麗な場面で
   材質別再現率の平均 **0.972**。分光角(SAM)の **0.995** にほとんど負けない。
   **汚れる前は分光の作り込みがほとんど効かない** —— 差が出るのは劣化してから。
2. ★★**何が消えるかは前処理の代数で決まる**。破片ごとの劣化は閉形式
   ``s(λ) = g·R(λ)·exp(-w·A_w(λ)) + (a·u(λ)+c)`` で、**SAM は g に不変**
   (方向しか見ない)、**2 階微分は 1 次式 a·u+c を消す**。予測どおり、
   乗算的な汚れ(0→0.8)で生 SAM は 0.995 → **0.990**、傾き(0→70°)で
   0.995 → **0.993**。加算ベースライン(0→0.6)は生 SAM を 0.995 →
   **0.827** に落とすのに、2 次微分は **全水準 0.995 のまま**動かない。
3. ★★**予想が外れた(1): 濡れは 2 次微分でほとんど消せた**。「水の吸収は
   波長に依存する乗算だから 2 階微分にも残る」と踏んでいた。実測は
   生 SAM 0.995 → **0.282**、連続体除去 0.992 → **0.293** と予測どおり壊滅
   する一方、2 次微分は 0.995 → **0.818** で耐える。**理由も代数だった** ——
   2 階微分はガウス帯を 1/σ² で重みづけるので、水の帯(σ≈70 nm)は材質の帯
   (σ≈43 nm)に対して **(43/70)² = 0.37 倍**に潰れる。水帯(1450 / 1940 nm)を
   捨てる手はそれでも効き、濡れ 0.2 で **0.903 → 0.976**。
4. ★**予想が外れた(2): 連続体除去は加算に「途中まで」強い**。上側凸包で
   割るのだから吸うはずで、加算 0.4 では確かに **0.983 対 0.865(生 SAM)**と
   勝つ。ところが加算 1.0 では **0.736 対 0.827** と**逆転**する。凸包は
   ベースラインの傾きは吸うが、加算された定数は吸収の谷を浅くするので、
   材質を分けている形そのものが潰れる。
5. ★★**特徴の無い材質は微分すると消える**。金属はほぼ平坦なので 2 次微分が
   0 に近づき、**方向しか見ない SAM は方向の無い相手に無力**になる ——
   綺麗な場面ですら金属の再現率 **0.06**。平坦度 ``||d2||/平均`` の門
   (校正で決めた **8.93e-05**)を 1 本足すと **0.98** に戻る。
6. ★★**乗算的な汚れが壊すのは分類ではなく検出**。分類だけなら 0.990 のまま
   なのに、検出まで入れた再現率は 0.995 → **0.716**。暗くなった破片が
   ベルトに沈む。**同じ数字に畳むと、不変性が成り立っていることが見えない**。
7. ★★**壊れ方は 4 種類あり、混ぜると順序が見えない**。全部入りの条件で
   被覆(重なりで隠れた面積)**19.4 %** / 未検出 **1.1 %** / 混合画素(境界)
   **11.9 %** / 誤分類 **16.7 %**。対照群で効く順序は 濡れ(**+0.148**)>
   乗算汚れ(+0.015)> 混合画素(+0.012)> 傾き(+0.009)> 加算汚れ
   (**-0.127**)。★**加算汚れを止めると逆に下がる** —— 2 次微分は加算に
   不変なので分類は変わらないが、加算ベースラインは破片を明るくして
   **ベルトから浮かせていた**(未検出 1.1 % → **17.3 %**)。同じ汚れが
   分類を壊しながら検出を助けている。
8. ★★**「似た組が先に壊れる」はライブラリの話で、劣化が強いと別の組が来る**。
   ペア間分光角の 1 位は **PP-PE(1.97°)**。雑音だけを 10 倍にした条件では
   実測の 1 位も **PP-PE**、順位相関 **0.84**(15 対)—— 予測が当たる。
   ところが全部入りでは 1 位が **紙-金属**(744 画素)に入れ替わり相関 0.70。
   濡れはどの材質にも同じ水の帯を足すので、特徴を持たない金属は
   「水の帯しか無いスペクトル」になり、O-H で定義されている紙に最も近づく。
9. ★**混合画素は捨てると精度が上がるが、組成は逆に悪くなる**。境界を捨てると
   材質別再現率は 0.822 → **0.834**。ところが面積基準の組成推定の誤差は
   **3.42 pp → 3.50 pp**。境界の割合は材質ごとに違い(PET 8 % ‥ 金属 18 %)、
   小さい破片ほど高いからで、捨てると小さい材質が体系的に動く。線形混合分解
   (:func:`fullseye.spec_unmix`)は**劣化が無ければ**境界の面積比を平均絶対
   誤差 **0.003** で当てるが、全部入りでは **0.183** —— 劣化は線形混合の
   仮定そのものを壊すので、組成でも **21.13 pp** と素朴な画素計数に勝てない。
...
def section_sweep(pair) -> dict:
    ...
    print("       水帯を捨てる手はそれでも効く。濡れ 0.2 で %.3f -> %.3f、"
          "濡れ 1.0 で %.3f -> %.3f。"
...
def section_pairs(ab: dict, pair) -> dict:
    ...
    print("  ★ところが全部入りでは 1 位が %s に入れ替わる(相関 %.2f)。"
          % (top_f, rho_f))
    print("     濡れは**どの材質にも同じ水の帯**を足す。特徴を持たない金属は"
          "濡れると\n     「水の帯しか無いスペクトル」になり、O-H の帯で"
          "定義されている紙といちばん近くなる。\n     「似た組が先に壊れる」は"
          "ライブラリの話であって、劣化が強いと別の組が先に来る。")
