...
**書類の 4 隅を全部枠に入れる** —— 隅 1 つが枠外 56 px までは辺の外挿で持つが
(4 隅 RMS 2.0 px)、80 px で 42 px に跳ねる。**影は 4 割まで**(影の強さ 0.45 で
4 隅 RMS 5.7 px、0.55 で 65 px)。**机は紙とはっきり違う明るさのものを**
(明度差 0.43 で 10 px、0.28 で検出不能)。
...
(d) **局所窓の op が小さい窓にしか届かない**。``mean_image``/``gray_tophat``/
    ...
    ``a``/``b`` が [0,1] である以上そこへ届かない。第 4 節で並べたところ、
    窓 9(届く上限)は地を平らにはするが**図の階調を潰す**(真の傾斜との相関
    0.94 → 0.28)。窓 61 なら 0.73 まで残る。この差は届かない窓の代償。
    (周波数側の ``dc_homomorphic`` だけは正規化カットオフなので大スケールに届く。)
(e) **2 値マスクに ``sobel_dir`` を当てると方向が 0/90 度に量子化される**。
    ``tools_geom.hough_line_trans_dir`` は 1 点 1 票(その点の勾配方向の
    ビンにだけ入れる)なので、量子化がそのまま結果になる —— 実測で 4 辺の
    うち 2 本が**ちょうど 0.00 度と 90.00 度**に潰れ、4 隅 RMS が 29 px に
    なった。先に ``gauss_filter(a=1.0)`` でぼかしてから方向を測ると **1.96 px**
    まで下がる。領域の境界の勾配方向を出す op が無く、この前処理は利用者が
    自分で気付くしかない(方向つき Hough の docstring にも注意書きが無い)。
"""
...
def main():
    ...
    figs.save_grid(
        ...
        caption="題に添えた数字は格子 RMS。アフィン(同名 op の取り違え)は例外を"
    ...
    print("        誤りが素直に出るのは行の高さと行間の方(遠い行ほど縮む)。")
    ...
    figs.save_grid(
        ...
        caption="下寄りの横長の帯が図の階調。窓 9 は地と一緒にそれも割ってしまう"
    ...
    print("     42 %)、薄い字も 76 % しか拾えない。地を割って平らにすると誤検出は 0 に")
    print("     なるが **薄い字は全部消える**(残り 0 %)—— 平らな地の上では大域しきい値")
    print("     が濃い字の側に寄るため。局所コントラストを持ち上げる手(illuminate /")
    print("     tophat / homomorphic)は薄い字を 99〜100 % 残すかわりに紙を 38〜99 % 字にする。")
    print("     図の階調は**窓が小さいほど壊れる** —— 9 px の局所平均はランプそのものと")
    print("     区別が付かず、割ると平らになって相関が 0.94 → 0.28 まで落ちる(段数は")
    print("     増えるが、それは元の傾斜ではなく割り算で持ち上がった雑音の段数)。")
