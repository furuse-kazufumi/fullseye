...
1. **ゼロ点(2 値化してボイド率だけ)は 5 条件を分けられない。** 体積率の真値を
   3.000 % にそろえた 5 条件で、推定は **2.46〜2.63 %** に収まり、条件間の開きは
   **0.17 ポイント**しかない。合否のしきい値を 5 % に置けば 5 条件とも合格、
   1 % に置けば 5 条件とも不合格。**この数字は形態にほぼ完全に盲目**です
   (ついでに真値より 12〜18 % 低い —— 2 値化が PSF のぼけた縁を削るので)。
2. ★**形態指標は同じ 5 条件を桁で分ける。** 界面離隔の中央値は層中央 60.0 µm に
   対し界面接触 10.0 µm(**6.0 倍**)、扁平度は球 0.97 に対し扁平ボイド 0.25
   (**3.9 倍**)、最近接間隔は散在 160.0 µm に対し連なり 20.0 µm(**8.0 倍**)。
   **1 個の数字に畳んだ瞬間に消えるのは、この 3 本の軸ぜんぶ**。
3. ★★**応力の代理指標(界面欠損率)は体積率が同じでも 0 → 14.68 % まで動く。**
   層中央の球は 0.00 %、界面に接する球は 6.92 %、**同じ体積の扁平ボイドは
   14.49 %(2.09 倍)**。閉形式の予測(球 7.78 % / 扁平 15.00 %、比 1.93)と
   1 ポイント以内で一致します。最大クラスタの跨ぎ率も散在 6.1 % に対し
   連なりは 57.8 %(球)/ 80.0 %(扁平)—— **同じ「ボイド率 3 %」が、
   熱経路を塞ぐ面積では 2.09 倍、割れ道の長さでは 13.1 倍違う**。
4. ★★**崖の予想が外れた —— ボイド率は粗いボクセルでも崩れない。** 予測は
   「最小寸法 / ボクセル = 2 で壊れる」= 球(径 115.7 µm)は 58 µm、扁平ボイド
   (厚み 40 µm)は 20 µm、でした。実測は **60 µm でも球 3.21 % / 扁平 3.09 %**
   で、10 µm の 2.60 / 2.53 % より**むしろ大きい**(80 % を割る点は掃引内に無し)。
   理由は、部分体積を線形の被覆率で積むと**個々のボイドを解像できなくても体積は
   保存される**から。合否の数字はしれっと出続けます —— ただし格子の位相を
   3 通り振ると 60 µm では **±1.36 ポイント**振れ、合否線が 3 % ならこの揺れ
   だけで結論が反転します。
5. ★★**先に死ぬのは形のほう。しかも向きが悪い。** 扁平度は 30 µm までは
   球 0.79 対 扁平 0.25(3.2 倍)と分離を保ちますが、60 µm では**両方とも
   測れません**(1 ボイドあたりのボクセルが足りず nan)。界面欠損率は
   **14.16 → 9.78 %(-31 %)**。つまり **合否の数字は据え置き(むしろ増える)の
   まま、危ない形の指標だけが「安全」側へ動く**。
6. ★**離隔の予想も外れた。** 予測は「層中央と界面接触の差 42.2 µm より粗い
   ボクセルでは判別不能」。実測は 10 / 15 / 20 / 30 / 60 µm で 層中央 60・60・
   60・60・120 µm 対 接触 10・15・20・30・80 µm —— **順序は最後まで残ります**。
   差が縮むのではなく両方がボクセル刻みに量子化されるからで、接触側(真値 0 µm)
   の読みは**ボクセル 1.0〜1.3 個ぶんに貼りつき**、層中央(真値 42.2 µm)は
   120 µm と **2.8 倍**に膨らむ。**「界面に接している」は判定できても
   「何 µm 離れている」は言えない**。
7. ★**しきい値を動かすと、指標が壊れるのではなく測っている対象が入れ替わる。**
   しきい値 0.30 → 0.60 でボイド率は 1.61 → 3.74 %(+133 %)、界面欠損率も
   11.22 → 18.77 % と単調に増えます。ところが連なり条件では **0.50 でボイドが
   融合して塊が 24 → 5 個(0.55 で 3 個)に落ち**、その瞬間「最近接ボイド間隔」は
   **20 → 340 µm に跳ね上がる** —— 隣が遠ざかったのではなく、隣が同じ塊になって
   数え上げから消えたから。合否と形態で最適なしきい値が違います。
8. ★**連結半径の掃引は、連なりを幾何どおりの位置で捕まえる。** 設計の最近接
   すき間は 連なり 17.3 µm / 散在 140.9 µm(**8 倍**)なので、塊の数が半減する
   半径の予測は 8.7 / 70.5 µm。実測は **10.0 / 75.0 µm** で、どちらも掃引の刻み
   以内。**ボイド率にはこの 8 倍の差がまったく出ません**(両条件とも 3 % 台)。
   雑音も同じで、sigma 0.15 では偽ボイドが **24 → 1794 個**に爆発するのに、
   体積率は 2.61 → 2.94 % しか動きません(**強いことが問題** —— 壊れているのに
   合否の数字からは気づけない)。
9. **正直に書く: この PoC は寿命そのものを測っていません。** 熱疲労寿命は
   Coffin-Manson 則と有限要素の応力場が要り、ここには無い。示せたのは
   **「合否 1 個の数字が形態に盲目であること」と「その盲目さがボクセルを粗くすると
   安全側でなく危険側に効くこと」まで**です。界面欠損率は応力の代理指標であって、
   寿命の予測値ではありません。

道具の穴は §7 に印字します(前向きの畳み込み op が無い / ``query_distance`` が
立方格子専用 / 扁平度と最近接間隔の op が無い / 界面に接するボイドの等値面は
切り出し方だけで体積が -35.6 % 変わり、``mesh_volume`` は黙って数字を返す)。
