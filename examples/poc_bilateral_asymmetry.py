...
def main():
    ...
    # 対角線(真値)からの離れ方が「利得」そのもの。残差最適面の線は真値より
    ...
    figs.save_plot("gain_vs_amplitude",
                   [("真値", _t, _t),
                    ...
                    ("ランドマーク面", _t, np.array([gains[a][2] for a in
                                                     (0.0, 0.4, 0.8, 1.6, 3.2, 6.4)]))],
                   xlabel="入れた非対称量 [mm]", ylabel="測った max [mm]",
                   title="面の決め方で利得が変わる",
                   caption="真値の線は利得 1.0。残差最適面は %.0f %% を系統的に取りこぼす。"
                           "ランドマーク面は利得 %.2f だが床が %.0f 倍高い。"
                           % (100 * (1 - g_opt), g_lmk, lm_floor / opt_floor))
    ...
    # ★穴 D の門をそのまま渡せる形にしておく(margin が先に潰れ、角度が後から跳ぶ)。
    figs.save_table("degenerate_margin",
                    ["標本", "候補 3 面のスコア", "margin", "真の正中面との角度"],
                    deg_rows, col_w=150,
                    title="margin は「面を外したこと」を先に知らせる",
                    caption="margin < 0.1 で採用を止めれば、66〜88 度の外しは弾ける。"
                            "op はこの値を返さない。")
