...
"""データセンターの温度場を疎なセンサから復元する —— 格子の死角が、ラック 1 本を消す。

サーバ室の熱管理は、ラック前面・背面に散らした温度センサを **3-D の温度場に補間して**
見ます(ASHRAE の熱環境ガイドラインが定める吸気温度の判定も、PUE を上げないための
冷気配分も、この場の上の量です)。ところが**センサの置き方**で、過負荷のラック 1 本が
場から消えます。この PoC は温度場を式で仕込んで、「消える量」を幾何から**先に予測して
から**測ります。

EXTEND: 実センサに差し替えるなら :func:`read` を BMS/DCIM の読み値に、
:func:`lattice` をフロアプランから起こしたセンサ座標に置き換えます。
**真値 (:func:`field`) は実データでは手に入りません** —— 実室では場の全点を知る手段が
無く(CFD も熱画像も、それ自体が別の誤差を持つ代替物です)、この PoC の「回復率」
「ピーク誤差」「位置誤差」は原理的に測れなくなります。さらに厄介なのは、崖の式
``exp(-3d²/8σ²)`` を評価するには**プルームの幅 σ が要る**のに、**その σ こそ疎な格子が
壊すもの**だという点です。現場でできるのは「σ をこれ以上細いとは仮定しない」という
上からの押さえだけで、必要間隔は σ から逆算する(``d ≤ 1.36 σ`` で半分)しかありません。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. **ゼロ点(全センサの平均を場全体に置く = 場は平らとみなす)は RMSE 2.192 °C で、
   過負荷ラックを 1 台も見つけない(見失い 3 台)**。いちばん良い RBF は 0.214 °C で
   **10.2 倍**良く、3 台とも見つける。以降の話はすべてこのゼロ点の上での話。
2. ★**崖は測る前に言える**。間隔 d の 3-D 格子では、ピークからいちばん遠い点は
   セルの中心 = ``d√3/2``。σ の等方ガウスなら見えるピークは
   ``exp(-(d√3/2)²/2σ²) = exp(-3d²/8σ²)`` 倍で、**半分に落ちる間隔は
   ``d* = σ√(8ln2/3) = 1.3596 σ``**(σ=0.55 / 0.35 / 0.22 m のラックで
   0.748 / 0.476 / 0.299 m)。この閉形式を先に印字してから測った ——
   **幾何だけを取り出した実測との差は 1.2e-15**。ここは検算であって発見ではない。
3. ★★**外れたのは閉形式ではなく「測り方」**。現場で測れるのは「復元した場のピーク」
   であって「そのラックの寄与」ではないので、**背景の復元誤差が同じ場所に載る**。
   素朴なピーク測定は予測を最大 **+0.2598** 超過し、その超過は「ラックを消した場」で
   測った床 0.2664 とほぼ同じ。σ=0.22 m のラックは d=1.20 m で幾何の回復
   **0.000014**(消滅)なのに素朴に測ると **0.1609** 残って見える ——
   **残っているのは背景の誤差**で、崖は実際より浅く見える。
4. ★**閉形式は曲線ではなく床**。ラックが格子のどこに落ちるかは設計で決められない。
   d=0.60 m で σ=0.35 m のラックの回復は、最悪位相(セル中心)0.3322 /
   現場の位相 0.8938 / 最良位相(センサ直上)1.0000 と 3 倍に跳ぶ。
   設計に使えるのは最悪位相の値だけで、「平均すればこれくらい見える」は
   **そのラックには通じない**。
5. **対照群 a: 乱数配置は死角を消さない**。同じ 1400 本を一様乱数で置くと、最近傍距離の
   中央値は 0.3222 m(Poisson の予測 0.3294 m)で、**格子の最悪距離 0.5196 m を
   6.17 % が超える**(予測 exp(-4πλr³/3) = 6.58 %、差 0.41 ポイント)。違うのは
   平均ではなく**言い切れるかどうか**: 12 通りの位相を振っても格子は幾何の下限を
   1 度も割らず(σ=0.22 m のラックで最悪 0.19977 ≥ 下限 0.06147)、乱数は 2 台で
   割った(0.03497)。★偶然だが、σ=0.55 m のラックでいちばん悪かった乱数の
   最近傍距離 0.51963 m は、格子の保証最悪 0.51962 m とほぼ同値だった。
6. **対照群 b: 崖の急さは補間法で動かない。動くのは持ち上がりのほう**。
   ``log(回復)`` を ``d²`` に当てた傾きは 最近傍 -3.0612 / 線形 -3.0612 /
   RBF -2.9679 /m² で、予測 ``-3/(8σ²) = -3.0612`` との差は最大 3.05 %。
   ★**ただし「崖の位置は手法で動かない」という予測は外した**: 半減間隔は
   最近傍 0.4777 / 線形 0.4777 / **RBF 0.6015 m** で 26 % ずれる。原因は係数
   **1.372 倍** —— 薄板スプラインは**内挿でありながら節点の値を超える**ので、山を
   一律に高く戻す。最近傍と線形が小数点以下まで一致するのは偶然ではなく、
   どちらも節点を超えないから(最悪位相の最大値はどちらも「いちばん近いセンサの読み」)。
7. ★★**「上向きに外れる手法は偽の峰も同じだけ立てる」——これも外した**。
   d=0.60 m での床(何も無いところに立つ旗)は 最近傍 0.975 °C / 線形 0.132 °C /
   RBF 0.164 °C で、**持ち上がる側のほうが低い**。信号 / 偽物の比は 3.07 → 22.63 →
   27.49。最近傍の床は雑音 0.15 °C の **6.5 倍**で、これは雑音ではなく
   **滑らかな背景勾配を階段で近似した段差**。この場面では上向きの外れに代償が無かった。
8. **物差しを 3 つ置くと、全部を同時に勝つ手法は無い**。RMSE は RBF(0.176 / 0.328 /
   0.742 °C)、ピーク誤差も RBF、**位置誤差は最近傍**(d=0.80 m で 0.150 m 対
   線形 0.660 m / RBF 0.187 m)。★RBF がピーク誤差で勝つのは原理的に正しいからでは
   なく、§6 の **1.37 倍の持ち上がり**がサンプリングの減衰を逆向きに打ち消しただけ
   (d=0.80 m で 最近傍 -5.43 °C → RBF -5.11 °C。どちらも真値に届いていない)。
9. **線形補間は端で必ず別の手法に化ける**。センサは部屋の内側にしか置けないので、
   評価点の 21.2 %(d=0.50 m)/ 39.7 %(0.80)/ 71.2 %(1.20)が Delaunay の凸包の
   外に出て最近傍にフォールバックする。「線形の結果」と呼んでいるものの 7 割が
   最近傍、ということが起こる。
10. **「偽の峰 0」は数え忘れではない**。しきい値を 32.0 → 28.0 °C に下げると同じ体積
    から偽の峰が 1 個出る(ホットアイルの帯そのものが峰になる)—— 門を壊して確かめた。

【グラウンドトゥルース】温度場を ``T(x,y,z) = 室温 24 °C + ホットアイルの帯
(y=4.2 m、幅 σ=0.9 m、+5 °C、上ほど熱い) - コールドアイルの床吹き出し
(y=1.4/7.0 m、幅 σ=0.7 m、-5.5 °C、``exp(-z/0.9)``) + Σ 過負荷ラックの等方ガウス``
で置いた(:func:`field`)。ラックは σ=0.55 m/+7 °C、σ=0.35 m/+9 °C、σ=0.22 m/+11 °C
の 3 台で、真のピークは 35.46 / 37.15 / 39.36 °C。プルームを**等方**にしたのは崖の
閉形式がちょうど成り立つようにするためで、実際のプルームは上へ伸びた異方形
(その場合 ``exp(-(d²/8)(1/σx²+1/σy²+1/σz²))`` に置き換わる)。センサは
真値 + ガウス雑音 σ=0.15 °C。回復率は**補間がセンサ値に線形**であることを使い、
「全部の場の復元」と「そのラックだけ消した場の復元」の差の最大として取り出した ——
こうすると背景の寄与が厳密に消え、閉形式が予言する量そのものになる。

来歴(公開文献のみ): ASHRAE TC 9.9 *Thermal Guidelines for Data Processing
Environments* —— 吸気温度の recommended 18-27 °C / class A1 allowable 15-32 °C
(この PoC の検出しきい値 32 °C はこの上限を取った)/ ISO/IEC 30134-2
...
def main() -> int:
    ...
    print("  * ★3 つ目の外し: 「持ち上がる手法は偽の峰も同じだけ持ち上げる」も"
          "外れた。最近傍の床は\n    %.3f °C(雑音の %.1f 倍)で、"
          "**滑らかな背景を階段で近似した段差**のほうが大きい。"
          % (mc["floor_c"]["nearest"], mc["floor_c"]["nearest"] / NOISE))
    print("  * 乱数配置は死角を消さない: %.2f %% が格子の最悪距離を超えた"
          "(Poisson の予測 %.2f %%)。格子は\n    幾何の下限を 1 度も割らず、"
          "乱数は %d 台で割った —— 同じ本数でも「言い切れるか」が違う。"
          % (100 * gr["frac"], 100 * gr["p_worse"], gr["below"]))
    print("  * 3 つの物差しを同時に勝つ手法は%s。"
          % ("無い" if mt["swapped"] else "在る"))
