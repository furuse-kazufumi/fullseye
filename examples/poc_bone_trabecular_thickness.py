...
1. **真値そのものが 2 つある**。閉形式(帯の幅の長さ加重平均)の Tb.Th は
   104.8 µm、画素から見た面積加重では 113.9 µm(+8.7 %)、距離変換の定義
   (最大内接円)で真値二値像から出すと 121.6 µm(面積加重比 +6.7 % = 交点の
   太り)、平板モデル 2·BV/BS は 118.4 µm(定義比 -2.6 %)。Tb.Sp は定義 803 µm に
   対し平板 450 µm(-44 %)—— 髄腔を「平板の隙間」と見なすから。**どの真値と
   比べるかで、しきい値の差より大きい 9〜16 % が先に動く**。
2. ★**解像度の崖は「平均」には来なかった。「分布」に来た**。画素 10 → 60 µm
   (1 骨梁 = 10.5 → 1.7 px)で、予想 1「細い骨梁が部分体積で消えて厚い側だけ
   残る生存バイアス」は外れ(消えた骨は 60 µm でも 0.0 %)。予想 2 の量子化
   (2r-1 の規約)は 60 µm で -29.5 %(予想 -25 %)。予想 3 の大津の太りは
   BV/TV +0.9 → +27.1 %。大津 → 直接法の Tb.Th 誤差が +7.3 % で済むのは
   **量子化の -29.5 % と太りが打ち消しているから**。60 µm では厚さの値が
   60 / 180 / 300 µm の 3 通りに潰れ、真値分布との重なりは 0.83 → 0.09。
3. ★★**雑音の崖は 2 方向から来る**。σ = 0.10 で斑点(骨と誤認した孤立画素)が
   0 → 7 個(予想 0.09 から、的中)、σ = 0.15 で骨梁の途切れが 0 → 2 本。
   斑点は Tb.Sp を**小さく**(σ = 0.20 で -79.3 %)、途切れは**大きく**引く
   はずだが、予想「σ = 0.40 で途切れ 13 件が Tb.Sp を正に転じる」は外れ、
   実測 -30.1 % —— 大津のしきい値が雑音で下がって帯が太る(BV/TV +110 %)
   ほうが勝つ。途切れの効果は平均には出ず、件数でしか見えない。
   ★opening r=1 は斑点を全部消すが**細い骨梁も切って**途切れが 5 → 16 本、
   骨片 20 個。面積オープニング 16 px は斑点だけを消す(途切れ 5 → 5、
   Tb.Sp 誤差 -79.3 → -9.7 %)。ぼかし → 大津は Tb.Th が +32.2 % 太る。
4. **しきい値 ±10 % で BV/TV と Tb.Th は同じ向きに動く**(0.40 → 0.60 で
   BV/TV +5.3 → -7.4 %、Tb.Th 直接 +6.5 → -8.2 %)。予想どおり —— 消える
   骨梁が無いので生存バイアスが働かず、単に帯が痩せる。Tb.Sp は -0.4 → +0.8 %
   でほぼ動かず(髄腔は帯より 7 倍広い)、Tb.N(平板)だけ -1.3 → -0.0 % で
   逆向き。
5. ★**ビームハードニング(カップ状バイアス)は「全体」では壊れず「場所」で壊れる**。
   β = 0.6 でも大津 1 本の BV/TV は -0.5 % —— 予想より頑健に見えるが、中心の
   Tb.Th 84 µm と縁の 112 µm で 25 % 違う(真値 113 / 110)。中心が痩せた分を
   縁が太って打ち消し、全体は合っているのに場所ごとには壊れている。
   retinex → exp → 大津で中心 95 / 縁 110 µm と**半分しか戻らない**(雑音を
   止めても 107 / 116 —— 窓 800 µm が骨梁間隔 1100 µm より狭く、局所平均に骨が
   混ざる)。★retinex のまま(log 域で)大津を取ると β = 0 でも BV/TV +19.6 %:
   しきい値が幾何平均 √(髄·骨) 側へ落ちて帯が太る。**バイアス補正は対数のまま
   二値化してはいけない**(exp で比に戻せば +0.6 %)。対照群(画素 20 µm、
   σ = 0.05、β = 0.3): 全部で BV/TV +1.6 % / Tb.Th -1.9 %、部分体積だけなら
   +0.2 % / +0.2 %。
