...
この PoC が示すこと(数字はすべて実行時に印字される実測値):
 ...
 1. ★ゼロ点(band 1 を 1 本のしきい値で 2 値化して数える)は、密接度 0.35 /
    塊の相関長 3 セルで **-4.2 ポイント**外す。同じ画像を線形混合分解に
    かけると **+0.02 ポイント**。対照群(標本化そのものの誤差 +0.01 ポイント)
    が効いていて、**この偏りは標本化ではなく数え方のせい**だと分かります。
 2. ★★**同じしきい値・同じ真値でも、塊の大きさで偏りが変わる**。密接度
    0.35 を固定し相関長を 3 -> 12 セルと振ると、硬い分類の偏りは
    **-4.2 -> -0.2 ポイント**。**周長率(混合画素の量)で説明がつき**、
    偏り = **-0.80 x 周長率 + 0.015**(決定係数 **R^2 = 0.984**)。分解の
    偏りは +0.02 〜 +0.10 ポイントで、周長にほとんど反応しません。
 3. ★★**偏りの符号は密接度で反転し、途中でゼロを横切る**。同じ塊の
    大きさ・同じしきい値で、密接度 0.15 -> 0.85 と振ると偏りは
    **-5.9 -> +6.0 ポイント**、**0.49 付近でゼロ**。そこは「正しい」のでは
    なく、**散らばった氷が食われる分と、散らばった水が塗り潰される分が
    釣り合っているだけ**です。密接度 0.5 付近だけ見て検証すると合格します。
 4. しきい値の位置は偏りをまるごと平行移動させる。0.30 -> 0.70 で偏りは
    **+13.1 -> -18.8 ポイント**動く(相関長 3 セル)。どこに置いても
    **塊の細かさに対する傾きは残る** —— しきい値の調整では模様依存は消えません。
 5. ★端成分の明るさが 5 % ずれると密接度は **-1.8 ポイント**(氷側を明るく
    見積もったとき)。分解の答えはほぼ (観測 - 水)/(氷 - 水) なので、
    **端成分の誤差をそのまま割り算で受け取る**。硬い分類はこの列で 1 桁も
    動きませんが、それは長所ではなく**そもそも割合を見ていない**からです。
 6. ★★**第 3 成分(薄氷)は必ず中途半端に配分される**。薄氷を混ぜた場面
    (厚氷 0.279 / 薄氷 0.071 / 水 0.650)で、2 端成分の分解は「氷 = 厚氷 +
    薄氷」を真値とすれば **-4.4 ポイント**、「氷 = 厚氷のみ」なら
    **+2.7 ポイント** —— **同じ数字が、真値の定義しだいで低くも高くも外れる**。
 7. ★**予想が外れた**。「薄氷を 3 番目の端成分に入れれば直る」と踏んで
    いたが、雑音 0.01 では **+3.8 / -2.3 ポイント**で改善しない。対照群
    (雑音 0)では **+0.01 ポイント**なので、悪いのは 3 端成分という考え方
    ではなく**バンドが 2 本しかないこと** —— 端成分 3 つ + 和 1 の制約は
    ちょうど決定系で、雑音を平均する余地がありません。第 3 成分に備えるには
    **アルゴリズムではなくバンドを増やす**のが筋でした。
