...
【★★ 番号つき所見(すべて実測。予想が外れたものはそう書く)】
  ...
  8. ★★ **時空間ボリューム (t, y, x) に 3-D 局所極大を掛けても検出器の
     ...
     **等方の立方近傍**なので、空間の 1 画素と時間の 1 フレーム(= 1.2 px
     の移動)を同じ物差しで比べる。min_distance=1 では真値の **137 %** も
     出るのに適合率は 67 %(出た点の 1/3 は粒子でない場所)、
     min_distance=3 では再現率 39 %。**どこにも正解が無い。**
     時間は空間ではない —— 体積として扱ってよいのは「見る」ときで、
     「測る」ときは軸ごとに物差しを変えないと壊れる。
