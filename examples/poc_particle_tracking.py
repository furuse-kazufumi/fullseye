...
【★★ 番号つき所見(すべて実測。予想が外れたものはそう書く)】
  ...
  8. ★★ **時空間ボリューム (t, y, x) に 3-D 局所極大を掛けても検出器の
     ...
     50.5 %** しかない —— 半分のコマで粒子が消える。**対照群で原因を
     確定した**: 粒子を止める(σ_step = 0)と再現率 99.9 %。つまり犯人は
     雑音ではなく**動き**。等方の立方近傍は時間方向にも厳密な最大を要求
     するので、動く粒子の中心画素の標本値が揺れると「前後のコマより高い
     コマ」しか残らない(無相関なら 1/3)。空間の 1 画素と時間の 1 フレーム
     を同じ物差しで比べている。**時間は空間ではない。**
