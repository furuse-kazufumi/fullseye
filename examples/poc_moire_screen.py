...
間隔だけで決まる閉形式で、実測 FFT と FFT 分解能の範囲で合う。ただし
**高調波のうなりも一緒に予測しないと危ない** —— 基本波のうなりが安全な
位置でも、3 次のうなりがムラの帯に落ちることがある」。

【所見(すべて下の節で実測。★ は重要、★★ は最重要)】

 1. 閉形式が効く。表示の空間周波数 k(カメラ画素あたりの縞数)に対して、
    m 次高調波のモアレ周波数は ``wrap(m·k)``([-0.5,0.5] へ折り返し)。
    k=3.05 で予測周期 20.00 / 6.67 / 4.00 px に対し実測 19.69 / 6.65 / 4.00 px
    (差は 512 点 FFT の分解能 1/512 cyc/px ぶん)。カメラを 0〜8 度回した
    ときの周期と向きの動きも同じ式で追える(6 節。0.5 度で 20.0→17.7 px)。

 2. ★★**2 つの失敗が打ち消し合う**。低域通過でならしてからムラの振幅を
    測ると、σ を上げるにつれて「モアレの漏れ込み(過大評価)」が減り
    「本物のムラの減衰(過小評価)」が増える。σ=8 px で**合計誤差 +0.9 %**
    ——数字だけ見れば完璧だが、内訳は漏れ +8.3 % / 減衰 -7.4 %。
    **対照群を 2 本(縞だけ / ムラだけ)置かないとこの打ち消しは見えない**(3 節)。

 3. ★★**3 つの手法が別々の理由で壊れる**(5 節)。
    (a) 平滑化は δ が f_m の数倍以内になると壊れる(δ=0.03 で +49 %)——
        これは光学の限界。
    (b) 素のノッチは **δ·L が整数のときだけ**効く。δ=0.25(128.00 ビン)で
        +0.7 %、δ=0.12(61.44 ビン)で +39.1 % —— 原因はスペクトルリーケージで、
        **解析窓の長さという software 側の問題**。窓長を δ·Lc が整数になるよう
        選ぶと +1.7 % まで直る(撮り直し不要)。
    (c) 窓長を合わせたノッチは、**2 次元での**ベクトル距離がノッチ半径を
        下回ると壊れる。δ=f_m のときでもムラの向きが 30 度違うので距離
        0.00404 が残り、-25 % で踏みとどまる。

 4. ★**高調波のうなりが本命、ただし画素開口が効く**。k=0.67(表示 1 縞を
    1.5 画素で撮る)は基本波のうなり 0.33 で安全に見えるのに、3 次のうなりが
    0.01(周期 100 px)でムラの帯に落ち、振幅はムラの 2.3 倍。実測の漏れは
    **真値の +203 %**。同じ 3 次でも k=3.05 では無害 —— 画素開口の sinc が
    9 次相当の周波数を潰すため。**k が小さいほど高調波の罠が効く**(7 節)。

 5. ★**撮影条件を計算で決められる**。k を整数にすると全高調波のうなりが
    同時に直流へ落ち、縞は一様になる(実測: 像の標準偏差 1.7e-14、
    縞の振幅は 0.0298 だった)。基本波を 1/1000 にする光学ローパス
    σ_opt = sqrt(ln(1000)/(2π²k²)) = 0.194 px も閉形式で出て、実測の漏れは
    真値の 0.01 %(7 節)。

 6. `fs.apply(..., "gauss_image")` の σ は **0.3〜3.0 に固定**で、ムラの
    スケール(σ≈8 px)の平滑化ができない(a=1.0 でムラ推定 +123 %)。
    `fs.ledger.background_flatten`(2 次曲面フィット)は 4 周期のムラを
    表現できず振幅の 4 % しか拾えない。周波数ノッチの op は 3 層のどこにも
    無い。8 節の「道具の穴」に実測つきで置いた。
...
def section5_separability():
    ...
    print("         区別できない(δ<=0.03 で誤差 +50 %s 超)。**光学の限界**。" % "%")
...
def section8_tool_gaps():
    ...
    f0 = 0.05
    a_h = np.cos(2 * np.pi * f0 * _J)
    a_d = np.cos(2 * np.pi * f0 * (_J + _I) / np.sqrt(2.0))
    rp_h = np.asarray(fs.radial_power_spectrum(a_h))
    rp_d = np.asarray(fs.radial_power_spectrum(a_d))
