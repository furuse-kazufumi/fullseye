...
上下水道やプラントの配管は、管内を走るカメラ・距離センサで内面を撮り、円筒を
開いた **2-D の展開図**にして腐食・減肉・溶接部を見ます(EN 13508-2 の目視符号、
ASME B31G の残存強度評価はどちらもこの図の上の量を使う)。ところが**展開の中心軸を
どこに取るか**で、腐食していない管に腐食が生まれ、本当の減肉が消えます。この PoC は
真値を自分で仕込んで、その「生まれる量」と「消える量」を別々に数えます。

EXTEND: 実センサに差し替えるなら :func:`survey` の戻り値 ``dist``(各 (z, θ) の
距離 [mm])を実測の距離画像に、``z_hit`` / ``th_hit`` を「そのビームが当たった点の
位置」に置き換えます。**真値の側 (:func:`loss_parts`) は実データでは手に入りません**
—— 実管では超音波肉厚計の点測定か、切り出して断面を測るしかなく、この PoC の
「種類ごとの検出率」は原理的に測れなくなります。だから合成でやる価値があります。
センサの軌跡(オフセットと傾き)は実機では走行体の姿勢センサで推定しますが、
その推定誤差がそのまま §7 の「多項式次数」の選択誤差になります。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. **ゼロ点(展開図の距離をそのまま呼び内半径と比べ、0.5 mm のしきい値)は、
   軸が合っていれば良く当たる**。孔食 99.1 % / 全周減肉 96.3 % / 管底腐食 98.4 %
   を検出し、偽陽性 0.00 %、体積誤差は -23.0 / -11.6 / -8.5 %。**問題は判定器では
   なく、その前の「軸をどこに置くか」にある**。
2. ★**軸が 4.0 mm ずれるだけで、腐食ゼロの真円の管の 44.8 % が「減肉」になる**。
   偽の減肉体積 116378 mm^3 は、本物の孔食 777 mm^3 の **149.9 倍**。
   幾何で先に予測してから測った —— 中心が e ずれた円は展開すると**振幅 e の
   1 周期の正弦波**になるので、旗が立つ割合は ``arccos(t/e)/π``。予測(厳密)と
   実測の差は最大 **1.84 ポイント**。★崖は**しきい値そのものの所**に立つ:
   0.40 mm で 0 %、しきい値と同じ 0.50 mm で 4.95 %(ここだけ予測 0.00 % を外す
   —— 雑音 σ=0.05 mm が旗を立てている)、0.55 mm で 11.78 %。
3. ★**軸ずれは 1 周期だけでなく、2 次で「全周」にも効く**。平均半径が
   ``-e²/(4R)`` だけ縮む(予測と実測の差は最大 **0.0016 mm**)。オフセット 6 mm は
   **全周減肉 0.60 mm の 30 % を打ち消す** —— 管が細く見えるので減肉が隠れる。
4. **1 周期(軸ずれ)と 2 周期(楕円化)は分離できる**。傾き 1.6 度でも 2 周期への
   漏れは 0.0405 mm(楕円化 1.2 mm の 3.4 %)。傾きの効きは 1 周期が **z に線形**
   (実測の傾き 0.0279、予測 tan α = 0.0279 mm/mm、差 0.0 %)。★2 周期への漏れの
   予想は 1 度外れた —— 斜め切りの楕円 R(sec α - 1)/2 = 0.0098 mm だけだと思って
   いたが実測 0.0405 mm。傾きが作る横ずれの 2 次項 <e²>/4R = 0.0293 を足して
   0.0390 でようやく合う(残差 4 %)。
5. ★★**分離できないのは軸ずれと「管底腐食」**。下水管でいちばん多い腐食は管底の
   三日月形で、その **79 % が k=1 に入っている** —— 軸ずれと同じ場所。だから
   1 周期を消す補正(E1)は偽陽性を 45.37 → 34.56 % に落とす代わりに、
   **管底腐食の検出率を 100.0 → 34.4 %、体積を -68 % にする**。
6. ★現場の定番(E2 = スライスごとに円をあてはめて残差を見る)は**さらに全周減肉を
   食う**(検出率 52.0 → 36.1 %、体積 -47 %)—— 平均半径を捨てるから。同じ理由で
   溶接ビードの高さが 1.58 → 0.01 mm(真値 1.50)になり、**全周に一様な形は
   何であれ見えなくなる**。
7. ★★**「軸をどこまで自由に動かしてよいか」という 1 本のつまみに全部が並ぶ**。
   1 周期の z 依存を n 次多項式に縛ると、n=0(平行移動)/ 1(傾き)/ 2(たわみ)/
   free(毎スライス自由)。★**予想は「上げるほど本物が消える」だったが、実測は
   両端で落ちる U 字**だった: n=0 は管底腐食 8.1 %(傾きを表せず、残った 1 周期が
   腐食を打ち消す = **補正しすぎではなく足りなくて消える**)、n=1 で 92.4 %、
   n=2 で 85.5 %、free で 48.6 %。偽の減肉は単調に減る(22.19 → 11.25 → 0.00 %)。
   **いちばん良い次数は「軸は直線、管はたわむ」という物理そのもの**で、
   E4(n=2)は 孔食 99.1 % / 全周減肉 76.2 % / 管底腐食 85.5 % を残して偽陽性
   0.76 %。孔食(局所)はどの次数でも 79 % 以上残る —— 消えるのは 1 周期に載って
   いる欠陥だけ。
8. ★**その E4 にも崖がある**。管底腐食が長くなるほど「軸のずれ」に見え、管長いっぱい
   (300 mm)まで伸びると検出率 86.6 → 10.9 %、推定した最深部 0.96 → 0.32 mm
   (真値 1.5 mm)。**z によらず一定な腐食は軸のずれと幾何的に区別できない** ——
   分けているのは「腐食は z で変わる」という仮定だけなので、管を長く取るしかない。
9. **同じ管を CT 側(ボクセル 1.5 mm)から測ると別の所で壊れる**。左右対称のはずの
   全周減肉が 4.91 / 6.04 mm と **1.13 mm** 食い違い(真値の減肉 0.60 mm より
   大きい)、esdf は健全部と減肉部を同じ -3.35 mm と答える。距離センサ側は同じ
   0.60 mm を見つける —— **どちらが良いかではなく、崖の場所が違う**。

【グラウンドトゥルース】内面半径 ``R_in(z, θ) = R0 + 楕円化 + 減肉`` を式で置き、
軸のたわみも ``z`` の 2 次式で与えた(:func:`loss_parts` / :func:`ovality` /
:func:`axis_offset`)。欠陥は 孔食(ガウス、深さ 2.5 mm)・全周減肉(0.60 mm)・
管底腐食(最深 1.5 mm、半周のコサイン)・溶接ビード(内側へ 1.5 mm)。体積は
``∫∫ (R_in² - R_ref²)/2 dθ dz`` を細かい格子で積分した値(孔食は閉形式
``2π σ_z σ_s D`` = 754.0 mm^3 と数値積分 776.6 mm^3 が +3.00 % で一致 ——
差は 2 次項 ``D²/2R`` の分)。測定はセンサ位置から内面への光線を二分法で解いた
厳密値に、雑音 σ=0.05 mm・欠測 3 %・鏡面の欠測帯を掛けたもの。

来歴(公開文献のみ): ISO 12181-1:2011 *Geometrical product specifications (GPS) —
Roundness* —— 真円度の調和成分(1 次 = 偏心、2 次 = 楕円)/ EN 13508-2:2003+A1:2011
*Investigation and assessment of drain and sewer systems outside buildings —
Visual inspection coding system* / ASME B31G-2012 *Manual for Determining the
Remaining Strength of Corroded Pipelines* —— 減肉の深さと長さから残存強度を出す規格。
"""
...
def section_correction() -> dict:
    ...
    print("  ★E3(軸は直線だと**先に決めて** 1 周期の z 依存を 1 次に縛り、"
          "楕円化の\n     2 周期も 1 次に縛る)は 管底腐食 %.1f %% / 全周減肉 "
          "%.1f %% / 孔食 %.1f %% を\n     残したまま偽陽性 %.2f %%。"
          % (100 * scs[3]["cres_rate"], 100 * scs[3]["band_rate"],
             100 * scs[3]["pit_rate"], 100 * scs[3]["false"]))
    print("     E1 と E2 の偽陽性が %.1f / %.1f %% と高いままなのは、"
          "どちらも 2 周期(楕円化 %.1f mm)を\n     触らないから —— "
          "**1 周期だけ消すのは「軸のずれだけ直す」ことであって、"
          "「形を直す」ことではない**。"
          % (100 * scs[1]["false"], 100 * scs[2]["false"], OVAL_A))
    print("  ★E3 に残る偽陽性 %.2f %% は**管のたわみ %.1f mm**(1 周期が z の"
          "2 次で動く)。\n     E4 で 2 次まで許すと偽陽性 %.2f %%、"
          "代わりに管底腐食が %.1f → %.1f %% へ。\n     **物理に合わせた分だけ"
          "偽物が減り、合わせすぎた分だけ本物が減る**。"
          % (100 * scs[3]["false"], BEND, 100 * scs[4]["false"],
             100 * scs[3]["cres_rate"], 100 * scs[4]["cres_rate"]))
...
# --------------------------------------------------------------------------- #
# 8. E4 の崖 —— 管底腐食が管いっぱいに伸びると軸ずれと区別できない              #
# --------------------------------------------------------------------------- #
...
def main() -> int:
    ...
    print("まとめ")
    print("=" * 78)
    i4 = sw["offs"].index(4.0)
    print("  * 軸が %.1f mm ずれると、腐食ゼロの管の %.1f %% に減肉の旗が立つ"
          "(幾何の予測との差 %.2f ポイント)。" % (4.0, sw["meas"][i4], sw["err"]))
    print("  * 1 周期を消せば偽物は消えるが、管底腐食の %.0f %% も k=1 に居るので"
          "一緒に消える(検出率 %.1f → %.1f %%)。"
          % (100 * sp["cres_k1_share"], 100 * co["scs"][0]["cres_rate"],
             100 * co["scs"][1]["cres_rate"]))
    print("  * 軸の動きを**物理どおりに縛る**と両方残る"
          "(E4: 管底腐食 %.1f %%・偽陽性 %.2f %%)。\n    "
          "ただし腐食が管長いっぱいまで伸びると %.1f %% まで落ちる"
          "(最深部 %.1f → %.2f mm)。"
          % (100 * co["scs"][4]["cres_rate"], 100 * co["scs"][4]["false"],
             ce["rate"][-1], CRES_D, ce["depth"][-1]))
