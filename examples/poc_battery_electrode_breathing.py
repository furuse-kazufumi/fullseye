...
1. **ゼロ点を 2 つ置く**。(Z1) 二値化して層の画素数を数えると、負極の
   +0.58 µm は量子化 1 px = 1.0 µm に埋もれて、**12 層中 0 層**が真値の
   3 分の 1 以内に入らない。(Z2) 固定しきい値 55 % の交差を線形補間する ——
   **これもサブピクセルなので、雑音なしなら +0.004495(真値 +0.004450)と
   ほぼ当たる**。問題は精度ではなく、4 章。
2. **提案(`fs.ledger.measure_pos` の勾配ピーク + 全境界の回帰)は、格子の上では
   完璧に見える**。13 本の境界位置の誤差は最大 **1.8e-14 px**、層ごとの伸びも
   負極 +1.014 / +0.958 / +1.013 %(真値 +1.00 %)、正極 +0.206 / +0.207 /
   +0.188 %(真値 +0.20 %)、セパレータ 0.000 %(真値 0)。
3. ★★**その「完璧」を疑ったら、検査のほうが甘かった**。2 章の真値は 12, 70,
   88 ... と**画素の中心にぴったり**乗っていた。積層まるごとを小数画素だけ
   0 → 0.95 とずらして測り直すと(20 通り x 13 本 = 260 点)、位置の誤差は
   **RMS 0.0088 px / 最大 0.0122 px** —— ずれ 0 のときだけ 1.8e-14 px で、
   ずれ 0.25 では -0.0119 px。放物線あてはめの**ピークロッキング**です。
   これは負極 1 層のひずみに **3.0 %** 効くので、2 章で層別が +1.014 /
   +0.958 % とばらついたのは雑音ではなく**これ**でした。
   ★`fs.ledger.piv_peak_locking` の指標 c0 は**この大きさには気づかない**
   (推定 1.37 / 真値 1.60、どちらも「一様」の 1 前後)—— op 自身の docstring が
   「決定的な診断は小数部を 0→0.9 まで振ること」と書いているとおりでした。
4. ★★**対照群がゼロ点を殺す**。伸びゼロのまま照明だけ変えると Z2 は
   **偽の伸び**を出す: オフセット +0.10 で **-1.07e-02 = 真値の 2.4 倍を逆符号で**、
   傾斜 ±30 % で -1.18e-04、ぼけ 1.6→2.4 px で -3.77e-04、ゲイン x1.30 では
   **交差が 12 本から 6 本に落ちて測定不能**。同じ条件で `measure_pos` は
   0.0 / 0.0 / -2.2e-06 / -8.5e-12 —— **勾配のピークは I → aI + b で厳密に
   動かない**から。★予想は 2 段で外れた: (1) 「ぼけが増えると隣の境界が
   引き合う」は σ=2.4 px では **-8.5e-12** で何も起きず(最も狭いセパレータ
   18 px でも 7.5σ 離れている)、5.0 px でようやく -8.5e-05(真値の 1.9 %)。
   (2) 6.0 px では偏りが育つ前に**境界が 1 本消えて測定不能** ——
   引き合いより先に検出が死ぬ。
5. ★**雑音の効き方は先に計算できる、ただし割引つき**。エッジ位置の
   Cramer-Rao 下界 σ_x = (σ_n/√W)/h・√(2√π σ_b) を先に書いてから測ると、
   σ_n = 0.02 / 0.05 / 0.10 / 0.20 で **実測 / 下界 = 2.47 / 2.62 / 2.54 / 2.59**。
   放物線あてはめは下界の約 2.6 分の 1 の効率で、**比は雑音でほとんど動かない**
   (係数 1 つで予測に使える)。★層別のばらつきの予測(σ_x√2/層厚)は
   3.28e-03 対 3.50e-03 と当たるのに、**積層全体の予測は 1.7 倍甘い**
   (6.51e-04 対 3.75e-04)—— 回帰は x 側にも誤差があるので、教科書の
   最小二乗の分散式では足りない。
6. ★★**いちばん欲しい数字が、いちばんばらつく**。σ_n = 0.10 で、積層全体の
   ひずみのばらつきは **6.5e-04**(真値 4.45e-03 の 15 %)なのに、
   負極 1 層だけのひずみは **3.3e-03**(真値 1.00e-02 の 33 %)—— **5.0 倍**。
   てこの腕が 480 px から 58 px に縮むだけの幾何で、推定器の出来ではない。
7. ★**全体のひずみにも 2 通りあり、精度と偏りが逆を向く**。端から端の比は
   定義上不偏(実測 +0.004434、残る -0.37 % は 3 章のピークロッキング)だが
   境界 2 本しか使わない。13 本の回帰は雑音に強い代わりに、**ひずみが
   一様でないと -2.65 % 偏る**(回帰は境界を重心からの距離で重みづけるが、
   真の平均は層厚で重みづける)。★対照群で確かめた: 全層に一様 0.45 % を
   掛けると回帰の偏りは **+3.8e-06 に消える**。σ_n = 0.10 では回帰の
   ばらつき 6.51e-04 に対し端から端は 1.39e-03(回帰が 53 % 小さい)、
   偏りは -2.44e-04 対 -1.82e-04 —— **どちらが良いかは雑音次第**。
8. ★★**崖は 2 段で、予測を外した**。コントラストを落とすと、まず数字が
   静かに悪くなる(CNR 7.6 → 3.8 でばらつき 2.26e-04 → 5.13e-04、予測どおり
   1/c に比例)。ところが **CNR 5.3 で境界の本数が壊れ始め**(24 回中 1 回、
   CNR 2.7 で 13 回、CNR 1.9 で 19 回)、予測した 3σ の崖(CNR 0.96)より
   **5.5 倍手前**で死ぬ。★さらに **CNR 1.9 の行はばらつきが 4.53e-04 と
   逆に良く見える** —— 数え損ねた回が落ちて易しい乱数だけが残るから。
   壊れ方は「足りない / 余分 / 数は合うが誤差」に分けて数えること。
...
def main() -> None:
    ...
    print("    本数の崩壊は CNR %.1f から始まり、予測した 3σ の崖 CNR %.2f より"
          " 手前に来た。" % (cliff["cnr_break"], cliff["cnr_pred"]))
