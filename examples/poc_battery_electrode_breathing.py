...
def section_controls() -> dict:
    ...
    print("  勾配ピークがゲインとオフセットに厳密に動かないのは、"
          "|dI/dr| のピーク位置が\n     I -> aI + b で変わらないから(a > 0)。"
          "傾斜と大きなぼけはその限りではない:")
    print("     傾斜 %+.2e / ぼけ 2.4 px %+.2e / ぼけ 6.0 px %+.2e"
          % (out["傾斜 +-30 %"][0], out["ぼけ 1.6->2.4 px"][0], out["ぼけ 1.6->6.0 px"][0]))
    print("  ★予想が外れた: 「ぼけが増えると隣の境界が引き合って偽の伸びが出る」と"
          "\n     踏んだが 2.4 px では %+.1e で何も起きない(最も狭いセパレータ"
          " 18 px でも %.1f σ 離れている)。σ=6.0 px にして初めて %+.1e。"
          % (out["ぼけ 1.6->2.4 px"][0], 18.0 / 2.4, out["ぼけ 1.6->6.0 px"][0]))
