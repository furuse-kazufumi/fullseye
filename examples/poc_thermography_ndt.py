...
def tsr_depth(ts, cube, order=9):
    ...
    厳密解に対して 0.6 % 以内で成り立つことを確かめてある(最悪 0.34 %、L=3.0 mm)。
...
def section1_check():
    ...
    print("     d = √(π α t*) は厳密解に対して**0.6 % 以内**で成立する")
    print("     (最悪は L=3.0 mm の比 0.9966 = 0.34 %。薄いほど良く当たる)。")
...
def section4_aspect(depth_map, ts, masks, d_hat, cube, sound):
    ...
    print("     深い側で +59 %〜+612 %。ただし直径 4 mm の浅い 2 つは −4 % と")
    print("     −12 % で、外れる向きは一方向ではない)。つまりさきほどの")
    print("     『縦横比の限界』は物理の限界では")
