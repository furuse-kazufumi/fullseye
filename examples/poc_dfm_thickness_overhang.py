...
# --------------------------------------------------------------------------- #
COUPON = ((28.0, 32.0), (3.0, 12.0), (12.0, 17.0))     # 薄壁 2 枚 + スロットを含む小片


def coupon(h):
    """薄壁 2 枚とスロットだけを切り出した小片(占有・グレー値・SDF)。
    ...
    return sdf, np.asarray(L.sdf_to_occupancy(sdf, iso=0.0), bool), gray, res
...
def section_orientation():
    ...
    dirs = [("Z+(設計どおり)", (0, 0, 1)), ("Z-(裏返す)", (0, 0, -1)),
            ("Y+", (0, 1, 0)), ("X+", (1, 0, 0)),
            ("Z から X へ 45 度", (1, 0, 1)), ("Z から Y へ 45 度", (0, 1, 1))]
