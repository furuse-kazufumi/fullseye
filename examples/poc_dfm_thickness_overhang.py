...
2. ★**「最大内接球なら 1 voxel 刻みになる」という予想は外れた**。予想は h 刻み、
   実測も **2h 刻み**(掃引 12 点すべてで 2h の倍数、しかも侵食と 1 つ残らず同じ値)。
   2 値格子の距離変換は「占有ボクセル中心から自由ボクセル中心まで」を測るので、
   半径ではなく直径が量子化される —— 内接球に替えても得しない。
3. **グレー値の探針だけが刻みを持たない**。3 voxel 以上の 5 点で誤差の中央値は
   侵食 0.205 mm / 内接球 0.205 mm に対し探針 0.011 mm。ただし
   ★**予想「2 voxel を切ると探針は壁を見つけられなくなる」も外れた** ——
   実測は見つけられないのではなく**見つけたと言って間違える**。T/h = 1.6, 1.5 で
   誤差 +0.992, +4.602 mm、返る壁の本数が 2 から 1 に落ち、
   壁 + スロット + 壁 = 4.500 mm を「肉厚」として返す。空を返して落ちるより厄介。
4. ★★**サポートが要る面積は、しきい値 45 度の両側で段差になる**。解析値で
   44.9 度なら 185.22 mm^2、45.1 度なら 576.10 mm^2 —— **0.2 度で 3.11 倍**。
   部品の中に「ちょうど 45 度の斜面」が 390.32 mm^2 あるからで、
   **推定誤差が 0.1 度でもあれば答えは 3 倍動く**。
5. ★**その段差を、丸めた形はきれいに消してしまう**。等値面の取り方だけを変えた
   3 条件(2 値から / 距離場から / 平滑化してから)で、45 度ちょうどの面の帰属が
   変わる。距離場から取ると段差は 390.88 -> 0.87 mm^2、つまり **100 % 消える**
   (44.9 度でも 546.39 mm^2 = 解析値 185.22 mm^2 の 195 % 過大)。平滑化
   sigma=1.5 voxel でも 390.88 -> 345.78 mm^2 と 12 % 削れる ——
   **「合わせた分だけ欠陥が消える」の面積版**。数字は滑らかになるが、
   造形機は丸まっていない形を作る。
6. **肉厚の誤差 [mm] と NG 面積 [mm^2] は連動しない**。同じ h=0.500 mm で
   肉厚の誤差は +0.500 mm(33.3 %)なのに、NG 面積の誤差は 11.1 %。
   逆に h=0.250 mm では肉厚の誤差がゼロでも NG 面積は 4.8 % ずれたまま。
   **片方の精度をもう片方の根拠にしてはいけない**。
7. **工具の入る隙間は「入る/入らない」の 2 値なので、刻みがそのまま誤判定になる**。
   設計 1.500 mm のスロット(入る工具半径の上限 0.750 mm)を測ると、誤判定は
   ★**両方向に出る**。粗さ 0.550 mm では隙間が 1.100 mm に痩せて見え、
   本当は入る r=0.60 の工具を落とす(安全側)。粗さ 0.500 mm では逆に
   2.000 mm へ**太って**見え、入らない r=0.80 の工具を通してしまう(危険側)。
   自由側の距離変換は「自由中心から占有中心まで」なので、壁が voxel の境目を
   またぐと隙間は 1 voxel ぶん広がる。
8. ★**造形方向を振っても、予想に反してほとんど効かない**。6 通りの解析値で
   最良は X+(横倒し)の 493.74 mm^2 = Z+ の 0.86 倍にしかならず、最悪の
   Z-(裏返す)は 2642.71 mm^2(4.59 倍)。板の裏 2349.7 mm^2 が Z+ 以外の
   どの向きでも寝た面になるため。★効いたのは向きではなく**設計**で、三角補強の
   斜面を 45 度から 50 度へ 5 度ずらすだけで 576.10 -> 158.94 mm^2(0.28 倍)。
   しきい値に貼りついた 1 面をずらすほうが、向きを 6 通り試すより効く。
