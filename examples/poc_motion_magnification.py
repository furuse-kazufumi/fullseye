...
def main():
    ...
    if figs.enabled():
        # 図: 1 行だけを時間方向に積んだスリット像(縦 = 時間)。0.5 px の振動は
        #     縞が真っ直ぐに見えるが、25 倍にすると縞のうねりとして目で見える。
        #     どちらのパネルも自分の値域で塗るので、見えているのは形の違い。
        v05 = make_clip([(0.5, FREQ, 0.0)])[0]
        m25 = M.motion_magnify(v05, 25.0, *BAND, FPS)["video"]
        figs.save_grid("slit_scan", [v05[:, H // 2, :], m25[:, H // 2, :]],
                       ["生 0.5 px", "25 倍 12.5 px"],
                       title="行 %d を時間方向に積む(縦 = 100 フレーム)" % (H // 2),
