...
4. **壊れる境界を数字で** —— 拡大率をいくら上げても壊れない。壊すのは
   ...
   (b) 被写体テクスチャが広帯域であること(利得が 4〜13 % 不足する。
   同じ映像でも「測る」ほうは 0.2 % 以内で当たる)、(c) 雑音。
...
def main():
    ...
    print("    → 拡大の利得は 1 割方不足し、映像は理想並進から 10〜30 % ずれる。")
    print("       1 本の副帯域に複数の空間周波数が同居すると、その和の位相は")
    print("       変位に線形でなくなるため。位相ベース処理に内在する制約で、")
    print("       調整では消えない。")
    print("    → 同じ映像でも**計測**は 0.2 % 以内で当たる。副帯域ごとに局所")
    ...
    print("      では 10〜30 % ずれているのに True のままだった。真に危険なのは")
    print("      位相増分の大きさではなく**副帯域あたりの空間周波数の数**で、")
    print("      それを見る指標が返り値に無い。")
    print("  (3) クリップ長が T*f/fps を整数にしていないことを誰も警告しない。")
    print("      3.7 Hz を T=105 で撮ると隣の帯域が 3.5 dB しか落ちず、漏話は")
    print("      -78 dB から -42 dB へ悪化し、尖頭値振幅は +14 % 過大に出る。")
    print("      例外も NaN も出ず、band_snr の band_bins からも読み取れない。")
    print("      _require_band はビンが 1 本でもあれば通す設計なので、ここに")
    print("      「ビンからの外れ量」を足せる余地がある。")
