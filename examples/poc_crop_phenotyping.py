...
"""作物の 3-D 表現型 —— 葉が重なると、隠れる前に**畳まれて**測れなくなる。

精密農業では点群や画像から**葉面積指数(LAI)・草高・葉角・受光**を測って生育と
収量を推定します。ところが群落が閉じると、上から見た植被率は飽和し、真の葉面積との
関係が崩れます。**どこで崩れるかを、真値を式で持ったまま**測ります。

EXTEND: 実測に差し替えるなら :func:`make_canopy` が返す辞書(葉 1 枚ごとの着生点・
方位・着生角・曲率・長さ・幅)を、3-D 計測した株の骨格に置き換えます。そのとき
**測れなくなるのは真値のほう** —— 葉面積 pi/4·L·W も、投影係数 G(theta) も、
面積加重の葉角分布も、この表があるから閉形式で出せています。実圃場では
:func:`canopy_buffers` の戻り値(最上面の高さ・法線・重なり枚数)のうち
**「重なり枚数」だけが原理的に取れません** —— この PoC の主役がそれです。
区画は周期境界にしてあるので縁の効果はゼロ。実圃場では縁が効きます。

この PoC が示すこと(数字はいずれも実行時に印字される実測値):

1. **ゼロ点(植被率 -> Beer-Lambert 反転)は薄い群落でだけ当たる**。教科書の
   k=0.5 で LAI 0.30 を +47.9 %、LAI 4.85 を -30.5 %。★**k を真値
   (G(0)=0.7276、閉形式)に直しても救われない** —— +1.7 % -> -52.2 % と
   むしろ悪化する。偏りは消光係数ではなく**群落の並び方**から来ている。
2. ★★**遮蔽は天頂から見た植被率を 1 ビットも変えない**。壊しているのは
   **投影が畳む分**のほうで、LAI 4.85 では 1 セルを平均 3.49 枚の葉が覆うのに
   1 枚として数える(畳んだ分 2.68 投影 m^2/m^2)。遮蔽が効くのは
   **別の推定器**(見えた葉を足す式)で、そちらは真値の 78.3 % を落とす
   (奪われた分 3.80 葉 m^2/m^2)。**2 つは単位からして別の量**で、
   同じ数字の言い換えではない。
3. ★**遮蔽の天井は閉形式で予測できる**。上から見える葉面積は
   ∫exp(-k s)ds = (1-e^-kL)/k なので **1/k = 1.374 m^2/m^2 が上限**。実測は
   LAI 7.28 で 1.135(予測の 0.825 倍)—— 予測は上限として正しく、足りない分は
   裸地が残るぶん。植被率まで込みの精密版 C/k = 1.181 なら比 0.960。
   **LAI を 4.0 倍(1.83 -> 7.28)にしても見える葉は 1.41 倍にしかならない。**
4. ★★**クランピングの予想が外れた**。「葉先は 0.61 m 届き条間は 0.75 m だから
   株冠は重なりきる -> Omega ≒ 1」と踏んだが、実測 Omega は LAI 0.30 の 1.017 から
   LAI 7.28 の **0.370** まで落ちる。株冠が重なっていても**株の中で葉どうしが
   同じ地面を何度も覆う**からで、★**対照群がそれを名指しする**: 葉の方位を
   互生(1 つの鉛直面)から乱数に変えるだけで、同じ LAI 4.85 のまま
   Omega 0.478 -> 0.946、推定 LAI 2.32 -> 4.59 と真値へ戻る。
5. ★**推定 LAI の天井も予測できる**(2 段階クランピング、Nilson 1971)。1 株の影を
   実測して 0.308 m^2、27 株 / 5.06 m^2 = 1.642、株どうしを Poisson で重ねると
   植被率の天井 0.807 -> **推定 LAI の天井 2.26**。実測は LAI 7.28 でも 2.70
   (予測比 1.195)。**葉を何枚足しても推定はこの近くで止まる。**
6. ★**崖は点密度の対数でしか動かない**。LAI 差 0.5 を判別率 84 % で見分けられる
   上限は 25 / 100 / 400 / 1600 点/m^2 で 1.52 / 3.33 / 5.16 / 7.28 ——
   **4 倍にするたびに +1.92 しか伸びない**。しかも予測(d'=1、Omega は薄い側で
   固定)は 3.23 / 5.64 / 8.31 / 11.00 で**どこでも 2.7 楽観的**。実測の Omega が
   厚くなるほど下がる分を、予測は勘定に入れていない。
7. ★**葉角を振っても誤差はほとんど動かない**(予想は外れ)。k を 0.930 -> 0.631 と
   1.5 倍振っても推定 LAI の誤差は -30.4 -> -34.4 % で、むしろ僅かに悪化する。
   k は植被率(下がる)と反転(割る)の両方に入って**相殺する** —— 残るのは
   クランピングで、それは葉角では動かない。★一方**風で葉が 20 度寝るだけ**で、
   葉面積が 1 mm^2 も変わらないのに k 固定の推定は +18.7 % 動く(k を更新すれば
   +2.1 % に縮むが、ゼロにはならない —— 寝た葉は重なりも増やす)。
8. ★★**「正しい重み」ほど法線の雑音を増幅する**。面積加重の k は
   調和平均 N/Σ(1/|n.z|) が正しい式で、メッシュに :func:`face_areas` で
   重みを付ければ 0.7193(閉形式の真値 0.7185、+0.10 %)。ところが**対照群 1**
   (真の法線を最上面から取る)ですでに 0.7751(+7.9 %)—— 見えている層は
   水平な葉に偏るので、法線が完璧でも母集団の葉角は復元できない。**対照群 2**
   (推定した法線)では 0.5021(-30.1 %)まで崩れ、**重みを付けない素の平均
   0.7099(-1.2 %)のほうが当たる**。1/|n.z| は |n.z| が小さいところで発散する。
9. ★**受光は測れるのに葉面積は測れない**。太陽が真上のとき fPAR は植被率そのもの
   = 0.8152(実測)。一様媒質モデル 1-exp(-k·LAI) は 0.9694 で **+18.9 % 過大** ——
   モデルは光を遮る量まで過大に見積もる。ただし fPAR は k で割って k を掛け戻す
   ので **k の誤差は約分される**。葉角分布が効くのは斜めの太陽で、球形と決め打つと
   天頂角 60 度で +8.7 %。
10. ★**数え方の規約だけで誤差の符号が変わる**。セル中心で判定するかぎり、セルを
   2.5 -> 80 mm と 32 倍粗くしても植被率は 0.7460 -> 0.7551 としか動かない
   (偏らない)。ところが実機の既定「セルに 1 点でも当たれば植被」に替えると
   縁のぶん太り、推定 LAI は 10 mm の **-37.0 %** から 80 mm の **+46.0 %** へ
   符号ごと反転する。危ないのはセルの大きさではなく**規約**。
11. **稈をカプセルで作ると、2 値化を 1 回挟むだけで収束が消える**。閉形式
   2πrh+4πr^2 に対し、SDF -> 2 値 -> marching cubes の面積誤差は
   -4.60 / +1.56 / -1.04 / +4.80 %(h = 4.0 / 2.8 / 2.0 / 1.4 mm)と**符号ごと揺れ**、
   h を詰めても縮まない。距離場のまま等値面を取れば
   -1.03 / -0.54 / -0.26 / -0.13 % と単調に縮む。**位相の問題であって分解能の
   問題ではない。** 稈は葉ではない(PAI = LAI + 0.265)ので、この経路の選択が
   そのまま「葉でないもの」の水増し量になる。

【グラウンドトゥルース】葉は「単位速度の中肋 + 幅 w(u) の帯」という解析曲面。
中肋の傾きを alpha(u)=beta-kappa·u と置くと水平前進も高さも閉形式になり、
|C'|=1 なので **片面葉面積 = ∫w(u)du = pi/4·L·W(楕円と同じ)**、
法線の鉛直成分は **|cos alpha(u)|** で、投影係数 G(theta) も 1 次元求積で厳密。
三角形メッシュに起こして :func:`mesh_area` / :func:`face_areas` で数え直すと
面積 -1.36 %(葉先の幅ゼロを避けて半セル内側から取るぶん)、面積加重 <|n.z|>
+0.10 % で一致する。重なりの平均枚数も閉形式 k·LAI と +0.14 % で合う。
稈は :func:`fullseye.ledger.capsule_sdf` の厳密カプセル(面積 2πrh+4πr^2 /
体積 πr^2h+4/3πr^3)。天頂からの見え方は点を撒かず、帯の (s,t) 座標を
**逆に解いて** セル中心が葉の下にあるかを判定するので、点密度で太らない厳密な
z-buffer になる。区画は周期境界(縁の効果ゼロ)。

来歴(公開文献のみ): Monsi & Saeki, *Jap. J. Botany* 14 (1953) 22 —— 群落内の
光分布と消光係数 / J. Ross, *The Radiation Regime and Architecture of Plant Stands*
(Junk, 1981) —— 投影係数 G(theta) / T. Nilson, *Agric. Meteorol.* 8 (1971) 25 ——
群落の間隙率とクランピング / J. M. Chen & T. A. Black, *Agric. For. Meteorol.* 57
(1991) 1 —— 有効 LAI とクランピング指数 Omega / J. M. Norman & G. S. Campbell,
*Plant Physiological Ecology* (1989) —— 葉角分布と G(theta) の測り方 /
W. E. Lorensen & H. E. Cline, "Marching Cubes", SIGGRAPH 1987 /
I. Quilez, *Distance functions*(公開の SDF 集)。
"""
