...
8. ★★**「正しい重み」ほど法線の雑音を増幅する**。面積加重の k は
   ...
   重みを付ければ 0.7283(真値 0.7276、+0.10 %)。ところが**対照群 1**(真の
   法線を最上面から取る)ですでに 0.7751(+6.5 %)—— 見えている層は水平な葉に
   偏るので、法線が完璧でも母集団の葉角は復元できない。**対照群 2**(推定した
   法線)では 0.5021(-31.0 %)まで崩れ、**重みを付けない素の平均 0.7099
   (-2.4 %)のほうが当たる**。1/|n.z| は |n.z| が小さいところで発散する。
9. ★**受光は測れるのに葉面積は測れない**。太陽が真上のとき fPAR は植被率そのもの
   = 0.8152(実測)。一様媒質モデル 1-exp(-k·LAI) は 0.9708 で **+19.1 % 過大** ——
