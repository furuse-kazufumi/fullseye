...
2. ★**ゼロ点に負ける領域がある** —— 「何もしない」と「アンシャープマスク」をゼロ点に
   ...
   「何もしない」にも抜かれる**。長さは **4.4 px 過大**でゼロ点以下。実用上の許容は
   角度 ±10 度・長さ ±3 px 程度しかない。核を **推定した** 場合(2 次元ケプストラム)
...
def main():
    ...
    print("  → 現実のブレ(画面外から画が流れ込む)を循環モデルで戻すと、縁の誤差が")
    print("     内部の 2 倍以上になる。復元が下手なのではなく **モデルが縁で嘘をつく**。")
