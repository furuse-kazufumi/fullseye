...
2. ★★**楕円体高を標高として使うと、高さは 36 m 間違うのに、傾斜は
   0.0069 度しか間違いません**。これが「引き算で消える誤差」と「消えない
   誤差」の分かれ目です。閉形式で先に予測して印字しました ——
   ジオイド高が一定なら傾斜の誤差は**厳密に 0**(実測 1.7e-13 度)、
   勾配 ``|∇N| = 1.208e-04`` があると誤差の上限は ``atan|∇N|`` = **0.006921 度**
   (実測 0.006921 度、予測との差 1.1e-15 度)。
3. ★★**同じ 36 m が、体積には全部効きます**。1 km 四方の造成で設計面を
   標高 30.0 m と決め、現況を楕円体高で入れると土量が **+3.62e+07 m^3**
   ずれます(閉形式 A・N̄ = A × 35.97 m と 5.5e-08 m^3 一致)。
   ★対照群: 設計面も同じ GNSS から作れば誤差は **0.00 m^3** ——
   **汚染された物差しで汚染された対象を測ると、誤差は消えます**。
4. ★★**浸水は 0 % か 100 % になります**。標高 32.0 m の水位に対し、真の
   浸水面積は 58081 セル中 **20076 セル(34.6 %)**。楕円体高で判定すると
   **0 セル(0.0 %)**、水位のほうに N を足し間違えると **58081 セル(100.0 %)**。
   ★0 % / 100 % は分母を疑えという話がありますが、これは小標本の産物では
   ありません —— 分母 58081 で、**構造的に**全滅しています(36 m の下駄が
   地形の起伏 41.6 m と同じ桁だから)。
5. ★**ほとんど平らな土地では、ジオイドの勾配が流向をひっくり返します**。
   氾濫原(真の勾配の中央値 8.6e-05)に ``|∇N| = 1.208e-04`` を足すと、
   下り方向が 90 度以上回るセルが **44223 / 56644(78.1 %)**。閉形式
   (``|∇H|^2 + ∇H・∇N < 0`` のセル)の予測 78.1 % と一致しました。
   D8 流向(``dem_flow_direction``)でも **48477 / 58081(83.5 %)** が別の
   隣へ流れます。★**傾斜の誤差 0.007 度は無視できても、平地では向きが逆に
   なります** —— 灌漑・下水・内水氾濫の計算はここで壊れます。
   対照群(N 一定)は 0 セル、丘陵地(勾配が 10 倍)では 0.6 % だけ。
6. **視通(差の量)にはほとんど効きません**。2304 組の見通しで、
   遮蔽余裕の変化は最大 **1.03 m**(閉形式の上限 ``|∇N|・d`` = 1.28 m 以内)、
   判定が変わった組は **6 / 2304(0.26 %)**。地球の曲率落ち(12 km で
   9.8 m)のほうが 10 倍効きます。
...
8. **epoch(座標の時刻)は年数 × 2.5 cm で効きます**。何年で何を割るかの表を
   §5 に出しました(代表値 2.5 cm/年: Scan-to-BIM の 5 mm は **0.2 年**、
...
10. ★**屈折は崖を 7.2 % 遠くへ動かします**。曲率だけなら許容 5 mm を割るのは
    **252 m** ですが、標準大気(k = 0.13)なら **270 m**。
...
def main():
    ...
    for stem in ("geoid", "enu", "datum", "ortho", "vertical", "epoch", "crs", "utm"):
