...
11. ★★**軸順と単位のうち、うるさく壊れるのは半分だけでした**。5 通りの
    ...
    ``dem_geodetic_to_ecef`` の範囲検査に当たる)。残る 50 %(位置の誤り
    中央値 6771 km)と、ラジアンを度として渡す(8883 km)・経度の符号反転
    (4800 km)・ECEF の x と y の入れ替え(4849 km)・高さがフィート
    (228 m)は **例外 0 件**。ECEF の入れ替えでは、返った緯度経度が
