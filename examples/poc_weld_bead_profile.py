...
【真値(閉形式)】
母材 h = 0 の上に、円弧の余盛(幅 %s mm・高さ %s mm)を x = %s mm を中心に
置き、両つま先にガウス形のアンダーカット(左 %s mm / 右 %s mm)を掘る。
この h(x) を三角測量の式 ``row = r0 + K h(x)`` で画像へ投影し、輝線を
1σ = %s px のガウスで描く。**画像から出した h を真値の h と直接比べられる。**
...
def est_robust(img, half=4, med=15):
...
def section2_estimators(truth: dict) -> None:
    ...
    print("     1 % 入れると 0.00258 対 0.00247 mm で**区別がつかない**。")
...
def section4_specular(truth: dict) -> None:
    ...
    print("     壊れる(飽和 1.3 % で偏り -0.025 mm)。重心は平頂でも重さの")
    print("     中心を取るので -0.007 mm で耐える。★**山の形を仮定する推定量**")
