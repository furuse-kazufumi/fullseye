...
5. ``nice_ticks`` の刻みは必ず ``1/2/5 x 10^k`` で等間隔かつ範囲内(45 通りで違反 0)。
   明示して描いた目盛りは既定の目盛りと**画素単位で同一**。
6. ``compare_frame`` の大きさ == ``左 + divider + 2*gap + 右``(誤差 0 画素)、
   2 枚は画素単位でそのまま、仕切りはちょうど指定色、隙間と余白は background。
