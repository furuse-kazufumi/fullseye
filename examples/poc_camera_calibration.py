...
(c) **``camera_calibration`` は不確かさも条件数も返さない**。返るのは
    ``reproj_rms`` だけ —— まさに本 PoC が「保証しない」と示している数字である。
    完全な退化(全視点が正面平行)は零空間の次元で fail-closed に弾くが、
    その判定は ``sv[-2] <= 1e-8 * sv[0]`` という崖であって、傾き 2 度は
    素通りして桁違いに誤った K を無警告で返す(第 4 章)。さらに **0.05 px の
    雑音を入れるとその崖自体をすり抜け**、実際に止めるのは後段の「fx が nan」
    の門になる —— 止まりはするが「退化」という診断名は利用者に届かない
    (第 6 章 a)。傾きの条件数を返す 1 つの数字があれば足りる。
