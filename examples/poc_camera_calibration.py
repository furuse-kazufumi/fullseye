...
(c) **``camera_calibration`` の退化検出の門が一度も発火しない(dead gate)**。
    実装は ``sv[-2] <= 1e-8 * sv[0]`` で「全視点が正面平行」を弾くつもりだが、
    教科書どおりの完全退化(傾き 0 度、面内回転のみ)でも実測 ``sv[-2]/sv[0] =
    1.7e-06`` —— しきい値の **170 倍**で素通りする。原因は V を**正規化していない
    ホモグラフィ**から組むこと: H の列は画素尺度 ~1e4 と 1(最終行)が混在し、
    V の特異値が 10 桁にまたがるので、相対しきい値 1e-8 には構造的に到達しない。
    結果として実際に止めているのは後段の「K が非有限/非正」の門で、fail-closed
    ではあるが著者が書いた「板を傾けよ」という診断文は**到達不能コード**に
    なっている(第 6 章 a で傾き 0/2/5/8/20/32 度を並べて実測)。直すなら
    Hartley 正規化を掛けた V で判定するか、しきい値を 1e-4 程度に上げる。
