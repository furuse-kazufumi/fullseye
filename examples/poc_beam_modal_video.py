...
1. **ゼロ点(梁の縁 1 画素の輝度 FFT)は f_1 を当てられない**(A_1 = 0.2 px で
   誤差 %(zp_f1)s Hz)。輝度は照明ちらつきの折り返し線(100 Hz → fps 128 で
   28.0 Hz)を**振動の %(zp_ratio)s 倍**で拾い、輝度から組んだモード形状は符号を
   持たない(MAC: モード 1 %(zp_mac1)s / 2 %(zp_mac2)s / 3 %(zp_mac3)s)。
   振幅 1 px なら f_1 は当たる(誤差 %(zp_f1_big)s Hz)が MAC_2 は %(zp_mac2_big)s。
2. ★**f_n は 3 モードとも %(f_max_err)s Hz 以内**(位相法、fps 128 / 2 s、
   A_1 = 0.2 px)。ところが同じ時系列から出した **ζ_1 は 3 通りの方法で 3 通り**:
   半値幅 %(z_hp)s(真値の %(z_hp_ratio)s 倍)/ 包絡線の対数勾配 %(z_env)s /
   減衰正弦波の当てはめ %(z_fit)s(真値 0.02)。半値幅の %(z_hp_ratio)s 倍は雑音では
   なく**窓長 2 s の分解能の下限**で、予測(矩形窓の主ローブ 0.886/T → ζ 下限
   %(z_floor)s)と一致。
3. ★★**振幅を 0.02 → 2 px で掃引すると壊れる順番が量ごとに違う**。位相法が
   合格し始める振幅は f_1(±0.05 Hz)%(ph_f)s px / MAC_2 > 0.95 %(ph_m2)s px /
   ζ_1(±25 %%)%(ph_z)s px / MAC_3 > 0.95 %(ph_m3)s px。PIV(窓 16 px)は
   f_1 %(pv_f)s / MAC_2 %(pv_m2)s / ζ_1 %(pv_z)s / MAC_3 %(pv_m3)s px。
   **f_1 が合っていても ζ_1 は 2 倍違う**振幅帯が両方の推定器に在る(PIV は
   0.1 px で f_1 誤差 %(pv_f_01)s Hz のまま ζ_1 = %(pv_z_01)s)。崖の順番は
   f → MAC_2 → ζ → MAC_3 で、**ζ が雑音にいちばん弱い**(f は 256 フレームの
   積分で決まり、ζ は包絡線の傾きなので雑音の床がそのまま「減衰しない成分」に見える)。
4. ★**位相法の返す wrap_limit_px(%(wrap)s px)は崖の位置を予測しなかった**。
   予想は「%(wrap)s px で位相が巻いて壊れる」、実測は A_1 = 2 px(先端 3.1 px、
   その %(wrap_ratio)s 倍)でも f_1 誤差 %(big_f)s Hz・MAC_1 %(big_mac)s で壊れない。
   返り値の限界値は局所空間周波数の最大(雑音画素)で決まっていて、実際の崖は
   模様の波長(5〜14 px)で決まる。
5. **fps を落とすとモード 2(18.80 Hz)は Nyquist で予測どおりに折り返る**:
   fps 32 → %(al32)s Hz、24 → %(al24)s Hz(予測 |f_2 − k·fps| との差 %(al_err)s Hz
   以内)。折り返した線は f_1 と区別がつかない —— fps 24 では 5.2 Hz の線を
   「新しいモード」と読んでしまう。★照明の折り返しは fps 48.5 で**ちょうど
   3.00 Hz = f_1 に落ちる**。輝度のゼロ点の包絡線は ζ_1 を %(zc_b)s と報告し
   (ちらつきは減衰しないので)、位相法は %(zc_d)s で生き残る(ちらつきは輝度の
   掛け算で位相を動かさない —— fps 128 では位相法の dy スペクトルのちらつき線は
   モード 1 の %(fl_d128)s 倍、輝度では %(fl_b128)s 倍)。
6. **対照群で原因を分ける**(A_1 = 0.2 px、全部入りに対して 1 要因ずつ止める):
   手ぶれを引かないと ζ_1 は %(c_all_z)s → %(c_noc_z)s(手ぶれ 0.8 / 2.3 Hz が
   帯域内に居る)、雑音を止めると MAC_3 が %(c_all_m3)s → %(c_nonoise_m3)s、
   ちらつき・ローリングシャッターは f/ζ/MAC を動かさない(予測: 梁の行をまたぐ
   時刻差 %(rs_dt)s ms はモード 3 で %(rs_ph)s rad の位相差だが、行方向に平均すると
   振幅 %(rs_gain)s 倍の減りにしかならない。実測 %(rs_meas)s 倍)。
