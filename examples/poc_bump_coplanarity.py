...
1. **ゼロ点(そりを引かずに最小二乗平面だけ引く = JEDEC の着座平面)を先に測る**。
   そり PV 50 µm を仕込むと共平面性は **54.31 µm**(真値 38.44 µm)、
   バンプ個体差の読み取り RMS 誤差は **8.84 µm** —— **個体差の 1σ(4 µm)の 2.2 倍**。
   仕様 ±12 µm で **256 本中 62 本が不合格**、うち **58 本は誤検出**。
2. ★**壊れ方は 2 種類あり、片方は誤検出ではなく見逃し**。ゼロ点は
   誤検出 58 本のかたわらで、真に仕様外の 5 本のうち **1 本を見逃す**
   (そりが正の側に居るバンプは、−20 µm 沈んでいても平面からは −12 µm を切らない)。
   2 次曲面を引くと誤検出 0 本 / 見逃し 0 本。**「不合格 62 本」と 1 つに畳むと、
   この 2 つが混ざる**。
3. ★**予想が外れた —— 3 次は 2 次より悪い**。読み取り RMS 誤差は
   1 次 8.84 → 2 次 **1.23** → 3 次 **1.37 µm**(2 次の **1.12 倍**)。
   予想は「高次ほど良い」だったが、仕込んだ高次成分は 4 次のロブなので
   3 次の項では 1 µm も取れず、**増えた 4 項がバンプ個体差と雑音を余計に吸う分だけ
   損をする**。次数は「足りるところで止める」もの。
4. **崖は幾何で先に予測できる**。そりの形(モードの混ぜ方)を固定すると、
   その次数で取り切れない残りは PV に**比例**する。バンプ位置で測った比例定数は
   r1 = **0.17664** / r2 = **0.02355**。個体差 1σ = 4 µm に並ぶ PV は
   1 次 **22.6 µm** / 2 次 **169.9 µm** と予測 —— 掃引の実測は **22.6 µm**(比 0.996)
   と **169.4 µm**(比 **0.998**)。**そりが 170 µm を超える薄型パッケージでは、
   2 次を引いても個体差は読めない**(PV 400 µm で RMS 誤差 9.44 µm、不合格 58 本)。
5. **対照群(そり PV = 0)**では 1 次 **0.23** / 2 次 **0.37 µm** で、差は 0.14 µm。
   壊していたのは当てはめでもバンプでもなく**そり**だと確かめられる
   (2 次のほうがわずかに悪いのは、6 項が本物の個体差を少し吸うから)。
6. ★★**引きすぎの害は「見えない」形で出る**。バンプ高さに**本物の低次不良**
   (中央がなだらかに 8 µm 沈む = ダイアタッチのボイド。**わざと厳密な 2 次にせず
   ガウス形**にした)を追加で仕込むと、2 次曲面はそれを**そりだと思って吸う**:
   中央−外周の真の差 **−3.78 µm** が残差では **−0.45 µm**(**88.2 % 消える**)。
   孤立した短小バンプの検出数は **5 → 5 本**で変わらないので、
   **「検出できている」ことが健全さの証拠にならない** ——
   真に仕様外のバンプは 5 → 7 本に増えたのに、見逃しが **0 → 2 本**に増える。
   消えた分は捨てられておらず、**報告されるそりが 30.06 → 36.20 µm(+6.14 µm)と
   膨らんでいる** —— 見る場所を変えれば残っている。
7. ★**検出は簡単な側**。そりを引いた画像(``background_flatten`` degree=2)に
   自動しきい値を掛けるだけで 256 本中 **256 本**を取りこぼしなく拾う
   (しきい値 60.46 µm)。難しいのは「見つける」ことではなく
   「読んだ高さから何を引くか」。

【グラウンドトゥルース】そり = 3 つの解析モード(ボウル ξ²+η² / サドル ξ²−η² /
4 次ロブ ξ⁴+η⁴−6ξ²η²)を PV 比 0.50 : 0.30 : 0.20 で混ぜて PV を指定値に正規化した
閉形式の場。バンプ高さ = 平均 200 µm・σ 4 µm の正規分布(seed 固定)+ 仕込んだ
短小 6 本(−20 µm。うち 1 本はもともと背が高く、沈めても ±12 µm に収まるので
「真に仕様外」は 5 本)。所見 6 では中央低下 8 µm(ガウス形)を追加。
計測雑音 σ 1.5 µm/画素。

★道具の穴(節 7 で assert つきで確かめている): ラベルごとの gray 統計 /
多項式曲面のロバスト当てはめ / JEDEC の着座平面 —— いずれも公開経路に無い。
``surface_form_error`` は docstring が「(residual, rms, pv) を返す」と書いているのに
台帳経由では **pv の float 1 個**しか返らない。進化 op の ``auto_threshold`` は
値域を [0,1] とみなすので、µm 単位の高さ場をそのまま渡すと **0.5 µm の位置**で切る
(実測: 前景 1141 画素 / 正解 437 画素)。
