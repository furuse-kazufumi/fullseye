...
5. ★速度は到達時刻面の**勾配の大きさの逆数**で出る。合流線から 4 px 以上
   離れた領域の中央値 3.01 px/ms(真値 3.0、+0.3 %)。メッシュの頂点法線
   (:func:`fullseye.ledger.vertex_normals`)から出すと 3.10(**+3.3 %**)——
   面積重み付き法線は隣の三角形と平均されるので、勾配より粗い。
...
7. ★もう 1 つの「到達」の定義 —— 最急上昇(:func:`fullseye.vol_edge_probe`)は
   振幅に依らない代わりに、**平滑化の量だけ時刻が前へずれる**。ずれは
   sqrt(sigma_t^2 + sigma_smooth^2) で予測でき、実測もそれに乗る。
   しきい値の定義はオフセットが閉形式、最急上昇の定義は**較正が要る**。
