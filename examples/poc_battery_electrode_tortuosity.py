...
``tau_transport`` / ``tau_geodesic`` はどちらも二値ボリュームだけから計算するので
**そのまま動きます**。真値がわりの ``tau_transport`` は輸送方程式の数値解なので、
...
1. **ゼロ点(Bruggeman τ = ε^(-0.5))を先に置く**。空隙率 ε = 0.421 の
   球状粒子床で、Bruggeman は τ = 1.542、輸送方程式を解いた実測は
   **τ = 2.163(-28.7 %)**。経験則は**必ず小さめに外す**。
2. ★**外し方は空隙率で単調に開く**。ε = 0.658 → 0.153 と振ると
   誤差は **-14.4 % → -83.5 %**。ε > 0.4 では Bruggeman 指数(D_eff ∝ ε^α)の
   実測は **α = 1.79** で 1.5 ではなく、ε < 0.25 では **α = 3.31** まで跳ねる。
   「1.5 乗則」は高空隙率の近似ですらない。
3. ★★**測地屈曲度は Bruggeman の τ ではない**。画像解析の論文が測る
   「最短経路長 / 直線距離」は ε = 0.421 で **τ_geo = 1.232** —— 実測 2.163 の
   **57 %** しかない。ところが★**その 2 乗が Bruggeman とほぼ一致する**
   (ε = 0.658 で 1.233 対 1.233、ε = 0.421 で 1.517 対 1.542)。
   **2 つの「よく合う数字」は、同じ向きに同じだけ間違っている**だけだった。
4. ★★**対照群 —— 同じ空隙率で粒子を扁平にすると τ は 4 倍になる**
   (カレンダリング)。ε = 0.441 に揃えた扁平粒子床で、
   厚み方向 τ_z = 7.323 / 面内 τ_x = 2.037(**3.6 倍の異方性**)。
   球状床は同じ ε で τ_z = 1.843 / τ_x = 1.906(1.03 倍 = 等方)。
   Bruggeman はどちらにも同じ 1.499 を返す —— **面内は当たり(-21 %)で
   厚み方向は壊滅(-80 %)**。セル設計に効くのは厚み方向のほうです。
5. **もっともらしい犯人を対照群で外した**。「閉気孔(孤立した空隙)が
   効いているのでは」を測ると、孤立空隙は全空隙の **0.12 / 0.60 / 2.65 %**
   (ε = 0.42 / 0.24 / 0.15)しかなく、τ の 2〜10 倍の外れを説明できない。
   効いているのは**首(くびれ)**で、空隙の内接半径の中央値は粒子半径の
   **0.40 倍**しかない。★ただし**そのスカラーも向きを分けられない**:
   扁平床の τ_z と τ_x は 3.6 倍違うのに、内接半径の分布は 1 つしかない。
6. ★**予想が外れた —— 解像度に崖が無い**。「voxel が首の直径を超えたところで
   崩れる」と予測して(首の直径 ≈ 粒子半径の 0.65 倍 = 粒子半径 1.5 voxel 相当)
   掃引したが、**崖は現れず、なだらかな片側の偏りだけ**が出た:
   voxel を 5.5 倍粗くすると τ_f は 1.771 → 3.041(**+72 %**)へ単調に増え、
   同じ間 **ε は 0.4448 → 0.4431(-0.4 %)しか動かない**。
   空隙率は解像度に鈍く、屈曲度は敏感で、しかも**警告が出ない**
   —— 壊れた図も、落ちた検出数も、何も出ないまま数字だけがずれる。
7. **数字が 3 つある以上、どれを報告したかを書かないと意味が無い**
   (Bruggeman 1.542 / 測地 1.232 / 輸送 2.163、同じ 1 個のボリュームから)。
...
N_SPHERE = 350
#: 扁平率 3 の粒子で ε を同じ 0.44 台に合わせる粒子数(6 章の対照群)
N_FLAKE, FLAT = 1040, 3.0
