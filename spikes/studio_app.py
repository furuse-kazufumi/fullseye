...
try:
    import code_editor as _code_editor  # IDE 級エディタ(定義ジャンプ/補完/矩形選択)
except Exception:
    _code_editor = None
...
class StudioWindow(QtWidgets.QMainWindow):
    def __init__(self) -> None:
        ...
        # builtin シーンをワンクリックで入れるチップ
        chips = QtWidgets.QWidget(); cb = QtWidgets.QHBoxLayout(chips); cb.setContentsMargins(0, 0, 0, 0)
        for nm in ("go2", "cassie", "apollo", "anymal", "spot"):
            b = QtWidgets.QPushButton(nm); b.setMaximumWidth(72)
            b.clicked.connect(lambda _=False, n=nm: self.sim_path.setText(n))
            cb.addWidget(b)
        sl.addWidget(chips)
        ...
        self.sim_gsplat_btn.setToolTip("シーンをオービット撮影→3DGS 学習(native gsplat 自動選択, 別プロセス)→全周GIF")
        self.sim_gsplat_btn.clicked.connect(self._train_3dgs)
        self.sim_motion_btn = QtWidgets.QPushButton("動かす 🏃")
        self.sim_motion_btn.setToolTip("body リグ付け→サイン波モーションで『動く3DGS』を生成→motion.gif")
        self.sim_motion_btn.clicked.connect(lambda: self._train_3dgs(motion=True))
        sb.addWidget(self.sim_mesh_btn); sb.addWidget(self.sim_cloud_btn)
        sb.addWidget(self.gsplat_quality); sb.addWidget(self.sim_gsplat_btn)
        sb.addWidget(self.sim_motion_btn)
        sl.addWidget(sbtns)
    ...
    def _train_3dgs(self, *_args, motion: bool = False) -> None:
        """シーンを 3DGS 化(fullseye_3dgs ランチャを別プロセス起動 → 完了で GIF を開く)。

        scene 欄は builtin名(go2/cassie/apollo/anymal/spot)か MJCF パス。backend は
        ランチャが自動選択。motion=True で『動く3DGS』(body リグ+モーション)。"""
        import os, tempfile
        ...
        gif_name = "motion.gif" if motion else "turntable.gif"
        ...
        args = [launcher, scene, "--out", outdir, "--quality", quality]
        if motion:
            args.append("--motion")
        proc.start(vpy, args)
        kind = "動く3DGS" if motion else "静止3DGS"
        self.statusBar().showMessage(
            f"{kind} 開始({scene}, {quality})… 別プロセスで実行中。完了で GIF を開きます")
