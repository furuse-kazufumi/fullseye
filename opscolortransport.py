...
``histogram_match``        順位を保つ単調写像。**厳密一致は同値の無い入力で
                           画素数が参照と等しいときだけ**(出力が参照の並べ
                           替えになる、実測 KS 距離 0)。画素数が違えば
                           分位点の再標本化で **KS 距離 ≤ 1/n + 1/m**(実測
                           4.8e-4 対 上限 3.0e-3)。整数画像(同値だらけ)の
                           既定 ``ties="average"`` は階段状に丸まり、8 階調
                           で KS 0.073、256 階調で 0.0037
    ...
    "histogram_match": [
        {
            ...
            "instead": "bins=None(既定。同値が無く画素数が等しければ厳密、"
                       "違えば KS 距離 ≤ 1/n + 1/m)",
        },
        {
            "when": "整数画像(同値の画素が多い)に既定 ties='average' で掛けたとき",
            "what": "同値の塊ごとに参照値の平均へ潰すので、出力の分布は参照と厳密には"
                    "一致しない(階段状に丸まる)。例外は出ない",
            "measured": "8 階調・1024 画素で参照との KS 距離 0.073、256 階調・4096 画素で 0.0037",
            "instead": "分布の厳密さが目的なら ties='break'(KS 0、ただし平坦部に偽の濃淡)",
        },
    ],
}
