...
# --------------------------------------------------------------------------- #
# 突出度: 大域中央値は帯域幅に依存し、狭帯域で順序が逆転する(2026-09-06)
# --------------------------------------------------------------------------- #
def test_global_prominence_inverts_in_a_narrow_band():
    """**純雑音のほうが本物の欠陥より大域突出度が高い**、を固定する。

    直す代わりに測って残す門。`peak_prominence` は広い帯域でしか順序を保たず、
...
def test_local_prominence_is_present_on_the_order_axis_too():
    ...
    assert abs(o["local_noise_floor"] - o["noise_floor"]) > 0.0
