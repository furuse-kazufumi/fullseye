...
def test_ledger_is_complete_and_every_op_has_an_implementation():
    ...
    # 2026-09-05: optscene "scene" 37 op を追加(80 → 117)。物理空間にシーンを組んで
    # 撮る層で、これで「レンダラを持てないので画像でなく限界を返す」線引きが外れた。
    # センサー/レンズ/光源/レイアウトの 4 オブジェクトもここに入る。
    assert len(opsoptics.OPSOPTICS) == 117
    ...
    assert len(opsoptics.list_ops("scene")) == 37
    ...
    seen = [n for entries in opsoptics._CATALOG.values() for n, *_ in entries]
    assert len(seen) == len(set(seen)), \
        f"duplicate op names in the ledger: {sorted({n for n in seen if seen.count(n) > 1})}"
    for cat, mod in (("appearance", "matappear"), ("interface", "glassmirror"),
                     ...
                     ("surface", "surfacelib"), ("scene", "optscene")):
