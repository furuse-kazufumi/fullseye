...
KNOWN_DEAD_BRIDGES = {
    "tb_indices_to_labels":
        ...
        "TYPE_TO_SORT は 'labels' を 3-D の volume へ写す。点群セグメンテーション 3 op と"
        "違って**点が手元に無い**ので体積へ焼く材料が無く、1-D を 3-D に仕立てるのは"
        "捏造になる。正しい直し方は台帳の型語彙を『体積のラベル』と『列のラベル』に"
        "分けることだが(実測 0/60)、新しい型に消費 op が無いと袋小路の型が増える(この repo が"
        "繰り返し踏んできた形)ので単独では入れない。",
