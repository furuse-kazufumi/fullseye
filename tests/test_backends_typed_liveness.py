...
# 2026-09-08: ``tb_indices_to_labels`` をここから外した —— **橋に載せるのをやめた**。
#
# 旧・台帳の診断は正しかった: 「reprconv の 'labels' は 1 次元なのに TYPE_TO_SORT は
# 'labels' を 3-D の volume へ写す。実測 0/60」。正しい直し方として「台帳の型語彙を
# 『体積のラベル』と『列のラベル』に分ける」と書いてあり、「消費 op の無い型を増やすと
# 袋小路になるので単独では入れない」と保留していた。その判断自体は妥当だが、
# **保留のあいだ op は登録されたまま毎回 fail-soft していた**(0/60 = 一度も走っていない)。
#
# 同じ回に探針の門を全 sort へ広げたところ、この op が「宣言 out と実際の形が
# 食い違う唯一の橋 op」として独立に出た。型語彙を増やさずに済ませる正しい形は
# **橋を架けないこと**なので ``backends_typed._OP_BRIDGE_SKIP`` へ移した。
# 台帳経由(``fullseye.ledger.indices_to_labels``)は今までどおり使える。
#
# ★この 1 本については記録が **3 つ**あった —— ここ、``gen_op_figures.DOMAIN_MISMATCH``、
# そして op 自身の 0/60 という実測。それでも「登録されているのに一度も走らない」状態は
# 続いていた。**知っていることと、判定に使っていることは別**(KNOWN_ISSUES §42)。
