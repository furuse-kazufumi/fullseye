...
#: 各 ``sp_*`` 関数は 1 行の委譲（docstring 無し）で、実際に登録される ``op.fn`` も
#: ``build()`` 内の ``_safe`` が返すクロージャ ``w`` であり ``__doc__`` を転記
#: しないため、素通しでは説明が消える。ここに Japanese の説明を書き ops.py の
#: 登録ループで Op.doc に積ませる。キーは op 名。座標系はどの op も CONTOUR
#: （``{"shape": (H, W), "cs": [1x2 の (row, col) 配列, ...]}``）で、単位は入力
#: 画像のピクセル（小数値＝サブピクセル位置）。
DOCS = {
    "sp_local_max_sub_pix": (
        "グレー値曲面の極大点をサブピクセル精度で検出する（HALCON "
        "``local_max_sub_pix`` の代替実装。``halcon=\"\"``: 同名の座標は既存の"
        "core op が既にカバー済みなので二重申告しない）。\n\n"
        "3x3 近傍に 2 次曲面 ``z = a0+a1 x+a2 y+a3 x^2+a4 y^2+a5 xy`` を最小二乗"
        "フィットし、勾配 0 の点へのオフセットを解いてサブピクセル位置を求める"
        "（ヘッセ行列が退化する場合は 1 次元放物線補間にフォールバック）。``a``"
        "は極大の顕著さ（周囲の最小値からの深さ）のしきい値を 0.01〜0.31 に振る"
        "（``thr = 0.01 + 0.30*a``）。``b`` は未使用。戻り値は CONTOUR、座標の"
        "単位は入力画像のピクセル（小数値＝サブピクセル）。検出点数は"
        "``count_contours`` で数えられる。"
    ),
    "sp_local_min_sub_pix": (
        "グレー値曲面の極小点をサブピクセル精度で検出する（HALCON "
        "``local_min_sub_pix`` に相当）。\n\n"
        "アルゴリズムは ``sp_local_max_sub_pix`` と対称（3x3 の 2 次曲面フィットで"
        "勾配 0 点へサブピクセル補正）。``a`` は極小の顕著さ（周囲の最大値からの"
        "深さ）のしきい値を 0.01〜0.31 に振り（``thr = 0.01 + 0.30*a``）、``b`` は"
        "未使用。戻り値は CONTOUR、座標の単位は入力画像のピクセル。暗いスポット"
        "・くぼみの検出に向く。"
    ),
    "sp_saddle_points_sub_pix": (
        "グレー値曲面の鞍点（サドルポイント）をサブピクセル精度で検出する"
        "（HALCON ``saddle_points_sub_pix`` に相当）。\n\n"
        "3x3 の 2 次曲面フィットのヘッセ行列が不定符号（``det H < 0``、両主曲率の"
        "符号が逆）になる画素を鞍点候補とし、勾配 0 点がセル内"
        "（``|dx|,|dy| <= 0.5``）に収まるものだけを残す。``a`` は鞍点の強さ"
        "（より小さい方の主曲率の絶対値）のしきい値を 1e-4〜0.0201 に振る"
        "（``thr = 1e-4 + 0.02*a``）。``b`` は未使用。戻り値は CONTOUR、座標の"
        "単位は入力画像のピクセル。"
    ),
    "sp_critical_points_sub_pix": (
        "極大・極小・鞍点のサブピクセル位置をまとめて検出する（HALCON "
        "``critical_points_sub_pix`` に相当）。\n\n"
        "``sp_local_max_sub_pix`` / ``sp_local_min_sub_pix`` / "
        "``sp_saddle_points_sub_pix`` の 3 つを同じしきい値 ``a`` で実行し、"
        "検出点を極大→極小→鞍点の順で 1 つの CONTOUR に連結する。``b`` は未使用。"
        "戻り値の座標単位は入力画像のピクセル（サブピクセル小数値）。個々の種類"
        "を区別せず「臨界点がどこにあるか」だけを知りたい場合に使う。"
    ),
    "sp_plateaus": (
        "同じ（量子化した）グレー値が連結した平坦領域（プラトー）の重心をサブ"
        "ピクセル位置として返す（HALCON ``plateaus`` に相当）。\n\n"
        "隣接画素同士がしきい値 ``q = 1e-6 + 0.05*a`` 以内で等しければ同じ平坦"
        "領域とみなす（Union-Find で連結成分化）。``a`` はこの許容量子化幅と"
        "最小面積（``min_area = 2 + round(6*a)``、これ未満の領域は無視）の両方を"
        "振る。``b`` は未使用。戻り値は CONTOUR で、各プラトーにつき 1 点（その"
        "領域の画素座標の平均、単位はピクセル）。入力が 200,000 画素を超える"
        "場合は安全側で空集合を返す（計算量ガード）。"
    ),
    "sp_lowlands_center": (
        "極小の「盆地」（周囲より低い平坦領域）の重心を返す（HALCON "
        "``lowlands_center`` に相当）。\n\n"
        "``sp_plateaus`` と同じ量子化による平坦領域抽出を行った上で、外周の画素"
        "がすべて厳密に高い（領域極小である）ものだけを残し、さらに深さ（外周の"
        "最小値との差）が ``min_depth = 0.01 + 0.40*a`` 以上のものだけを採用する。"
        "``b`` は未使用。戻り値は CONTOUR、各盆地につき 1 点（重心座標、単位は"
        "ピクセル）。入力が 200,000 画素を超える場合は安全側で空集合を返す。"
    ),
}
