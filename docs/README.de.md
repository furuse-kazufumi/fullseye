...
## PoC-Serie — 117 reale Aufgaben mit Grundwahrheit
...
| shape_descriptors (3) | [`poc_real_texture_invariance`](https://github.com/furuse-kazufumi/fullseye/blob/master/examples/poc_real_texture_invariance.py) 実写テクスチャを回す(回転不変は等方な素材でだけ成り立つ)<br>[`poc_rotation_invariance_audit`](https://github.com/furuse-kazufumi/fullseye/blob/master/examples/poc_rotation_invariance_audit.py) 「回転しても同じ」と言える量はどれか(実写の硬貨を 72 角度で監査)<br>[`poc_texture_rotation_identity`](https://github.com/furuse-kazufumi/fullseye/blob/master/examples/poc_texture_rotation_identity.py) 回しても素材を見分けられるか(実写テクスチャ 3 種 x 12 角度) |
...
**Gemessener Inhalt**: von 1912 Notizen verweisen **1894** auf mindestens ein lauffähiges Beispiel (18 ohne), **1907** haben einen Nutzungsabschnitt ab 120 Zeichen (5 nur eine Zeile). Die Struktur (Aufrufform, Typen, anschließbare Ops) haben alle 1912.
