...
**2,144개**의 노트를 주제 클러스터로 계층화하고, 각 클러스터에 LLM 요약을 붙인 "내비게이션이 있는 코퍼스"도 만들 수 있습니다. 내부적으로는 [RAPTOR](https://github.com/gadievron/raptor) 포크의 `corpus2skill`(TF-IDF + k-means + LLM 요약)을 사용하고 있지만, **이는 어디까지나 선택적인 최적화이며 필수가 아닙니다**. 요구 사항은 "`docs/ops`를 입력으로 클러스터별 SKILL.md 계층을 출력"하는 것뿐이므로, 동등한 도구라면 무엇으로든 대체할 수 있습니다.
