...
Fullseye の 3-D オペレータ群(`ops3d` = 372 の型付き op)を、**実問題を解く実行可能な事例**（全 121 件）で示します。
