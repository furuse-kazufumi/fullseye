...
#### [write_image が書けなかった事実を捨てて無言で戻り、uint16 が 8 bit に潰れ、読めない理由が全部「無い」だった](hardening/image-io-dropped-write-failures-and-crushed-16-bit.md)

GenSpark 第 18・19 報(N75 / N76 / N77 / N78 / N79、N72 の同族、N81 の音声分)。全部 master で再現した。

見つけた PoC: `genspark_external_review` / 直した所: `api.py`, `imgio.py`, `ops.py`, `dsp.py` / 門: `test_write_refuses_a_missing_directory_instead_of_pretending`, `test_write_refuses_an_unwritable_extension_naming_the_writable_ones`, `test_ppm_takes_a_grey_image_and_reads_back`, `test_read_errors_say_which_kind_of_failure`, `test_uint16_is_written_as_16_bit_and_float_depth_options_are_lossless`, `test_float_default_is_8_bit_as_documented_and_depth_needs_a_capable_extension` / 状態: fixed

#### [台帳の引き方が未知の名前に黙って空を返し、write_wav の path が台帳でデータ扱いされていた](hardening/ledger-lookups-returned-empty-for-unknown-names.md)
...
`compactness` の頭打ちを直したあと、**同じ型が他にも無いか**を機械的に探した。 「単調に形を変える列を渡して、出力が止まる op」を数える探針で 9 本出た。

見つけた PoC: `shape_factors_closed_form` / 直した所: `backends_auto.py` / 門: `test_a_region_feature_responds_to_at_least_one_deformation`, `test_a_contour_feature_responds_to_at_least_one_deformation`, `test_height_width_ratio_reports_tall_objects_honestly`, `test_the_moment_features_keep_growing_on_long_shapes`, `test_the_probe_would_catch_a_squashed_feature`, `test_the_constant_ledger_names_ops_that_really_are_constant` / 状態: fixed
...
見つけた PoC: `threshold_family_agreement` / 直した所: `backends_r3.py` / 門: `test_the_region_is_the_bright_side`, `test_a_global_automatic_threshold_gets_the_box_exactly`, `test_they_all_agree_with_each_other`, `test_the_complement_is_what_the_gate_catches`, `test_the_itk_convention_is_the_one_we_had_wrong` / 状態: fixed
...
#### [sort `table` が「行のリスト」と「諸元 dict」の両方を指し、消費 op が口ごもって落ちた](hardening/table-sort-mixes-rows-and-spec-dicts.md)
...
見つけた PoC: `tools/chain_fuzz.py` / 直した所: `optscene.py`, `fourierdesc.py`, `tools/chain_fuzz.py`, `ops1d.py`, `opsoptics.py` / 門: `test_table_consumers_refuse_a_list_of_rows_with_the_op_name`, `test_a_wrong_kind_of_spec_is_refused_too`, `test_peak_subbin_and_register_light_ledger_out_match_what_they_return`, `test_nonfinite_allowlist_additions_are_documented_and_really_nonfinite`, `test_the_eight_suspect_replays_are_white_now` / 状態: fixed

#### [機能ゲートが `match` ソートを知らず、正しい op を「実装されていない」と数えていた](hardening/functional-gate-did-not-know-the-match-sort.md)
