...
- 2026-09-09 — 回しても素材を見分けられるか ―― 「回転不変」と名の付く記述子を、実写で数える(poc_texture_rotation_identity)
...
## 77. 回しても素材を見分けられるか ―― 「回転不変」と名の付く記述子を、実写で数える

[![回しても素材を見分けられるか ―― 「回転不変」と名の付く記述子を、実写で数える](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/poc/poc_texture_rotation_identity/01_texture_rotations_720.jpg)](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/poc/poc_texture_rotation_identity/01_texture_rotations.png)

*↑ **回しても素材を見分けられるか ―― 「回転不変」と名の付く記述子を、実写で数える** ―― テクスチャ記述子には「回転不変」と名の付く符号化があり、GLCM には角度がある。ここで問うのは**記述子がどれだけ動いたか**ではなく、**回した自分は、まだ別の素材より近いか** ―― 振れ幅は「大きい / 小さい」としか言えないが、取り違えの回数は「使えるか / 使えないか」を直接答える。素材は CC0 の実写 3 種(brick / grass / gravel)で、**異方(brick の横目地)と等方(grass, gravel)を混ぜてある** —— どちらか片方だけだと、結論が素材の性質か記述子の性質か分けられない。回転は `reshape=False` + 反射境界で行い、**中央を小さく切り出す**(外周のゼロ埋めを統計に混ぜると、記述子でなく縁を測ることになる)。★まず**分母**を見る: 回す前の 0 度で 3 素材が 0.0448 離れていることを確かめてから、取り違えを数える。★**GLCM の未使用だった `b` を 4 方向平均(0/45/90/135 度)に配線した**(既定 `b=0.5` は 0 度のままなので、既存の結果は 1 ビットも変わらない)。★★**予測を外した**: 「異方な brick では平均が逆効果になる」と読んでいたが、効き方を決めていたのは素材の異方性ではなく**共起距離 `a`** だった。距離 1 では **3/36 → 0/36** と効き、既定の距離 2 ではどちらも 0/36 ながら brick の振れ幅が分離の **1.36 → 0.63 倍**へ下がり(余裕が増える)、距離 4 では **3/36 → 8/36 と悪化**する。悪化の原因は平均ではなく**分母** —— 距離を伸ばすと3 素材の 0 度での差そのものが **0.0448 → 0.0221** と半分に潰れ、残り少ない差が平均で先に消える。**長い距離で平均を掛ける前に、素材が分かれているかを確かめること。**★**LBP の「回転不変」符号化は、実写では救いにならない**: 既定(不変でない)で **19/36**、`uniform`(回転不変)で **18/36** —— 名前に「不変」と付いていても半分前後を取り違える。最も異方な brick が **11/12** と一番悪く、等方な gravel は **0/12** で無傷。**不変性は記述子の性質であると同時に、素材の性質でもある。***
...
*↑ 測定の図 ―― 3 素材を同時に回す。GLCM は角度 0 度固定なので、回すと grass が 12 角度中 3 回「別の素材のほうが近い」と答える。*

```
py -3.11 examples/poc_texture_rotation_identity.py
```

ソース: [examples/poc_texture_rotation_identity.py](https://github.com/furuse-kazufumi/fullseye/blob/master/examples/poc_texture_rotation_identity.py)

使用 op(ノートへ): [`annotate_table`](https://furuse.work/ops/annotate/paper/annotate_table.html) · [`cooc_feature_matrix`](https://furuse.work/ops/2d/texture/cooc_feature_matrix.html) · [`reflect`](https://furuse.work/ops/3d/optics/reflect.html) · [`sk_lbp`](https://furuse.work/ops/2d/texture/sk_lbp.html) · [`text_box`](https://furuse.work/ops/annotate/text/text_box.html)
...
## 79. (x, y, t) で数える ―― 通過台数とオクルージョン、そして L/V という 1 つの定数
...
## 81. 粒子追跡を (行, 列, 時刻) の体積として測る ―― 誤リンクの向きは 1 種類ではない
...
## 83. 構造物の微小振動を映像から測る ―― モーション拡大は「測る」役に立つのか
...
## 85. 庫内の滞留はどこで生まれたか ―― 待ちの種類を分けずに数えると全部「混雑」になる
...
## 91. 点群位置合わせの収束域 ―― 初期姿勢がどれだけずれたら壊れるか
...
## 92. 実写のステレオ写真で測る ―― 合成では出ない 3 つの躓き
...
## 93. 「回転しても同じ」と言える量はどれか —— 実写の硬貨を 72 角度で回して数える
...
## 94. 色恒常性(ホワイトバランス)―― 「効く手法」は無い、あるのは効く条件だけ
...
## 95. 多波長で層を剥がす ―― 下絵・地塗り・上塗り・褪色を、真値を握ったまま分離する
...
## 96. 偏光で鏡面反射を剥がす ―― フレネルの式で真値を作り、分離結果を突き合わせる
...
## 97. 実写の免疫染色を色で分ける ―― 見張り役が、見張るべき誤りにだけ盲目だった
...
## 98. 改竄検出を ROC で語る ―― 「見つかった 1 枚」ではなく、偽陽性を固定したときの検出率
...
## 99. 手持ちで撮った書類をまっすぐに戻す ―― 台形補正と影除去を、真値と突き合わせて測る
...
## 100. カメラ指紋(PRNU)で「どのカメラで撮ったか」を当てる ―― 指紋は枚数で育ち、保存ボタンで消える
...
## 101. 絵画のひび割れ網 ―― 3 指標のうち撮影条件で壊れるのは分岐次数だけ
...
## 102. 壊れたメッシュを直してから測る ―― 消えるのは欠陥の数で、戻るのは量ではない
...
## 104. CAD と実測点群の差分検査 ―― 合わせた分だけ欠陥が消え、無い所にへこみが出る
...
## 105. 造形しやすさを形から測る —— しきい値に貼りついた面は、丸めた分だけ判定が飛ぶ
...
## 106. 対称性で欠けを補う —— 仮定した面がずれた分だけ、復元は嘘をつく
...
## 107. 積層の反りは層の履歴が決める —— 均した面積は置き場所を捨てる
...
## 109. 接合層のボイドを 1 個の数字に畳む ―― 畳んだ分だけ、寿命に効く形が消える
...
## 111. 構造物を年ごとに測り返す —— 測る場所がずれると、劣化は進んだように見える
...
## 112. 設計と実物の食い違いを部屋から測る ―― 合わせの妥協角は、無傷の部材へ配られる
...
## 113. 配管内面の減肉を展開図で測る ―― 軸を決めた分だけ、管底の腐食が消える
...
## 115. 人と機械の安全距離 —— 代表点に置き換えた分だけ、危険が消える
