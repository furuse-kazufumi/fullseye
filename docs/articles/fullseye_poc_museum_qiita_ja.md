...
**2026-09-16 — Fullseye 0.2.0 を公開しました**(`pip install -U fullseye`)。この展示館に関わるのは 3 つで、**うち 2 つは前回の版で書いたことの訂正**です。

- **展示の数字を引くときの DOI が変わります。** 0.2.0 の version DOI は [10.5281/zenodo.22772164](https://doi.org/10.5281/zenodo.22772164)、concept DOI [10.5281/zenodo.22761195](https://doi.org/10.5281/zenodo.22761195) は従来どおり常に最新版へ解決します。走らせた版を必ず添えてください。
- **訂正(1): MCP サーバが `pip install` だけで動くようになりました。** 前回の版では「今は checkout 限定で、pip 版からは理由つきで止まります」と書きました。原因は、索引と op ノートをリポジトリ相対で読んでいたことです。checkout のテスト 28 件は全部緑のままで、**門が事故の起きない場所にだけ立っていた**という、この展示館が何度も書いてきた型そのものでした。索引とノートの frontmatter を配布物に同梱し、**wheel だけを入れた仮想環境から実際に起動して往復する門**を足してあります。
- **訂正(2): 他の言語からの入口が 5 op ではなくなりました。** 前回は「5 op だけの C ABI」と書きましたが、この版では共有ライブラリの関数 1 本(`fs_apply`)に op 名と JSON を渡すと、**2-D レジストリの単入力 901 op** が C / C++ / C# / Lua / Python から呼べます。契約の 5 op は Rust のネイティブ経路、それ以外は DLL に埋め込んだ CPython 経由で、**どちらの経路で走ったかが必ず結果に返ります**。型付き台帳の 1,024 op と多入力の 17 op は、画像 / 領域のハンドル 1 つでは運べないので理由つきで拒みます(次の版の課題)。
- **検索に載る op が 918 → 1,942 になりました。** 機械可読索引が型付き台帳 33 族(光学・3D・信号・音響・トモグラフィほか)を数えるようになったためで、各展示の「使用 op」から辿れる範囲が広がっています。既存 op の画素の振る舞いは変えていません。
...
## 26. 全数の 2-D 検査と抜き取りの 3-D 検査を対応づける ―― 格子は自分自身に重なる
...
## 27. 竣工した部屋の壁を測る —— 外接直方体は寸法でなく部屋の向きを測っている
...
## 28. 電極の呼吸を µm で測る ―― サブピクセルなら何でもよいわけではない
...
## 31. ダイの傾きと TSV の位置ずれを 1 つの CT から分ける ―― 傾きは回転まで偽装する
...
## 35. 白色干渉計でナノメートルの段差をどこまで正確に測れるか
...
## 37. 多ビーム測深で海底が笑う ―― 音速を取り違えると、壊れるのは外側ビームだけ
...
## 38. 粒度分布を画像から測る ―― 融合と縁切れが逆向きに効き、途中で打ち消し合う
...
## 39. 光弾性で応力を測る ―― 巻き戻しが最初に失敗するのは等方点
...
## 41. 実写のコインを数えて測る ―― 当たっている答えに、余裕があるとは限らない
...
## 42. ねじの輪郭からピッチ・フランク角・有効径 ―― 傾きは左右のフランクに逆符号で出る
...
## 43. 堆積物の在庫量 ―― 誰も測っていない「山の下の地面」が答えを決める
...
## 45. 表面粗さ Sa / Sq / Sz は標本化とカットオフにどこまで耐えるか
...
## 46. 骨梁の厚さ・間隔・骨体積率 ―― 平板モデルと直接法は同じ画像で別の値になる
...
## 47. 重なった細胞をどう数えるか ―― 個数・過分割・過統合を別々に測る
...
## 48. 蛍光の共局在は漏れ込みで嘘をつく ―― Pearson と Manders は別の場所で壊れる
...
## 49. MRI のバイアス場と組織面積 ―― 灰白質と白質は逆向きに壊れ、足すと隠れる
...
## 51. 血管網を抜いて分岐を測る ―― ヒゲ、分岐近傍の径の過大、そして指数の脆さ
...
## 55. 変化検出と位置合わせ誤差 ―― 偽陽性はエッジの帯、しかも崖つき
...
## 56. 疎な温度センサから 3-D 熱場を復元する ―― 格子の死角がラックを消す
...
## 57. 地形を測る ―― 傾斜・水の流れ・日当たりを閉形式と突き合わせる
...
## 59. 座標は「もっともらしい数字」のまま数十メートル間違う ―― 楕円体高・測地成果・平面近似
...
## 61. 太陽光発電所のドローン熱画像 —— 温度差を測っているつもりで、風と角度を測っている
...
## 62. 実写の深宇宙に既知の星を仕込む ―― 汚染は測定値と信頼度を同じ向きに嘘つかせる
...
## 63. 河川表面流速を斜め動画から測る(LSPIV)―― 速度の誤差と流量の誤差は別に数える
...
## 66. 縁が暗い天体の輪郭はどこか ―― 周辺減光があると「50 % 法」は半径を小さく見る
...
## 67. 星の位置は何分の 1 画素まで測れて、どこで崖に落ちるか
...
## 69. 畑の緑を数える ―― 被覆率の真値を画素ごとの葉の面積率で持つ
...
## 70. 河川の水位を斜め写真から測る ―― 透視を無視した「行番号」は弓なりに外れる
...
## 71. 手ブレはどこまで戻せるか ―― 核を自分で作り、掛けて、戻して、元と比べる
...
## 72. 疑似カラーは読み手の判断を変える —— 無い境目を数え、位置を先に当てる
...
## 73. ハエの複眼は光場センサ ―― 神経重ね合わせの「重ねると頑健」は膝の手前まで
...
## 75. 霞を剥がす ―― 大気散乱モデルで真値を作り、透過率と大気光を別々に採点する
...
## 76. 光子を数えて距離を測る ―― 何個数えれば何ミリまで出るのか
...
## 77. ハエの視覚前段を op の連鎖で ―― 合成の空を回る/前進すると、何が読めて何が読めないか
...
## 79. ライトフィールドから深度を出す ―― 既知の深度で作った光場に、ゼロ点を並べて突きつける
...
## 81. 超解像は情報を増やすのか ―― 真値を持ったまま縮小して、戻して、数える
...
## 82. 動画から固有振動数・減衰比・モード形状を同定する ―― f は最後まで生き残り、ζ が先に嘘をつく
...
## 85. 構造物の微小振動を映像から測る ―― モーション拡大は「測る」役に立つのか
...
## 86. 粒子追跡を (行, 列, 時刻) の体積として測る ―― 誤リンクの向きは 1 種類ではない
...
## 90. (x, y, t) で数える ―― 通過台数とオクルージョン、そして L/V という 1 つの定数
...
## 91. 庫内の滞留はどこで生まれたか ―― 待ちの種類を分けずに数えると全部「混雑」になる
...
## 96. 点群位置合わせの収束域 ―― 初期姿勢がどれだけずれたら壊れるか
...
## 97. 「回転しても同じ」と言える量はどれか —— 実写の硬貨を 72 角度で回して数える
...
## 98. 多波長で層を剥がす ―― 下絵・地塗り・上塗り・褪色を、真値を握ったまま分離する
...
## 99. 偏光で鏡面反射を剥がす ―― フレネルの式で真値を作り、分離結果を突き合わせる
...
## 100. 実写の免疫染色を色で分ける ―― 見張り役が、見張るべき誤りにだけ盲目だった
...
## 101. 色恒常性(ホワイトバランス)―― 「効く手法」は無い、あるのは効く条件だけ
...
## 102. 手持ちで撮った書類をまっすぐに戻す ―― 台形補正と影除去を、真値と突き合わせて測る
...
## 104. 絵画のひび割れ網 ―― 3 指標のうち撮影条件で壊れるのは分岐次数だけ
...
## 105. カメラ指紋(PRNU)で「どのカメラで撮ったか」を当てる ―― 指紋は枚数で育ち、保存ボタンで消える
...
## 108. CAD と実測点群の差分検査 ―― 合わせた分だけ欠陥が消え、無い所にへこみが出る
...
## 109. 作物の葉面積を上から測る —— 隠れるより先に、投影が畳んでしまう
...
## 110. 接合層のボイドを 1 個の数字に畳む ―― 畳んだ分だけ、寿命に効く形が消える
...
## 111. 造形しやすさを形から測る —— しきい値に貼りついた面は、丸めた分だけ判定が飛ぶ
...
## 114. 壊れたメッシュを直してから測る ―― 消えるのは欠陥の数で、戻るのは量ではない
...
## 116. 配管内面の減肉を展開図で測る ―― 軸を決めた分だけ、管底の腐食が消える
...
## 117. 積層の反りは層の履歴が決める —— 均した面積は置き場所を捨てる
...
## 118. 人と機械の安全距離 —— 代表点に置き換えた分だけ、危険が消える
...
## 119. 設計と実物の食い違いを部屋から測る ―― 合わせの妥協角は、無傷の部材へ配られる
...
## 120. 構造物を年ごとに測り返す —— 測る場所がずれると、劣化は進んだように見える
...
## 121. 対称性で欠けを補う —— 仮定した面がずれた分だけ、復元は嘘をつく
