> **言語 / Language**: **English** · [日本語](https://github.com/furuse-kazufumi/fullseye/blob/master/docs/articles/fullseye_overview_qiita_ja.md)
...
![The real shape model of asteroid Itokawa (49,152 facets, nothing decimated) on a turntable in a physically based custom renderer — Hapke BRDF, hard 0.53-deg-sun ray-cast shadows, zero ambient, band-limited relief and angular boulders (all hand-written in numpy)](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/showcase_turntable_itokawa.gif?v=2)

This is the real shape model of asteroid **25143 Itokawa** — the Gaskell model built from Hayabusa spacecraft observations, published in the JAXA DARTS archive, all 49,152 facets **kept, nothing decimated** — with the camera and the Sun of this article's protagonist **Fullseye**'s custom 3D renderer orbiting it (phase angle fixed at 45°). Loading the shape, adaptive tessellation to 1.5 m facets (geometry unchanged), band-limited relief per wavelength, angular boulders scattered on the D^-3.1 law, the Hapke BRDF, shadows ray-cast with the Sun's angular diameter — **all of it is hand-written numpy** (zero ambient light, one exposure for every frame).
...
## A Science Museum on Paper — 151 Exhibits of Playing With Ops
...
### The Astro-Stacking Wing — Watching a Theoretical Limit Break
...
<sub>`wingstudio_studio_help.gif` — 24 frames / 3 fps / 1000×720 px / 0.53 MB / SHA-256 `49fc9ab4fffe092c`</sub>
...
<sub>`wingstudio_studio_editor.gif` — 24 frames / 6 fps / 1060×740 px / 0.36 MB / SHA-256 `ce1154f194aeb759`</sub>
...
<sub>`wingstudio_studio_opsearch.gif` — 17 frames / 4 fps / 1280×800 px / 0.54 MB / SHA-256 `7c1af28cc7a99e86`</sub>
...
<sub>`wingstudio_studio_pipeline.gif` — 24 frames / 4 fps / 1280×800 px / 0.54 MB / SHA-256 `e395b77263a3e87a`</sub>
...
<small>A single frame still reads (still thumbnail: [wingopt_pipeline_flow_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_pipeline_flow_thumb.jpg)). 6 frames / 700 ms per frame / 940×514 px / 0.30 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_limit_crossover_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_limit_crossover_thumb.jpg)). 42 frames / 10 fps / 1000×474 px / 0.46 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_cos4_falloff_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_cos4_falloff_thumb.jpg)). 36 frames / 10 fps / 1000×494 px / 1.66 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_mtf_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_mtf_thumb.jpg)). 34 frames / 10 fps / 1000×536 px / 0.99 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_dof_coc_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_dof_coc_thumb.jpg)). 37 frames / 10 fps / 1000×496 px / 0.44 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_res_vs_dof_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_res_vs_dof_thumb.jpg)). 43 frames / 10 fps / 1000×548 px / 0.33 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_airy_rayleigh_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_airy_rayleigh_thumb.jpg)). 33 frames / 10 fps / 1000×516 px / 2.31 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_polarizer_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_polarizer_thumb.jpg)). 31 frames / 10 fps / 1000×492 px / 2.65 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_abcd_rays_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_abcd_rays_thumb.jpg)). 39 frames / 10 fps / 1000×474 px / 0.53 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_illumination_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_illumination_thumb.jpg)). 33 frames / 10 fps / 1000×502 px / 0.30 MB.</small>
...
<small>A single frame still reads (still thumbnail: [wingopt_pixel_pitch_thumb.jpg](https://raw.githubusercontent.com/furuse-kazufumi/fullseye/master/docs/articles/assets/thumbs/wingopt_pixel_pitch_thumb.jpg)). 40 frames / 10 fps / 1000×502 px / 0.58 MB.</small>
...
## What Grew on 2026-09-20 — a Third-Party AI Used It Hard, and Every Finding Was Verified One by One

No ops were added this round. Instead, an external AI (GenSpark) was asked to **use 0.2.0 hard in two environments — core (`pip install fullseye`) and all (`fullseye[all]`)** — and sent back 54 reports (197 findings, N1–N197) with the actual outputs. The procedure was plain: **reproduce each one on the current master; fix the bug-side items with a test, a runnable example and a hardening note in the same commit; keep the design-side items in a table with the reason they stay**. Several findings were later withdrawn by the reporter itself ("87 ops collapse to identity" was the fallback for inputs of the wrong sort; "there is no Insert button" was an oversight) — **a third-party finding is a probe, not a verdict**. Reproduce first, then adopt.

The fixes shipped as **0.2.1** (`pip install -U fullseye`, [Release v0.2.1](https://github.com/furuse-kazufumi/fullseye/releases/tag/v0.2.1), Zenodo version DOI [10.5281/zenodo.22851588](https://doi.org/10.5281/zenodo.22851588)). No new ops and no change to the typed-contract vocabulary, hence a patch release.

| What was fixed | Before | After |
|---|---|---|
| **"Absent" vs "not installed"** | An op whose optional backend is missing and a misspelled op both raise the same `KeyError: unknown operator` | From the shipped index (module / requires), `MissingBackendError` (a `KeyError` subclass) **names the missing extra and the `pip install "fullseye[...]"` line**. A name that truly does not exist points to `fullseye has` and `op_find` |
| **How to write `run_pipeline` stages** | Mixed tuples / dicts / strings gave opaque TypeErrors | Five forms (`"a,b"`, names, tuples, `(name, {"a": ..})`, dicts) give the same result; a wrong form raises a TypeError that **names the cause** (swapped arguments, unknown knob, a stage that does not start with a name) |
| **Inputs with no conversion** | String and complex arrays passed as "all zeros" under the fallback policy | TypeError **regardless of policy**. Turning an input with nowhere to go into zeros is not a fallback, it is a lie |
| **Empty and tiny inputs** | 271 ops died with raw scipy / numpy errors (`axis 1 is out of bounds`) | Zero-size inputs stop at the gate with one sentence; exceptions from inside an op on a tiny image are **annotated with the op name and the input shape** (type and message unchanged) |
| **Non-finite outputs** | NaN / inf outputs were sanitized silently | Recorded in the ledger as `non_finite_output: N of M`; `on_error="raise"` stops. Ops that return NaN by design (`tb_fly_tau_from_expansion`) sit in an exemption list |
| **CLI** | No `--version`; help examples in the checkout spelling; `parity` collided with argparse; `samples` demanded an action; `coverage` hit FileNotFoundError in the wheel | All fixed. **The help examples follow how you invoked it** (`fullseye` vs `py -3.11 imgevolve.py`) |
| **Studio's run key** | Ctrl+R unbound; F5 ignored unapplied edits and ran the old pipeline | F5 / Ctrl+R **apply pending edits first**, then run. A single-op run that fell back says "FALLBACK" in the title |
| **`engine.load` on an instance** | `e = FullseyeEngine(); e.load(p)` discarded the classmethod's return value, and the empty engine returned the input unchanged | Loads into the instance (`from_dict` / `from_ops` too). Dict stages accepted; a bad stage or an out-of-range `upto` raises `ValueError`; `to_python` starts with a coding line and the `--ops` string |
| **Empty op names, narrow floats** | `apply(img, "")` resolved to `lowpass` (it matched the empty HALCON alias) and ran. float16 / float32 were not promoted to the float64 contract; scipy rejects float16 and a copy of the input came back | Empty names are unknown. float16 / float32 are **promoted losslessly to float64** (not recorded) |
| **Pose helpers and matrices** | `pose_to_hom_mat3d` could not take its own 4×4 output back | Accept 6/7-vectors, 3×3 and 4×4 |
| **N-ary listing and knobs** | The 17 n-ary ops (`add_image`, …) were callable but absent from `op_names()`; whether `a` / `b` do anything was prose only; no way to call a two-input op from the CLI | `op_names(include_nary=True)` (default unchanged), a `knobs` field on every `list_ops()` row (461 ops measured, `None` when not), and `fullseye apply add_image a.png out.png --input2 b.png` |
| **Ledger lookups** (reports 15–22) | `op_producers("gaussian")` returned `[]` silently, `op_presets("no_such")` returned `{}`, `op_run("write_wav")` used the demo array as a file name and the stdlib `Wave_write` printed "Exception ignored", inputs with no sample seed were passed as `None` and died with IndexError | Unknown sorts, op names and ops are **ValueError** (`op_sorts()` lists the sort names); a path parameter is never a data input in the ledger; `op_run` says "no built-in sample for input sort 'lab' — pass it explicitly" |
| **Image I/O** (reports 18–19) | `write_image` discarded OpenCV's `False` and **returned silently with no file** (ppm, missing parent directory, unwritable extension); `read_image` raised FileNotFoundError for everything; a `uint16` image was crushed to 8 bits on write although reads keep 16 bits | The facade delegates to `imgio`: unwritable targets stop with one sentence, read failures are told apart (a directory is IsADirectoryError), `uint16` stays 16-bit, float is documented as 8-bit by default with `depth=16` / `"float"` lossless, and 8-bit quantisation rounds (error 1/255 → 1/510) |
| **Non-array images** (report 26) | `apply("abc", "gaussian")` **returned "abc" unchanged** under the default fallback policy (a str / dict / scalar is not an ndarray, so the check never ran) | Ops that take a raster convert first and run the same check; a 0-d input is a TypeError under every policy |
| **Name resolution, made visible** (report 20) | Several ops claim the same HALCON alias (68 of 387) and the rows did not say which one runs; `list_ops(sort="IMAGE")` matched nothing | The rule was always one (exact name, else an explicit table). Rows carry `halcon_peers`, a gate requires an explicit row for every shared alias without a canonical op, sort / search fold case and accents, and an unknown sort is a ValueError |
| **One rule for stage forms** (report 53, 0.2.2) | The same stage `{"op": "otsu"}` ran under `run_pipeline`, but `diagnose_stages` said "unknown operator {'op': 'otsu'}" and `fullseye.Pipeline` died with `unhashable type: 'dict'` — four places read stages and only two had gates | Op-name extraction lives in one function, `engine.stage_name`; a gate pins that all four entry points read a stage alike, and a broken stage becomes an error row of the validator |
| **Classes leaked into the MCP search surface** (report 41, 0.2.2) | "Callable means op" let 41 classes (`Image`, `Pipeline`, `MissingBackendError`, `TcpChannel` …) appear as ops in `fullseye_search_ops` | Classes and modules are excluded (facade 1,069 → 1,028, not one real function lost). The "474 names not in the table" compared against the HALCON mapping table; the remaining 460 are real functions |
| **Ledger overflow, studio `--help`, namespace** (reports 43, 44, 50, 0.2.2) | The bounded ring dropped old events silently so counts did not add up; `fullseye-studio --help` aborted (SIGABRT) on a display-less Linux; `fullseye.os` resolved | `fallback_overflow()` reports evicted events (an identity is the gate); `--help` / `--version` answer without starting Qt and a missing display stops with one sentence; import helpers are removed from the facade |
| **strict_mode covered only some guards** (report 55 = re-measurement of 0.2.1, 0.2.2) | `with strict_mode(): apply(gray, "access_channel")` returned an array and only logged the fallback (`on_error="raise"` did stop); `op_run(img, name)` died with unhashable, `python -m fullseye` did not exist, and `fullseye index` from the wheel wrote a third index into site-packages | strict now means raise on every path when `on_error` is None; `op_run` explains the order and points registry ops to `apply`; `python -m fullseye` is the CLI; `index` names the difference from the shipped index with reasons and never writes inside the package |
| **Wrappers, missing inputs, empty success** (remaining candidates reproduced, 0.2.2) | `apply(Image(img), …)` stopped with "scalar Image"; `op_run("blend_mode", rgb)` raised a raw TypeError; `fullseye algo run quicksort` printed `[]` with rc 0 when `--seq` was missing; `op_find` returned an empty `doc` | `Image` is unwrapped to `.array`; a missing input reads "missing input(s) top (rgb) — this op takes base, top"; a missing `--seq` is refused with an example; `doc` is filled from the note's first line |

**Design kept, with reasons on record (20+ items)**: the default `on_error="fallback"` (an industrial line does not stop a batch for one bad frame; set `FULLSEYE_ON_ERROR=raise` for strict runs) / one warning per op (the ledger `fullseye.fallbacks()` has every event) / knobs `a`, `b` in [0,1] (the evolution genome; out-of-range is recorded and clamped) / 2-D ops accepting 3-D input stays a known issue in the ledger / index tiers are summed (registry 931 + n-ary 17 + ledger 1048) / `op_names()` does not mix n-ary in by default / no `apply2` — and so on.
