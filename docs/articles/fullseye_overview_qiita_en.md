...
**Design kept, with reasons on record (14 items)**: the default `on_error="fallback"` (an industrial line does not stop a batch for one bad frame; set `FULLSEYE_ON_ERROR=raise` for strict runs) / one warning per op (the ledger `fullseye.fallbacks()` has every event) / knobs `a`, `b` in [0,1] (the evolution genome; out-of-range is recorded and clamped) / 2-D ops accepting 3-D input stays a known issue in the ledger / index tiers are summed (registry 931 + n-ary 17 + ledger 1048) / `op_names()` does not mix n-ary in by default / no `apply2` — and so on.
