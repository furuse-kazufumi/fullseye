...
- 2026-09-24 — Every Speedup Is the Same Arithmetic Regrouped: Scoring Attention with Identities (poc_attention_identities)

## The wings (151 exhibits)
...
*↑ **Every Speedup Is the Same Arithmetic Regrouped: Scoring Attention with Identities** ―― Every speed and memory trick in the lineage that leads to large language models turns out to be a regrouping of the same arithmetic rather than an approximation, and each identity can be checked to machine precision. The operators are a new family of ten, written in numpy alone - no torch - and the input is a sequence of image patches (a 64 by 64 picture cut into sixty-four eight-by-eight tiles), so attention here is literally the entrance to a vision transformer and the claims hold on the picture side too. The first finding is that tiled attention with an online softmax, the core of FlashAttention, is exact: across seven tile sizes from one row to sixty-four the worst disagreement with the one-pass computation is 1.33e-15. What makes it fast is not a different computation but never forming the T by T matrix at all. A prediction was wrong here: error was expected to accumulate with the number of tiles, and it does, but sixty-four tiles give 1.33e-15 against 1.22e-15 for one - a factor of 1.1. The second is that linear attention is associativity and nothing else: (QK^T)V and Q(K^T V) agree to 1.18e-15, and the causal running-state form to 9.39e-16. The quadratic and linear costs are two bracketings of one product and the crossover sits at T equal to d. A second prediction was half right: the ratio grows with T over d, but at T of 2048 it stalls at fifteen or sixteen instead of the thirty-two the formula promises, because the quadratic path becomes bandwidth-bound. The third is that under a causal mask, rewriting the future does not move a single bit of the past - exactly zero - and this is not a tautology, because the keys and values from row thirty-two onward really were replaced with different random numbers, and the second half moves by 0.403. That is the reason a key-value cache is sound, and stepping one token at a time matches the batch forward pass to 5.92e-16. The fourth is that the sparsity of attention is an integer: a causal mask leaves exactly T(T+1)/2 = 2080 non-zero weights and a window of nine leaves the sum of min(i+1, W) = 540 - claims that need no floating-point tolerance at all. The fifth is that without a positional code attention does not see the order of the patches: shuffling them and unshuffling the output returns the original to 4.4e-16, while with rotary embedding it does not (0.018). That is precisely what a positional code is for, and on the picture it is the difference between the same image and a different one. The sixth is that rotary embedding is a rotation, so it preserves the norm - 4.44e-16 across all sixty-four rows - and its inner product depends only on the relative position: holding the difference at seven and sliding the absolute position through forty values spreads the product by 1.11e-15. The seventh is that grouped-query attention collapses exactly onto the two schemes it interpolates: with as many key-value heads as query heads it equals multi-head attention to the last bit, with one it equals multi-query attention, and the middle differs from both by 0.942. Alongside these, RMS normalisation returns an output whose RMS is exactly one (off by 2.22e-16, and by 1.53e-04 once an epsilon of 1e-6 is added - that is what the epsilon costs); attention is a convex combination so its output never leaves the input range, yet the range collapses by a factor of 35.4, because averaging destroys contrast; and subtracting the row maximum is not decoration, since at large scores the naive exponential breaks fifteen of sixty-four rows and the stable form breaks none. One more prediction was wrong and is kept: the intermediate state of the online softmax was expected to approach the answer monotonically, and it does not - it rises before it falls - because the running maximum rebuilds the denominator each time it updates, so a partial value is not even an approximation. Eight distinct mistakes are refused by name, so it is always readable which operator rejected what. Twenty-seven checks and ten figures.*
