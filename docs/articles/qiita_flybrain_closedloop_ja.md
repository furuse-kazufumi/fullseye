...
いま走っているのは 2 つです。**配線をシャッフルした対照群**(次数を保存したまま接続先を入れ替えたネットワークを、同じ初期化から同じ課題で進化させる)で「配線の取り分」を進化の側からも測ること。そして**細胞型を段階的に黙らせる実験**で、どの機能がどの段で消えるかを並べること。

もう 1 つ気になっているのは、第 6 節の乖離です。行動の目的だけでは方向選択性が出ないのなら、**段階的な課題の列**(明暗 → ON/OFF → 向き → 回転)なら出るのか。予備実験では、後の段を学ぶと前の段の成績が落ちる**忘却**が出ました(0.044 → 0.017)。累積的な目的にすれば消えるのか、これから測ります。
