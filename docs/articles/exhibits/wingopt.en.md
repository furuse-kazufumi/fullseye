...
*↑ **The one road from design to verdict** — six steps, "design → limit → virtual part → capture → inspect → verdict", cut into frames you can stop on one at a time. Fixing the system fixes **16.264 µm/pixel**, out of which comes an optical limit of **32.53 µm** (sampling-bound); a 120 µm scratch is then 7.38 pixels, and at the end an IoU of **0.4228** calls it detected — the scoring works because **the ground-truth mask does not move when the capture blurs it** (the verdict is `marginal`). Ops used: `system_geometry`, `resolving_power`, `system_feasibility`, `surface_texture`, `defect_scratch`, `composite_defect`, `defect_stats`, `image_formation`, `draw_polyline`, `draw_circle`.*
...
*↑ **A map of the detection limit** — measure the detection rate over the plane of defect size (horizontal, logarithmic) against contrast (vertical) and **the optical limit of 32.53 µm (sampling-bound) stands still as a vertical line**, while the actual detection boundary (the white line = the measured 50 % contour) lies over to its right. At a contrast of 0.06 it takes 53 µm (1.64x the limit); raise the contrast to 0.40 and 28 µm (0.85x) is enough — **the right-hand side is not a lens problem**. Ops used: `render_part`, `system_geometry`, `resolving_power`, `draw_polyline`, `draw_line`.*
