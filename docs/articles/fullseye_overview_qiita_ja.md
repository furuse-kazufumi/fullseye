...
*↑ 手続き生成した手の骨格（手根骨8・中手骨5・指骨14＝計27骨、名前の話の節に出てきた `hand_hero.png` と同じ被写体）の SDF を占有ボクセル格子に落とし込み、`marching_cubes`（等値面レベル 0.5）でメッシュ抽出して骨格標本風に回したもの。掌を上にして寝かせ、俯瞰で1回転させています。ボリューム→メッシュ→レンダまで一気通貫です。*個々のアルゴリズムは PCL（C++）や Open3D に散在しますが、**「純 numpy・型付き・全 op に機械可読ノート付き」で1つのライブラリがこの範囲を持っている構成は、あまり見ない**と思います。産業の縞投影計測からロボットの把持姿勢まで、同じ書き味で届くのが狙いです。
...
GPU 化については、「性能の正直な話（GPU）」の節に書いた通り、CPU 上の実測はすでにありますが、**op ごとに GPU 常駐させて E2E（エンドツーエンド）で速くする**という本丸は、実機（RTX 5090）が到着してからの検証待ちです。ここで先走って「GPU なら劇的に速くなるはずです」と書かないのは、CPU の数字と GPU の数字を混同しないという規律を、限界を語るときにも同じ強度で適用しているからです。
