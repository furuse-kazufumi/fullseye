...
なぜ CI のフルスイートがこのバグを見逃したのかを、もう一段だけ掘っておきます。CI の `test` ジョブは `pip install -e ".[opencv,skimage,pil,wavelets,gui,video,volume,gltf,lidar,pcd]"` という**盛りだくさんの extras 込みでインストール**してからテストを走らせています（torch だけは重さの都合で外している、というのは第1波の文脈と同じです）。この構成では `scikit-image` が常に存在するので、`recon3d` の無条件 import は**一度も失敗しません**。つまり、フルスイートを何度回しても、**このバグには絶対に気づけない**構造だったわけです。「テストを厚くする」ことと「テストが検証している依存の組み合わせを広げる」ことは別の軸で、今回は後者が抜けていました。**素のインストールを実行して確かめる `core-minimal` ジョブ**（第1波の節で触れた、`import fullseye` と `import ops3d, pipeline3d, measure3d` を素の環境で実行するジョブ）を CI に持っていたにもかかわらず、`v0.1.0` 公開時点ではこのジョブの対象に含めていなかった箇所が、まさにこの穴でした。`v0.1.1` の修正と同時に、この検証範囲も広げています。
