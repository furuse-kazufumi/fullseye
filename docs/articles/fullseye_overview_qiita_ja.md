...
## 追記3(2026-08-31): 3D に「処理領域」と「境界」を、そしてマインクラフト風の歩き回り

> **Status: Production-ready / Verified**(この追記の分で 3D op は **279 → 285**、専用テストと GT 例を追加)

この追記の 2 本は、どちらも**私(著者)の側の気づきが起点**です。ひとつめは「**2D にあって 3D に無いものは何か。HALCON の domain(処理領域)の概念は 3D 側にあるのか? boundary 系は? どちらもメモリ消費を抑えるために必要そうだ**」という問い。目録を突き合わせたら、その勘は両方とも当たっていました ―― 3D 側 55 ファミリのどこにも domain も voxel 境界もいなかったのです。

**1. domain ファミリ(4 op): 512³ の CT を全部持ち歩かない。** `vol_reduce_domain`(マスク外を黙らせる)→ `vol_crop_domain`(前景のタイトな直方体へ切り出し、**戻すための offset ごと返す**)→ 処理 → `vol_uncrop`(元の座標系へ厳密に貼り戻す)。検証例では 96³ の合成 CT から対象の球だけ切り出して**メモリ 1/34(実測)**。3D の Hessian 系 op(血管強調など)にはメモリ都合のサイズ上限があるのですが、その上限を「あきらめる」のではなく「正攻法で通す」ための道具です。

**2. boundary ファミリ(2 op): 中身を捨てて殻だけ持つ。** 2D の `region_boundary` の voxel 版 `vol_boundary`(6/18/26 連結、内側/外側)と、殻をそのまま物理 mm 座標の点群にする `vol_boundary_points`。中実の球は殻だけ残すと **19%(実測)**まで痩せ、しかもその痩せた点群のまま球フィットが**中心誤差 0.000 mm**で閉じました(半径は 4.33/4.5 mm ―― 内側境界のボクセル中心は表面より半ボクセル内側にいるので、**わずかに小さく出るのが正しい**。この系統誤差も例の中で明記して検証しています)。「voxel の世界から点群の世界へ、一番安い橋を架ける」op でもあります。

ここでも敵対的検証を先にかけて、自分のコードから 2 件のバグを摘出してから出しています(crop の既定マスクが 0.5 閾値で灰色ボクセルを取りこぼす/巨大 shape 指定で整数オーバーフローが容量チェックをすり抜ける)。

**3. マインクラフト風ウォークスルー(Studio)。** ふたつめの気づきは「**3D データの中を視点を変えながら歩き回れたら、博物館や科学館で実物を見ているような臨場感が出るのでは**」というもの。Studio の 3D ビューアはこれまで軌道カメラ(対象の周りを回る)だけでしたが、**F キーで一人称モード**に入れるようにしました。WASD で歩き、ドラッグで見回し、ホイールで歩く速さを変え、R で入口に戻る。臨場感の核は**透視投影**(近いものは大きく、遠いものは小さく)で、この投影は自作のソフトウェア描画なので、**ピンホールカメラの解析式と 10⁻¹³ の精度で一致すること**を独立に検証しました。遠近で明るさと点の大きさも減衰させています。軌道カメラ側のコードは 1 行も変えていません(既存テスト 178 件が回帰ゼロ)。

CT の中を歩いて欠陥を探す、点群の遺跡スキャンの中を歩く ―― 「データを見る」が「データの中に立つ」に変わると、案外気づくものが違います。

## 追記4(2026-08-31 深夜): 3D 大増強、RLE の正体、そして「1D は足りないのではなく繋がっていなかった」

> **Status: Production-ready / Verified**(この追記の分で 3D op は **285 → 310**、1D 目録 **37 op** 新設、テストは **6,535 件**全緑)

追記3 の後、「他に欠けているものは?」「もっと効率的な方法は?」「1D も足りないのでは?」と掘り続けた一晩の成果です。3 つの発見がありました。

**1. HALCON region の効率の正体は run-length でした。** HALCON が 1000 万画素の画像から抜いた領域を何千個もメモリに置けるのは、region がビットマップでなく**水平線分(run)のリスト**だから。3D 側にその等価物を作ったら(`vol_rle_encode` 系 9 op)、384³ の部品マスクが dense の **1/145(実測)**になり、体積・バウンディングボックスは復号せず run のまま **300〜1000 倍速**で答えが出ました。和・積・差の集合演算も run のまま — 192³ 同士で **3.1ms** です。おまけに「run の中はラベルが必ず一定」という構造上の事実を使うと、成分分解も run 単位の振り分けで済みます。メモリ 3 兄弟(crop=どこを計算するか / RLE=何を保持するか / タイル処理=一度に RAM に載せる量)がこれで揃いました。

**2. 3D の残りギャップを一晩で 15 op 埋めました(並列 AI 3 系統+統合)。** CT windowing(放射線科が毎日回す「窓」— 軟部窓では骨が飽和し、骨窓では軟部が背景と同化する、を機械検証)、幾何変換(回転方向の規約を `np.rot90` との bit 一致で機械固定 — 符号の反転は敵対検証の代表格なので、テストが規約の番人になるようにしました)、virtual probe(プローブ 1 本でパイプ壁厚 2.042mm vs 真値 2.000mm)、3D FFT フィルタ、Richardson–Lucy デコンボリューション。

ここで白状すると、RL の最初の docstring に私は「10 反復で RMSE 半減」と書きました。実測したら **0.81x** です。半減しない理由ごと(残差は縁の階段が支配し、RL はそこをゆっくりしか直せない)docstring を書き直し、代わりに RL が本当に最適化している量 — 推定を再ぼかしすると観測に 0.021x で一致する「前方一貫性」— を正直な主張として固定しました。

さらに敵対的検証で**確定バグ 9 件**。旗艦は「**偶数サイズの PSF で RL の随伴演算子が 1 ボクセルずれ、10 反復で 10⁹¹ に発散する**」— 'same' 畳み込みのクロップ規約が偶数軸で真の随伴からずれる、という数学のバグです。修正後は随伴の内積恒等式が誤差 0.0、奇数 PSF では旧実装とビット一致(挙動不変)を確認しています。ほかにアンダーフロー→NaN の無言化 3 件、無言の切り捨て 2 件など — 「符号の反転」と「ゼロ割(とその親戚のアンダーフロー・ほぼゼロ正規化)」はやはり敵対検証の二大横綱でした。

**3. 1D は「足りない」のではなく「繋がっていなかった」。** HALCON の funct_1d ファミリ(1D 関数の平滑化・微分・極値・照合、23 関数)に対応する実装が、リポジトリの中で**完全な孤児**になっていました — どこからも import されず、パッケージ登録もなく、テストもカタログもなし。さらに調べると facade の対応表は既にこのモジュールを参照していて、つまり **PyPI からインストールした版では funct_1d 系 23 op が ModuleNotFoundError で全滅していた**ことも判明(出荷バグとして修正)。品質採用(fail-closed 化+解析 GT テスト 41 件: sin の微分≈cos、ゼロ交差=kπ、減衰振動から周期・時定数・遅延を真値どおり復元)し、信号処理 16 関数(dsp)と合わせて **1D 統一目録 37 op** を新設しました。2D の measure1d・3D の probe・センサー系列は、どれも「プロファイル (x,y) を取り出す」で合流し、その後段が funct1d — 3 つの次元が一本の線で繋がります。

ギャップ監査の教訓をひとつ: 「無い op」を探すだけでは足りません。「**あるのに世界に接続されていない op**」— 実装されテストに値するのに、目録・パッケージ・facade のどこかで鎖が切れているもの — を探すこと。今日それで 23+16 個の関数が息を吹き返しました。

## 追記5(2026-09-01): op を鎖にして揺さぶる — 拡散収束ファザーが「単体では見えないバグ」を掘り当てた

> **Status: Converged / Verified**(連鎖ファザー 2000 本で疑義署名 **0 件**まで収束、テストは **6,630 件**全緑)

この追記は、著者の「**各 op をつなぐ方策を実施して、昔やったように拡散と収束を繰り返して、そのなかで不具合を発見していく流れをやってみましょう**」という一言から始まった数日分です。個々の op の単体テストと敵対的検証は追記4 までで散々やりました。次の敵は、**単体では全部通るのに、op を鎖にしたときだけ現れるバグ**です。

**仕組みは「型付きプール+ランダム連鎖+署名収束」。** 各 op の宣言(in 型 → out 型)に従って、プールから引数を引き、ランダムに op を連鎖させ(拡散)、産物をプールに戻す。例外や異常はすべて (種別, op, 例外, メッセージ) の**署名**に潰して数える(収束)。署名の分類がこのファザーの心臓部です — 文書化された ValueError は **CONTRACT(白)**、生の TypeError/IndexError は **SUSPECT(契約の穴)**、宣言と違う型を返したら **TYPEMISS(型の嘘)**、NaN/Inf の無言戻りは **NONFINITE**、そして産物がプール上限を超えたら黙って捨てずに **GROWTH として記録**します(切り捨てを無言でやらない、はファザー自身にも適用するルールです)。

**ファザー自身も 2 回、現場で鍛えられました。** 初走行は拡大系 op の連鎖でプールの体積が指数増殖し、34GB の voxel に半径 1 のモルフォロジーが 20 分貼り付く「ハングもどき」で停止(→ プール投入上限+GROWTH 記録で修正)。次は **4 万制御点の TPS フィットが内部で 12GB の密行列を確保**して停止 — こちらはプール上限では防げません。入力は小さいのに **内部割当だけが巨大になる**、敵対的検証の新しい家系(第 6 家系)です。同じパターンで PSF 生成(σ に桁違いの値が紛れると 64GB)、CPD 対応行列も一掃し、op 側に fail-closed の上限を置き、TPS 変形はチャンク化して**ビット一致のまま**有界メモリにしました。

**一番の収穫は、3 波にわたって追いかけた幽霊の正体でした。** fit_zernike(Zernike 係数フィット)が稀に全係数 NaN を返す — 単体では何度やっても再現せず、推測でパッチを当てるのは禁じ手なので3 波の間「グレー」のまま持ち越していました。今回の連鎖トレースでようやく再現条件が判明: **float64 では有限な 1e39 級の値(増幅系連鎖の産物)が、torch 経路の float32 キャストで inf に化ける**。すると grid_sample と lstsq が NaN を**無言で**返します。修正は「キャスト後の有限性を検証する」ヘルパ 1 個 — ただし同じキャストは 20 箇所あるので、**1e39 を実際に流し込んで**確認し、無言 NaN を出した 4 op(fit_zernike / polar_unwrap / cylinder_unwrap / scene_flow_lk)だけを直しました。sobel3d などは実測で無事だったので触っていません(実測該当だけ直す、も規律のうちです)。

**もう一つの発見は「inf はバグではなく、契約かどうか」という整理です。** ファザーは sdf_subtract(SDF の差集合)の inf 出力に旗を立てました。でも調べると、出所は esdf(ユークリッド符号付き距離場)の「全部自由空間なら +inf」という**文書化された契約**で、min/max 代数はそれを**数学的に正確に**伝播していただけ — つまり無実です。ところが同じ家族の sdf_smooth_union(滑らかな和集合)だけは、内部が算術(inf−inf、inf×0)なので **inf 入力で全 NaN** — こちらは本物のバグでした。ブレンド帯 |a−b|<k が inf では退化することを使い、厳密に min へ退化させて修正。レジストレーション系の rmse=inf(「マッチなし」の番兵値)も契約と判定し、ファザーの許可リストに明記しました。**「非有限を出した op」を責めるのではなく「非有限の出所と契約」を追う** — これがこの回の教訓です。

ほかにも入口契約の穴を 2 系統(FPFH 記述子に**別の点群の法線**を混ぜると近傍 index が範囲を越えて生 IndexError → 対応検証を入口に / エピポーラ幾何系に (N,1) 列ベクトルを渡すと重心計算の c[1] で落ちる → (N,2) 契約を 4 入口に)。数学ファミリの第一陣 16 op(線形代数・統計・多項式補間 — 「数学辞典に載る問題を全部」を北極星に、まず計算可能な足場から)もこの期間に入りました。

収束の軌跡だけ数字で: 初回走行 **103 署名** → 型の嘘 22 件と契約の穴を三段で修正 → 最終走行(2000 連鎖・同一 seed)で**疑義署名 0 件・残るは文書化済み ValueError の 63 種(白)だけ**。「拡散と収束」という言い方は比喩ではなくて、署名数が実際に単調収束していく様子を毎走行の集計で眺められます。単体テスト・敵対的検証に続く**第三の品質保証層**として、このファザーは CI 級の常備品になりました。

## 追記6(2026-09-01): 部品を買う前に「見えるかどうか」を動画で確かめる ―― 仮想マシンビジョン環境と、新しい 2 分野

> **Status: Measured / Verified**(新規 34 op、連鎖ファザー 1500 本で**疑義署名 0 件**・新 2 族とも **17/17 到達**、テスト **7,300 件超**全緑)

追記5 のファザーを走らせながら、著者から続けて 3 つ要望が来ました。「**vision award にある様な事に繋がる機能があるといいね**」「**マシンビジョンの仮想環境ができる方が良いのは確かだし、そのつもりで光学系の op をそろえてる**」「**動画でデモが出来るようになってるといいね**」。この追記はその 3 つに答えた数日分です。

### まず動画から ―― 「何 µm から見えるのか」を目で見る
...
左が仮想の部品を撮った画像、右が検査器の答えと正解マスクの重ね合わせです。欠陥を **20 µm から 400 µm まで 48 段階で大きくしていく**と、あるところで急に検出が始まります。画面の数字は全部その場で計算した実測値で、絵に合わせて書いた飾りではありません。

この系(f=35 mm・作動距離 200 mm・f/4・画素ピッチ 3.45 µm・2448×2048)で測った値はこうです。
...
**光学限界の 1.41 倍**の大きさが要る、というのがこの構成の答えです。この「1.41 倍」が本題で、**大きければアルゴリズムか照明の問題、1 倍に近ければレンズの問題**という切り分けがここで決まります。カタログの数字を眺めていても、この比は出てきません。

もう 1 本は逆向きの掃引 ――「**設計を変えると限界が動く**」です。
...
欠陥は 100 µm に固定したまま、作動距離だけを 120 mm から 700 mm まで動かします。画素サイズが 8.38 → 65.55 µm に膨らみ、光学限界が 18.40 → 131.13 µm へ悪化していく。見どころが 3 つあって、**律速が途中で入れ替わる**(作動距離 160.5 mm を境に、回折律速から標本化律速へ)、**光学限界が 100 µm を追い越すのが約 551.6 mm**(ここから先は原理的に解けない帯)、そして**実際に検出が保つのは約 403.3 mm まで**。原理の限界より 150 mm ほど手前で、実力の限界が先に来ます。

動画はどちらも決定的で、同じコマンドで再生成すると **SHA-256 まで一致**します。描画は Fullseye 自身の `imagedraw` op と numpy 合成で、matplotlib は使っていません(数値ラベルだけ Pillow のテキスト描画)。GIF と mp4 は同一のフレーム列から書き出し、書き出し後に読み戻してフレーム数を照合しています ―― 動画は「それらしく見える」ことが致命的に簡単なので、検証を挟まないと自分でも信用できません。

### 仕組み ―― 「画像を作る」のではなく「限界を返す」

この仮想環境は 3 層です。**`visiondesign`** が閉形式の光学(ガウス結像式・Nyquist・Airy 基準・錯乱円・cos⁴ 則)、**`defectgen`** が数式による欠陥生成、**`visionlab`** がその全部を「設計 → 限界 → 部品を作る → 撮る → 検査する → 判定」の一本に繋ぐ層です。

**レンダラを持たない**という制約が、むしろ設計を決めました。モンテカルロ光輸送は Fullseye の依存方針とも計算方針とも合いません。そこで**「画像」ではなく「限界」を返す**側に倒しています。分解能・被写界深度・周辺光量・コントラスト伝達は、どれも数十年前から教科書にある閉形式で、実装は**それらの配置**にすぎません。

`defectgen` の方は「欠陥は確率幾何である」という見方です。傷はランダムウォーク、孔食は点過程、割れは分岐する経路、質感は帯域制限ノイズ。**この見方の利点は、絵が綺麗なことではなく、画素完全な正解マスクが定義から無料で出てくること**です。注釈を付ける作業が存在しません。

### 「落とした理由を混ぜない」を 2 回やった

この層で敵対的検証が捕まえた不具合のうち、同じ性質のものが 2 つありました。どちらも**「検出できなかった」に別の理由が紛れ込む**という形です。

1 つ目は掃引の集計。**1 画素未満で描けなかった**ものと、**外から渡された検査器が例外を投げた**ものを、検出率 0% に混ぜていました。混ぜると、設計やアルゴリズムが不当に責められます。今は `unrenderable` と `detector_failed` を別々に数え、評価できた試行が 0 なら検出率は `0%` ではなく `None` を返します。

2 つ目はもっと質が悪くて、**レポートが「光学限界に未到達」と書いていたのに、実はレンズはちゃんと解像していた**というものです。原因は、`resolvable` という 1 つの判定に **横分解能**と**被写界深度**という独立な 2 軸を畳んでいたこと。ある構成で測ると、横分解能の限界は 30.7 µm で 60 µm の欠陥は余裕で解像できているのに、被写界深度 0.52 mm が要求公差 1.00 mm に足りないせいで、どのサイズも `resolvable` に格上げされない。そして見出しには「光学限界に未到達」と出る。**これを読んだ人はレンズを買いに行きますが、直すべきは絞りか公差かフォーカス機構です。**
...
横分解能は閉形式なので、掃引の刻み方に関わらず**必ず数値が出ます**。「未到達」と書ける場所ではありませんでした。

### 業界のアワードを、op 発想の恒常的な入力にする

著者の「**この手の Vision 関連イベントの情報から新しい op の検討をしてみるのもいいかもしれない**」という一言から、産業ビジョンの展示会とアワードを一次情報で読む手順書(`docs/INDUSTRY_SIGNALS.md`)を作りました。論文コーパスとは**判定軸が違う**のが理由です。査読者が見るのは「学術的に新規か」、審査員が見るのは「今まさに産業が金を払う価値があると判断したか」。後者は**仕様が確定していて検証すべき数値がある**という意味になります。

在庫確認で 1 回間違えかけたので、その罠も手順に残しました。**キーワード 1 つで「0 件」と結論してはいけません** ―― `hyperspect` と `spectral` で引いて「ハイパースペクトルは無い」と書きかけましたが、実体は `spec_` 接頭辞の 14 op でした。同じ罠が他にもあり、結局「**有る / 無い**」の 2 値ではなく「**有るが未登録**」「**有るが規約外**」を含む 4 状態で記録することにしました。
...
### 新しい 2 分野 ―― ライトフィールドと光子計数

調査で「3 つの表面すべてで 0 件」だった分野のうち、2 つを実装しました(残りの 4 分野 ―― 鏡面反射の分離、モーション増幅、ドップラー、コヒーレンス走査干渉 ―― は次の候補として記録済みです)。

**ライトフィールド(17 op)** は 4 次元 (視点 V, 視点 U, 高さ H, 幅 W) を扱う族です。1 回の露光から視点を切り出し、後からピントを変え、深度を出す。既存のステレオ(2 眼)やフォーカススタック(実カメラの合焦)とは入力が違うので、型で分けてあります。

この族で一番気に入っている数字は、**リフォーカスの利得が角度分解能に一致する**という教科書の結果を、実装が独立に再現したことです。
...
この比はハードコードではなく、**許容錯乱円を変えて `depth_of_field` を 2 回呼んだ結果**です。6.0016 の 0.0016 はその 2 回の計算の差であって、丸めた数字ではありません。代償も同じ表に出ていて、空間分解能は 2048×2448 が 341×408 / 256×306 / 204×244 に落ちます。**得るものと失うものが同じ計算から出てくる**のが、この族の気持ちよさです。

**光子計数・時間分解(17 op)** は、光が「連続した明るさ」ではなく**数えられる粒**になる領域です。Poisson 標本化、Anscombe 変換(分散安定化)、SPAD のデッドタイム、TCSPC の到達時刻ヒストグラム、そこからの距離と蛍光寿命。既存の `aug_read_noise`(加法ガウスの読み出し雑音)とは雑音モデルが違い、両者が出会うのは Anscombe の一般化形だけです。

デッドタイムの実装は教科書式 `m = n/(1+nτ)` と**ビット単位で一致**し、逆変換の往復誤差は 1.7e-16。dToF の距離推定は 1.5 m を注文して **1.5000003 m** が返ります(対数放物線当てはめ)。

### 「発見ゼロ」が頑健さに見えていた ―― ファザーの穴を 3 つ

新しい 34 op を連鎖ファザーへ繋いだところ、**ファザー自身の穴が 3 つ**出ました。3 つとも「**バグが無い**」と「**そもそも実行されていない**」を区別できない、という同じ形をしています。追記5 でファザーを「第三の品質保証層」と書きましたが、その層自体が嘘をつきうる、という話です。

**1 つ目 ―― 既定値つきの引数を一切上書きできなかった。** ある op の既定値がプールの固定サイズと噛み合わないと、その op は**毎回 ValueError で弾かれ、一度も実行されないまま**「発見ゼロ」に数えられます。実際 `lf_from_mla` の既定 `angular=(5,5)` は 32×32 を割り切れず、1200 連鎖で一度も走っていませんでした。op 名で狙い撃ちするヒントだけ既定値も上書きできるようにして解決(名前レベルのヒントを既定値に効かせると既存 op の挙動が一斉に変わるので、そちらは据え置き)。

**2 つ目 ―― カバレッジが数しか出なかった。** 「304/417」と言われても、残る 113 が**頑健だから発見が無い**のか**そもそも到達不能**なのかが分かりません。族ごとの内訳を出すようにしたら、光子計数族が `10/17` ―― つまり **fail-closed が効きすぎて 7 op が一度も実行されていない**ことが即座に分かりました。ファザーの汎用 `signal` プールは負値を持つ正弦波で、光子カウントは非負が契約なので、毎回きれいに拒否されていたのです。守りが完璧に働いた結果、**その守りの奥が一度も試されない**。

**3 つ目 ―― 署名が数値で分裂していた。** 良いエラーメッセージほど「負の bin が 127 個、最小 -1.176」のように**その実行固有の数**を含みます。素の文字列で同一視すると、同じ 1 件の問題が実行のたびに別署名になる。署名が 99 → 238 に膨れた増分のほぼ全部がこれでした。署名を作る前に数値を伏せるだけで解決します。
...
新しい 2 族とも **17/17 到達**、疑義署名は **0 件**、そして署名数が **238 → 40** に収束しました。

### 型を分ける、という判断を 2 回

「非負の 1 次元配列」を全部同じ型にしなかったのは、実測に基づく判断です。光子カウントのヒストグラム(値域 0〜数千)を SPAD のデッドタイム則に渡すと、**例外も出ず、恒等写像に限りなく近い値が返ります**(相対変化 1.1e-4)。本物の計数レート(1e3〜1e7 Hz)なら 33.3% 変わるところです。つまり**飽和も fail-closed も麻痺型の非単射性も、一度も踏まれない**。型を分けないと、この「もっともらしい通過」が永久に検出されません。

しかも物理が違います。デッドタイムは検出器の**レート流**に効くもので、時間 bin ヒストグラムに bin ごとに掛かるものではありません(ヒストグラム側の正しい歪みモデルは別にあります)。同じ型にすると、**進化探索が物理的に誤った連鎖を「正当な型接続」として学習します**。

同じ判断を 4 次元のライトフィールドでもしています。既存の「画像の列」型に潰すと「どの視点か」が消えて、リフォーカスもエピポーラ平面も定義できなくなる。**型は入れ物の形ではなく、意味の約束です。**

### 進化語彙は増えたのか ―― 増えていなかった、という実測

ここで正直な話を 1 つ。**カタログに 34 op 足しても、進化探索の語彙は 1 つも増えていませんでした。** 橋渡し op が 0 件です。理由は 2 段階でした。

まず、新しい型に対応する進化側の型が無かった。これは足せば済みます。ところが足しても足りなくて、**その族の入口**(画像 → ライトフィールド、深度 → 到達時刻立方体)が**既存の画像型を入力に取る**のです。既定語彙に入れると既存型の候補リストが動き、**ゲノムから op への写像が変わって過去のチャンピオンが黙って書き換わります**。かといって入口抜きで消費側だけ足せば、**誰もその型を産まないので永久に到達不能な死んだ語彙**が増えるだけ ―― ファザーで踏んだのと同じ罠です。

結論は「**入口と消費側を必ず一緒に、同じモードへ置く**」。既定語彙は 801 op のまま 1 つも動かさず(写像の不変性は保つ)、広い語彙(opt-in)側で 873 → **890 op**、新 2 族の橋は 5 → **22 本**になりました。この不変量はテストで固定してあります ――「**その型を産む op が語彙に無いのに、食う op だけある**」状態を検出したら落ちます。

### 来歴の規律を、文章から検査へ

もう 1 つ、著者から鋭い指摘がありました。「**同じ名前で実装すると問題ないか?**」

答えは重要で、**コードが独立に書かれていても、来歴は書いたものが全て**です。実際、新しいモジュールの docstring に商用製品名が「このモジュールが存在する理由」として書き込まれる事故が起きました。コードは公開文献から独立に起こしたものでしたが、それでも由来の記録は汚れます。

そこで規律を三分法として書き直し、**検査に落としました**。
...
旧版の規則は「文書のどこにも書くな」でしたが、**リポジトリ自身がそれを守っていませんでした**。守れない規則は監査もできないので、実態に合う形へ直して、機械検査を書きました(モジュール名・関数名・モジュール docstring・公開 docstring の 4 面を走査、免除には理由の記述を必須にしてあります)。

### この期間の数字

| 項目 | 前 | 後 |
|---|---|---|
| 型付きカタログ | 400 op | **434 op** |
| 進化語彙(既定) | 801 op | **801 op**(写像の不変性を保つため意図的に据え置き) |
| 進化語彙(広い、opt-in) | 873 op | **890 op** |
| per-op ドキュメント | 1,155 件 | **1,189 件** |
| ファザー署名(1500 連鎖) | 238 | **40** |
| 新 2 族の到達 | ― | **17/17 と 17/17** |

敵対的検証で見つけて直した実バグは、ライトフィールド 3 件(無言の Inf / 生の numpy エラー漏れ / サイズ上限の穴)、光子計数 5 件(非正規化数のアンダーフローによる無言 NaN / 型の嘘 / 文字列の暗黙パース / 平坦ヒストグラムで argmax が bin 0 を拾う / 既定値の地雷)、仮想環境 3 件。加えて、**書いた本人が docstring の数値を 3 件、実測して自己訂正**しています ―― 「立ち上がりを含めると寿命が短く出る」は実測では**逆で長く出る**、「Anscombe 経由の方がデノイズが良い」は線形平滑では**負ける**、など。良い結果だけ残さない、はここでも同じです。

## 追記7(2026-09-02): 「一語も持っていない」5 分野を埋め、そして「発見ゼロ」が嘘をつくことを知った

> **Status: Measured / Verified**(新規 8 族 111 op、連鎖ファザー 1500 本で**疑義署名 0 件**、テスト **8,169 件**全緑。進化が新族で **+44.5%**、ただし 1 件は **−28.2%** で負けた)

追記6 で作った「業界のアワードを op 発想の恒常的な入力にする手順書」には、**fullseye が 3 つの在庫表面すべてで 0 件だった 4 分野**が記録してありました。この追記は、**その 4 つを全部埋めた**数日分です。途中で著者から「**1D が扱えるなら音響データも扱えるよね**」「**4 元数画像も使えたら面白いことができるかな?**」という 2 つの問いが来て、それも足しました。

### 何が入ったか

| 族 | op | この族を足す理由が測定に出ている点 |
|---|---|---|
| ライトフィールド | 17 | リフォーカス利得が角度分解能に一致(6.0016 / 8.0038 / 10.0075) |
| 光子計数・時間分解 | 17 | デッドタイム式が教科書と**ビット一致**、dToF が 1.5 m → 1.5000003 m |
| 鏡面反射の分離 | 13 | 影 k 灯で素の最小二乗が崩れ median/ransac は雑音床。**k=4 が共通の破綻点** |
| モーション増幅 | 9 | **崖の位置を閉形式で説明**(J₀ の第一零点 2.4048/k = 3.0619 画素) |
| 四元数画像 | 19 | 色空間の 3 次元回転だけが本物の差(下記の正直な結論) |
| FMCW レンジ-ドップラー | 8 | 弱い標的が矩形窓では漏れに埋没、hann で復活(−24.57 → −43.56 dB) |
| 音響・振動診断 | 19 | **包絡線スペクトルで 107.000000 Hz、生スペクトルの同じ周波数は 4.3e-16** |
| コヒーレンス走査干渉 | 9 | **位相シフト法が λ/4 ちょうどで破綻**、誤差は常に λ/2 の整数倍 |

型付きカタログは **511 op**、進化レジストリは **847 op** になりました。

### この期間で一番の収穫 ―― 「発見ゼロ」は頑健さではなかった

追記5 で連鎖ファザーを「第三の品質保証層」と書きました。今回わかったのは、**その層自体が嘘をつく**ということです。

3D 族が 1500 連鎖で 310 中 204 しか実行されていない理由を掘ったところ、op が頑健だったのではなく、**ファザーが黙って飛ばしていた**のが原因でした。未到達 100 件の内訳はこうです。
...
**70 件が「記録も残さずスキップ」**。足りなかったのはカメラ内部行列、姿勢、RANSAC の閾値、voxel 化の範囲と解像度といった、ごく普通の引数でした。ヒントを足して経路を開いた瞬間、**これまで一度も動いていなかったところから本物の型不整合が 5 件**出てきます。
...
そこで台帳全体の健全性検査を書いたら、**同じクラスがさらに 7 件**出ました。直せなかった 3 件は理由つきで一覧に明記し、「**ちょうどこの集合だけ**」を検査するようにしてあります ―― 直したのに残っていても、新しい乖離が増えても落ちます。

**同じ形をこの期間に 4 回踏みました。**

1. **既定値つきの引数を一切上書きできなかった。** 既定値がプールの寸法と噛み合わない op は毎回 ValueError で弾かれ、**一度も実行されないまま**「発見ゼロ」に数えられる。
2. **カバレッジが数しか出なかった。** 「304/417」では、残る 113 が頑健なのか到達不能なのかが分からない。族ごとの内訳を出したら、光子計数族が 10/17 ―― **fail-closed が効きすぎて 7 op が一度も実行されていない**と即座に分かりました。
3. **署名がメッセージ中の数値で分裂していた。** 良いエラーメッセージほど「負の bin が 127 個、最小 −1.176」のような実行固有の数を含むので、同じ 1 件が毎回別署名になる。
4. **上の 70 件。**

型の到達可能性は不動点で解けます(初期プールの型から、入力が揃う op の出力型を足して閉じるまで回す)。実測すると 434 op 中、**構造的に到達不能なのは 1 件だけ**でした。残りは全部「引かれなかっただけ」だったのです。

### 型を分ける判断を、実測で 6 回

「同じ形だから同じ型でいい」は、静かに間違う系を作ります。判断基準は 1 つで、**混ぜたときに例外が出るのか、もっともらしく間違った数字が返るのか**。後者なら分けます。
...
- 逆に本物の偏光掃引を測光ステレオに渡すと、真の法線が `(0,0,1)` の平らな面に対して **34 度ずれた法線**を残差 21% で返す。同じ op が本物の測光データなら 0.000115 度です。**差は 296,000 倍**。

私も独立に測って **35.15 度 対 0.000000 度**を確認しました。例外も NaN も出ない、自信を持った嘘です。

同じ判断を、時間軸が先頭の動画 (T,H,W) と空間ボクセル、到達時刻立方体 (H,W,T)、色の四元数とモノジェニック信号、複素のビート立方体と実の光子ヒストグラム、z 走査スタックでも行いました。**逆に、音響信号は既存の 1 次元型に相乗りさせました** ―― 任意の実 1 次元配列は本当に妥当な音響信号なので、型を宣言しても嘘にならない代わりに**守るものが無い**。危険は配列ではなく `rate` スカラの側で、同じ録音を 48000 Hz として読むと欠陥が 107 Hz ではなく 200.625 Hz と報告され、やはり例外は出ません。だから防御をスカラ側に置きました。
...
- **Riesz / モノジェニック信号は、同じ日に作ったばかりの複素ステアラブル版に負けました。** 同一オクターブに 2 方位があると誤差 **1.30e-01 対 4.4e-16**。2 オクターブ離すと両方機械精度になるので、原因が「単一平面波モデルが radial band では偽」だと特定できています。25% の画素が rank 0 になり、理論上の勝ち(斜め格子)も実現しませんでした。**勝ちは 2 つだけ** ―― 雑音下で約 2 倍、増幅で 2.21 倍速い。

負けを先に書くのは、この記事の趣味ではなく**実装の指針**です。負けた理由が特定できているから、次に何を直せばいいかが決まります。

### 既存版が壊れることを、測定に出す

新しい族を足す理由は「新しいから」では足りません。**既存のやり方がどこで壊れるか**を同じ土俵で示す必要があります。コヒーレンス走査干渉がその一番きれいな例でした。

| 真の段差 | 位相シフト法(既存) | コヒーレンス法(新規) |
|---|---|---|
...
壊れ始めは **λ/4 = 0.15 µm ちょうど**。誤差は常に λ/2 の整数倍 ―― 縞の次数が飛ぶので、**構造を持った嘘**になります。そして例外も NaN も警告も出ません。λ を 0.8 µm に変えると破綻点も 0.20 µm へ移動することまで確認してあります。

音響でも同じことをしました。軸受欠陥の **包絡線スペクトルのピークは 107.000000 Hz** ですが、**生のスペクトルの同じ周波数は 4.3e-16** です。欠陥はそこに無い。次数比分析も、回転速度が変わる信号では素朴なスペクトルが 7% に潰れて 66.5 Hz に広がるところ、角度領域へ移せば 0.999371 / 幅 0 bin になります。

### 進化が、新しい族を実際に使った

ここが今回の本題でした。**語彙を 511 op に広げても、それを使う仕事が無ければ空回りします。**

実際、進化ループを回すと落選理由の筆頭がこれでした。
...
**課題 12 件がすべて古い型**だったのです。そこで新しい型の課題を 4 つ足しました。真値はどれも前方モデルから厳密に作れます(答えを知ったうえで入力を合成する)。結果は **locked holdout** ―― champion に対して一度だけ採点する、本当に触れていない分割 ―― で測りました。

| 課題 | 恒等 | 手(既存 op 単体の最良) | 進化 | 手比 |
|---|---|---|---|---|
| 光子ヒストグラムのデノイズ | 0.2664 | 0.5371 | **0.7760** | **+44.5%** |
| 振動している場所の地図 | 0.0000 | 0.6973 | **0.8868** | **+27.2%** |
| ライトフィールドの視差地図 | 0.0000 | 0.5794 | 0.5907 | +2.0% |
| 鏡面反射の除去 | 0.4115 | 0.8406 | 0.6039 | **−28.2%** |
...
**そして、負けた 1 件がいちばん教育的でした。** 鏡面反射の除去で進化が見つけた champion は、**四元数族を経由する、族をまたいだ経路**です(色画像 → 四元数 → 色空間回転 → 色画像)。観測用の分割では 0.776 と手の 0.84 に迫って見えたのに、**触れていない分割では 0.628**まで落ちました。seed 間のばらつきも標準偏差 0.190。**観測用の分割だけ見ていたら「惜しい」と読んでいた**ところです。勝った振動地図は逆に、触れていない分割のばらつきが標準偏差 0.001 でした。ばらつきを開示する仕組みが、そのまま効いた実例です。

### 自分と Agent の誤りを 5 件、記録に残しました

この記事でいちばん残したいのはここです。

**1. 進化の伸びを +38% と書きかけました。** 手の基準線 0.3945 と進化の 0.546 を比べたのですが、**この 2 つは別の抽出で測った数字**でした。同じ分割で測り直すと手は 0.5794 で、実際の差は **+2.0%**。違う抽出どうしを比べると 20 倍近く盛れます。

**2. 手の基準線に、弱い方の op を置いていました。** 昇格ゲートが「既存 op 単体の最良」を全探索して、**私が置いた手より 2 倍以上強い op がある**と示しました。弱い方を手にすると課題が実際より易しく見えます。基準線は「最初に思いついた手」ではなく「**既存で最良の手**」であるべきでした。

**3. 型の検査を書くとき、自分の述語が間違っていました。** `pose` 型に検査を入れたら 6 件挙がったのですが、うち 4 件は**私が `isinstance(np.ndarray)` と書いたせい**でした。GPU 対応の op は torch のテンソルを返すのがこの repo の約束で、**型ではなく形で判定すべき**だったのです。直したら残ったのはちょうど 3 件で、そちらは本物でした(1 つは `pose` を名乗りながら R も t も持っていなかった)。

**4. Agent が自分に有利な数字を、自分で訂正しました。** 四元数を作った Agent が「四元数の合成は 10 万回でドリフト 0.0、行列は 4.4e-10」を四元数の優位として報告していました。ところがその 4.4e-10 は、私が公開したばかりの `pose_quat` の別のバグ(正規化で分母に 1e-12 を足していた)が非直交な行列を食わせていた結果でした。正しくは **4.33e-14** ―― 4 桁小さく、ただの丸め誤差で、四元数の優位ではありません。**自分に有利な数字こそ先に測り直す**、という良い実例です。

**5. Agent が、コードコメントに書いたまま一度も測っていなかった数字を摘発しました。** ガイドを書く過程で自分の未実測値に気づき、実測表に差し替えています。しかも実測すると、**高 pad 側では書いていたより悪く**(純白色雑音が存在しない強い過渡 +4.09 を報告)、**低 pad 側では軽かった**。捏造した 1 組の数字では伝わらないスケール依存性でした。

### 副産物 ―― 公開した直後の API にバグがあった

四元数の相談で在庫を確認したところ、四元数・双対四元数の関数が **28 個実在するのに、どちらの facade からも 1 つも引けない**状態でした。配線した直後、Agent が 3 件の無言の誤りを報告してきます。
...
原因は `norm + 1e-12` で割っていたことでした。零長を fail-closed にして厳密除算に直し、**直交性のずれは 4.4e-16** になっています。切り分けで 1 つ分かったのは、4.0e-12 という数字は `quat_to_hom_mat3d` が内部で `quat_normalize` を呼ぶため **ε が二重に効いていた**からで、外側の式だけ再現しても現行コードでは出ないことでした。

同じクラスの取りこぼしを兄弟コードごと一掃するのは、この repo の規律です。サイズ上限が **float64 昇格の後**に効いていて防ぐはずの確保を防げていなかった件も 2 モジュールで直しました(0 バイトの view で 0.0000 秒拒否を確認)。

### この期間の数字

| 項目 | 前 | 後 |
|---|---|---|
| 型付きカタログ | 400 op | **511 op** |
| 進化レジストリ | 801 op | **847 op** |
| 進化の課題 | 12 件 | **16 件**(うち 4 件が新しい型) |
| per-op ドキュメント | 1,155 件 | **1,264 件** |
| ファミリガイド | 16 本 | **24 本** |
| テスト | 6,630 件 | **8,169 件**(全緑) |

敵対的検証で見つけて直した実バグは、8 族あわせて **30 件超**。すべて「例外が出る」ではなく「**黙って間違った数字を返す**」型です。角度が正弦で折り返されて 95 度と 85 度が**ビット完全に同一**になる、開口の無いアレイが **−90 度を自信を持って返す**、パッドを跨ぐフレームが最も衝撃的に見えて**存在しない過渡を報告する**、端で切れた包絡線が **76% 間違った高さを返す**、波長の nm/µm 取り違えが**完全に無症状**―― どれも、走らせて測らなければ見えないものでした。
