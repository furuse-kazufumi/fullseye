...
ここは初出版で**破綻点を取り違えていた**ので、訂正を兼ねて書きます。当初は「**8 灯のうち 4 灯が遮蔽されると崩れる**」と書きました。実際そのとおりの数字が出ていた ―― k=4 で法線の平均誤差 **70.5 度**。ところが同時に、**推定器は「8 灯すべてを信じた」と報告していました**。一番外している所で、診断が何も警告しない。

根本は Woodham のモデル `I = a·max(n·L, 0)` にあります。**測定値 0 は「`n·L ≤ 0`」という不等式**なのに、線形解法はそれを等式 `n·L = 0` として読みます。しかも「アルベド 0」の退化解は**潰れた全フレームを厳密に再現する**ので、遮蔽灯の集合が**無矛盾な仮説として真の解を上回れて**しまう。だから中央値もランザックも、真っ黒な灯の側を「多数派」として選んでいました。

規則を「**当てはめが許容差内で説明でき、かつ測定値自身が同じ許容差より 0 から離れている**」に変えると、こうなります。

| 遮蔽灯 | 修正前の平均誤差 | 修正後 |
|---|---|---|
| 0〜3 灯 | 0.0001 度 | 0.0001 度 |
| **4 灯** | **70.5 度**(8/8 全部を信じたと報告) | **0.000115 度** |
| 6 灯(生きた灯 2 本) | 8.99 度(**答えを返す**) | **全画素 NaN**(解けないと言う) |

6 灯の行が要点です。生きた灯が 2 本 = **未知数 3・式 2 の劣決定**で、原理的に解けません。修正前はそこで「8.99 度」というもっともらしい答えを返していました ―― 浅い丘だったので最小ノルム解がたまたま近かっただけで、別の形状なら黙って大きく外れます。

**そして本物の破綻点は遮蔽ではなく、ハイライト汚染でした。** 正の外れ値 `+3.0` を 8 灯中 4 灯に足すと、ransac 65.4 度 / median 7.4 度(1〜3 灯なら 0.0001 度)。**中央値の破綻点がちょうど 50 % なので、4/8 で崩れること自体は理論どおり**です。遮蔽はモデルの読み違いで解けるようになり、汚染は理論限界として残りました。
...
### この表は、一度この規律を破って作られていました

ここは記事の訂正です。**上の表の初出版は、1 行の中で基準線と champion を別の抽出から取っていました。**

公開後に「表が現行コードで再現しない」と分かり、原因を追いました。総当たりの結果、旧表の基準線 `0.2664 / 0.5371` は **cfg seed 2** の locked 分割、`0.6973` と `0.5794` と `0.4115 / 0.8406` は **cfg seed 1** の locked 分割に一意に一致しました。ところが champion 側は **cfg seed 0** の走行です。つまり**基準線と進化の値が別の抽出**で、しかも**課題ごとに cfg seed が違って**いました。進化列(`0.7760 / 0.8868 / 0.5907 / 0.6039`)に至ってはどの cfg seed でも再現せず、保存された成果物のどれにも存在しません。逆に**成果物に残っていた値(`0.7845 / 0.8941 / 0.5465 / 0.6277`)は、今日の再測定と 4 桁一致**します。

**「比較は同一 split で取れ」と書いた記事の、看板の表がそれを破っていた**わけです。

穴の場所も特定できました。`robust.py` は基準線のファイルが無ければ **`base = {}` として先へ進み、基準線を `null` のまま書き出す**作りでした。例外は出ません。実際 `out/rb_*/` に基準線のファイルは 1 つも無く、**誰も測っていませんでした**。表に書いた数字は、成果物から読んだのではなく別に計算したもので、だから後から突き合わせられなかった。

直したのは 3 つです。**`robust.py` が進化を始める前に自分で基準線を測って JSON に書く**(分割の解決規則は `evolve.run` と同一)、**`null` が残ったら abort する**(fail-closed)、そして**ディスク上のファイルと食い違ったら両方を残す**(どちらも黙って勝たせない)。

もう 1 つ罠が見つかりました。**workdir に基準線の JSON があると、`evolve.run` が初期集団の個体 0 を乱択最良ゲノムで置き換えるため、進化結果そのものが変わります。** 上のコマンドで空ディレクトリを指定しているのはそのためです。
