...
共有 **2,153 篇運算子說明**(呼叫方式、型別契約、HALCON 對應、參考文獻、來源)與 **56 篇族群指南**。依維度的入口:

**實測涵蓋**: 演化運算子 932/932、型別台帳 1200/1212、單行門面 `fullseye.<名稱>` 596/1230 —— **門面側僅涵蓋一半**。

**內容實測**: 2158 篇中，附有可執行範例的 **2101** 篇(57 篇沒有)，用法說明 120 字以上的 **2124** 篇(34 篇僅一行)。結構(呼叫形式、型別、可銜接運算子)2158 篇全有。
...
完整地圖，確保**沒有任何文件無法從索引到達**(`docs/ops/` 下的 2,153 篇運算子說明與 56 篇族群指南從上面的「尋找運算子」進入; 文章見 [articles/](articles/README.md))。**內文多為日文。**
