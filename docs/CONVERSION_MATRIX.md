...
```
型 58 / 変換ペア 164 / 変換を行う op のべ 443
生成器のある型 37 / 出力型に述語のある型 56

■ 袋小路 (16) ― 産めるのに、そこから他の型へ出られない
   angle            ← 1 型から来る
   axes             ← 1 型から来る
   countrate        ← 0 型から来る
   cscalar          ← 1 型から来る
...
■ 誰も食べない型 (14) ― 出力専用。終端なら正しい
   axes, cscalar, curvature, flow, frame, gradient, graph, hessian, indices, pairs, pointmap, roots, rot_scale, shift
...
■ 片道の変換 (110) ― A→B はあるが B→A が無い(戻せないのが正しい場合も多い)
   ...
   points           → flow              nearest_neighbor_flow, nearest_neighbor_flow, smooth_flow
   ...
   signal           → pairs             create_funct_1d_pairs, create_funct_1d_pairs, invert_funct_1d
   ...
   voxel            → flow              scene_flow_lk, scene_flow_lk
