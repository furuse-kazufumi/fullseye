...
也可以把 **1,947 篇**筆記依主題分層叢集，並為每個叢集附上 LLM 摘要，做成「附導覽的語料庫」。我們內部使用的是 [RAPTOR](https://github.com/gadievron/raptor) 分支的 `corpus2skill`（TF-IDF + k-means + LLM 摘要），但**這只是一種選用的最佳化，並非必要**。要求僅僅是「以 `docs/ops` 為輸入，輸出依叢集劃分的 SKILL.md 階層」，因此任何具同等功能的工具都可以替代。
