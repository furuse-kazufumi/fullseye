...
也可以将 **1,964 条**笔记按主题聚类分层，并为每个聚类附上 LLM 摘要，做成"带导航的语料库"。我们内部使用的是 [RAPTOR](https://github.com/gadievron/raptor) 分支的 `corpus2skill`（TF-IDF + k-means + LLM 摘要），但**这只是一种可选的优化，并非必需**。要求仅仅是"以 `docs/ops` 为输入，输出按聚类划分的 SKILL.md 层级结构"，因此任何等效的工具都可以替代。
