...
- `sdf_*` のプリミティブは球と直方体だけ。**円筒と半空間が無い**(DFM の穴も斜面も
  自前で書くことになる)。
- **面ごとの面積**を返す op が無い(`mesh_area` は総和だけ)。オーバーハング面積は
  「面ごとの面積 × 法線の判定」なので、外積を呼び手が書く。
- 境界ループの抽出・自己交差の検出・開いたメッシュの符号付き体積が無い。
- 点群の変化検出に M3C2(法線方向に距離を取る)が無い。点群 → DEM のラスタ化、
  地面点の分類、LoD も無い。
