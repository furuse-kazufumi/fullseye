...
**1,947개의 연산자 노트**(호출 형식, 타입 계약, HALCON 대응, 참고문헌, 출처)와 **49개의 패밀리 가이드**가 있습니다. 차원별 입구:

**실측 커버리지**: 진화 연산자 901/901, 타입 台帳 1025/1037, 한 줄 파사드 `fullseye.<이름>` 547/1122 — **파사드는 아직 절반**.

**내용 실측**: 1952건 중 실행 가능한 예제가 붙은 것은 **1912**건(40건은 없음), 사용법이 120자 이상인 것은 **1933**건(19건은 한 줄 요약). 구조(호출 형식·타입·다음 연산자)는 1952건 모두.
...
**색인에서 닿지 않는 문서를 만들지 않기** 위한 전체 지도입니다(`docs/ops/`의 연산자 노트 1,947건과 패밀리 가이드 49건은 위의 「연산자 찾기」에서, 기사는 [articles/](articles/README.md)에서). **본문은 대부분 일본어입니다.**
