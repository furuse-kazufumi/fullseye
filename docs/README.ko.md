...
**2,025개의 연산자 노트**(호출 형식, 타입 계약, HALCON 대응, 참고문헌, 출처)와 **50개의 패밀리 가이드**가 있습니다. 차원별 입구:

**실측 커버리지**: 진화 연산자 931/931, 타입 台帳 1073/1085, 한 줄 파사드 `fullseye.<이름>` 568/1202 — **파사드는 아직 절반**.

**내용 실측**: 2030건 중 실행 가능한 예제가 붙은 것은 **1977**건(53건은 없음), 사용법이 120자 이상인 것은 **2001**건(29건은 한 줄 요약). 구조(호출 형식·타입·다음 연산자)는 2030건 모두.
...
**색인에서 닿지 않는 문서를 만들지 않기** 위한 전체 지도입니다(`docs/ops/`의 연산자 노트 2,025건과 패밀리 가이드 50건은 위의 「연산자 찾기」에서, 기사는 [articles/](articles/README.md)에서). **본문은 대부분 일본어입니다.**
