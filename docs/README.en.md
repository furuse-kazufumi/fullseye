...
## PoC series — 136 real problems solved against a ground truth
...
**2,094 per-operator notes** (call form, type contract, HALCON counterpart, references, provenance) and **56 family guides**. Entry points by dimension:

**Measured coverage**: evolvable ops 931/931, typed ledger 1142/1154, one-line facade `fullseye.<name>` 568/1202 — **the facade is only half covered** (the rest are helpers, classes and re-exported modules).

**Measured substance**: of 2099 notes, **2042** link at least one runnable example (57 have none) and **2067** have a usage section of 120+ characters (32 are a one-line summary). The structure (call form, types, ops that chain next) is present in all 2099.
...
The complete map, so that **no document is unreachable from this index** (the 2,094 per-op notes and 56 family guides under `docs/ops/` are reached from the operator section above; articles from [articles/](articles/README.md)). One unreachable file fails `tests/test_docs_index_reachable.py`. **Most bodies are in Japanese.**
