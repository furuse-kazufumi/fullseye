...
## PoC series — 137 real problems solved against a ground truth
...
**2,103 per-operator notes** (call form, type contract, HALCON counterpart, references, provenance) and **56 family guides**. Entry points by dimension:

**Measured coverage**: evolvable ops 931/931, typed ledger 1151/1163, one-line facade `fullseye.<name>` 568/1202 — **the facade is only half covered** (the rest are helpers, classes and re-exported modules).

**Measured substance**: of 2108 notes, **2051** link at least one runnable example (57 have none) and **2074** have a usage section of 120+ characters (34 are a one-line summary). The structure (call form, types, ops that chain next) is present in all 2108.
...
## Document map — all 199

The complete map, so that **no document is unreachable from this index** (the 2,103 per-op notes and 56 family guides under `docs/ops/` are reached from the operator section above; articles from [articles/](articles/README.md)). One unreachable file fails `tests/test_docs_index_reachable.py`. **Most bodies are in Japanese.**
