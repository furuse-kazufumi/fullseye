...
## PoC series — 142 real problems solved against a ground truth
...
**2,153 per-operator notes** (call form, type contract, HALCON counterpart, references, provenance) and **56 family guides**. Entry points by dimension:

**Measured coverage**: evolvable ops 932/932, typed ledger 1200/1212, one-line facade `fullseye.<name>` 596/1230 — **the facade is only half covered** (the rest are helpers, classes and re-exported modules).

**Measured substance**: of 2158 notes, **2101** link at least one runnable example (57 have none) and **2124** have a usage section of 120+ characters (34 are a one-line summary). The structure (call form, types, ops that chain next) is present in all 2158.
...
## Document map — all 204

The complete map, so that **no document is unreachable from this index** (the 2,153 per-op notes and 56 family guides under `docs/ops/` are reached from the operator section above; articles from [articles/](articles/README.md)). One unreachable file fails `tests/test_docs_index_reachable.py`. **Most bodies are in Japanese.**
