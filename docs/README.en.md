...
**1,912 per-operator notes** (call form, type contract, HALCON counterpart, references, provenance) and **49 family guides**. Entry points by dimension:
...
**Measured substance**: of 1912 notes, **1894** link at least one runnable example (18 have none) and **1907** have a usage section of 120+ characters (5 are a one-line summary). The structure (call form, types, ops that chain next) is present in all 1912.
...
The complete map, so that **no document is unreachable from this index** (the 1,912 per-op notes and 49 family guides under `docs/ops/` are reached from the operator section above; articles from [articles/](articles/README.md)). One unreachable file fails `tests/test_docs_index_reachable.py`. **Most bodies are in Japanese.**
