...
## PoC series — 138 real problems solved against a ground truth
...
**2,111 per-operator notes** (call form, type contract, HALCON counterpart, references, provenance) and **56 family guides**. Entry points by dimension:

**Measured coverage**: evolvable ops 931/931, typed ledger 1159/1171, one-line facade `fullseye.<name>` 576/1210 — **the facade is only half covered** (the rest are helpers, classes and re-exported modules).

**Measured substance**: of 2116 notes, **2059** link at least one runnable example (57 have none) and **2082** have a usage section of 120+ characters (34 are a one-line summary). The structure (call form, types, ops that chain next) is present in all 2116.
...
## Document map — all 200

The complete map, so that **no document is unreachable from this index** (the 2,111 per-op notes and 56 family guides under `docs/ops/` are reached from the operator section above; articles from [articles/](articles/README.md)). One unreachable file fails `tests/test_docs_index_reachable.py`. **Most bodies are in Japanese.**
