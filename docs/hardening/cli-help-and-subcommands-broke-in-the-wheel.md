...
8. **第 12 報の追加**(N19 / N25 / N26): `fullseye --version` が rc 2 で usage しか出なかった → `--version` を足す。`algo run` / `apply` の実行時メッセージも `_prog()` で呼ばれ方に合わせる。`has <typo>` の「not in the HALCON reference」は綴りの間違いまで HALCON の話に見えるので、「op 名でも HALCON 別名でもリファレンスの operator でもない」+ 探し方に。同報の N20(範囲外入力の出力が [0,1] を出る)= `extra_checks` の設計、N21(a=1e9 の丸め)= 記録 + once-per-op 警告、N22 / N23 = 第 1・3 陣で直し済み、N24(警告は初回のみ)= 文書化済みの設計(`fullseye.fallbacks()` / `FULLSEYE_ON_ERROR=raise`)。 CI(Linux)では `os.path.basename` が `\` を区切りと見ないので Windows 形の argv[0] が丸ごと 1 要素になり、この判定が手元だけ緑だった —— 区切りは `[\/]` の両方で切る。
9. **第 13 報「ネイティブクラッシュ 3 件」の正体**: `sample_photo` / `sample_photo_raw` / `sample_photo_rgb` に未登録名を渡すと `realdata` が **`SystemExit(1)`** を投げていた(PoC 用に理由を印字して落ちる作り)。公開 API なので呼び手のプロセスごと終わり、第三者のスモークでは「ワーカーが消滅」と観測された。`realdata.RealDataError(RuntimeError)` に替え、fail-closed は例外で守る。
