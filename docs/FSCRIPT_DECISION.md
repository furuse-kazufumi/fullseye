...
- ★★**Runtime に専有 CPU コア/優先度を確保する(裾の主対策・実測で確定, §9.1)**。裾(最悪 118.7 ms)の
  主因は **他プロセスとの CPU コア競合による cv2 内部スレッドのプリエンプト**。**6 コア空けるだけで、18 個の
  busy プロセスが同居しても最悪 21.7 ms(クリーン同等)** に収まる。∴ 産線 PC では Runtime プロセスに
  affinity/優先度でコアを確保し、他ワークロードにコアを奪わせない運用を **導入要件**とする。
- ★**cv2 のスレッド数を明示的に抑える**(`cv2.setNumThreads`、副対策)。全コア飽和という悪条件下でも
  cv2 単スレッド化で最悪 73→57 ms に縮む(p50 は上がる代償)。専有コアと併用する。
