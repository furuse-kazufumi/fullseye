...
- HALCON の JIT は**ネイティブコードではなく最適化バイトコード**へのコンパイルで、
  速度向上は**制御フローが重いコードに限られ、演算子呼び出しが連続するだけのコードでは効果は無視できる**
  — と MVTec 自身が文書化している。**これは本書 §1.4 の実測と完全に一致する。**
- **同一ベンダの二実行系(インタプリタ / JIT)ですら意味論が乖離し、公式ドキュメントで注意喚起されている。**
- さらに HALCON は **export 先で「性能のため」に IDE 側の安全チェックを意図的に落としている**
  (未初期化 tuple 要素アクセス等)。
→ 250 人規模のベンダですら二実行系の意味論乖離を防ぎきれていない。
  一人で同じ二本立てを採れば、**「IDE では例外、配布では未定義値」という最悪クラスのバグ面**を背負う。
  fail-closed を方針とする以上(CLAUDE.md)、**単一実行系のほうが方針と整合する。**
...
- ★**常駐 Python オブジェクト数に上限を設け、監視する**。外部検証の実測で gen2 GC のポーズは
  常駐 live オブジェクト数に対し **3 万個 = 0.69 ms / 30 万個 = 8.47 ms / 300 万個 = 232 ms** と増える。
  → **10 ms サイクルで GC を 1 ms 以内に収めるなら常駐 5 万個以下**。
  本測定で GC が一度も発火しなかったのは、この負荷が Python オブジェクトをほぼ常駐させないため
  (numpy 配列は refcount 解放)。**24 時間分のトレンドやレシピを dict/list で持てば簡単に破綻する。**
  ∴ 「Python だから危ない」ではなく **「何個常駐させる設計にしたか」**が制御対象。
...
| N3 | オフライン復元 | インストーラ・wheel・ネイティブ DLL・ドライバ・ライセンス・モデルを完全保存。★**凍結は onedir + MSI**(`--onefile` は起動のたびに %TEMP% へ再展開し **+1.8 秒**、かつ Qt DLL を埋め込むと LGPL 準拠の説明が困難)。★リリースビルドは**使い捨てクリーン venv / CI コンテナ**で行う(汚れた site-packages から過剰収集すると同じアプリが **5.4 GB** になる実測あり) |
...
**★この分離は一時的な回避策ではなく、恒久的な境界である**(敵対検証の指摘で明確化):
進化のゲノムは **固定長 float ベクトル → N_SLOTS 個の線形ステージ**(`ops.decode` / `Stage(op,a,b,sort)`)
という**平坦表現**であり、正直な holdout ゲートもこの表現の上でしか動かない。
**実行時制御変数を持つ言語プログラムは、この表現に写像できない。**
→ ∴ **言語プログラム全体が進化の対象になることは、原理的に無い**。
  進化できるのは `EvolvableBlock`(分岐・副作用の無い直列部分集合)だけであり、
  それは制約ではなく**正しい境界**である。外側の言語が取得・分岐・測定・動作を担い、
  block の中だけが進化する — この役割分担を今後も動かさない。
