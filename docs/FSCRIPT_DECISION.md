...
| ゲート | 判定 | 外れた場合 |
|---|---|---|
| **G1** | 増分 1(型と意味論を閉じる = xfail 5 件を pass)を **4 週**で終える | 見積りが楽観的だったと判断し、**制限付き Python サブセット案**(判断 3 の第 3 案)へ切り替えを検討 |
| **G2** | 増分 2(産業プロファイル 20〜40 op + per-op 差分ゲート)を **8 週**で終える | 産業プロファイルの op 数を 10 に減らして再挑戦。それも外れたら Path C/D の再評価 |
| **G3** | 増分 3(ウォッチ)で**実レシピを step 実行しながら image/region/domain を観測**できる | 独自 DSL を続ける根拠(判断 6 の第 1 位)が未達 → 設計全体を再検討 |
