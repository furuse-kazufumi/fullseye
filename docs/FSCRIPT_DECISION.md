...
| (b) 検査ロジックを走らせる**言語処理系**が Python でよいか | **問題なし(実測 +0.4〜4.7%)** | 実測 §1 / HALCON 自身が本番にインタプリタを出荷(§1.3) |
...
| N4 | SBOM / ライセンス | 第三者ライセンス一覧・GPL 混入なし・Qt/PySide の条件を明示(**要法務確認**) |
...
**native の選択順**: ① **cv2 で足りるものは cv2**(実測済・即効。ライセンス Apache-2.0)→
