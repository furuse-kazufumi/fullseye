...
- ★★**Runtime に専有 CPU コアを確保する(裾の唯一の確実策・実測で確定, §9.1)**。裾(最悪 118.7 ms)の
  主因は **他プロセスとの CPU コア競合による cv2 内部スレッドのプリエンプト**。**6 コア空けるだけで、18 個の
  busy プロセスが同居しても最悪 21.7 ms(クリーン同等)** に収まる。これは**プロセス内で設定するものではなく
  OS/配布レベルの運用要件**(産線 PC で Runtime に他ワークロードを同居させない/affinity で隔離)。**導入要件**とする。
- ★**cv2 のスレッド数を抑える**(`cv2.setNumThreads`、副対策・弱い)。全コア飽和下では効果は小さく noisy
  (cv2-threads=2 で改善せず、別 run の =1 で 57 ms=ばらつき)。専有コアがある前提の微調整に留める。
- ★**プロセス優先度(`SetPriorityClass HIGH`)は未検証**。本 PC の py 起動コンテキストでは適用されず(戻り値 0)、
  裾への効果を実証できていない(`FullseyeRuntime.high_priority` が実適用可否を返す)。昇格/サービス化が要る見込み。
- ★**タイマ分解能(`timeBeginPeriod(1)`)は本製品の実検査では裾に効かず**(§9)。合成空ループの数字は非転移。
...
| **N1b** | **裾を詰めること(★原因特定済 2026-08-16)** | かつて同 soak で **最悪 118.7 ms(p50 の 4.6 倍)** を観測したが、**原因を特定した = 外部の CPU コア競合による cv2 内部スレッドのプリエンプト**(`FSCRIPT_MEASUREMENTS.md` §9.1 の A/B、evidence-based)。**クリーンなアイドル機では 54,000 サイクル走っても最悪 21.0 ms(1.24 倍)= pipeline 固有の裾はタイト**。全コア飽和で同一 pipeline が最悪 ~73 ms へ膨れ、正本の裾域を再現。**∴ 118.7 ms は環境要因で pipeline 固有でない**。GC / リーク / タイマ分解能は主因でない。**受入条件を更新: (1)産線 PC で Runtime に専有コア/優先度を確保する運用要件(6 コア空けるだけで 18 個の busy プロセスがいても最悪 21.7 ms=クリーン同等)、(2)`cv2.setNumThreads` で cv2 スレッドを抑える(飽和下で最悪 73→57 ms)。** バルク(p50 ~17-26 ms)の内訳は connection(cv2)~12 ms + threshold(numpy)~4.9 ms で、ページフォールト ~14,000/サイクル(毎サイクル新規 4MP 確保)= バッファ pool 化は p50 削減策(裾とは別軸) |
