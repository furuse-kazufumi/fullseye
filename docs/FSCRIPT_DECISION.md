...
| **N1b** | **裾を詰めること(★最優先の技術課題)** | 同 soak で **p50 26 ms に対し p99 55.7 ms(2.1 倍)/ 最悪 118.7 ms(4.6 倍)**。**製造業に出せるのは最悪値なので、現状の実力は「4 MP ≒ 最悪 120 ms」であって「26 ms」ではない**。**裾を詰めることは、平均を速くすることより優先度が高い**。原因は未診断(OS スケジューリング / 他プロセス / アロケータ / cv2 スレッドプール / ページフォールト / `timeBeginPeriod`)= **次に測るべき最優先項目** |
