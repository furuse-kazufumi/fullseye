...
category: 見つける
ops: [glyph_correct_spec, glyph_make_spec, glyph_find_text_lines, glyph_rewrite_line, glyph_find_plate, glyph_typeface_noise_floor, glyph_rendering_noise_floor, glyph_distance, glyph_replace, glyph_split_cells, glyph_fonts]
...
直し方は 2 つ。`mode="repair_flagged"`(既定)は床を超えた字**だけ**置き換えて正しい字には触らず、置換後に床より近くならなければ**元に戻します**(`failed_verification`)。`mode="rewrite_line"` は bbox の行を**同じ書体で丸ごと描き直す**ので、見逃し・誤検出が結果に残らず字数の違いも直りますが、書体は変わります。看板の板は `glyph_find_plate` で起こせます(縁の直線 4 本の交点で四隅を取る)。行の位置(bbox)を人が測りたくなければ `glyph_make_spec(rgb, texts)` が暗い字の行を上から検出して指示書を作ります(MCP では items の bbox を省くだけ)。版面が取れなければ `layout.status` で断り、bbox の無い指示書では 1 画素も直しません。
...
**向かない**: ★**文字認識(OCR)ではありません**。何が書いてあるかは教えてもらう前提です。★**行の位置は呼ぶ側が与えるか、`glyph_make_spec` の自動検出(暗い字 / 明るい地、CJK の等幅だけ。白抜き文字は取れない)に頼ります**。bbox を外したときの見逃しは `rewrite_line` でも防げません。★縁取り・影・グラデーションの文字は色が多峰なので**断ります**(`reason_code = multimodal_colour`)。黙って塗りません。★字が小さい(マス幅 90 px 未満)と距離の順序が壊れるので、看板は先に板を起こして拡大してください。★`mismatch` の判定(壊れたマスの割合 2/3 以上かつ距離の中央値 0.10 以上で `unrelated`)は生成画像 26 枚と合成での実測から置いた目安で、保証ではありません —— 割合と距離そのもの(`mismatch_fraction` / `mismatch_distance`)を併記します。★書体は環境のものを使います(`glyph_fonts()`)。CJK を描ける書体が無い環境では例外になります。
...
- op: `glyph_correct_spec`(入口)/ `glyph_make_spec`(bbox の自動検出)/ `glyph_find_text_lines` / `glyph_rewrite_line` / `glyph_find_plate` / `glyph_typeface_noise_floor` / `glyph_rendering_noise_floor` / `glyph_distance` / `glyph_replace` / `glyph_split_cells`
