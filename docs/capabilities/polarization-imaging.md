...
category: 光と色
ops: [polarization_demosaic, polarization_demosaic_color, polarization_stokes, polarization_dolp_map, polarization_separate, stokes_analyze, mueller_from_intensities, mueller_checks, mueller_element, mueller_apply]
...
**向かない**: ★demosaic は**双線形**だけで、エッジ適応や勾配法はありません(輪郭では 1 画素幅のにじみが出ます)。★`mueller_from_intensities` は生成器・検光子が**正確に既知**である前提で、素子の誤差は結果にそのまま乗ります(較正は別の問題)。★`mueller_checks` の `tol` は測定雑音の水準で置いてください。雑音つきで回復した行列は最小固有値が僅かに負に出るのが普通で、tol を勘で小さくすると「非物理」に化けます(例で実演)。
...
- op: `polarization_demosaic` / `polarization_demosaic_color` / `mueller_from_intensities` / `mueller_checks`(2026-09-18、optics 台帳 `polarization`)、既存の `polarization_stokes` / `polarization_dolp_map` / `polarization_separate` / `stokes_analyze` / `mueller_element` / `mueller_apply`
