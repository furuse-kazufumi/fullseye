...
> **★次 = フェーズ I の残り**(優先順): (1)**650op レジストリの fail-open 是正**(`backends._safe` が全例外を飲み region に空 region=『欠陥ゼロ』を返す=Runtime に致命的。fslib は既に fail-closed=対比テスト固定済。次は Runtime プロファイルのロード時 self-check=全 op のバックエンド実在検証、§1.6b/R4)。(2)**裾を詰める**(soak 52,800 サイクルで p50 26ms に対し最悪 118.7ms/4.6倍、原因未診断=最優先の技術課題 N1b)。(3)golden vector 形式の確定 + fail-closed ロード時マニフェスト検証。言語スコープは絞ったまま(Vector/STLコンテナ/例外/procedure/regexp は入れない)。**A/B の分岐は Q1/Q2 の答え待ち**。fscript/fslib/fullseye_abi.h の commit は local(auto-commit hook)・push 判断は human-gate。
>
