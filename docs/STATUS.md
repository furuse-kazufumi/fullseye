...
> **★★次セッション最優先(2026-08-15 その2 完了時点)= Fullseye Script フェーズ I-2「意味論を閉じる」。** 正本 = `docs/FSCRIPT_DECISION.md`(要件定義+基本設計・確定案)/ `docs/FSCRIPT_MEASUREMENTS.md`(実測記録)。**確定** = Python で進める / 実行モデルは VM 一本(MVTec 公式で HDevEngine=インタプリタと確認)/ **A(普通の Python)か B(独自 DSL)かは未決** — 4案x2レンズの独立採点で割れ、決定的情報は顧客側(Q1: 工程技術者の Python を産線 PC で実行してよいか / Q2: レシピを書き換えるのはベンダか顧客か)。**最初の約6週間は A/B/D で同一作業**ゆえ共通核(フェーズ I)から着手する。**I-1 は完了**(commit 16955d0): `fullseye_abi.h` を仕様として先に書き `fslib.py` を適合実装に、`tests/test_abi_conformance.py` 19 tests がヘッダを機械パースして検査(dict が境界を越えない / Region が storage を公開しない 等)。**次 = I-2**: `tests/test_fscript.py` の **strict xfail 5 件を pass にする**(値域の内容依存正規化 / Tuple の + が連結 / iconic の暗黙真偽化 / 比較の .any() 潰れ / value_kind の型推測)= `fscript` を `fslib` の型モデルへ載せ替える。**★最重要の既知欠陥**: 650op レジストリは fail-open(`backends._safe` が全例外を飲み region に空 region=『欠陥ゼロ』を返す)。進化には正しく Runtime には致命的。**★裾が残課題**: soak 52,800 サイクルで p50 26ms に対し最悪 118.7ms(4.6倍)、原因未診断。言語スコープは絞った(Vector/STLコンテナ/例外/procedure/regexp は入れない)。full suite 4372 passed / 5 xfailed。全て push 済。
>
