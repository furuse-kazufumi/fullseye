...
> **★次 = フェーズ I の残り**(優先順): (1)**裾を詰める(最優先の技術課題 N1b)**(soak 52,800 サイクルで p50 26ms に対し最悪 118.7ms/4.6倍、原因未診断。候補=OS スケジューリング/他プロセス/アロケータ/cv2 スレッドプール/ページフォールト/`timeBeginPeriod`。ツール=`tools/bench_soak.py`、★熱定常で測る[[feedback_benchmark_thermal_steady_state]])。(2)Runtime ローダーへ `require_ready` を配線 + golden vector 形式確定 + fail-closed マニフェスト検証。言語スコープは絞ったまま(Vector/STLコンテナ/例外/procedure/regexp は入れない)。**A/B の分岐は Q1/Q2 の答え待ち**。fscript/fslib/fullseye_abi.h の commit は local(auto-commit hook)・push 判断は human-gate。
>
