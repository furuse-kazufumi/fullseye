...
> **★N1b 裾の原因特定完了(2026-08-16, evidence-based, commit 予定)= 外部 CPU コア競合による cv2 内部スレッドのプリエンプト**。`bench_soak --diagnose` の A/B(`FSCRIPT_MEASUREMENTS.md §9.1`): クリーンなアイドル機は 54,000 cycle でも **max 21.0ms(1.24x)= pipeline 固有の裾はタイト**、全コア飽和で同一 pipeline が **max ~73ms**(cv2 の gauss/connection が 4-25x に膨張・numpy threshold は不変)= 正本 118.7ms は環境要因で pipeline 固有でない。**対策=★コア確保が唯一の確実策**(6 コア空けるだけで 18 busy プロセス同居でも max 21.7ms=OS/配布の運用要件)。cv2.setNumThreads は弱く noisy、`SetPriorityClass HIGH` は本環境で未適用=未検証(`FullseyeRuntime.high_priority` が可否を返す)、タイマ/GC/リークは主因でない。R4 と N1b 受入条件に反映済。
