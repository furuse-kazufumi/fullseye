...
> **★★次セッション最優先(ユーザー指示 2026-08-15 exit 時)= Fullseye Script 言語の「要件定義 → 基本設計」を外部AIも交えて確定する。** 実装はまだ。設計正本 = `docs/FSCRIPT_LANGUAGE.md`(本セッションで Codex 審議 + ユーザーとの長い設計対話を統合済)。動作実証済 PoC = `fscript.py`(実ルールベースアルゴリズムが走ることを smoke test で確認)。memory = [[project_fullseye_script_language_2026_08_15]]。**次セッションの進め方**: (1)`docs/FSCRIPT_LANGUAGE.md` を読む(要件・アーキ・型システム・ウォッチ・DLL段階・クロスプラットフォームが全て記載)→ (2)外部AI(Codex/他)と**要件定義(何を満たすか)→ 基本設計(モジュール分割・型システム FImage/Tuple/Vector/STLコンテナ・VM 命令セット・ウォッチ API・L1 ライブラリ契約)**を詰めて確定 → (3)確定後に増分1実装(L1 を fslib 分離 + HALCON 型システムクラス化 + bytecode VM)。★ユーザーが教示した HALCON 忠実点(Image=pixels+domain / control=Tuple[int/real/string混在] / Vector / STLコンテナ / XLD / handle / 型・文字列変換 / 高速性=VM+compile二モード)を要件に必ず含める。全 fscript 関連 commit は local WIP(未 push、push 可否は要確認)。
>
