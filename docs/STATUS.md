...
> **★N1b 裾の原因特定完了(2026-08-16, evidence-based, commit 予定)= 外部 CPU コア競合による cv2 内部スレッドのプリエンプト**。`bench_soak --diagnose` の A/B(`FSCRIPT_MEASUREMENTS.md §9.1`): クリーンなアイドル機は 54,000 cycle でも **max 21.0ms(1.24x)= pipeline 固有の裾はタイト**、全コア飽和で同一 pipeline が **max ~73ms**(cv2 の gauss/connection が 4-25x に膨張・numpy threshold は不変)= 正本 118.7ms は環境要因で pipeline 固有でない。**対策=(主)Runtime に専有コア/優先度**(6 コア空けるだけで 18 busy プロセス同居でも max 21.7ms)**+(副)cv2.setNumThreads 抑制**(飽和下 73→57ms)。タイマ/GC/リークは主因でない。R4 と N1b 受入条件に反映済。
> **★次 = フェーズ I の残り**(優先順): (1)Runtime ローダーを実 CLI/配布イメージへ結線(常駐プロセス・**コア affinity/優先度**・cv2 スレッド抑制・PLC state machine・deadline/ERROR/TIMEOUT)。(2)任意: バッファ pool 化で p50 削減(~14,000 ページフォールト/cycle=毎 cycle 新規 4MP 確保。裾でなくバルク軸)。言語スコープは絞ったまま(Vector/STLコンテナ/例外/procedure/regexp は入れない)。**A/B の分岐は Q1/Q2 の答え待ち**。fscript/fslib/fsruntime/fullseye_abi.h の commit は local(auto-commit hook)・push 判断は human-gate。
>
