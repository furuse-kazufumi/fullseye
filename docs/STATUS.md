...
> **★次 = フェーズ I の残り**(優先順): (1)**裾を詰める(最優先 N1b)= 正本長(~25分/52,800 サイクル)の熱定常 soak で 118.7ms を再現**してから原因を切り分け(タイマは降格済。残候補=他プロセス/アロケータ断片化/cv2 スレッドプール/ページフォールト。★熱定常で測る[[feedback_benchmark_thermal_steady_state]])。(2)Runtime ローダーへ `require_ready` を配線 + golden vector 形式確定 + fail-closed マニフェスト検証。言語スコープは絞ったまま(Vector/STLコンテナ/例外/procedure/regexp は入れない)。**A/B の分岐は Q1/Q2 の答え待ち**。fscript/fslib/fullseye_abi.h の commit は local(auto-commit hook)・push 判断は human-gate。
>
