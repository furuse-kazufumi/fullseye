...
# glass_catalog — OPTICS `design` op

- **データ種**: `なし` → `table`(引数だけで決まる op —— 画像やデータの入力を取らない)
- **呼び出し**: `import fullseye as fs; fs.ledger.glass_catalog(name=None)` (実装を直接呼ぶなら `import raytrace; raytrace.glass_catalog(name=None)`、台帳から引くなら `opsoptics.get("glass_catalog")`)
...
[lens_system](lens_system.md) · [thick_lens](thick_lens.md) · [glass](glass.md) · [example_system](example_system.md) · [sellmeier](sellmeier.md) · [paraxial_trace](paraxial_trace.md) · [seidel_coefficients](seidel_coefficients.md) · [spot_stats](spot_stats.md)
