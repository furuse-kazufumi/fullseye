...
- **呼び出し**: `import fullseye as fs; fs.ledger.rgb_to_quaternion(image_rgb) -> 'np.ndarray'` (実装を直接呼ぶなら `import quatimage; quatimage.rgb_to_quaternion(image_rgb) -> 'np.ndarray'`、台帳から引くなら `opsquat.get("rgb_to_quaternion")`)
