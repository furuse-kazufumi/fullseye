...
- **データ種**: `qimage` → `image2d`
- **呼び出し**: `import fullseye as fs; fs.ledger.monogenic_orientation(qimage, display: 'bool' = False) -> 'np.ndarray'` (実装を直接呼ぶなら `import quatimage; quatimage.monogenic_orientation(qimage, display: 'bool' = False) -> 'np.ndarray'`、台帳から引くなら `opsquat.get("monogenic_orientation")`)
