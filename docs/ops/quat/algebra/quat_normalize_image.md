...
- **データ種**: `qimage` → `qimage`
- **呼び出し**: `import fullseye as fs; fs.ledger.quat_normalize_image(qimage) -> 'np.ndarray'` (実装を直接呼ぶなら `import quatimage; quatimage.quat_normalize_image(qimage) -> 'np.ndarray'`、台帳から引くなら `opsquat.get("quat_normalize_image")`)
