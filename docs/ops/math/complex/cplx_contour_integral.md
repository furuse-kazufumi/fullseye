...
- **呼び出し**: `import fullseye as fs; fs.ledger.cplx_contour_integral(z, fz)` (実装を直接呼ぶなら `import mathops; mathops.cplx_contour_integral(z, fz)`、台帳から引くなら `opsmath.get("cplx_contour_integral")`)
