...
- **呼び出し**: `import fullseye as fs; fs.ledger.cplx_argument_principle(z, fz)` (実装を直接呼ぶなら `import mathops; mathops.cplx_argument_principle(z, fz)`、台帳から引くなら `opsmath.get("cplx_argument_principle")`)
