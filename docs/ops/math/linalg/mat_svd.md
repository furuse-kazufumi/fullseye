...
- **データ種**: `matrix` → `table`
- **呼び出し**: `import fullseye as fs; fs.ledger.mat_svd(a, full_matrices=False)` (実装を直接呼ぶなら `import mathops; mathops.mat_svd(a, full_matrices=False)`、台帳から引くなら `opsmath.get("mat_svd")`)
