...
10 op / 5 カテゴリ(numpy と scipy のみ。台帳は `opsblob.py`、実体は `blob2d.py`):
...
- **split(3)** — `blob_distance` / `blob_seeds` / `blob_split`: 触れ合って 1 個になった塊を分水嶺で戻す。距離は**画素単位**(正規化しない)、種の `h` は**絶対値**。
...
## 触れ合った塊を割る(split の 3 op)

連結成分は「触れているかどうか」しか見ないので、重なった細胞や粒子は 1 個になります。割るには **距離 → 種 → 分水嶺** の 3 段です。

```python
import numpy as np
import fullseye as fs

# 半径 20 と 14 の円板を重ねて置く(真値は重ねる前の面積 1257 と 613)
yy, xx = np.mgrid[0:100, 0:100]
mask = ((yy - 50) ** 2 + (xx - 36) ** 2 <= 400) | ((yy - 50) ** 2 + (xx - 66) ** 2 <= 196)

lab = fs.ledger.blob_label(mask)
print("融合して", int(lab.max()), "個に見える")          # -> 1

d = fs.ledger.blob_distance(mask)                        # 画素単位の距離
seeds = fs.ledger.blob_seeds(d, 1.0)                     # h は絶対値
parts = fs.ledger.blob_split(lab, seeds, d)
f = fs.ledger.blob_features(parts)
print("割れた:", f["n"], "個  面積", np.sort(f["area"])[::-1].astype(int).tolist())
# -> 2 個  面積 [1149, 680]
```

3 つを別々の op にしてあるのは、**種の選び方がいちばん効くノブ**だからです。1 本にまとめると `h` の意味が隠れます。

### 押さえておく偏り

- **重なりは小さいほうへ寄ります。** 分水嶺は 2 つの種の中間で切るので、上の例では真値 1257 / 613 に対して **1149 / 680**。大きいほうが損をして小さいほうが得をする —— これは算法の定義どおりで、誤差ではありません。粒径分布を出すなら、この偏りを承知で使うこと。
- **`h` は絶対値です。** `fs.op.xsk2_h_maxima` は入力を `[0,1]` に切り詰めるうえ `h` が正規化画像に対する比なので、生の距離マップには使えません(画像に大きい物体が 1 つ入るだけで、小さい物体側の実効 `h` が上がる)。ここはそのどちらもしません。
- **種を持たない塊はそのまま残します。** 割る材料が無いのに消すのは嘘になるので。総面積は必ず保存します(門で確認)。
- **速さ**: 種の再構成は**連結成分ごとの外接箱の中でだけ**回します。背景を通る伝播が消えるので 512×512 に 200 個の円で **237.7 ms → 15.3 ms(15.5 倍)**、結果は全画素一致(門 `test_the_per_component_reconstruction_matches_the_global_one`)。

## この族で測れないこと(正直に)

- **割れるのは「距離の山が 2 つ以上ある」場合だけです。** 深く重なって山が 1 つに融けた対は、`h` をどこまで下げても割れません(下げると今度は雑音の凹凸で割りすぎる)。`poc_particle_sizing` の実測では、面積率 21.9 % の場面で融合した塊が 54 件ありました。
