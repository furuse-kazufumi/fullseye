...
- **呼び出し**: `import fullseye as fs; fs.ledger.dem_enu_from_geodetic(lat_deg, lon_deg, h_m, lat0_deg, lon0_deg, h0_m)` (実装を直接呼ぶなら `import demops; demops.dem_enu_from_geodetic(lat_deg, lon_deg, h_m, lat0_deg, lon0_deg, h0_m)`、台帳から引くなら `opsdem.get("dem_enu_from_geodetic")`)
