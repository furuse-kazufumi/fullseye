...
- **データ種**: `voxel` → `sdf`
- **呼び出し**: `import fullseye as fs; fs.ledger.esdf(occupancy, voxel_size=1.0)` (実装を直接呼ぶなら `import occupancy; occupancy.esdf(occupancy, voxel_size=1.0)`、台帳から引くなら `ops3d.get("esdf")`)
