...
- **データ種**: `points` → `voxel`
- **呼び出し**: `import fullseye as fs; fs.ledger.occupancy_grid(points, bounds, res)` (実装を直接呼ぶなら `import occupancy; occupancy.occupancy_grid(points, bounds, res)`、台帳から引くなら `ops3d.get("occupancy_grid")`)
