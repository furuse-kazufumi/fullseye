...
- **データ種**: `voxel` → `voxel`
- **呼び出し**: `import fullseye as fs; fs.ledger.morph_dilate3d(vol, r=1, device='cpu', se='cube')` (実装を直接呼ぶなら `import match3d; match3d.morph_dilate3d(vol, r=1, device='cpu', se='cube')`、台帳から引くなら `ops3d.get("morph_dilate3d")`)
