...
- **データ種**: `voxel` → `voxel`
- **呼び出し**: `import fullseye as fs; fs.ledger.vol_fft_lowpass(vol, cutoff, spacing=None)` (実装を直接呼ぶなら `import volfreq; volfreq.vol_fft_lowpass(vol, cutoff, spacing=None)`、台帳から引くなら `ops3d.get("vol_fft_lowpass")`)
