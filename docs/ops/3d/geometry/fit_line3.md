...
# fit_line3 — 3D `geometry` op

- **データ種**: `points` → `primitive`
- **呼び出し**: `import fullseye as fs; fs.ledger.fit_line3(points) -> 'dict'` (実装を直接呼ぶなら `import measure3d; measure3d.fit_line3(points) -> 'dict'`、台帳から引くなら `ops3d.get("fit_line3")`)
