...
- **呼び出し**: `import fullseye as fs; fs.ledger.vol_rle_encode(vol_binary) -> 'VolRLE'` (実装を直接呼ぶなら `import volregion; volregion.vol_rle_encode(vol_binary) -> 'VolRLE'`、台帳から引くなら `ops3d.get("vol_rle_encode")`)
