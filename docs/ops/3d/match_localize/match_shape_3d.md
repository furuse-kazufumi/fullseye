...
- **呼び出し**: `import fullseye as fs; fs.ledger.match_shape_3d(vol, template, device='cpu', mc=0.05, subvoxel=True)` (実装を直接呼ぶなら `import match3d; match3d.match_shape_3d(vol, template, device='cpu', mc=0.05, subvoxel=True)`、台帳から引くなら `ops3d.get("match_shape_3d")`)
