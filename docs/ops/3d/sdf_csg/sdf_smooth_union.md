...
- **データ種**: `sdf × sdf` → `sdf`
- **呼び出し**: `import fullseye as fs; fs.ledger.sdf_smooth_union(a, b, k)` (実装を直接呼ぶなら `import sdf_ops; sdf_ops.sdf_smooth_union(a, b, k)`、台帳から引くなら `ops3d.get("sdf_smooth_union")`)
