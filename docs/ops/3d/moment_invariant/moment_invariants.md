...
- **呼び出し**: `import fullseye as fs; fs.ledger.moment_invariants(points) -> 'np.ndarray'` (実装を直接呼ぶなら `import moments3d; moments3d.moment_invariants(points) -> 'np.ndarray'`、台帳から引くなら `ops3d.get("moment_invariants")`)
