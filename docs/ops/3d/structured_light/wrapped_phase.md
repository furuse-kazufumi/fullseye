...
- **データ種**: `images` → `image2d`
- **呼び出し**: `import fullseye as fs; fs.ledger.wrapped_phase(images) -> 'np.ndarray'` (実装を直接呼ぶなら `import fringe; fringe.wrapped_phase(images) -> 'np.ndarray'`、台帳から引くなら `ops3d.get("wrapped_phase")`)
