...
# laplacian_smooth — 3D `mesh_process` op

- **データ種**: `mesh` → `mesh`
- **呼び出し**: `import fullseye as fs; fs.ledger.laplacian_smooth(mesh: 'Sequence', iters: 'int' = 10, lam: 'float' = 0.5) -> 'Mesh'` (実装を直接呼ぶなら `import mesh_smooth; mesh_smooth.laplacian_smooth(mesh: 'Sequence', iters: 'int' = 10, lam: 'float' = 0.5) -> 'Mesh'`、台帳から引くなら `ops3d.get("laplacian_smooth")`)
...
[taubin_smooth](taubin_smooth.md) · [decimate_qem](decimate_qem.md) · [face_normals](face_normals.md) · [vertex_normals](vertex_normals.md) · [mesh_area](mesh_area.md) · [vertex_curvature](vertex_curvature.md) · [face_areas](face_areas.md) · [mesh_volume](mesh_volume.md)
