...
- **データ種**: `images` → `voxel`
- **呼び出し**: `import fullseye as fs; fs.ledger.carve(silhouettes: 'Sequence[np.ndarray]', Ks: 'Sequence[np.ndarray]', Rs: 'Sequence[np.ndarray]', ts: 'Sequence[np.ndarray]', bounds: 'Bounds', res: 'int') -> 'np.ndarray'` (実装を直接呼ぶなら `import visualhull; visualhull.carve(silhouettes: 'Sequence[np.ndarray]', Ks: 'Sequence[np.ndarray]', Rs: 'Sequence[np.ndarray]', ts: 'Sequence[np.ndarray]', bounds: 'Bounds', res: 'int') -> 'np.ndarray'`、台帳から引くなら `ops3d.get("carve")`)
...
[visual_hull](visual_hull.md) · [synthesize_silhouette](synthesize_silhouette.md) · [carve_look_at](carve_look_at.md)
