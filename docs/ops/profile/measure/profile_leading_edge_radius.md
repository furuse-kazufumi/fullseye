...
- **呼び出し**: `import fullseye as fs; fs.ledger.profile_leading_edge_radius(contour, frac=0.001, n_fit=24)` (実装を直接呼ぶなら `import profileops; profileops.profile_leading_edge_radius(contour, frac=0.001, n_fit=24)`、台帳から引くなら `opsprofile.get("profile_leading_edge_radius")`)
