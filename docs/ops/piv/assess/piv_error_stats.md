...
- **呼び出し**: `import fullseye as fs; fs.ledger.piv_error_stats(flow, truth, trim=1)` (実装を直接呼ぶなら `import pivops; pivops.piv_error_stats(flow, truth, trim=1)`、台帳から引くなら `opspiv.get("piv_error_stats")`)
