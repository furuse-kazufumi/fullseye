...
13 op / 5 カテゴリ(numpy のみ。台帳は `opspiv.py`、実体は `pivops.py`):

- **synth(2)** — `piv_synth_particles` / `piv_synth_pair`: 既知の変位場を持つ画像対を作る。**この族の全テストの真値の供給源**。
- **estimate(2)** — `piv_cross_correlate` / `piv_multipass`: 本体。窓ごとの FFT 相互相関と、粗→細の多段。
- **validate(2)** — `piv_outlier_mask` / `piv_replace_outliers`: 正規化中央値検定と穴埋め。
- **field(4)** — `piv_vorticity` / `piv_divergence` / `piv_flow_magnitude` / `piv_to_velocity`: 場の微分と単位変換。
- **assess(3)** — `piv_sample_at_windows` / `piv_error_stats` / `piv_peak_locking`: 真値との突き合わせと系統誤差の測定。
...
型を増やすときの本 repo の条件(**種を持つ op が無ければ永久に未実行になる**)は満たしています —— 生成が 5 op、消費が 9 op、族の中で閉じています。両方向の fail-closed も実測済み:
