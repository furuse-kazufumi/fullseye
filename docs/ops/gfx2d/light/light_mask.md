...
- **データ種**: `rgb × rgb` → `rgb`
- **呼び出し**: `import fullseye as fs; fs.ledger.light_mask(base, light, ambient=0.0)` (実装を直接呼ぶなら `import gfx2d; gfx2d.light_mask(base, light, ambient=0.0)`、台帳から引くなら `opsgfx2d.get("light_mask")`)
