...
# num_points_funct_1d — ONED `function` op

- **データ種**: `signal` → `measurement`
- **呼び出し**: `import fullseye as fs; fs.ledger.num_points_funct_1d(y) -> 'int'` (実装を直接呼ぶなら `import funct1d; funct1d.num_points_funct_1d(y) -> 'int'`、台帳から引くなら `ops1d.get("num_points_funct_1d")`)
