---
op: hx_fill_interlace
dim: 2d
category: halcon_ext
in: image
out: image
halcon: fill_interlace
...
# hx_fill_interlace — 2D `halcon_ext` op

- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "hx_fill_interlace", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `fill_interlace`(意味・パラメータは HALCON リファレンスが参考になる)

![hx_fill_interlace: input → output](../../_fig/hx_fill_interlace.png)
...
![hx_fill_interlace: other inputs](../../_fig/hx_fill_interlace.inputs.jpg)
