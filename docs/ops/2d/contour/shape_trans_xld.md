...
# shape_trans_xld — 2D `contour` op

- **データ種**: `contour` → `contour`
- **呼び出し**: `fullseye.apply(img, "shape_trans_xld", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `shape_trans_xld`(意味・パラメータは HALCON リファレンスが参考になる)
