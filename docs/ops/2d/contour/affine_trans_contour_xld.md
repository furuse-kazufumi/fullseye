---
op: affine_trans_contour_xld
dim: 2d
category: contour
in: contour
out: contour
halcon: affine_trans_contour_xld
...
# affine_trans_contour_xld — 2D `contour` op

- **データ種**: `contour` → `contour`
- **呼び出し**: `fullseye.apply(img, "affine_trans_contour_xld", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `affine_trans_contour_xld`(意味・パラメータは HALCON リファレンスが参考になる)

![affine_trans_contour_xld: input → output](../../_fig/affine_trans_contour_xld.png)

*図は合成の入力 128×128 で実際に走らせた出力。左が入力、右が出力。点群は上から見た散布(明るさ = z)、1-D 列は折れ線、体積は z 方向の最大値投影、動画は中央フレーム、複素画像は振幅、絵にならない返り値は値そのもの。*

**つまみ a を振る**(0.1 / 0.5 / 0.9、もう一方は既定):

![affine_trans_contour_xld: knob a sweep](../../_fig/affine_trans_contour_xld.a.jpg)

*つまみ b は出力を変えない(実測: 0.1 / 0.5 / 0.9 で同一)。*

**段階**(前置きの op → この op。左から順):

![affine_trans_contour_xld: stages](../../_fig/affine_trans_contour_xld.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![affine_trans_contour_xld: other inputs](../../_fig/affine_trans_contour_xld.inputs.jpg)
...
affine_trans_contour_xld 0.50 0.50
```
