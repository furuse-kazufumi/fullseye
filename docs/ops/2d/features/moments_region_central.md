---
op: moments_region_central
dim: 2d
category: features
in: region
out: feature
halcon: moments_region_central
...
# moments_region_central — 2D `features` op

- **データ種**: `region` → `feature`
- **呼び出し**: `fullseye.apply(img, "moments_region_central", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `moments_region_central`(意味・パラメータは HALCON リファレンスが参考になる)

![moments_region_central: input → output](../../_fig/moments_region_central.png)
...
![moments_region_central: stages](../../_fig/moments_region_central.chain.jpg)
...
```program
threshold 0.50 0.50
moments_region_central 0.50 0.50
```
