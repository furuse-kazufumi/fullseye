...
# area_holes — 2D `features` op

- **データ種**: `region` → `feature`
- **呼び出し**: `fullseye.apply(img, "area_holes", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `area_holes`(意味・パラメータは HALCON リファレンスが参考になる)

![area_holes: input → output](../../_fig/area_holes.png)
...
![area_holes: stages](../../_fig/area_holes.chain.jpg)
