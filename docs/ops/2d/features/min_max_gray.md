...
# min_max_gray — 2D `features` op

- **データ種**: `image` → `feature`
- **呼び出し**: `fullseye.apply(img, "min_max_gray", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `min_max_gray`(意味・パラメータは HALCON リファレンスが参考になる)

![min_max_gray: input → output](../../_fig/min_max_gray.png)
