...
# convexity — 2D `features` op

- **データ種**: `region` → `feature`
- **呼び出し**: `fullseye.apply(img, "convexity", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `convexity`(意味・パラメータは HALCON リファレンスが参考になる)

![convexity: input → output](../../_fig/convexity.png)
...
![convexity: stages](../../_fig/convexity.chain.jpg)
