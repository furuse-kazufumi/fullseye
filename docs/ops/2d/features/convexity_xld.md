...
# convexity_xld — 2D `features` op

- **データ種**: `contour` → `feature`
- **呼び出し**: `fullseye.apply(img, "convexity_xld", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `convexity_xld`(意味・パラメータは HALCON リファレンスが参考になる)

![convexity_xld: input → output](../../_fig/convexity_xld.png)
...
![convexity_xld: stages](../../_fig/convexity_xld.chain.jpg)
