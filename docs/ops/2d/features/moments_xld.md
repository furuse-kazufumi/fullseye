...
# moments_xld — 2D `features` op

- **データ種**: `contour` → `feature`
- **呼び出し**: `fullseye.apply(img, "moments_xld", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `moments_xld`(意味・パラメータは HALCON リファレンスが参考になる)

![moments_xld: input → output](../../_fig/moments_xld.png)
...
![moments_xld: stages](../../_fig/moments_xld.chain.jpg)
