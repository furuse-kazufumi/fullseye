...
# diameter_region — 2D `features` op

- **データ種**: `region` → `feature`
- **呼び出し**: `fullseye.apply(img, "diameter_region", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `diameter_region`(意味・パラメータは HALCON リファレンスが参考になる)

![diameter_region: input → output](../../_fig/diameter_region.png)
...
![diameter_region: stages](../../_fig/diameter_region.chain.jpg)
