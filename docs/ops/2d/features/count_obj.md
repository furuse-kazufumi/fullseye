...
- **データ種**: `region` → `feature`
- **呼び出し**: `fullseye.apply(img, "count_obj", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `count_obj`(意味・パラメータは HALCON リファレンスが参考になる)

![count_obj: input → output](../../_fig/count_obj.png)
...
![count_obj: stages](../../_fig/count_obj.chain.jpg)
