---
op: orientation_region
dim: 2d
category: features
in: region
out: feature
halcon: orientation_region
...
# orientation_region — 2D `features` op

- **データ種**: `region` → `feature`
- **呼び出し**: `fullseye.apply(img, "orientation_region", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `orientation_region`(意味・パラメータは HALCON リファレンスが参考になる)

![orientation_region: input → output](../../_fig/orientation_region.png)
...
![orientation_region: stages](../../_fig/orientation_region.chain.jpg)
