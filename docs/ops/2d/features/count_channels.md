...
# count_channels — 2D `features` op

- **データ種**: `color` → `feature`
- **呼び出し**: `fullseye.apply(img, "count_channels", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `count_channels`(意味・パラメータは HALCON リファレンスが参考になる)

![count_channels: input → output](../../_fig/count_channels.png)
...
![count_channels: stages](../../_fig/count_channels.chain.jpg)
