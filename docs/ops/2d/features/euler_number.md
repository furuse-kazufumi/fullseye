...
# euler_number — 2D `features` op

- **データ種**: `region` → `feature`
- **呼び出し**: `fullseye.apply(img, "euler_number", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `euler_number`(意味・パラメータは HALCON リファレンスが参考になる)

![euler_number: input → output](../../_fig/euler_number.png)
...
![euler_number: stages](../../_fig/euler_number.chain.jpg)
