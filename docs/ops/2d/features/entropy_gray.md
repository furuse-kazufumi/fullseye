...
- **データ種**: `image` → `feature`
- **呼び出し**: `fullseye.apply(img, "entropy_gray", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `entropy_gray`(意味・パラメータは HALCON リファレンスが参考になる)

![entropy_gray: input → output](../../_fig/entropy_gray.png)
