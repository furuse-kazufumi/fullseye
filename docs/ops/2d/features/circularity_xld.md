...
# circularity_xld — 2D `features` op

- **データ種**: `contour` → `feature`
- **呼び出し**: `fullseye.apply(img, "circularity_xld", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `circularity_xld`(意味・パラメータは HALCON リファレンスが参考になる)

![circularity_xld: input → output](../../_fig/circularity_xld.png)
...
![circularity_xld: stages](../../_fig/circularity_xld.chain.jpg)
