...
# diameter_xld — 2D `features` op

- **データ種**: `contour` → `feature`
- **呼び出し**: `fullseye.apply(img, "diameter_xld", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `diameter_xld`(意味・パラメータは HALCON リファレンスが参考になる)

![diameter_xld: input → output](../../_fig/diameter_xld.png)
...
![diameter_xld: stages](../../_fig/diameter_xld.chain.jpg)
