---
op: rectangularity_xld
dim: 2d
category: features
in: contour
out: feature
halcon: rectangularity_xld
...
# rectangularity_xld — 2D `features` op

- **データ種**: `contour` → `feature`
- **呼び出し**: `fullseye.apply(img, "rectangularity_xld", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `rectangularity_xld`(意味・パラメータは HALCON リファレンスが参考になる)

![rectangularity_xld: input → output](../../_fig/rectangularity_xld.png)
...
![rectangularity_xld: stages](../../_fig/rectangularity_xld.chain.jpg)
