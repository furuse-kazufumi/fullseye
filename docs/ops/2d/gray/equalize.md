...
- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "equalize", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `equ_histo_image`(意味・パラメータは HALCON リファレンスが参考になる)

![equalize: input → output](../../_fig/equalize.png)
...
![equalize: other inputs](../../_fig/equalize.inputs.jpg)
