...
- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "sk_corner_harris", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `points_harris`(意味・パラメータは HALCON リファレンスが参考になる)

![sk_corner_harris: input → output](../../_fig/sk_corner_harris.png)
...
![sk_corner_harris: knob a sweep](../../_fig/sk_corner_harris.a.jpg)

*つまみ b は出力を変えない(実測: 0.1 / 0.5 / 0.9 で同一)。*

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![sk_corner_harris: other inputs](../../_fig/sk_corner_harris.inputs.jpg)
