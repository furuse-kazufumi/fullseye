...
- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "roberts", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `roberts`(意味・パラメータは HALCON リファレンスが参考になる)

![roberts: input → output](../../_fig/roberts.png)
...
![roberts: other inputs](../../_fig/roberts.inputs.jpg)
