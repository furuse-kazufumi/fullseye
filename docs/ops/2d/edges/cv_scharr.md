...
# cv_scharr — 2D `edges` op

- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "cv_scharr", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `edges_image`(意味・パラメータは HALCON リファレンスが参考になる)

![cv_scharr: input → output](../../_fig/cv_scharr.png)
...
![cv_scharr: other inputs](../../_fig/cv_scharr.inputs.jpg)
