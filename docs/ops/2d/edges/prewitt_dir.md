...
# prewitt_dir — 2D `edges` op

- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "prewitt_dir", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `prewitt_dir`(意味・パラメータは HALCON リファレンスが参考になる)

![prewitt_dir: input → output](../../_fig/prewitt_dir.png)
...
![prewitt_dir: other inputs](../../_fig/prewitt_dir.inputs.jpg)
