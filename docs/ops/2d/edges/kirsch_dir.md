...
# kirsch_dir — 2D `edges` op

- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "kirsch_dir", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `kirsch_dir`(意味・パラメータは HALCON リファレンスが参考になる)

![kirsch_dir: input → output](../../_fig/kirsch_dir.png)
...
![kirsch_dir: other inputs](../../_fig/kirsch_dir.inputs.jpg)
