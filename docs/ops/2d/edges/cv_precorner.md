...
# cv_precorner — 2D `edges` op

- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "cv_precorner", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `corner_response`(意味・パラメータは HALCON リファレンスが参考になる)

![cv_precorner: input → output](../../_fig/cv_precorner.png)
...
![cv_precorner: other inputs](../../_fig/cv_precorner.inputs.jpg)
