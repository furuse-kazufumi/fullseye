...
# symm_difference — 2D `nary` op

- **データ種**: `region × region` → `region`
- **呼び出し**: 入力 2 枚の op なので `fullseye.apply` では呼べない。`g = fullseye.FullseyeGraph(); g.add("out", "symm_difference", ["in1", "in2"][:2], a=0.5, b=0.5)` (実装は `import imgops_nary; {o.name: o for o in imgops_nary.build_nary()}["symm_difference"].fn(inputs, a, b)`)
- **HALCON 相当**: `symm_difference`(意味・パラメータは HALCON リファレンスが参考になる)
