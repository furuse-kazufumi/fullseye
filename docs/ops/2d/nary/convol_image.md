...
# convol_image — 2D `nary` op

- **データ種**: `image × image` → `image`
- **呼び出し**: 入力 2 枚の op なので `fullseye.apply` では呼べない。`g = fullseye.FullseyeGraph(); g.add("out", "convol_image", ["in1", "in2"][:2], a=0.5, b=0.5)` (実装は `import imgops_nary; {o.name: o for o in imgops_nary.build_nary()}["convol_image"].fn(inputs, a, b)`)
- **HALCON 相当**: `convol_image`(意味・パラメータは HALCON リファレンスが参考になる)
