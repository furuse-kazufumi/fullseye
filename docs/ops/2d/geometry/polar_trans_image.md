...
- **データ種**: `image` → `image`
- **呼び出し**: `fullseye.apply(img, "polar_trans_image", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `polar_trans_image`(意味・パラメータは HALCON リファレンスが参考になる)

![polar_trans_image: input → output](../../_fig/polar_trans_image.png)
...
![polar_trans_image: other inputs](../../_fig/polar_trans_image.inputs.jpg)
