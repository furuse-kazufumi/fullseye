...
in: image
out: region
halcon: auto_threshold
examples: [gallery2d_segmentation, poc_battery_electrode_breathing, poc_bump_coplanarity, threshold_family_agreement]
...
- **データ種**: `image` → `region`
- **呼び出し**: `fullseye.apply(img, "auto_threshold", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `auto_threshold`(意味・パラメータは HALCON リファレンスが参考になる)

![auto_threshold: input → output](../../_fig/auto_threshold.png)
...
![auto_threshold: other inputs](../../_fig/auto_threshold.inputs.jpg)
