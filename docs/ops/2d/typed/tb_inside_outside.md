...
![tb_inside_outside: input → output](../../_fig/tb_inside_outside.png)
...
![tb_inside_outside: stages](../../_fig/tb_inside_outside.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_inside_outside: other inputs](../../_fig/tb_inside_outside.inputs.jpg)
...
tb_inside_outside 0.50 0.50
```

## 実行できる例(この op を実際に呼ぶ検証済みサンプル)

次の例は元の台帳 op `inside_outside` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
