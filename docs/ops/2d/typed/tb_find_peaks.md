...
# tb_find_peaks — 2D `typed` op

- **データ種**: `signal` → `signal`
- **呼び出し**: `fullseye.apply(img, "tb_find_peaks", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)

![tb_find_peaks: input → output](../../_fig/tb_find_peaks.png)
...
![tb_find_peaks: stages](../../_fig/tb_find_peaks.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_find_peaks: other inputs](../../_fig/tb_find_peaks.inputs.jpg)

## 使い方

Peak indices (scipy.signal.find_peaks) — impacts / defect echoes.

Typed bridge of the 1d op ``find_peaks`` into the 2-D evolution registry: the same implementation, called under the ``op(v, a, b)`` convention. This op has no tunable parameter; ``a`` and ``b`` are unused.
...
tb_find_peaks 0.50 0.50
```

## 実行できる例(この op を実際に呼ぶ検証済みサンプル)

次の例は元の台帳 op `find_peaks` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
