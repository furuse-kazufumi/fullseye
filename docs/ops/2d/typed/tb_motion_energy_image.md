...
# tb_motion_energy_image — 2D `typed` op

- **データ種**: `video` → `video`
- **呼び出し**: `fullseye.apply(img, "tb_motion_energy_image", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)

![tb_motion_energy_image: input → output](../../_fig/tb_motion_energy_image.png)
...
![tb_motion_energy_image: knob b sweep](../../_fig/tb_motion_energy_image.b.jpg)

**段階**(前置きの op → この op。左から順):

![tb_motion_energy_image: stages](../../_fig/tb_motion_energy_image.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_motion_energy_image: other inputs](../../_fig/tb_motion_energy_image.inputs.jpg)

**動き**(GIF: フレーム / 視点 / スライスを順に。静止の図が完成形で、GIF は補助):

![tb_motion_energy_image: animation](../../_fig/tb_motion_energy_image.gif)
...
tb_motion_energy_image 0.50 0.50
```

## 実行できる例(この op を実際に呼ぶ検証済みサンプル)

次の例は元の台帳 op `motion_energy_image` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
