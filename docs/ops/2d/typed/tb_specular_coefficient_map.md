...
![tb_specular_coefficient_map: input → output](../../_fig/tb_specular_coefficient_map.png)
...
![tb_specular_coefficient_map: stages](../../_fig/tb_specular_coefficient_map.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_specular_coefficient_map: other inputs](../../_fig/tb_specular_coefficient_map.inputs.jpg)
...
次の例は元の台帳 op `specular_coefficient_map` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
