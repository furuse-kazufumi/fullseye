...
# tb_lf_epi — 2D `typed` op

- **データ種**: `lightfield` → `image`
- **呼び出し**: `fullseye.apply(img, "tb_lf_epi", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)

![tb_lf_epi: input → output](../../_fig/tb_lf_epi.png)
...
![tb_lf_epi: stages](../../_fig/tb_lf_epi.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_lf_epi: other inputs](../../_fig/tb_lf_epi.inputs.jpg)
...
次の例は元の台帳 op `lf_epi` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
