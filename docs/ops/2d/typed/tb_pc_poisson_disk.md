...
- **データ種**: `points` → `points`
- **呼び出し**: `fullseye.apply(img, "tb_pc_poisson_disk", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)

![tb_pc_poisson_disk: input → output](../../_fig/tb_pc_poisson_disk.png)
...
![tb_pc_poisson_disk: stages](../../_fig/tb_pc_poisson_disk.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_pc_poisson_disk: other inputs](../../_fig/tb_pc_poisson_disk.inputs.jpg)
...
tb_pc_poisson_disk 0.50 0.50
```

## 実行できる例(この op を実際に呼ぶ検証済みサンプル)

次の例は元の台帳 op `pc_poisson_disk` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
