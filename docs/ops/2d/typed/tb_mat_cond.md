...
![tb_mat_cond: input → output](../../_fig/tb_mat_cond.png)
...
![tb_mat_cond: stages](../../_fig/tb_mat_cond.chain.jpg)
...
次の例は元の台帳 op `mat_cond` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
