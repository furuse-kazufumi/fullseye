...
![tb_tcspc_background_subtract: input → output](../../_fig/tb_tcspc_background_subtract.png)
...
![tb_tcspc_background_subtract: knob b sweep](../../_fig/tb_tcspc_background_subtract.b.jpg)

**段階**(前置きの op → この op。左から順):

![tb_tcspc_background_subtract: stages](../../_fig/tb_tcspc_background_subtract.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_tcspc_background_subtract: other inputs](../../_fig/tb_tcspc_background_subtract.inputs.jpg)
...
次の例は元の台帳 op `tcspc_background_subtract` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
