...
Typed bridge of the videostream op ``exponential_foreground`` into the 2-D evolution registry: the same implementation, called under the ``op(v, a, b)`` convention. ``a`` drives ``alpha`` (default 0.05) and ``b`` drives ``threshold`` (default 0.1).
