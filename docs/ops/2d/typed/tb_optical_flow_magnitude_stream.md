---
op: tb_optical_flow_magnitude_stream
dim: 2d
category: typed
in: video
out: video
...
# tb_optical_flow_magnitude_stream — 2D `typed` op

- **データ種**: `video` → `video`
- **呼び出し**: `fullseye.apply(img, "tb_optical_flow_magnitude_stream", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)

![tb_optical_flow_magnitude_stream: input → output](../../_fig/tb_optical_flow_magnitude_stream.png)
...
![tb_optical_flow_magnitude_stream: stages](../../_fig/tb_optical_flow_magnitude_stream.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_optical_flow_magnitude_stream: other inputs](../../_fig/tb_optical_flow_magnitude_stream.inputs.jpg)

**動き**(GIF: フレーム / 視点 / スライスを順に。静止の図が完成形で、GIF は補助):

![tb_optical_flow_magnitude_stream: animation](../../_fig/tb_optical_flow_magnitude_stream.gif)
...
tb_optical_flow_magnitude_stream 0.50 0.50
```

## 実行できる例(この op を実際に呼ぶ検証済みサンプル)

次の例は元の台帳 op `optical_flow_magnitude_stream` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
