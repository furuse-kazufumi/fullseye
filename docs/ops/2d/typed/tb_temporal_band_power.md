...
# tb_temporal_band_power — 2D `typed` op

- **データ種**: `video` → `image`
- **呼び出し**: `fullseye.apply(img, "tb_temporal_band_power", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)

![tb_temporal_band_power: input → output](../../_fig/tb_temporal_band_power.png)
...
![tb_temporal_band_power: stages](../../_fig/tb_temporal_band_power.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![tb_temporal_band_power: other inputs](../../_fig/tb_temporal_band_power.inputs.jpg)
...
tb_temporal_band_power 0.50 0.50
```

## 実行できる例(この op を実際に呼ぶ検証済みサンプル)

次の例は元の台帳 op `temporal_band_power` を呼ぶもの。この橋渡し op は同じ実装を `fn(v, a, b)` 規約に合わせただけなので、挙動はそのまま当てはまる(呼び出し形だけ違う)。
