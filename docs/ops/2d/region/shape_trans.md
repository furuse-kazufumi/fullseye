...
# shape_trans — 2D `region` op

- **データ種**: `region` → `region`
- **呼び出し**: `fullseye.apply(img, "shape_trans", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `shape_trans`(意味・パラメータは HALCON リファレンスが参考になる)

![shape_trans: input → output](../../_fig/shape_trans.png)
...
![shape_trans: stages](../../_fig/shape_trans.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![shape_trans: other inputs](../../_fig/shape_trans.inputs.jpg)
