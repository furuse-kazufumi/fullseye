...
# thinning_golay — 2D `region` op

- **データ種**: `region` → `region`
- **呼び出し**: `fullseye.apply(img, "thinning_golay", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `thinning_golay`(意味・パラメータは HALCON リファレンスが参考になる)

![thinning_golay: input → output](../../_fig/thinning_golay.png)
...
![thinning_golay: stages](../../_fig/thinning_golay.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![thinning_golay: other inputs](../../_fig/thinning_golay.inputs.jpg)

## 使い方

領域を細線化する。実装は skimage の ``thin``（Guo–Hall 型の反復細線化）で完全に 1 画素幅になるまで繰り返し、a, b は未使用。
