...
# opening_golay — 2D `region` op

- **データ種**: `region` → `region`
- **呼び出し**: `fullseye.apply(img, "opening_golay", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `opening_golay`(意味・パラメータは HALCON リファレンスが参考になる)

![opening_golay: input → output](../../_fig/opening_golay.png)

*図は合成の入力 128×128 で実際に走らせた出力。左が入力、右が出力。点群は上から見た散布(明るさ = z)、1-D 列は折れ線、体積は z 方向の最大値投影、動画は中央フレーム、複素画像は振幅、絵にならない返り値は値そのもの。*

**つまみ a を振る**(0.1 / 0.5 / 0.9、もう一方は既定):

![opening_golay: knob a sweep](../../_fig/opening_golay.a.jpg)

*つまみ b は出力を変えない(実測: 0.1 / 0.5 / 0.9 で同一)。*

**段階**(前置きの op → この op。左から順):

![opening_golay: stages](../../_fig/opening_golay.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![opening_golay: other inputs](../../_fig/opening_golay.inputs.jpg)

## 使い方

円形構造要素（disk, 半径 ``_rad(a)``=1〜4）による二値オープニング（opening、収縮の後に膨張）。小さな突起や孤立点を除去する。b は未使用。
