---
op: r3_background_seg
dim: 2d
category: region
in: region
out: region
halcon: background_seg
...
- **データ種**: `region` → `region`
- **呼び出し**: `fullseye.apply(img, "r3_background_seg", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `background_seg`(意味・パラメータは HALCON リファレンスが参考になる)

![r3_background_seg: input → output](../../_fig/r3_background_seg.png)
...
![r3_background_seg: stages](../../_fig/r3_background_seg.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![r3_background_seg: other inputs](../../_fig/r3_background_seg.inputs.jpg)

## 使い方

領域の背景（前景の補集合）を連結成分に分け、小さすぎる成分を捨てて返す（HALCON ``background_seg`` に相当）。

背景を連結成分ラベリングし、``a``（0〜1）を「最大背景成分の面積に対する相対しきい値」としてそれ未満の成分を除去する（``a=0`` で全背景＝正真正銘の``background_seg`` 結果）。``b`` は未使用。
