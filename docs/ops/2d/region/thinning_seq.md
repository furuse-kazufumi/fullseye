...
# thinning_seq — 2D `region` op

- **データ種**: `region` → `region`
- **呼び出し**: `fullseye.apply(img, "thinning_seq", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `thinning_seq`(意味・パラメータは HALCON リファレンスが参考になる)

![thinning_seq: input → output](../../_fig/thinning_seq.png)
...
![thinning_seq: stages](../../_fig/thinning_seq.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![thinning_seq: other inputs](../../_fig/thinning_seq.inputs.jpg)
