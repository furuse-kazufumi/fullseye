...
# select_shape — 2D `region` op

- **データ種**: `region` → `region`
- **呼び出し**: `fullseye.apply(img, "select_shape", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `select_shape`(意味・パラメータは HALCON リファレンスが参考になる)

![select_shape: input → output](../../_fig/select_shape.png)
...
![select_shape: stages](../../_fig/select_shape.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![select_shape: other inputs](../../_fig/select_shape.inputs.jpg)
