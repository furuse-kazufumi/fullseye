...
# xsk3_rank_majority — 2D `region` op

- **データ種**: `region` → `region`
- **呼び出し**: `fullseye.apply(img, "xsk3_rank_majority", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)

![xsk3_rank_majority: input → output](../../_fig/xsk3_rank_majority.png)

*図は合成の入力 128×128 で実際に走らせた出力。左が入力、右が出力。点群は上から見た散布(明るさ = z)、1-D 列は折れ線、体積は z 方向の最大値投影、動画は中央フレーム、複素画像は振幅、絵にならない返り値は値そのもの。*

**つまみ a を振る**(0.1 / 0.5 / 0.9、もう一方は既定):

![xsk3_rank_majority: knob a sweep](../../_fig/xsk3_rank_majority.a.jpg)

*つまみ b は出力を変えない(実測: 0.1 / 0.5 / 0.9 で同一)。*

**段階**(前置きの op → この op。左から順):

![xsk3_rank_majority: stages](../../_fig/xsk3_rank_majority.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![xsk3_rank_majority: other inputs](../../_fig/xsk3_rank_majority.inputs.jpg)

## 使い方

局所多数決フィルタ(skimage ``filters.rank.majority``)。xmh_majority と同じ多数決平滑化の skimage 版で、円盤近傍内で過半数の画素が 1 なら 1 にする。
