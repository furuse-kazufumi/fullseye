...
# region_boundary — 2D `region` op

- **データ種**: `region` → `region`
- **呼び出し**: `fullseye.apply(img, "region_boundary", a=0.5, b=0.5)` (2-D は 1 画像 + 2 スカラつまみ `a,b∈[0,1]` のモデル)
- **HALCON 相当**: `boundary`(意味・パラメータは HALCON リファレンスが参考になる)

![region_boundary: input → output](../../_fig/region_boundary.png)
...
![region_boundary: stages](../../_fig/region_boundary.chain.jpg)

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![region_boundary: other inputs](../../_fig/region_boundary.inputs.jpg)
