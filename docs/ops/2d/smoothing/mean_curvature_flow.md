---
op: mean_curvature_flow
dim: 2d
category: smoothing
in: image
out: image
halcon: mean_curvature_flow
...
![mean_curvature_flow: input → output](../../_fig/mean_curvature_flow.png)
...
![mean_curvature_flow: knob a sweep](../../_fig/mean_curvature_flow.a.jpg)

*つまみ b は出力を変えない(実測: 0.1 / 0.5 / 0.9 で同一)。*

**別の画像でも**(合成シーン / 写真 / 硬貨。上段が入力、下段がその出力。つまみは既定):

![mean_curvature_flow: other inputs](../../_fig/mean_curvature_flow.inputs.jpg)
...
**つまみ(実測)**: ``a`` は**段階的**に効き、切り替わるのは a ≈ 0.17、0.33、0.49、0.67、0.83、0.99(実測。刻み 0.02 の掃きで測った位置)。
