...
- **データ種**: `score` → `position`
- **呼び出し**: `import fullseye as fs; fs.ledger.score_to_position(score)` (実装を直接呼ぶなら `import reprconv; reprconv.score_to_position(score)`、台帳から引くなら `opsreprconv.get("score_to_position")`)
