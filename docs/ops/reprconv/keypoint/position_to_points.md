...
- **呼び出し**: `import fullseye as fs; fs.ledger.position_to_points(position)` (実装を直接呼ぶなら `import reprconv; reprconv.position_to_points(position)`、台帳から引くなら `opsreprconv.get("position_to_points")`)
