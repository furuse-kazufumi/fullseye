...
- **呼び出し**: `import fullseye as fs; fs.ledger.descriptor_to_matrix(descriptor)` (実装を直接呼ぶなら `import reprconv; reprconv.descriptor_to_matrix(descriptor)`、台帳から引くなら `opsreprconv.get("descriptor_to_matrix")`)
