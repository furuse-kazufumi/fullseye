...
- **データ種**: `normals` → `pairs`
- **呼び出し**: `import fullseye as fs; fs.ledger.normals_to_angles(normals)` (実装を直接呼ぶなら `import reprconv; reprconv.normals_to_angles(normals)`、台帳から引くなら `opsreprconv.get("normals_to_angles")`)
