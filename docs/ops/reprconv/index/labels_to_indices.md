...
- **データ種**: `labels` → `indices`
- **呼び出し**: `import fullseye as fs; fs.ledger.labels_to_indices(labels)` (実装を直接呼ぶなら `import reprconv; reprconv.labels_to_indices(labels)`、台帳から引くなら `opsreprconv.get("labels_to_indices")`)
