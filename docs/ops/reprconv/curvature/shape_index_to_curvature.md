...
- **呼び出し**: `import fullseye as fs; fs.ledger.shape_index_to_curvature(pairs)` (実装を直接呼ぶなら `import reprconv; reprconv.shape_index_to_curvature(pairs)`、台帳から引くなら `opsreprconv.get("shape_index_to_curvature")`)
