...
- **呼び出し**: `import fullseye as fs; fs.ledger.curvature_to_table(curvature)` (実装を直接呼ぶなら `import reprconv; reprconv.curvature_to_table(curvature)`、台帳から引くなら `opsreprconv.get("curvature_to_table")`)
