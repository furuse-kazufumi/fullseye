...
- **データ種**: `vector` → `shift`
- **呼び出し**: `import fullseye as fs; fs.ledger.vector_to_shift(vector)` (実装を直接呼ぶなら `import reprconv; reprconv.vector_to_shift(vector)`、台帳から引くなら `opsreprconv.get("vector_to_shift")`)
