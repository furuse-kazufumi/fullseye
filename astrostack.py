...
「星がきれい」だからではなく、**検算できる** からだ:
  ...
  * **drizzle は面積(総フラックス)を保存する。** 入力画素を「しずく(drop)」
    ...
    (実測: 相対誤差 0.0、``tests/test_astrostack.py`` で固定)。
...
def noise_sigma(image, method="mad", kappa=3.0, iters=5):
    ...
    *method* ``"mad"`` は中央絶対偏差の ``1.4826`` 倍(正規分布のときに標準偏差と
    ...
    —— 実測(``tests/test_astrostack.py``)では、星入りフレームで素の std は
    真の σ の 2 倍以上に膨らむのに対し、MAD 推定は 1 % 台で当たる。
...
def aperture_photometry(image, centers, r_aperture=5.0, r_inner=8.0,
    ...
    ガウシアン星に対して、半径 ``r`` の開口が拾う割合は
    ``1 - exp(-r^2/(2 sigma^2))``。実測では ``sigma=1.5``、``r=4.5``
    (``= 3 sigma``)で理論値 0.98889 に対し測定 0.98889、相対誤差 4e-6。
...
def sigma_clip_stack(frames, mode="sigma_clip", kappa=3.0, iters=5,
    ...
    **破綻点は 50 %。** 中心を中央値で取る以上、汚染フレームが半数を超えた
    画素では中央値そのものが汚染側に乗り、クリップは**正しい方**を捨てる。
    これはこの実装の不具合ではなく中央値の定義そのもので、``center="mean"``
    にすればもっと早く(汚染 1 枚でも)壊れる。テストは 0〜60 % の汚染率で
    誤差を測り、**壊れる側もそのまま残してある**。
