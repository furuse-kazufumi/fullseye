...
def find_shape_model(model, image, min_score: float = 0.5, step: int = 2,
    ...
    ★★ 返り値 ``angle`` の符号の向き(2026-09-02 に明文化・実装は不変)★★
        単位は **度**。意味は「**テンプレートをこれだけ回すと画像中の出現に重なる**」
        —— すなわち ``scipy.ndimage.rotate(template, angle)`` に渡す角度そのもの
        (実測: ``scene = ndimage.rotate(T, x)`` を探索させると返り値は x に一致、
        x = -30/-15/0/+15/+30 の 5 点で score 1.000)。

        画素座標での正確な向き: 中心からの変位 (dr, dc) は正の角 θ で

            dr' = dr·cosθ - dc·sinθ ,   dc' = dr·sinθ + dc·cosθ

        に写る(実測: 中心の真上 (dr,dc)=(-40,0) が θ=+30° で (-34.67,-20.06)、
        閉形式は (-34.641,-20.000))。row が下向き・col が右向きの画面座標では
        これは **反時計回り**。したがって「上が 0°・時計回りが正」の作図規約で
        そのまま描くと **鏡像になる**。時計回り規約に直すには符号を反転すること。
    """
